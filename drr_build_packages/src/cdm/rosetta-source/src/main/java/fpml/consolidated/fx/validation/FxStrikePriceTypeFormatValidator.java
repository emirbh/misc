		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "FxStrikePrice", path, "", res.getError());
				}
				return success("FxStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "FxStrikePrice", path, "");
			})
			.collect(toList());
	}
