		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventException", ValidationResult.ValidationType.CARDINALITY, "CreditEventException", path, "", res.getError());
				}
				return success("CreditEventException", ValidationResult.ValidationType.CARDINALITY, "CreditEventException", path, "");
			})
			.collect(toList());
	}
