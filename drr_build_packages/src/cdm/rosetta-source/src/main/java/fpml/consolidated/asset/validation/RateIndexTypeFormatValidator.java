		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateIndex", ValidationResult.ValidationType.TYPE_FORMAT, "RateIndex", path, "", res.getError());
				}
				return success("RateIndex", ValidationResult.ValidationType.TYPE_FORMAT, "RateIndex", path, "");
			})
			.collect(toList());
	}
