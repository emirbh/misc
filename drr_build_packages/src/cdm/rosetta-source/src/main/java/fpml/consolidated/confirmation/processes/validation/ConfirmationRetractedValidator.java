		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationRetracted", ValidationResult.ValidationType.CARDINALITY, "ConfirmationRetracted", path, "", res.getError());
				}
				return success("ConfirmationRetracted", ValidationResult.ValidationType.CARDINALITY, "ConfirmationRetracted", path, "");
			})
			.collect(toList());
	}
