...
public class FxAverageStrikeTypeFormatValidator implements Validator<FxAverageStrike> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAverageStrike", ValidationResult.ValidationType.TYPE_FORMAT, "FxAverageStrike", path, "", res.getError());
				}
				return success("FxAverageStrike", ValidationResult.ValidationType.TYPE_FORMAT, "FxAverageStrike", path, "");
			})
			.collect(toList());
	}
