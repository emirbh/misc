		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxPerformanceLeg", ValidationResult.ValidationType.TYPE_FORMAT, "FxPerformanceLeg", path, "", res.getError());
				}
				return success("FxPerformanceLeg", ValidationResult.ValidationType.TYPE_FORMAT, "FxPerformanceLeg", path, "");
			})
			.collect(toList());
	}
