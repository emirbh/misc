		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationException", ValidationResult.ValidationType.TYPE_FORMAT, "AllocationException", path, "", res.getError());
				}
				return success("AllocationException", ValidationResult.ValidationType.TYPE_FORMAT, "AllocationException", path, "");
			})
			.collect(toList());
	}
