		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegalEntity", ValidationResult.ValidationType.TYPE_FORMAT, "LegalEntity", path, "", res.getError());
				}
				return success("LegalEntity", ValidationResult.ValidationType.TYPE_FORMAT, "LegalEntity", path, "");
			})
			.collect(toList());
	}
