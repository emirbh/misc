		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MortgageSector", ValidationResult.ValidationType.TYPE_FORMAT, "MortgageSector", path, "", res.getError());
				}
				return success("MortgageSector", ValidationResult.ValidationType.TYPE_FORMAT, "MortgageSector", path, "");
			})
			.collect(toList());
	}
