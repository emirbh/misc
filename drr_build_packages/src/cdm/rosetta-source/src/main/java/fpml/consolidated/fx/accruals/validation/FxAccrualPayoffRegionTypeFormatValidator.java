		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualPayoffRegion", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualPayoffRegion", path, "", res.getError());
				}
				return success("FxAccrualPayoffRegion", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualPayoffRegion", path, "");
			})
			.collect(toList());
	}
