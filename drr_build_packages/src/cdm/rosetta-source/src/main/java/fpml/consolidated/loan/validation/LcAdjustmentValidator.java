...
public class LcAdjustmentValidator implements Validator<LcAdjustment> {
	...
	private List<ComparisonResult> getComparisonResults(LcAdjustment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("relatedBorrowing", (Boolean) o.getRelatedBorrowing() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcAdjustment", ValidationResult.ValidationType.CARDINALITY, "LcAdjustment", path, "", res.getError());
				}
				return success("LcAdjustment", ValidationResult.ValidationType.CARDINALITY, "LcAdjustment", path, "");
			})
			.collect(toList());
	}
