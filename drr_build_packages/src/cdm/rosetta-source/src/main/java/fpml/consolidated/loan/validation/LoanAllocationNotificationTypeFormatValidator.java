		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationNotification", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocationNotification", path, "", res.getError());
				}
				return success("LoanAllocationNotification", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocationNotification", path, "");
			})
			.collect(toList());
	}
