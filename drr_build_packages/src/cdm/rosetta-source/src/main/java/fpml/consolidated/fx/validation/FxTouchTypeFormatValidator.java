		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTouch", ValidationResult.ValidationType.TYPE_FORMAT, "FxTouch", path, "", res.getError());
				}
				return success("FxTouch", ValidationResult.ValidationType.TYPE_FORMAT, "FxTouch", path, "");
			})
			.collect(toList());
	}
