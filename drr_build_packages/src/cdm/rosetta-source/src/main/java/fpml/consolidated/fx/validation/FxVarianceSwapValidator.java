...
public class FxVarianceSwapValidator implements Validator<FxVarianceSwap> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxVarianceSwap", ValidationResult.ValidationType.CARDINALITY, "FxVarianceSwap", path, "", res.getError());
				}
				return success("FxVarianceSwap", ValidationResult.ValidationType.CARDINALITY, "FxVarianceSwap", path, "");
			})
			.collect(toList());
	}
