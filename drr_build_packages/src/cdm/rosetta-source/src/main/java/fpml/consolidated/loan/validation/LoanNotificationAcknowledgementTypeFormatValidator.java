		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanNotificationAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "LoanNotificationAcknowledgement", path, "", res.getError());
				}
				return success("LoanNotificationAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "LoanNotificationAcknowledgement", path, "");
			})
			.collect(toList());
	}
