		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNoticeDocument", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventNoticeDocument", path, "", res.getError());
				}
				return success("CreditEventNoticeDocument", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventNoticeDocument", path, "");
			})
			.collect(toList());
	}
