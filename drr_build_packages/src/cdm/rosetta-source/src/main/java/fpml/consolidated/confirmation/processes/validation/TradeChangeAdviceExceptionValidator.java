...
public class TradeChangeAdviceExceptionValidator implements Validator<TradeChangeAdviceException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdviceException", ValidationResult.ValidationType.CARDINALITY, "TradeChangeAdviceException", path, "", res.getError());
				}
				return success("TradeChangeAdviceException", ValidationResult.ValidationType.CARDINALITY, "TradeChangeAdviceException", path, "");
			})
			.collect(toList());
	}
