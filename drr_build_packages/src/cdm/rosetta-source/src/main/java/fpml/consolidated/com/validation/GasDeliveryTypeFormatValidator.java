		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GasDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "GasDelivery", path, "", res.getError());
				}
				return success("GasDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "GasDelivery", path, "");
			})
			.collect(toList());
	}
