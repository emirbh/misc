		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FinancialSwapLeg", ValidationResult.ValidationType.CARDINALITY, "FinancialSwapLeg", path, "", res.getError());
				}
				return success("FinancialSwapLeg", ValidationResult.ValidationType.CARDINALITY, "FinancialSwapLeg", path, "");
			})
			.collect(toList());
	}
