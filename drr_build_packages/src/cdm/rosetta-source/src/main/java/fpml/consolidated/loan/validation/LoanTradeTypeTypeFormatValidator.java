...
public class LoanTradeTypeTypeFormatValidator implements Validator<LoanTradeType> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeType", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeType", path, "", res.getError());
				}
				return success("LoanTradeType", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeType", path, "");
			})
			.collect(toList());
	}
