		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacility", ValidationResult.ValidationType.CARDINALITY, "AbstractFacility", path, "", res.getError());
				}
				return success("AbstractFacility", ValidationResult.ValidationType.CARDINALITY, "AbstractFacility", path, "");
			})
			.collect(toList());
	}
