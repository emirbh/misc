...
public class RegulatoryWithdrawalTypeFormatValidator implements Validator<RegulatoryWithdrawal> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegulatoryWithdrawal", ValidationResult.ValidationType.TYPE_FORMAT, "RegulatoryWithdrawal", path, "", res.getError());
				}
				return success("RegulatoryWithdrawal", ValidationResult.ValidationType.TYPE_FORMAT, "RegulatoryWithdrawal", path, "");
			})
			.collect(toList());
	}
