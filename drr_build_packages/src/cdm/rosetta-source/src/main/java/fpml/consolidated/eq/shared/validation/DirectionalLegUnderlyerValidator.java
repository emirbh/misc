		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DirectionalLegUnderlyer", ValidationResult.ValidationType.CARDINALITY, "DirectionalLegUnderlyer", path, "", res.getError());
				}
				return success("DirectionalLegUnderlyer", ValidationResult.ValidationType.CARDINALITY, "DirectionalLegUnderlyer", path, "");
			})
			.collect(toList());
	}
