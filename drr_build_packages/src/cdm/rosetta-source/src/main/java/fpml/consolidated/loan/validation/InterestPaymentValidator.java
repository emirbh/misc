...
public class InterestPaymentValidator implements Validator<InterestPayment> {
	...
	private List<ComparisonResult> getComparisonResults(InterestPayment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("lenderShareCalculationMethod", (InterestCalculationMethodEnum) o.getLenderShareCalculationMethod() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestPayment", ValidationResult.ValidationType.CARDINALITY, "InterestPayment", path, "", res.getError());
				}
				return success("InterestPayment", ValidationResult.ValidationType.CARDINALITY, "InterestPayment", path, "");
			})
			.collect(toList());
	}
