		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPricingDates", ValidationResult.ValidationType.CARDINALITY, "CommodityPricingDates", path, "", res.getError());
				}
				return success("CommodityPricingDates", ValidationResult.ValidationType.CARDINALITY, "CommodityPricingDates", path, "");
			})
			.collect(toList());
	}
