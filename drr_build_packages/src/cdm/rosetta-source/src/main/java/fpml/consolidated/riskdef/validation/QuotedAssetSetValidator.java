		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuotedAssetSet", ValidationResult.ValidationType.CARDINALITY, "QuotedAssetSet", path, "", res.getError());
				}
				return success("QuotedAssetSet", ValidationResult.ValidationType.CARDINALITY, "QuotedAssetSet", path, "");
			})
			.collect(toList());
	}
