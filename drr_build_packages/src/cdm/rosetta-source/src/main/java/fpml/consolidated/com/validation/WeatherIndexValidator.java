...
public class WeatherIndexValidator implements Validator<WeatherIndex> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeatherIndex", ValidationResult.ValidationType.CARDINALITY, "WeatherIndex", path, "", res.getError());
				}
				return success("WeatherIndex", ValidationResult.ValidationType.CARDINALITY, "WeatherIndex", path, "");
			})
			.collect(toList());
	}
