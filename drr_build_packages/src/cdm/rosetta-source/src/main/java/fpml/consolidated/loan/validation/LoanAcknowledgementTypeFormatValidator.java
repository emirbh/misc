...
public class LoanAcknowledgementTypeFormatValidator implements Validator<LoanAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAcknowledgement", path, "", res.getError());
				}
				return success("LoanAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAcknowledgement", path, "");
			})
			.collect(toList());
	}
