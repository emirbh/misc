		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PenaltyRateChange", ValidationResult.ValidationType.TYPE_FORMAT, "PenaltyRateChange", path, "", res.getError());
				}
				return success("PenaltyRateChange", ValidationResult.ValidationType.TYPE_FORMAT, "PenaltyRateChange", path, "");
			})
			.collect(toList());
	}
