		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodDates", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodDates", path, "", res.getError());
				}
				return success("CalculationPeriodDates", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodDates", path, "");
			})
			.collect(toList());
	}
