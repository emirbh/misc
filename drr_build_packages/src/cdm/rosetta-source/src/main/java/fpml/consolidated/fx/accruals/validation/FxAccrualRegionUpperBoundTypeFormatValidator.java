		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualRegionUpperBound", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualRegionUpperBound", path, "", res.getError());
				}
				return success("FxAccrualRegionUpperBound", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualRegionUpperBound", path, "");
			})
			.collect(toList());
	}
