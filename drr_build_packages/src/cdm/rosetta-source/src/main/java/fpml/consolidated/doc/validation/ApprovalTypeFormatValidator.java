		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Approval", ValidationResult.ValidationType.TYPE_FORMAT, "Approval", path, "", res.getError());
				}
				return success("Approval", ValidationResult.ValidationType.TYPE_FORMAT, "Approval", path, "");
			})
			.collect(toList());
	}
