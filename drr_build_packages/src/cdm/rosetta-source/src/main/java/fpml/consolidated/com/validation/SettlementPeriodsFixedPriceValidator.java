		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPeriodsFixedPrice", ValidationResult.ValidationType.CARDINALITY, "SettlementPeriodsFixedPrice", path, "", res.getError());
				}
				return success("SettlementPeriodsFixedPrice", ValidationResult.ValidationType.CARDINALITY, "SettlementPeriodsFixedPrice", path, "");
			})
			.collect(toList());
	}
