		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Message", ValidationResult.ValidationType.TYPE_FORMAT, "Message", path, "", res.getError());
				}
				return success("Message", ValidationResult.ValidationType.TYPE_FORMAT, "Message", path, "");
			})
			.collect(toList());
	}
