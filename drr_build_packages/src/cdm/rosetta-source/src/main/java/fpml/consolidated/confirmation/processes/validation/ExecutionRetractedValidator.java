...
public class ExecutionRetractedValidator implements Validator<ExecutionRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionRetracted", ValidationResult.ValidationType.CARDINALITY, "ExecutionRetracted", path, "", res.getError());
				}
				return success("ExecutionRetracted", ValidationResult.ValidationType.CARDINALITY, "ExecutionRetracted", path, "");
			})
			.collect(toList());
	}
