		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasketConstituent", ValidationResult.ValidationType.TYPE_FORMAT, "BasketConstituent", path, "", res.getError());
				}
				return success("BasketConstituent", ValidationResult.ValidationType.TYPE_FORMAT, "BasketConstituent", path, "");
			})
			.collect(toList());
	}
