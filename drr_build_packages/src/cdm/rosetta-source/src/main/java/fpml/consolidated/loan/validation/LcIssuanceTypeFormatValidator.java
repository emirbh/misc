		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcIssuance", ValidationResult.ValidationType.TYPE_FORMAT, "LcIssuance", path, "", res.getError());
				}
				return success("LcIssuance", ValidationResult.ValidationType.TYPE_FORMAT, "LcIssuance", path, "");
			})
			.collect(toList());
	}
