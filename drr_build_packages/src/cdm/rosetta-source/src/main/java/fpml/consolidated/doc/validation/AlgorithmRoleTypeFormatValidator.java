		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AlgorithmRole", ValidationResult.ValidationType.TYPE_FORMAT, "AlgorithmRole", path, "", res.getError());
				}
				return success("AlgorithmRole", ValidationResult.ValidationType.TYPE_FORMAT, "AlgorithmRole", path, "");
			})
			.collect(toList());
	}
