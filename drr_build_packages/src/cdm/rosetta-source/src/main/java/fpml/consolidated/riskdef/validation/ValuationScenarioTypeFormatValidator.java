		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationScenario", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationScenario", path, "", res.getError());
				}
				return success("ValuationScenario", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationScenario", path, "");
			})
			.collect(toList());
	}
