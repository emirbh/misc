...
public class LcRateChangeValidator implements Validator<LcRateChange> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcRateChange", ValidationResult.ValidationType.CARDINALITY, "LcRateChange", path, "", res.getError());
				}
				return success("LcRateChange", ValidationResult.ValidationType.CARDINALITY, "LcRateChange", path, "");
			})
			.collect(toList());
	}
