		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ServiceProcessingStep", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceProcessingStep", path, "", res.getError());
				}
				return success("ServiceProcessingStep", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceProcessingStep", path, "");
			})
			.collect(toList());
	}
