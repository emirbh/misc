		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IntermediaryInformation", ValidationResult.ValidationType.TYPE_FORMAT, "IntermediaryInformation", path, "", res.getError());
				}
				return success("IntermediaryInformation", ValidationResult.ValidationType.TYPE_FORMAT, "IntermediaryInformation", path, "");
			})
			.collect(toList());
	}
