		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReferenceType", ValidationResult.ValidationType.TYPE_FORMAT, "ReferenceType", path, "", res.getError());
				}
				return success("ReferenceType", ValidationResult.ValidationType.TYPE_FORMAT, "ReferenceType", path, "");
			})
			.collect(toList());
	}
