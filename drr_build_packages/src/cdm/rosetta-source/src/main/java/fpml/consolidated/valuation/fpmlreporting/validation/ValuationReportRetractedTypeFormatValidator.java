...
public class ValuationReportRetractedTypeFormatValidator implements Validator<ValuationReportRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationReportRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationReportRetracted", path, "", res.getError());
				}
				return success("ValuationReportRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationReportRetracted", path, "");
			})
			.collect(toList());
	}
