		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RoutingId", ValidationResult.ValidationType.TYPE_FORMAT, "RoutingId", path, "", res.getError());
				}
				return success("RoutingId", ValidationResult.ValidationType.TYPE_FORMAT, "RoutingId", path, "");
			})
			.collect(toList());
	}
