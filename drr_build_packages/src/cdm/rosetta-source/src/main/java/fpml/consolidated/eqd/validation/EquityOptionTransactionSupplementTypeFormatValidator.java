		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityOptionTransactionSupplement", ValidationResult.ValidationType.TYPE_FORMAT, "EquityOptionTransactionSupplement", path, "", res.getError());
				}
				return success("EquityOptionTransactionSupplement", ValidationResult.ValidationType.TYPE_FORMAT, "EquityOptionTransactionSupplement", path, "");
			})
			.collect(toList());
	}
