...
public class CommoditySwapValidator implements Validator<CommoditySwap> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySwap", ValidationResult.ValidationType.CARDINALITY, "CommoditySwap", path, "", res.getError());
				}
				return success("CommoditySwap", ValidationResult.ValidationType.CARDINALITY, "CommoditySwap", path, "");
			})
			.collect(toList());
	}
