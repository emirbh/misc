		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxCrossRateObservable", ValidationResult.ValidationType.CARDINALITY, "FxCrossRateObservable", path, "", res.getError());
				}
				return success("FxCrossRateObservable", ValidationResult.ValidationType.CARDINALITY, "FxCrossRateObservable", path, "");
			})
			.collect(toList());
	}
