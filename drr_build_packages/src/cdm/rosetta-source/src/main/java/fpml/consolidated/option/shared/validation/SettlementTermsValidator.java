		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementTerms", ValidationResult.ValidationType.CARDINALITY, "SettlementTerms", path, "", res.getError());
				}
				return success("SettlementTerms", ValidationResult.ValidationType.CARDINALITY, "SettlementTerms", path, "");
			})
			.collect(toList());
	}
