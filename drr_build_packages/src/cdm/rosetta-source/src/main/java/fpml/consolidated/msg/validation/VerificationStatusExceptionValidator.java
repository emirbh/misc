		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VerificationStatusException", ValidationResult.ValidationType.CARDINALITY, "VerificationStatusException", path, "", res.getError());
				}
				return success("VerificationStatusException", ValidationResult.ValidationType.CARDINALITY, "VerificationStatusException", path, "");
			})
			.collect(toList());
	}
