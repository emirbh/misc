		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractMaturityChange", ValidationResult.ValidationType.CARDINALITY, "LoanContractMaturityChange", path, "", res.getError());
				}
				return success("LoanContractMaturityChange", ValidationResult.ValidationType.CARDINALITY, "LoanContractMaturityChange", path, "");
			})
			.collect(toList());
	}
