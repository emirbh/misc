 ...
 * @version 2.1.1
 */
@RosettaDataRule("FixedPriceLegChoice3")
@ImplementedBy(FixedPriceLegChoice3.Default.class)
public interface FixedPriceLegChoice3 extends Validator<FixedPriceLeg> {
	
	String NAME = "FixedPriceLegChoice3";
	...
	class Default implements FixedPriceLegChoice3 {
		...
		private ComparisonResult executeDataRule(FixedPriceLeg fixedPriceLeg) {
			try {
				return choice(MapperS.of(fixedPriceLeg), Arrays.asList("notionalQuantitySchedule", "notionalQuantity", "settlementPeriodsNotionalQuantity", "quantityReference"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements FixedPriceLegChoice3 {
