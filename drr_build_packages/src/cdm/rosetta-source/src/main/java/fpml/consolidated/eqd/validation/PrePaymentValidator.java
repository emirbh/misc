...
public class PrePaymentValidator implements Validator<PrePayment> {
	...
	private List<ComparisonResult> getComparisonResults(PrePayment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("prePayment", (Boolean) o.getPrePayment() != null ? 1 : 0, 0, 1), 
				checkCardinality("prePaymentAmount", (NonNegativeMoney) o.getPrePaymentAmount() != null ? 1 : 0, 0, 1), 
				checkCardinality("prePaymentDate", (AdjustableDate) o.getPrePaymentDate() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrePayment", ValidationResult.ValidationType.CARDINALITY, "PrePayment", path, "", res.getError());
				}
				return success("PrePayment", ValidationResult.ValidationType.CARDINALITY, "PrePayment", path, "");
			})
			.collect(toList());
	}
