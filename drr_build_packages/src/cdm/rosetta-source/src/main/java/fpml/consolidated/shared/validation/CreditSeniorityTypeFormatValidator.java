		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditSeniority", ValidationResult.ValidationType.TYPE_FORMAT, "CreditSeniority", path, "", res.getError());
				}
				return success("CreditSeniority", ValidationResult.ValidationType.TYPE_FORMAT, "CreditSeniority", path, "");
			})
			.collect(toList());
	}
