		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeSummary", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeSummary", path, "", res.getError());
				}
				return success("LoanTradeSummary", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeSummary", path, "");
			})
			.collect(toList());
	}
