		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityStrike", ValidationResult.ValidationType.TYPE_FORMAT, "EquityStrike", path, "", res.getError());
				}
				return success("EquityStrike", ValidationResult.ValidationType.TYPE_FORMAT, "EquityStrike", path, "");
			})
			.collect(toList());
	}
