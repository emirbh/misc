		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReasonCode", ValidationResult.ValidationType.TYPE_FORMAT, "ReasonCode", path, "", res.getError());
				}
				return success("ReasonCode", ValidationResult.ValidationType.TYPE_FORMAT, "ReasonCode", path, "");
			})
			.collect(toList());
	}
