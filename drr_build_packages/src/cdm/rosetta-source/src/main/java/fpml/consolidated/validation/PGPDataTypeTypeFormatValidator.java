		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PGPDataType", ValidationResult.ValidationType.TYPE_FORMAT, "PGPDataType", path, "", res.getError());
				}
				return success("PGPDataType", ValidationResult.ValidationType.TYPE_FORMAT, "PGPDataType", path, "");
			})
			.collect(toList());
	}
