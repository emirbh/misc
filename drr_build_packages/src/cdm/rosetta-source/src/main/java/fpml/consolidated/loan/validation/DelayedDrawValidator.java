...
public class DelayedDrawValidator implements Validator<DelayedDraw> {
	...
	private List<ComparisonResult> getComparisonResults(DelayedDraw o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("mustDrawByDate", (ZonedDateTime) o.getMustDrawByDate() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DelayedDraw", ValidationResult.ValidationType.CARDINALITY, "DelayedDraw", path, "", res.getError());
				}
				return success("DelayedDraw", ValidationResult.ValidationType.CARDINALITY, "DelayedDraw", path, "");
			})
			.collect(toList());
	}
