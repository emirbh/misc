...
public class CreditEventExceptionTypeFormatValidator implements Validator<CreditEventException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventException", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventException", path, "", res.getError());
				}
				return success("CreditEventException", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventException", path, "");
			})
			.collect(toList());
	}
