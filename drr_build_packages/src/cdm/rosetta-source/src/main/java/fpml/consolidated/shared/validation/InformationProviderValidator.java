		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InformationProvider", ValidationResult.ValidationType.CARDINALITY, "InformationProvider", path, "", res.getError());
				}
				return success("InformationProvider", ValidationResult.ValidationType.CARDINALITY, "InformationProvider", path, "");
			})
			.collect(toList());
	}
