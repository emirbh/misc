		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("X509DataType", ValidationResult.ValidationType.TYPE_FORMAT, "X509DataType", path, "", res.getError());
				}
				return success("X509DataType", ValidationResult.ValidationType.TYPE_FORMAT, "X509DataType", path, "");
			})
			.collect(toList());
	}
