		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RetrievalMethodType", ValidationResult.ValidationType.CARDINALITY, "RetrievalMethodType", path, "", res.getError());
				}
				return success("RetrievalMethodType", ValidationResult.ValidationType.CARDINALITY, "RetrievalMethodType", path, "");
			})
			.collect(toList());
	}
