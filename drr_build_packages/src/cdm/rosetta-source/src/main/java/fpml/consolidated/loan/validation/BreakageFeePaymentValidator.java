...
public class BreakageFeePaymentValidator implements Validator<BreakageFeePayment> {
	...
	private List<ComparisonResult> getComparisonResults(BreakageFeePayment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("breakageFeeClaimDate", (ZonedDateTime) o.getBreakageFeeClaimDate() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BreakageFeePayment", ValidationResult.ValidationType.CARDINALITY, "BreakageFeePayment", path, "", res.getError());
				}
				return success("BreakageFeePayment", ValidationResult.ValidationType.CARDINALITY, "BreakageFeePayment", path, "");
			})
			.collect(toList());
	}
