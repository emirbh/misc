		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowNotional", ValidationResult.ValidationType.TYPE_FORMAT, "CashflowNotional", path, "", res.getError());
				}
				return success("CashflowNotional", ValidationResult.ValidationType.TYPE_FORMAT, "CashflowNotional", path, "");
			})
			.collect(toList());
	}
