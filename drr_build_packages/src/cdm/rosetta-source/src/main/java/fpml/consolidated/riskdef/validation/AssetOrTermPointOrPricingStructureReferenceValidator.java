		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetOrTermPointOrPricingStructureReference", ValidationResult.ValidationType.CARDINALITY, "AssetOrTermPointOrPricingStructureReference", path, "", res.getError());
				}
				return success("AssetOrTermPointOrPricingStructureReference", ValidationResult.ValidationType.CARDINALITY, "AssetOrTermPointOrPricingStructureReference", path, "");
			})
			.collect(toList());
	}
