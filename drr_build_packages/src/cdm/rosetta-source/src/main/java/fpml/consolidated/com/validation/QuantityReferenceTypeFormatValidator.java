		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuantityReference", ValidationResult.ValidationType.TYPE_FORMAT, "QuantityReference", path, "", res.getError());
				}
				return success("QuantityReference", ValidationResult.ValidationType.TYPE_FORMAT, "QuantityReference", path, "");
			})
			.collect(toList());
	}
