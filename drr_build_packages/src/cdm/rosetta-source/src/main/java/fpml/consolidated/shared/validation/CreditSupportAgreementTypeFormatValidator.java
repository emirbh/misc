		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditSupportAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "CreditSupportAgreement", path, "", res.getError());
				}
				return success("CreditSupportAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "CreditSupportAgreement", path, "");
			})
			.collect(toList());
	}
