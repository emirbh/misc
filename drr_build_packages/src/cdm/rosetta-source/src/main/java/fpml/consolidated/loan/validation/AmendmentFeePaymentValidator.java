		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AmendmentFeePayment", ValidationResult.ValidationType.CARDINALITY, "AmendmentFeePayment", path, "", res.getError());
				}
				return success("AmendmentFeePayment", ValidationResult.ValidationType.CARDINALITY, "AmendmentFeePayment", path, "");
			})
			.collect(toList());
	}
