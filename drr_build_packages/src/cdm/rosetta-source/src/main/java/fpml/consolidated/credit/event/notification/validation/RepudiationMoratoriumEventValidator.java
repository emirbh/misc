		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RepudiationMoratoriumEvent", ValidationResult.ValidationType.CARDINALITY, "RepudiationMoratoriumEvent", path, "", res.getError());
				}
				return success("RepudiationMoratoriumEvent", ValidationResult.ValidationType.CARDINALITY, "RepudiationMoratoriumEvent", path, "");
			})
			.collect(toList());
	}
