		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeterminationMethod", ValidationResult.ValidationType.CARDINALITY, "DeterminationMethod", path, "", res.getError());
				}
				return success("DeterminationMethod", ValidationResult.ValidationType.CARDINALITY, "DeterminationMethod", path, "");
			})
			.collect(toList());
	}
