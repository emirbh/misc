		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResetDatesReference", ValidationResult.ValidationType.CARDINALITY, "ResetDatesReference", path, "", res.getError());
				}
				return success("ResetDatesReference", ValidationResult.ValidationType.CARDINALITY, "ResetDatesReference", path, "");
			})
			.collect(toList());
	}
