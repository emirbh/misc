		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractMaturityChange", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractMaturityChange", path, "", res.getError());
				}
				return success("LoanContractMaturityChange", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractMaturityChange", path, "");
			})
			.collect(toList());
	}
