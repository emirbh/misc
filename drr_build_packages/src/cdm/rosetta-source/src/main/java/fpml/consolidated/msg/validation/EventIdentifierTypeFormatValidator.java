		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "EventIdentifier", path, "", res.getError());
				}
				return success("EventIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "EventIdentifier", path, "");
			})
			.collect(toList());
	}
