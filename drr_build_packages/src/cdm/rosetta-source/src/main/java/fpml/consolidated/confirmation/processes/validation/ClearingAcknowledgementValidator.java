...
public class ClearingAcknowledgementValidator implements Validator<ClearingAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "ClearingAcknowledgement", path, "", res.getError());
				}
				return success("ClearingAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "ClearingAcknowledgement", path, "");
			})
			.collect(toList());
	}
