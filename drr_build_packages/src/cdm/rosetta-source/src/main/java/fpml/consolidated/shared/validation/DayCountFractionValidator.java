		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DayCountFraction", ValidationResult.ValidationType.CARDINALITY, "DayCountFraction", path, "", res.getError());
				}
				return success("DayCountFraction", ValidationResult.ValidationType.CARDINALITY, "DayCountFraction", path, "");
			})
			.collect(toList());
	}
