		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasicQuotation", ValidationResult.ValidationType.TYPE_FORMAT, "BasicQuotation", path, "", res.getError());
				}
				return success("BasicQuotation", ValidationResult.ValidationType.TYPE_FORMAT, "BasicQuotation", path, "");
			})
			.collect(toList());
	}
