		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DealIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "DealIdentifier", path, "", res.getError());
				}
				return success("DealIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "DealIdentifier", path, "");
			})
			.collect(toList());
	}
