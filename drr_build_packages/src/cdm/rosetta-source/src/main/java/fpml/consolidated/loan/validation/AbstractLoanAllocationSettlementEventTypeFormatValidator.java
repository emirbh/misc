		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanAllocationSettlementEvent", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractLoanAllocationSettlementEvent", path, "", res.getError());
				}
				return success("AbstractLoanAllocationSettlementEvent", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractLoanAllocationSettlementEvent", path, "");
			})
			.collect(toList());
	}
