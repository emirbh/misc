...
public class ExecutionAdviceValidator implements Validator<ExecutionAdvice> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdvice", ValidationResult.ValidationType.CARDINALITY, "ExecutionAdvice", path, "", res.getError());
				}
				return success("ExecutionAdvice", ValidationResult.ValidationType.CARDINALITY, "ExecutionAdvice", path, "");
			})
			.collect(toList());
	}
