		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditDefaultSwapChoice", ValidationResult.ValidationType.CARDINALITY, "CreditDefaultSwapChoice", path, "", res.getError());
				}
				return success("CreditDefaultSwapChoice", ValidationResult.ValidationType.CARDINALITY, "CreditDefaultSwapChoice", path, "");
			})
			.collect(toList());
	}
