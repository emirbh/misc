		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeNovationContent", ValidationResult.ValidationType.TYPE_FORMAT, "TradeNovationContent", path, "", res.getError());
				}
				return success("TradeNovationContent", ValidationResult.ValidationType.TYPE_FORMAT, "TradeNovationContent", path, "");
			})
			.collect(toList());
	}
