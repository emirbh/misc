...
public class LoanAllocationFeeOwedValidator implements Validator<LoanAllocationFeeOwed> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationFeeOwed", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationFeeOwed", path, "", res.getError());
				}
				return success("LoanAllocationFeeOwed", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationFeeOwed", path, "");
			})
			.collect(toList());
	}
