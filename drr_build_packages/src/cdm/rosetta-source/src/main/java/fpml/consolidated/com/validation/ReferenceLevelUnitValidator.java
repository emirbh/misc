		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReferenceLevelUnit", ValidationResult.ValidationType.CARDINALITY, "ReferenceLevelUnit", path, "", res.getError());
				}
				return success("ReferenceLevelUnit", ValidationResult.ValidationType.CARDINALITY, "ReferenceLevelUnit", path, "");
			})
			.collect(toList());
	}
