...
public class GenericSecurityValidator implements Validator<GenericSecurity> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericSecurity", ValidationResult.ValidationType.CARDINALITY, "GenericSecurity", path, "", res.getError());
				}
				return success("GenericSecurity", ValidationResult.ValidationType.CARDINALITY, "GenericSecurity", path, "");
			})
			.collect(toList());
	}
