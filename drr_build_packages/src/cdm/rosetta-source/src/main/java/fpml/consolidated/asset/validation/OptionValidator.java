		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Option", ValidationResult.ValidationType.CARDINALITY, "Option", path, "", res.getError());
				}
				return success("Option", ValidationResult.ValidationType.CARDINALITY, "Option", path, "");
			})
			.collect(toList());
	}
