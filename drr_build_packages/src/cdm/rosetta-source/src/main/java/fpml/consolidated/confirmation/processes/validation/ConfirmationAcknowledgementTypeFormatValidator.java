...
public class ConfirmationAcknowledgementTypeFormatValidator implements Validator<ConfirmationAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationAcknowledgement", path, "", res.getError());
				}
				return success("ConfirmationAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationAcknowledgement", path, "");
			})
			.collect(toList());
	}
