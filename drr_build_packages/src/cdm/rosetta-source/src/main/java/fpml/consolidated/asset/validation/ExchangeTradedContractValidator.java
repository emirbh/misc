...
public class ExchangeTradedContractValidator implements Validator<ExchangeTradedContract> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTradedContract", ValidationResult.ValidationType.CARDINALITY, "ExchangeTradedContract", path, "", res.getError());
				}
				return success("ExchangeTradedContract", ValidationResult.ValidationType.CARDINALITY, "ExchangeTradedContract", path, "");
			})
			.collect(toList());
	}
