		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureProperty", ValidationResult.ValidationType.CARDINALITY, "SignatureProperty", path, "", res.getError());
				}
				return success("SignatureProperty", ValidationResult.ValidationType.CARDINALITY, "SignatureProperty", path, "");
			})
			.collect(toList());
	}
