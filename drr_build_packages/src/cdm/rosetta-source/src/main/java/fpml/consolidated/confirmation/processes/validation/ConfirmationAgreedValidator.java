		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationAgreed", ValidationResult.ValidationType.CARDINALITY, "ConfirmationAgreed", path, "", res.getError());
				}
				return success("ConfirmationAgreed", ValidationResult.ValidationType.CARDINALITY, "ConfirmationAgreed", path, "");
			})
			.collect(toList());
	}
