		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GasQuality", ValidationResult.ValidationType.TYPE_FORMAT, "GasQuality", path, "", res.getError());
				}
				return success("GasQuality", ValidationResult.ValidationType.TYPE_FORMAT, "GasQuality", path, "");
			})
			.collect(toList());
	}
