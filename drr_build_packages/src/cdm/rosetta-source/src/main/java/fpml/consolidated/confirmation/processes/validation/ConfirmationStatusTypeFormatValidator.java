...
public class ConfirmationStatusTypeFormatValidator implements Validator<ConfirmationStatus> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationStatus", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationStatus", path, "", res.getError());
				}
				return success("ConfirmationStatus", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationStatus", path, "");
			})
			.collect(toList());
	}
