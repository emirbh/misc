		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BermudaExercise", ValidationResult.ValidationType.CARDINALITY, "BermudaExercise", path, "", res.getError());
				}
				return success("BermudaExercise", ValidationResult.ValidationType.CARDINALITY, "BermudaExercise", path, "");
			})
			.collect(toList());
	}
