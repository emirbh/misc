		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTrade", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTrade", path, "", res.getError());
				}
				return success("LoanTrade", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTrade", path, "");
			})
			.collect(toList());
	}
