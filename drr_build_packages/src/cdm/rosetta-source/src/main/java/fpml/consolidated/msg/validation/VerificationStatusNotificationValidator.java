		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VerificationStatusNotification", ValidationResult.ValidationType.CARDINALITY, "VerificationStatusNotification", path, "", res.getError());
				}
				return success("VerificationStatusNotification", ValidationResult.ValidationType.CARDINALITY, "VerificationStatusNotification", path, "");
			})
			.collect(toList());
	}
