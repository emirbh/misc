		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EarlyTerminationProvision", ValidationResult.ValidationType.TYPE_FORMAT, "EarlyTerminationProvision", path, "", res.getError());
				}
				return success("EarlyTerminationProvision", ValidationResult.ValidationType.TYPE_FORMAT, "EarlyTerminationProvision", path, "");
			})
			.collect(toList());
	}
