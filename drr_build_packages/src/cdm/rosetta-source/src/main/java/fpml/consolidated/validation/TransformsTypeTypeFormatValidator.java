		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransformsType", ValidationResult.ValidationType.TYPE_FORMAT, "TransformsType", path, "", res.getError());
				}
				return success("TransformsType", ValidationResult.ValidationType.TYPE_FORMAT, "TransformsType", path, "");
			})
			.collect(toList());
	}
