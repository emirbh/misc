...
public class LoanTradeConfirmationValidator implements Validator<LoanTradeConfirmation> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeConfirmation", ValidationResult.ValidationType.CARDINALITY, "LoanTradeConfirmation", path, "", res.getError());
				}
				return success("LoanTradeConfirmation", ValidationResult.ValidationType.CARDINALITY, "LoanTradeConfirmation", path, "");
			})
			.collect(toList());
	}
