		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractPartyProfileId", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractPartyProfileId", path, "", res.getError());
				}
				return success("AbstractPartyProfileId", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractPartyProfileId", path, "");
			})
			.collect(toList());
	}
