		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonCorrectableRequestMessage", ValidationResult.ValidationType.TYPE_FORMAT, "NonCorrectableRequestMessage", path, "", res.getError());
				}
				return success("NonCorrectableRequestMessage", ValidationResult.ValidationType.TYPE_FORMAT, "NonCorrectableRequestMessage", path, "");
			})
			.collect(toList());
	}
