		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityFrequencyType", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityFrequencyType", path, "", res.getError());
				}
				return success("CommodityFrequencyType", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityFrequencyType", path, "");
			})
			.collect(toList());
	}
