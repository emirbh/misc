		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateLimits", ValidationResult.ValidationType.TYPE_FORMAT, "RateLimits", path, "", res.getError());
				}
				return success("RateLimits", ValidationResult.ValidationType.TYPE_FORMAT, "RateLimits", path, "");
			})
			.collect(toList());
	}
