...
public class AbstractLoanAllocationEventValidator implements Validator<AbstractLoanAllocationEvent> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanAllocationEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanAllocationEvent", path, "", res.getError());
				}
				return success("AbstractLoanAllocationEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanAllocationEvent", path, "");
			})
			.collect(toList());
	}
