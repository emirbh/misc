		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("YieldCurveValuation", ValidationResult.ValidationType.CARDINALITY, "YieldCurveValuation", path, "", res.getError());
				}
				return success("YieldCurveValuation", ValidationResult.ValidationType.CARDINALITY, "YieldCurveValuation", path, "");
			})
			.collect(toList());
	}
