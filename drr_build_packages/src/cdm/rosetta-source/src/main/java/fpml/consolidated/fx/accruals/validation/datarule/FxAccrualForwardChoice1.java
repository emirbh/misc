 ...
 * @version 2.1.1
 */
@RosettaDataRule("FxAccrualForwardChoice1")
@ImplementedBy(FxAccrualForwardChoice1.Default.class)
public interface FxAccrualForwardChoice1 extends Validator<FxAccrualForward> {
	
	String NAME = "FxAccrualForwardChoice1";
	String DEFINITION = "required choice settlementDate, settlementSchedule";
	
	class Default implements FxAccrualForwardChoice1 {
		...
		private ComparisonResult executeDataRule(FxAccrualForward fxAccrualForward) {
			try {
				return choice(MapperS.of(fxAccrualForward), Arrays.asList("settlementDate", "settlementSchedule"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements FxAccrualForwardChoice1 {
