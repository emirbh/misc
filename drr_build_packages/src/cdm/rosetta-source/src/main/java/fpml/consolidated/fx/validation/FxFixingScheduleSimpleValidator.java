		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFixingScheduleSimple", ValidationResult.ValidationType.CARDINALITY, "FxFixingScheduleSimple", path, "", res.getError());
				}
				return success("FxFixingScheduleSimple", ValidationResult.ValidationType.CARDINALITY, "FxFixingScheduleSimple", path, "");
			})
			.collect(toList());
	}
