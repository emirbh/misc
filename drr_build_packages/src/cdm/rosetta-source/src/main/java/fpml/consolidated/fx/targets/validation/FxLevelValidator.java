...
public class FxLevelValidator implements Validator<FxLevel> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxLevel", ValidationResult.ValidationType.CARDINALITY, "FxLevel", path, "", res.getError());
				}
				return success("FxLevel", ValidationResult.ValidationType.CARDINALITY, "FxLevel", path, "");
			})
			.collect(toList());
	}
