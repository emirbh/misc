...
public class FxExchangedCurrencyValidator implements Validator<FxExchangedCurrency> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxExchangedCurrency", ValidationResult.ValidationType.CARDINALITY, "FxExchangedCurrency", path, "", res.getError());
				}
				return success("FxExchangedCurrency", ValidationResult.ValidationType.CARDINALITY, "FxExchangedCurrency", path, "");
			})
			.collect(toList());
	}
