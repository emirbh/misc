...
public class ExecutionAcknowledgementValidator implements Validator<ExecutionAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "ExecutionAcknowledgement", path, "", res.getError());
				}
				return success("ExecutionAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "ExecutionAcknowledgement", path, "");
			})
			.collect(toList());
	}
