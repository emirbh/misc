...
public class StrikeSpreadValidator implements Validator<StrikeSpread> {
	...
	private List<ComparisonResult> getComparisonResults(StrikeSpread o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("upperStrike", (OptionStrike) o.getUpperStrike() != null ? 1 : 0, 0, 1), 
				checkCardinality("upperStrikeNumberOfOptions", (BigDecimal) o.getUpperStrikeNumberOfOptions() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StrikeSpread", ValidationResult.ValidationType.CARDINALITY, "StrikeSpread", path, "", res.getError());
				}
				return success("StrikeSpread", ValidationResult.ValidationType.CARDINALITY, "StrikeSpread", path, "");
			})
			.collect(toList());
	}
