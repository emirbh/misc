		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonPeriodicFixedPriceLeg", ValidationResult.ValidationType.TYPE_FORMAT, "NonPeriodicFixedPriceLeg", path, "", res.getError());
				}
				return success("NonPeriodicFixedPriceLeg", ValidationResult.ValidationType.TYPE_FORMAT, "NonPeriodicFixedPriceLeg", path, "");
			})
			.collect(toList());
	}
