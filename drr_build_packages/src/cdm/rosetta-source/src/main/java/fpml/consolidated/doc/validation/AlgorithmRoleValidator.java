		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AlgorithmRole", ValidationResult.ValidationType.CARDINALITY, "AlgorithmRole", path, "", res.getError());
				}
				return success("AlgorithmRole", ValidationResult.ValidationType.CARDINALITY, "AlgorithmRole", path, "");
			})
			.collect(toList());
	}
