		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcType", ValidationResult.ValidationType.TYPE_FORMAT, "LcType", path, "", res.getError());
				}
				return success("LcType", ValidationResult.ValidationType.TYPE_FORMAT, "LcType", path, "");
			})
			.collect(toList());
	}
