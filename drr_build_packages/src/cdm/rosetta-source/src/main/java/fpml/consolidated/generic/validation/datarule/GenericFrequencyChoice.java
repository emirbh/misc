...
public interface GenericFrequencyChoice extends Validator<GenericFrequency> {
	...
	class Default implements GenericFrequencyChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, GenericFrequency genericFrequency) {
			ComparisonResult result = executeDataRule(genericFrequency);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "GenericFrequency", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "GenericFrequency", path, DEFINITION, failureMessage));
		}
