...
public class TransformsTypeValidator implements Validator<TransformsType> {
	...
	private List<ComparisonResult> getComparisonResults(TransformsType o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("transform", (List<? extends TransformType>) o.getTransform() == null ? 0 : o.getTransform().size(), 1, 0)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransformsType", ValidationResult.ValidationType.CARDINALITY, "TransformsType", path, "", res.getError());
				}
				return success("TransformsType", ValidationResult.ValidationType.CARDINALITY, "TransformsType", path, "");
			})
			.collect(toList());
	}
