		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcTermination", ValidationResult.ValidationType.TYPE_FORMAT, "LcTermination", path, "", res.getError());
				}
				return success("LcTermination", ValidationResult.ValidationType.TYPE_FORMAT, "LcTermination", path, "");
			})
			.collect(toList());
	}
