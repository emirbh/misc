		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TimeDimension", ValidationResult.ValidationType.TYPE_FORMAT, "TimeDimension", path, "", res.getError());
				}
				return success("TimeDimension", ValidationResult.ValidationType.TYPE_FORMAT, "TimeDimension", path, "");
			})
			.collect(toList());
	}
