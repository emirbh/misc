...
public class EquityDerivativeLongFormBaseValidator implements Validator<EquityDerivativeLongFormBase> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityDerivativeLongFormBase", ValidationResult.ValidationType.CARDINALITY, "EquityDerivativeLongFormBase", path, "", res.getError());
				}
				return success("EquityDerivativeLongFormBase", ValidationResult.ValidationType.CARDINALITY, "EquityDerivativeLongFormBase", path, "");
			})
			.collect(toList());
	}
