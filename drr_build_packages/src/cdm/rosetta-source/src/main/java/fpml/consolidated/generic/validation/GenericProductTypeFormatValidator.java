		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericProduct", ValidationResult.ValidationType.TYPE_FORMAT, "GenericProduct", path, "", res.getError());
				}
				return success("GenericProduct", ValidationResult.ValidationType.TYPE_FORMAT, "GenericProduct", path, "");
			})
			.collect(toList());
	}
