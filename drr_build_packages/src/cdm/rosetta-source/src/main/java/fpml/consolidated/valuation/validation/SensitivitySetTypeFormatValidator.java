		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SensitivitySet", ValidationResult.ValidationType.TYPE_FORMAT, "SensitivitySet", path, "", res.getError());
				}
				return success("SensitivitySet", ValidationResult.ValidationType.TYPE_FORMAT, "SensitivitySet", path, "");
			})
			.collect(toList());
	}
