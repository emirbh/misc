...
public interface CalculationPeriodDatesChoice1 extends Validator<CalculationPeriodDates> {
	...
	String DEFINITION = "optional choice terminationDate, relativeTerminationDate";
	...
	class Default implements CalculationPeriodDatesChoice1 {
		...
		private ComparisonResult executeDataRule(CalculationPeriodDates calculationPeriodDates) {
			try {
				return choice(MapperS.of(calculationPeriodDates), Arrays.asList("terminationDate", "relativeTerminationDate"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
