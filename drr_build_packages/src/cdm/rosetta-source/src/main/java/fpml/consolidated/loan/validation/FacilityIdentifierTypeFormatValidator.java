		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityIdentifier", path, "", res.getError());
				}
				return success("FacilityIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityIdentifier", path, "");
			})
			.collect(toList());
	}
