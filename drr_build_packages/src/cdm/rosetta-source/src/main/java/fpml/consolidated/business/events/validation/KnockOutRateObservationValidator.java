...
public class KnockOutRateObservationValidator implements Validator<KnockOutRateObservation> {
	...
	private List<ComparisonResult> getComparisonResults(KnockOutRateObservation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("rebatePayment", (NonNegativePayment) o.getRebatePayment() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("KnockOutRateObservation", ValidationResult.ValidationType.CARDINALITY, "KnockOutRateObservation", path, "", res.getError());
				}
				return success("KnockOutRateObservation", ValidationResult.ValidationType.CARDINALITY, "KnockOutRateObservation", path, "");
			})
			.collect(toList());
	}
