 ...
 * @version 2.1.1
 */
@RosettaDataRule("ReferenceInformationChoice")
@ImplementedBy(ReferenceInformationChoice.Default.class)
public interface ReferenceInformationChoice extends Validator<ReferenceInformation> {
	
	String NAME = "ReferenceInformationChoice";
	String DEFINITION = "optional choice referenceObligation, noReferenceObligation, unknownReferenceObligation";
	
	class Default implements ReferenceInformationChoice {
		...
		private ComparisonResult executeDataRule(ReferenceInformation referenceInformation) {
			try {
				return choice(MapperS.of(referenceInformation), Arrays.asList("referenceObligation", "noReferenceObligation", "unknownReferenceObligation"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements ReferenceInformationChoice {
