...
public class MetalPhysicalLegValidator implements Validator<MetalPhysicalLeg> {
	...
	private List<ComparisonResult> getComparisonResults(MetalPhysicalLeg o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("metal", (Metal) o.getMetal() != null ? 1 : 0, 0, 1), 
				checkCardinality("deliveryPeriods", (CommodityDeliveryPeriods) o.getDeliveryPeriods() != null ? 1 : 0, 0, 1), 
				checkCardinality("deliveryConditions", (MetalDelivery) o.getDeliveryConditions() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MetalPhysicalLeg", ValidationResult.ValidationType.CARDINALITY, "MetalPhysicalLeg", path, "", res.getError());
				}
				return success("MetalPhysicalLeg", ValidationResult.ValidationType.CARDINALITY, "MetalPhysicalLeg", path, "");
			})
			.collect(toList());
	}
