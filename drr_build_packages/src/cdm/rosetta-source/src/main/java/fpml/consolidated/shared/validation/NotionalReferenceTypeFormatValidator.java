		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotionalReference", ValidationResult.ValidationType.TYPE_FORMAT, "NotionalReference", path, "", res.getError());
				}
				return success("NotionalReference", ValidationResult.ValidationType.TYPE_FORMAT, "NotionalReference", path, "");
			})
			.collect(toList());
	}
