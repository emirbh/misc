		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityFeature", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityFeature", path, "", res.getError());
				}
				return success("FacilityFeature", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityFeature", path, "");
			})
			.collect(toList());
	}
