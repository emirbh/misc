		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssociationToAssetId", ValidationResult.ValidationType.CARDINALITY, "AssociationToAssetId", path, "", res.getError());
				}
				return success("AssociationToAssetId", ValidationResult.ValidationType.CARDINALITY, "AssociationToAssetId", path, "");
			})
			.collect(toList());
	}
