		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DualCurrencyStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "DualCurrencyStrikePrice", path, "", res.getError());
				}
				return success("DualCurrencyStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "DualCurrencyStrikePrice", path, "");
			})
			.collect(toList());
	}
