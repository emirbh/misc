package fpml.consolidated.option.shared.meta;

import com.rosetta.model.lib.annotations.RosettaMeta;
import com.rosetta.model.lib.meta.RosettaMetaData;
import com.rosetta.model.lib.qualify.QualifyFunctionFactory;
import com.rosetta.model.lib.qualify.QualifyResult;
import com.rosetta.model.lib.validation.Validator;
import com.rosetta.model.lib.validation.ValidatorFactory;
import com.rosetta.model.lib.validation.ValidatorWithArg;
import fpml.consolidated.option.shared.Knock;
import fpml.consolidated.option.shared.validation.KnockTypeFormatValidator;
import fpml.consolidated.option.shared.validation.KnockValidator;
import fpml.consolidated.option.shared.validation.datarule.KnockChoice;
import fpml.consolidated.option.shared.validation.exists.KnockOnlyExistsValidator;
import java.util.Arrays;
import java.util.Collections;
import java.util.List;
import java.util.Set;
import java.util.function.Function;


/**
 * @version 2.1.1
 */
@RosettaMeta(model=Knock.class)
public class KnockMeta implements RosettaMetaData<Knock> {

	@Override
	public List<Validator<? super Knock>> dataRules(ValidatorFactory factory) {
		return Arrays.asList(
			factory.<Knock>create(KnockChoice.class)
		);
	}
	
	@Override
	public List<Function<? super Knock, QualifyResult>> getQualifyFunctions(QualifyFunctionFactory factory) {
		return Collections.emptyList();
	}
	
	@Override
	public Validator<? super Knock> validator(ValidatorFactory factory) {
		return factory.<Knock>create(KnockValidator.class);
	}

	@Override
	public Validator<? super Knock> typeFormatValidator(ValidatorFactory factory) {
		return factory.<Knock>create(KnockTypeFormatValidator.class);
	}

	@Deprecated
	@Override
	public Validator<? super Knock> validator() {
		return new KnockValidator();
	}

	@Deprecated
	@Override
	public Validator<? super Knock> typeFormatValidator() {
		return new KnockTypeFormatValidator();
	}
	
	@Override
	public ValidatorWithArg<? super Knock, Set<String>> onlyExistsValidator() {
		return new KnockOnlyExistsValidator();
	}
}
