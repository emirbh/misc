		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationEvent", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocationEvent", path, "", res.getError());
				}
				return success("LoanAllocationEvent", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocationEvent", path, "");
			})
			.collect(toList());
	}
