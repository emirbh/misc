		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnSwapLegUnderlyer", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnSwapLegUnderlyer", path, "", res.getError());
				}
				return success("ReturnSwapLegUnderlyer", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnSwapLegUnderlyer", path, "");
			})
			.collect(toList());
	}
