		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualAverageStrikeReference", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualAverageStrikeReference", path, "", res.getError());
				}
				return success("FxAccrualAverageStrikeReference", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualAverageStrikeReference", path, "");
			})
			.collect(toList());
	}
