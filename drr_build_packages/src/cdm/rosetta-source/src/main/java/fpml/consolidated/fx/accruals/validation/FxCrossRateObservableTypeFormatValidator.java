		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxCrossRateObservable", ValidationResult.ValidationType.TYPE_FORMAT, "FxCrossRateObservable", path, "", res.getError());
				}
				return success("FxCrossRateObservable", ValidationResult.ValidationType.TYPE_FORMAT, "FxCrossRateObservable", path, "");
			})
			.collect(toList());
	}
