		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingRefused", ValidationResult.ValidationType.CARDINALITY, "ClearingRefused", path, "", res.getError());
				}
				return success("ClearingRefused", ValidationResult.ValidationType.CARDINALITY, "ClearingRefused", path, "");
			})
			.collect(toList());
	}
