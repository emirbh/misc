		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxPivot", ValidationResult.ValidationType.CARDINALITY, "FxPivot", path, "", res.getError());
				}
				return success("FxPivot", ValidationResult.ValidationType.CARDINALITY, "FxPivot", path, "");
			})
			.collect(toList());
	}
