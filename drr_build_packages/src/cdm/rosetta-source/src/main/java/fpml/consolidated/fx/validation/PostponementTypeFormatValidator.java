...
public class PostponementTypeFormatValidator implements Validator<Postponement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Postponement", ValidationResult.ValidationType.TYPE_FORMAT, "Postponement", path, "", res.getError());
				}
				return success("Postponement", ValidationResult.ValidationType.TYPE_FORMAT, "Postponement", path, "");
			})
			.collect(toList());
	}
