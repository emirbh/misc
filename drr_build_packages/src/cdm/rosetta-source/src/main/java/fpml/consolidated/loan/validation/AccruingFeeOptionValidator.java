		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeeOption", ValidationResult.ValidationType.CARDINALITY, "AccruingFeeOption", path, "", res.getError());
				}
				return success("AccruingFeeOption", ValidationResult.ValidationType.CARDINALITY, "AccruingFeeOption", path, "");
			})
			.collect(toList());
	}
