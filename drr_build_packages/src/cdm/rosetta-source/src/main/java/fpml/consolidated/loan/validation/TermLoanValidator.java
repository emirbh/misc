		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TermLoan", ValidationResult.ValidationType.CARDINALITY, "TermLoan", path, "", res.getError());
				}
				return success("TermLoan", ValidationResult.ValidationType.CARDINALITY, "TermLoan", path, "");
			})
			.collect(toList());
	}
