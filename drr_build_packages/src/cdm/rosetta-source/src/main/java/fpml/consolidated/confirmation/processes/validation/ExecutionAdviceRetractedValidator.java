		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdviceRetracted", ValidationResult.ValidationType.CARDINALITY, "ExecutionAdviceRetracted", path, "", res.getError());
				}
				return success("ExecutionAdviceRetracted", ValidationResult.ValidationType.CARDINALITY, "ExecutionAdviceRetracted", path, "");
			})
			.collect(toList());
	}
