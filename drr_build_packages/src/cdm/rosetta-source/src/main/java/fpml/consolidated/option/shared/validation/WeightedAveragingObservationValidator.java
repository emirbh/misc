		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeightedAveragingObservation", ValidationResult.ValidationType.CARDINALITY, "WeightedAveragingObservation", path, "", res.getError());
				}
				return success("WeightedAveragingObservation", ValidationResult.ValidationType.CARDINALITY, "WeightedAveragingObservation", path, "");
			})
			.collect(toList());
	}
