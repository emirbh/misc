		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RelativeDateOffset", ValidationResult.ValidationType.TYPE_FORMAT, "RelativeDateOffset", path, "", res.getError());
				}
				return success("RelativeDateOffset", ValidationResult.ValidationType.TYPE_FORMAT, "RelativeDateOffset", path, "");
			})
			.collect(toList());
	}
