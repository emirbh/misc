		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionType", ValidationResult.ValidationType.CARDINALITY, "OptionType", path, "", res.getError());
				}
				return success("OptionType", ValidationResult.ValidationType.CARDINALITY, "OptionType", path, "");
			})
			.collect(toList());
	}
