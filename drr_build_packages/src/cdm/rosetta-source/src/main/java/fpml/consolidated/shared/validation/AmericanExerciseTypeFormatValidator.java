		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AmericanExercise", ValidationResult.ValidationType.TYPE_FORMAT, "AmericanExercise", path, "", res.getError());
				}
				return success("AmericanExercise", ValidationResult.ValidationType.TYPE_FORMAT, "AmericanExercise", path, "");
			})
			.collect(toList());
	}
