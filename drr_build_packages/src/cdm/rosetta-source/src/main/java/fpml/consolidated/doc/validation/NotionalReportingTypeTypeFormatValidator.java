		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotionalReportingType", ValidationResult.ValidationType.TYPE_FORMAT, "NotionalReportingType", path, "", res.getError());
				}
				return success("NotionalReportingType", ValidationResult.ValidationType.TYPE_FORMAT, "NotionalReportingType", path, "");
			})
			.collect(toList());
	}
