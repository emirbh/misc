		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentDates", ValidationResult.ValidationType.TYPE_FORMAT, "PaymentDates", path, "", res.getError());
				}
				return success("PaymentDates", ValidationResult.ValidationType.TYPE_FORMAT, "PaymentDates", path, "");
			})
			.collect(toList());
	}
