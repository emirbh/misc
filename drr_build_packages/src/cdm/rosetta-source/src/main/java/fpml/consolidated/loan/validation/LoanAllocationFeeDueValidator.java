...
public class LoanAllocationFeeDueValidator implements Validator<LoanAllocationFeeDue> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationFeeDue", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationFeeDue", path, "", res.getError());
				}
				return success("LoanAllocationFeeDue", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationFeeDue", path, "");
			})
			.collect(toList());
	}
