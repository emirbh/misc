		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ElectricityDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "ElectricityDelivery", path, "", res.getError());
				}
				return success("ElectricityDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "ElectricityDelivery", path, "");
			})
			.collect(toList());
	}
