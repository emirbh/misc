...
public class DealStatementTypeFormatValidator implements Validator<DealStatement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DealStatement", ValidationResult.ValidationType.TYPE_FORMAT, "DealStatement", path, "", res.getError());
				}
				return success("DealStatement", ValidationResult.ValidationType.TYPE_FORMAT, "DealStatement", path, "");
			})
			.collect(toList());
	}
