		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DSAKeyValue", ValidationResult.ValidationType.TYPE_FORMAT, "DSAKeyValue", path, "", res.getError());
				}
				return success("DSAKeyValue", ValidationResult.ValidationType.TYPE_FORMAT, "DSAKeyValue", path, "");
			})
			.collect(toList());
	}
