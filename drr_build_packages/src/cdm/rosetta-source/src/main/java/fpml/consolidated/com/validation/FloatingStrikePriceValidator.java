		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingStrikePrice", ValidationResult.ValidationType.CARDINALITY, "FloatingStrikePrice", path, "", res.getError());
				}
				return success("FloatingStrikePrice", ValidationResult.ValidationType.CARDINALITY, "FloatingStrikePrice", path, "");
			})
			.collect(toList());
	}
