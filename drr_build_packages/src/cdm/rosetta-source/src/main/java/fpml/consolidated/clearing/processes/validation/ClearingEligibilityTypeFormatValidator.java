...
public class ClearingEligibilityTypeFormatValidator implements Validator<ClearingEligibility> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingEligibility", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingEligibility", path, "", res.getError());
				}
				return success("ClearingEligibility", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingEligibility", path, "");
			})
			.collect(toList());
	}
