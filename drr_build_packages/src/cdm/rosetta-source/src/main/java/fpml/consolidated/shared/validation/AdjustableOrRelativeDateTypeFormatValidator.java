		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableOrRelativeDate", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableOrRelativeDate", path, "", res.getError());
				}
				return success("AdjustableOrRelativeDate", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableOrRelativeDate", path, "");
			})
			.collect(toList());
	}
