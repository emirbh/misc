		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementInstruction", path, "", res.getError());
				}
				return success("SettlementInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementInstruction", path, "");
			})
			.collect(toList());
	}
