...
public class FundingFeePaymentValidator implements Validator<FundingFeePayment> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FundingFeePayment", ValidationResult.ValidationType.CARDINALITY, "FundingFeePayment", path, "", res.getError());
				}
				return success("FundingFeePayment", ValidationResult.ValidationType.CARDINALITY, "FundingFeePayment", path, "");
			})
			.collect(toList());
	}
