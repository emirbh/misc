...
public class LegacyLoanInterestPaymentValidator implements Validator<LegacyLoanInterestPayment> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegacyLoanInterestPayment", ValidationResult.ValidationType.CARDINALITY, "LegacyLoanInterestPayment", path, "", res.getError());
				}
				return success("LegacyLoanInterestPayment", ValidationResult.ValidationType.CARDINALITY, "LegacyLoanInterestPayment", path, "");
			})
			.collect(toList());
	}
