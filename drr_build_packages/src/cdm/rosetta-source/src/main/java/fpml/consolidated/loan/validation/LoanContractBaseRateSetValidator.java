		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractBaseRateSet", ValidationResult.ValidationType.CARDINALITY, "LoanContractBaseRateSet", path, "", res.getError());
				}
				return success("LoanContractBaseRateSet", ValidationResult.ValidationType.CARDINALITY, "LoanContractBaseRateSet", path, "");
			})
			.collect(toList());
	}
