		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualTriggerReference", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualTriggerReference", path, "", res.getError());
				}
				return success("FxAccrualTriggerReference", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualTriggerReference", path, "");
			})
			.collect(toList());
	}
