		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CrossRate", ValidationResult.ValidationType.CARDINALITY, "CrossRate", path, "", res.getError());
				}
				return success("CrossRate", ValidationResult.ValidationType.CARDINALITY, "CrossRate", path, "");
			})
			.collect(toList());
	}
