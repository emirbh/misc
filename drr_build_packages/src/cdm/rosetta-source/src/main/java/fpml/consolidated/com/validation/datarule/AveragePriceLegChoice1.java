 ...
 * @version 2.1.1
 */
@RosettaDataRule("AveragePriceLegChoice1")
@ImplementedBy(AveragePriceLegChoice1.Default.class)
public interface AveragePriceLegChoice1 extends Validator<AveragePriceLeg> {
	
	String NAME = "AveragePriceLegChoice1";
	...
	class Default implements AveragePriceLegChoice1 {
		...
		private ComparisonResult executeDataRule(AveragePriceLeg averagePriceLeg) {
			try {
				return choice(MapperS.of(averagePriceLeg), Arrays.asList("calculationDates", "calculationPeriods", "calculationPeriodsSchedule", "calculationPeriodsReference", "calculationPeriodsScheduleReference", "calculationPeriodsDatesReference"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements AveragePriceLegChoice1 {
