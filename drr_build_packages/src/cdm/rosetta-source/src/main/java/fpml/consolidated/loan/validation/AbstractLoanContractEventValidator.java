...
public class AbstractLoanContractEventValidator implements Validator<AbstractLoanContractEvent> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanContractEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanContractEvent", path, "", res.getError());
				}
				return success("AbstractLoanContractEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanContractEvent", path, "");
			})
			.collect(toList());
	}
