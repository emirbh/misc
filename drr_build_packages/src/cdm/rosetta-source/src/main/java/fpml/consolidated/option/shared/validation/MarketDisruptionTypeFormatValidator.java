		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarketDisruption", ValidationResult.ValidationType.TYPE_FORMAT, "MarketDisruption", path, "", res.getError());
				}
				return success("MarketDisruption", ValidationResult.ValidationType.TYPE_FORMAT, "MarketDisruption", path, "");
			})
			.collect(toList());
	}
