		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractAdjustment", path, "", res.getError());
				}
				return success("LoanContractAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractAdjustment", path, "");
			})
			.collect(toList());
	}
