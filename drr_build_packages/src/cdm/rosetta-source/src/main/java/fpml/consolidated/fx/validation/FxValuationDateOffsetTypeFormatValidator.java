...
public class FxValuationDateOffsetTypeFormatValidator implements Validator<FxValuationDateOffset> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxValuationDateOffset", ValidationResult.ValidationType.TYPE_FORMAT, "FxValuationDateOffset", path, "", res.getError());
				}
				return success("FxValuationDateOffset", ValidationResult.ValidationType.TYPE_FORMAT, "FxValuationDateOffset", path, "");
			})
			.collect(toList());
	}
