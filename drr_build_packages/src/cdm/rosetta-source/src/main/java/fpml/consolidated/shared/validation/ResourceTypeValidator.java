		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResourceType", ValidationResult.ValidationType.CARDINALITY, "ResourceType", path, "", res.getError());
				}
				return success("ResourceType", ValidationResult.ValidationType.CARDINALITY, "ResourceType", path, "");
			})
			.collect(toList());
	}
