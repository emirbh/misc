...
public class EquityDerivativeShortFormBaseValidator implements Validator<EquityDerivativeShortFormBase> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityDerivativeShortFormBase", ValidationResult.ValidationType.CARDINALITY, "EquityDerivativeShortFormBase", path, "", res.getError());
				}
				return success("EquityDerivativeShortFormBase", ValidationResult.ValidationType.CARDINALITY, "EquityDerivativeShortFormBase", path, "");
			})
			.collect(toList());
	}
