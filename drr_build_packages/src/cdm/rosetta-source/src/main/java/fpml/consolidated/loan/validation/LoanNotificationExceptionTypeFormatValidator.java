		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanNotificationException", ValidationResult.ValidationType.TYPE_FORMAT, "LoanNotificationException", path, "", res.getError());
				}
				return success("LoanNotificationException", ValidationResult.ValidationType.TYPE_FORMAT, "LoanNotificationException", path, "");
			})
			.collect(toList());
	}
