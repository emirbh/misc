...
public class RegulatoryReportingProductValidator implements Validator<RegulatoryReportingProduct> {

	private List<ComparisonResult> getComparisonResults(RegulatoryReportingProduct o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, RegulatoryReportingProduct o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegulatoryReportingProduct", ValidationResult.ValidationType.CARDINALITY, "RegulatoryReportingProduct", path, "", res.getError());
				}
				return success("RegulatoryReportingProduct", ValidationResult.ValidationType.CARDINALITY, "RegulatoryReportingProduct", path, "");
			})
			.collect(toList());
	}
