		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementRateSource", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementRateSource", path, "", res.getError());
				}
				return success("SettlementRateSource", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementRateSource", path, "");
			})
			.collect(toList());
	}
