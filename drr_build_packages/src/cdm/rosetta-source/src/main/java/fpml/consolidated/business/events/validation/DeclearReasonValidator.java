		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeclearReason", ValidationResult.ValidationType.CARDINALITY, "DeclearReason", path, "", res.getError());
				}
				return success("DeclearReason", ValidationResult.ValidationType.CARDINALITY, "DeclearReason", path, "");
			})
			.collect(toList());
	}
