		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSettlementPeriodBarrier", ValidationResult.ValidationType.TYPE_FORMAT, "FxSettlementPeriodBarrier", path, "", res.getError());
				}
				return success("FxSettlementPeriodBarrier", ValidationResult.ValidationType.TYPE_FORMAT, "FxSettlementPeriodBarrier", path, "");
			})
			.collect(toList());
	}
