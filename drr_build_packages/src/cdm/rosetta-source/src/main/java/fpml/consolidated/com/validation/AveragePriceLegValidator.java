...
public class AveragePriceLegValidator implements Validator<AveragePriceLeg> {
	...
	private List<ComparisonResult> getComparisonResults(AveragePriceLeg o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("pricingStartDate", (AdjustableDate) o.getPricingStartDate() != null ? 1 : 0, 0, 1), 
				checkCardinality("calculation", (FloatingLegCalculation) o.getCalculation() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragePriceLeg", ValidationResult.ValidationType.CARDINALITY, "AveragePriceLeg", path, "", res.getError());
				}
				return success("AveragePriceLeg", ValidationResult.ValidationType.CARDINALITY, "AveragePriceLeg", path, "");
			})
			.collect(toList());
	}
