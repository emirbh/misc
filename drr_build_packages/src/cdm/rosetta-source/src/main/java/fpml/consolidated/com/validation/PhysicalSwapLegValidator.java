		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PhysicalSwapLeg", ValidationResult.ValidationType.CARDINALITY, "PhysicalSwapLeg", path, "", res.getError());
				}
				return success("PhysicalSwapLeg", ValidationResult.ValidationType.CARDINALITY, "PhysicalSwapLeg", path, "");
			})
			.collect(toList());
	}
