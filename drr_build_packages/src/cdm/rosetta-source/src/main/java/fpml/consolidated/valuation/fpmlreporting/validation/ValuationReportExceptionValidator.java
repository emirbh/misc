...
public class ValuationReportExceptionValidator implements Validator<ValuationReportException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationReportException", ValidationResult.ValidationType.CARDINALITY, "ValuationReportException", path, "", res.getError());
				}
				return success("ValuationReportException", ValidationResult.ValidationType.CARDINALITY, "ValuationReportException", path, "");
			})
			.collect(toList());
	}
