...
public class OrderIdentifierValidator implements Validator<OrderIdentifier> {
	...
	private List<ComparisonResult> getComparisonResults(OrderIdentifier o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("orderId", (OrderId) o.getOrderId() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OrderIdentifier", ValidationResult.ValidationType.CARDINALITY, "OrderIdentifier", path, "", res.getError());
				}
				return success("OrderIdentifier", ValidationResult.ValidationType.CARDINALITY, "OrderIdentifier", path, "");
			})
			.collect(toList());
	}
