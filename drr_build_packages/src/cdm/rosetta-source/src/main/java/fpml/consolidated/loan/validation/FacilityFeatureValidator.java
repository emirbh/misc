		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityFeature", ValidationResult.ValidationType.CARDINALITY, "FacilityFeature", path, "", res.getError());
				}
				return success("FacilityFeature", ValidationResult.ValidationType.CARDINALITY, "FacilityFeature", path, "");
			})
			.collect(toList());
	}
