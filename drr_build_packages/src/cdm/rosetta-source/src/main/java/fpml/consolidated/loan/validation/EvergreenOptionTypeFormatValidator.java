		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EvergreenOption", ValidationResult.ValidationType.TYPE_FORMAT, "EvergreenOption", path, "", res.getError());
				}
				return success("EvergreenOption", ValidationResult.ValidationType.TYPE_FORMAT, "EvergreenOption", path, "");
			})
			.collect(toList());
	}
