		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractAdjustment", ValidationResult.ValidationType.CARDINALITY, "LoanContractAdjustment", path, "", res.getError());
				}
				return success("LoanContractAdjustment", ValidationResult.ValidationType.CARDINALITY, "LoanContractAdjustment", path, "");
			})
			.collect(toList());
	}
