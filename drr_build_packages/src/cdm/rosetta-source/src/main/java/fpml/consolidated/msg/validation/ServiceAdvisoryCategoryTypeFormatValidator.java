		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ServiceAdvisoryCategory", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceAdvisoryCategory", path, "", res.getError());
				}
				return success("ServiceAdvisoryCategory", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceAdvisoryCategory", path, "");
			})
			.collect(toList());
	}
