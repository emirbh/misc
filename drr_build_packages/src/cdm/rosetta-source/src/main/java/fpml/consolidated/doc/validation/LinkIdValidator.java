		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LinkId", ValidationResult.ValidationType.CARDINALITY, "LinkId", path, "", res.getError());
				}
				return success("LinkId", ValidationResult.ValidationType.CARDINALITY, "LinkId", path, "");
			})
			.collect(toList());
	}
