		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingContext", ValidationResult.ValidationType.CARDINALITY, "PricingContext", path, "", res.getError());
				}
				return success("PricingContext", ValidationResult.ValidationType.CARDINALITY, "PricingContext", path, "");
			})
			.collect(toList());
	}
