...
public class CollateralAllocationRejectedTypeFormatValidator implements Validator<CollateralAllocationRejected> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationRejected", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralAllocationRejected", path, "", res.getError());
				}
				return success("CollateralAllocationRejected", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralAllocationRejected", path, "");
			})
			.collect(toList());
	}
