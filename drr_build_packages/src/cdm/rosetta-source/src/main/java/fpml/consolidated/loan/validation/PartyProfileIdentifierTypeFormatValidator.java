		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyProfileIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "PartyProfileIdentifier", path, "", res.getError());
				}
				return success("PartyProfileIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "PartyProfileIdentifier", path, "");
			})
			.collect(toList());
	}
