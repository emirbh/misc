...
public class UtilizationPeriodValidator implements Validator<UtilizationPeriod> {
	...
	private List<ComparisonResult> getComparisonResults(UtilizationPeriod o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("utilizationAmounts", (UtilizationAmounts) o.getUtilizationAmounts() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UtilizationPeriod", ValidationResult.ValidationType.CARDINALITY, "UtilizationPeriod", path, "", res.getError());
				}
				return success("UtilizationPeriod", ValidationResult.ValidationType.CARDINALITY, "UtilizationPeriod", path, "");
			})
			.collect(toList());
	}
