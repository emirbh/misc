		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DataProvider", ValidationResult.ValidationType.CARDINALITY, "DataProvider", path, "", res.getError());
				}
				return success("DataProvider", ValidationResult.ValidationType.CARDINALITY, "DataProvider", path, "");
			})
			.collect(toList());
	}
