		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocation", ValidationResult.ValidationType.CARDINALITY, "LoanAllocation", path, "", res.getError());
				}
				return success("LoanAllocation", ValidationResult.ValidationType.CARDINALITY, "LoanAllocation", path, "");
			})
			.collect(toList());
	}
