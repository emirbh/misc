		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeightedPartialDerivative", ValidationResult.ValidationType.TYPE_FORMAT, "WeightedPartialDerivative", path, "", res.getError());
				}
				return success("WeightedPartialDerivative", ValidationResult.ValidationType.TYPE_FORMAT, "WeightedPartialDerivative", path, "");
			})
			.collect(toList());
	}
