		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Validation", ValidationResult.ValidationType.TYPE_FORMAT, "Validation", path, "", res.getError());
				}
				return success("Validation", ValidationResult.ValidationType.TYPE_FORMAT, "Validation", path, "");
			})
			.collect(toList());
	}
