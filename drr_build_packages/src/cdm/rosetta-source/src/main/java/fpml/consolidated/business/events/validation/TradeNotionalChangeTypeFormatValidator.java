		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeNotionalChange", ValidationResult.ValidationType.TYPE_FORMAT, "TradeNotionalChange", path, "", res.getError());
				}
				return success("TradeNotionalChange", ValidationResult.ValidationType.TYPE_FORMAT, "TradeNotionalChange", path, "");
			})
			.collect(toList());
	}
