		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanTradeEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanTradeEvent", path, "", res.getError());
				}
				return success("AbstractLoanTradeEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanTradeEvent", path, "");
			})
			.collect(toList());
	}
