		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedPayerReceiver", ValidationResult.ValidationType.CARDINALITY, "IdentifiedPayerReceiver", path, "", res.getError());
				}
				return success("IdentifiedPayerReceiver", ValidationResult.ValidationType.CARDINALITY, "IdentifiedPayerReceiver", path, "");
			})
			.collect(toList());
	}
