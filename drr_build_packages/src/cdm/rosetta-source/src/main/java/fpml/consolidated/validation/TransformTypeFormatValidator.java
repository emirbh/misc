...
public class TransformTypeFormatValidator implements Validator<Transform> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Transform", ValidationResult.ValidationType.TYPE_FORMAT, "Transform", path, "", res.getError());
				}
				return success("Transform", ValidationResult.ValidationType.TYPE_FORMAT, "Transform", path, "");
			})
			.collect(toList());
	}
