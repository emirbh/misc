 ...
 * @version 2.1.1
 */
@RosettaDataRule("FxScheduleChoice6")
@ImplementedBy(FxScheduleChoice6.Default.class)
public interface FxScheduleChoice6 extends Validator<FxSchedule> {
	
	String NAME = "FxScheduleChoice6";
	String DEFINITION = "optional choice dateAdjustments, dayType";
	
	class Default implements FxScheduleChoice6 {
		...
		private ComparisonResult executeDataRule(FxSchedule fxSchedule) {
			try {
				return choice(MapperS.of(fxSchedule), Arrays.asList("dateAdjustments", "dayType"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements FxScheduleChoice6 {
