		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityLeg", ValidationResult.ValidationType.CARDINALITY, "VolatilityLeg", path, "", res.getError());
				}
				return success("VolatilityLeg", ValidationResult.ValidationType.CARDINALITY, "VolatilityLeg", path, "");
			})
			.collect(toList());
	}
