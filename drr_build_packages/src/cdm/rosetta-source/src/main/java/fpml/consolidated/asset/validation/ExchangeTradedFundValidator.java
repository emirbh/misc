...
public class ExchangeTradedFundValidator implements Validator<ExchangeTradedFund> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTradedFund", ValidationResult.ValidationType.CARDINALITY, "ExchangeTradedFund", path, "", res.getError());
				}
				return success("ExchangeTradedFund", ValidationResult.ValidationType.CARDINALITY, "ExchangeTradedFund", path, "");
			})
			.collect(toList());
	}
