		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingContext", ValidationResult.ValidationType.TYPE_FORMAT, "PricingContext", path, "", res.getError());
				}
				return success("PricingContext", ValidationResult.ValidationType.TYPE_FORMAT, "PricingContext", path, "");
			})
			.collect(toList());
	}
