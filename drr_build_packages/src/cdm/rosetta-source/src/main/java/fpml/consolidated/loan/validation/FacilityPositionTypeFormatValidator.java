		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityPosition", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityPosition", path, "", res.getError());
				}
				return success("FacilityPosition", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityPosition", path, "");
			})
			.collect(toList());
	}
