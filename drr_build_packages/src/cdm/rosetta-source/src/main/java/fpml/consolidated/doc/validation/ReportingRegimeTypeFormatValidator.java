		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingRegime", ValidationResult.ValidationType.TYPE_FORMAT, "ReportingRegime", path, "", res.getError());
				}
				return success("ReportingRegime", ValidationResult.ValidationType.TYPE_FORMAT, "ReportingRegime", path, "");
			})
			.collect(toList());
	}
