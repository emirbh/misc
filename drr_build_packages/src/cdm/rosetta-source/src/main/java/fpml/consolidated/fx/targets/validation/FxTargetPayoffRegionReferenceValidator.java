		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetPayoffRegionReference", ValidationResult.ValidationType.CARDINALITY, "FxTargetPayoffRegionReference", path, "", res.getError());
				}
				return success("FxTargetPayoffRegionReference", ValidationResult.ValidationType.CARDINALITY, "FxTargetPayoffRegionReference", path, "");
			})
			.collect(toList());
	}
