...
public class IdentifiedAssetWithPartyValidator implements Validator<IdentifiedAssetWithParty> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedAssetWithParty", ValidationResult.ValidationType.CARDINALITY, "IdentifiedAssetWithParty", path, "", res.getError());
				}
				return success("IdentifiedAssetWithParty", ValidationResult.ValidationType.CARDINALITY, "IdentifiedAssetWithParty", path, "");
			})
			.collect(toList());
	}
