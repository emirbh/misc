...
public class ConsentGrantedValidator implements Validator<ConsentGranted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentGranted", ValidationResult.ValidationType.CARDINALITY, "ConsentGranted", path, "", res.getError());
				}
				return success("ConsentGranted", ValidationResult.ValidationType.CARDINALITY, "ConsentGranted", path, "");
			})
			.collect(toList());
	}
