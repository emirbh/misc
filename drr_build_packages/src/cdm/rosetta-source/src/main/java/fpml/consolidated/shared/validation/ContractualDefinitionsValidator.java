		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContractualDefinitions", ValidationResult.ValidationType.CARDINALITY, "ContractualDefinitions", path, "", res.getError());
				}
				return success("ContractualDefinitions", ValidationResult.ValidationType.CARDINALITY, "ContractualDefinitions", path, "");
			})
			.collect(toList());
	}
