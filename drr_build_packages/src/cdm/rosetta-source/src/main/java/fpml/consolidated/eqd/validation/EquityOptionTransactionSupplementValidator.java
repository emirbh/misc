...
public class EquityOptionTransactionSupplementValidator implements Validator<EquityOptionTransactionSupplement> {
	...
	private List<ComparisonResult> getComparisonResults(EquityOptionTransactionSupplement o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("exchangeTradedContractNearest", (Boolean) o.getExchangeTradedContractNearest() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityOptionTransactionSupplement", ValidationResult.ValidationType.CARDINALITY, "EquityOptionTransactionSupplement", path, "", res.getError());
				}
				return success("EquityOptionTransactionSupplement", ValidationResult.ValidationType.CARDINALITY, "EquityOptionTransactionSupplement", path, "");
			})
			.collect(toList());
	}
