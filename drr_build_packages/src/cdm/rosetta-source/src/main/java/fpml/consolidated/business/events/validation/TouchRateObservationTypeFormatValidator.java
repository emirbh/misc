		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TouchRateObservation", ValidationResult.ValidationType.TYPE_FORMAT, "TouchRateObservation", path, "", res.getError());
				}
				return success("TouchRateObservation", ValidationResult.ValidationType.TYPE_FORMAT, "TouchRateObservation", path, "");
			})
			.collect(toList());
	}
