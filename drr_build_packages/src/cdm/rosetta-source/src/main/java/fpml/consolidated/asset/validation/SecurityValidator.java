		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Security", ValidationResult.ValidationType.CARDINALITY, "Security", path, "", res.getError());
				}
				return success("Security", ValidationResult.ValidationType.CARDINALITY, "Security", path, "");
			})
			.collect(toList());
	}
