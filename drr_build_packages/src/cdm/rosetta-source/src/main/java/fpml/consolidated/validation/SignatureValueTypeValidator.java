		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureValueType", ValidationResult.ValidationType.CARDINALITY, "SignatureValueType", path, "", res.getError());
				}
				return success("SignatureValueType", ValidationResult.ValidationType.CARDINALITY, "SignatureValueType", path, "");
			})
			.collect(toList());
	}
