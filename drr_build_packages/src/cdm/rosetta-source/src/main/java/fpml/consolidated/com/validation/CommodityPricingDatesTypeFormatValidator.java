		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPricingDates", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityPricingDates", path, "", res.getError());
				}
				return success("CommodityPricingDates", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityPricingDates", path, "");
			})
			.collect(toList());
	}
