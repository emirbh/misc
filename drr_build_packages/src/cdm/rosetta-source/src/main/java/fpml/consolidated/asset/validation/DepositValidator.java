		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Deposit", ValidationResult.ValidationType.CARDINALITY, "Deposit", path, "", res.getError());
				}
				return success("Deposit", ValidationResult.ValidationType.CARDINALITY, "Deposit", path, "");
			})
			.collect(toList());
	}
