		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrectableRequestMessage", ValidationResult.ValidationType.TYPE_FORMAT, "CorrectableRequestMessage", path, "", res.getError());
				}
				return success("CorrectableRequestMessage", ValidationResult.ValidationType.TYPE_FORMAT, "CorrectableRequestMessage", path, "");
			})
			.collect(toList());
	}
