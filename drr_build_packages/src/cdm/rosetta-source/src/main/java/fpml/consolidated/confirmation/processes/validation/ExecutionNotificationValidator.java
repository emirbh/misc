...
public class ExecutionNotificationValidator implements Validator<ExecutionNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionNotification", ValidationResult.ValidationType.CARDINALITY, "ExecutionNotification", path, "", res.getError());
				}
				return success("ExecutionNotification", ValidationResult.ValidationType.CARDINALITY, "ExecutionNotification", path, "");
			})
			.collect(toList());
	}
