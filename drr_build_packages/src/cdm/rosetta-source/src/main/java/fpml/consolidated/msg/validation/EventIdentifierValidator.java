		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventIdentifier", ValidationResult.ValidationType.CARDINALITY, "EventIdentifier", path, "", res.getError());
				}
				return success("EventIdentifier", ValidationResult.ValidationType.CARDINALITY, "EventIdentifier", path, "");
			})
			.collect(toList());
	}
