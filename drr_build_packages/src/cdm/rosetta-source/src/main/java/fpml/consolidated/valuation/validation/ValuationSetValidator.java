		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationSet", ValidationResult.ValidationType.CARDINALITY, "ValuationSet", path, "", res.getError());
				}
				return success("ValuationSet", ValidationResult.ValidationType.CARDINALITY, "ValuationSet", path, "");
			})
			.collect(toList());
	}
