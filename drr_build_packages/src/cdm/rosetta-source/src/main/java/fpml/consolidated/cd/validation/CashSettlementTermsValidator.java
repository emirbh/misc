		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashSettlementTerms", ValidationResult.ValidationType.CARDINALITY, "CashSettlementTerms", path, "", res.getError());
				}
				return success("CashSettlementTerms", ValidationResult.ValidationType.CARDINALITY, "CashSettlementTerms", path, "");
			})
			.collect(toList());
	}
