		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationDisputed", ValidationResult.ValidationType.CARDINALITY, "ConfirmationDisputed", path, "", res.getError());
				}
				return success("ConfirmationDisputed", ValidationResult.ValidationType.CARDINALITY, "ConfirmationDisputed", path, "");
			})
			.collect(toList());
	}
