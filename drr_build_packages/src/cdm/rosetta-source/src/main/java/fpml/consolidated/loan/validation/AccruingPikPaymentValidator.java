		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingPikPayment", ValidationResult.ValidationType.CARDINALITY, "AccruingPikPayment", path, "", res.getError());
				}
				return success("AccruingPikPayment", ValidationResult.ValidationType.CARDINALITY, "AccruingPikPayment", path, "");
			})
			.collect(toList());
	}
