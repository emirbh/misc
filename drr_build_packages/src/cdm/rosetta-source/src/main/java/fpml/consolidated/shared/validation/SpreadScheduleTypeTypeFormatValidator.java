		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SpreadScheduleType", ValidationResult.ValidationType.TYPE_FORMAT, "SpreadScheduleType", path, "", res.getError());
				}
				return success("SpreadScheduleType", ValidationResult.ValidationType.TYPE_FORMAT, "SpreadScheduleType", path, "");
			})
			.collect(toList());
	}
