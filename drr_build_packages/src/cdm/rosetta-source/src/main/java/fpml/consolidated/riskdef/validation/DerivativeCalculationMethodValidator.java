		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DerivativeCalculationMethod", ValidationResult.ValidationType.CARDINALITY, "DerivativeCalculationMethod", path, "", res.getError());
				}
				return success("DerivativeCalculationMethod", ValidationResult.ValidationType.CARDINALITY, "DerivativeCalculationMethod", path, "");
			})
			.collect(toList());
	}
