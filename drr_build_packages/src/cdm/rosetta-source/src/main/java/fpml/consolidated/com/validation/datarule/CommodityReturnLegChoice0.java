 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityReturnLegChoice0")
@ImplementedBy(CommodityReturnLegChoice0.Default.class)
public interface CommodityReturnLegChoice0 extends Validator<CommodityReturnLeg> {
	
	String NAME = "CommodityReturnLegChoice0";
	String DEFINITION = "required choice relativePaymentDates, paymentDates, masterAgreementPaymentDates";
	
	class Default implements CommodityReturnLegChoice0 {
		...
		private ComparisonResult executeDataRule(CommodityReturnLeg commodityReturnLeg) {
			try {
				return choice(MapperS.of(commodityReturnLeg), Arrays.asList("relativePaymentDates", "paymentDates", "masterAgreementPaymentDates"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements CommodityReturnLegChoice0 {
