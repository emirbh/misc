		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxVolatilitySwap", ValidationResult.ValidationType.CARDINALITY, "FxVolatilitySwap", path, "", res.getError());
				}
				return success("FxVolatilitySwap", ValidationResult.ValidationType.CARDINALITY, "FxVolatilitySwap", path, "");
			})
			.collect(toList());
	}
