		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Quotation", ValidationResult.ValidationType.CARDINALITY, "Quotation", path, "", res.getError());
				}
				return success("Quotation", ValidationResult.ValidationType.CARDINALITY, "Quotation", path, "");
			})
			.collect(toList());
	}
