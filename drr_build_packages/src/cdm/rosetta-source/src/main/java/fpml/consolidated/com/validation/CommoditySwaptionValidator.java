...
public class CommoditySwaptionValidator implements Validator<CommoditySwaption> {
	...
	private List<ComparisonResult> getComparisonResults(CommoditySwaption o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("commoditySwap", (CommoditySwaptionUnderlying) o.getCommoditySwap() != null ? 1 : 0, 1, 1), 
				checkCardinality("physicalExercise", (CommodityPhysicalExercise) o.getPhysicalExercise() != null ? 1 : 0, 0, 1), 
				checkCardinality("premium", (CommodityPremium) o.getPremium() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySwaption", ValidationResult.ValidationType.CARDINALITY, "CommoditySwaption", path, "", res.getError());
				}
				return success("CommoditySwaption", ValidationResult.ValidationType.CARDINALITY, "CommoditySwaption", path, "");
			})
			.collect(toList());
	}
