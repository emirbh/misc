		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CountryCode", ValidationResult.ValidationType.TYPE_FORMAT, "CountryCode", path, "", res.getError());
				}
				return success("CountryCode", ValidationResult.ValidationType.TYPE_FORMAT, "CountryCode", path, "");
			})
			.collect(toList());
	}
