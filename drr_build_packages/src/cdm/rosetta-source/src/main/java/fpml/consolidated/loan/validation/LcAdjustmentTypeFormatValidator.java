		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "LcAdjustment", path, "", res.getError());
				}
				return success("LcAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "LcAdjustment", path, "");
			})
			.collect(toList());
	}
