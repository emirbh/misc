...
public class LoanAllocationConfirmationValidator implements Validator<LoanAllocationConfirmation> {

	private List<ComparisonResult> getComparisonResults(LoanAllocationConfirmation o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanAllocationConfirmation o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationConfirmation", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationConfirmation", path, "", res.getError());
				}
				return success("LoanAllocationConfirmation", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationConfirmation", path, "");
			})
			.collect(toList());
	}
