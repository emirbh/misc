		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSettlementSchedule", ValidationResult.ValidationType.CARDINALITY, "FxSettlementSchedule", path, "", res.getError());
				}
				return success("FxSettlementSchedule", ValidationResult.ValidationType.CARDINALITY, "FxSettlementSchedule", path, "");
			})
			.collect(toList());
	}
