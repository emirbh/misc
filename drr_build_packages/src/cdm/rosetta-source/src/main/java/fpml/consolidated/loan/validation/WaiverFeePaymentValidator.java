...
public class WaiverFeePaymentValidator implements Validator<WaiverFeePayment> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WaiverFeePayment", ValidationResult.ValidationType.CARDINALITY, "WaiverFeePayment", path, "", res.getError());
				}
				return success("WaiverFeePayment", ValidationResult.ValidationType.CARDINALITY, "WaiverFeePayment", path, "");
			})
			.collect(toList());
	}
