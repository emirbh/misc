...
public class RequestClearingRetractedValidator implements Validator<RequestClearingRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestClearingRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestClearingRetracted", path, "", res.getError());
				}
				return success("RequestClearingRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestClearingRetracted", path, "");
			})
			.collect(toList());
	}
