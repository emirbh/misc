		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DSAKeyValue", ValidationResult.ValidationType.CARDINALITY, "DSAKeyValue", path, "", res.getError());
				}
				return success("DSAKeyValue", ValidationResult.ValidationType.CARDINALITY, "DSAKeyValue", path, "");
			})
			.collect(toList());
	}
