		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFixing", ValidationResult.ValidationType.CARDINALITY, "FxFixing", path, "", res.getError());
				}
				return success("FxFixing", ValidationResult.ValidationType.CARDINALITY, "FxFixing", path, "");
			})
			.collect(toList());
	}
