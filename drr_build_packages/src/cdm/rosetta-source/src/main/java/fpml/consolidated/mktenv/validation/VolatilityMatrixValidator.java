		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityMatrix", ValidationResult.ValidationType.CARDINALITY, "VolatilityMatrix", path, "", res.getError());
				}
				return success("VolatilityMatrix", ValidationResult.ValidationType.CARDINALITY, "VolatilityMatrix", path, "");
			})
			.collect(toList());
	}
