		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentDatesReference", ValidationResult.ValidationType.CARDINALITY, "PaymentDatesReference", path, "", res.getError());
				}
				return success("PaymentDatesReference", ValidationResult.ValidationType.CARDINALITY, "PaymentDatesReference", path, "");
			})
			.collect(toList());
	}
