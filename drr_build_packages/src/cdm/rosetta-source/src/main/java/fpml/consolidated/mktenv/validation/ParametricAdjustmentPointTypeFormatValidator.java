		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ParametricAdjustmentPoint", ValidationResult.ValidationType.TYPE_FORMAT, "ParametricAdjustmentPoint", path, "", res.getError());
				}
				return success("ParametricAdjustmentPoint", ValidationResult.ValidationType.TYPE_FORMAT, "ParametricAdjustmentPoint", path, "");
			})
			.collect(toList());
	}
