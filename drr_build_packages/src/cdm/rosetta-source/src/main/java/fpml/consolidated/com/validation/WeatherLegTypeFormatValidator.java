		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeatherLeg", ValidationResult.ValidationType.TYPE_FORMAT, "WeatherLeg", path, "", res.getError());
				}
				return success("WeatherLeg", ValidationResult.ValidationType.TYPE_FORMAT, "WeatherLeg", path, "");
			})
			.collect(toList());
	}
