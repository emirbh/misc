...
public class LcRenewalValidator implements Validator<LcRenewal> {
	...
	private List<ComparisonResult> getComparisonResults(LcRenewal o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("finalExpiryDate", (ZonedDateTime) o.getFinalExpiryDate() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcRenewal", ValidationResult.ValidationType.CARDINALITY, "LcRenewal", path, "", res.getError());
				}
				return success("LcRenewal", ValidationResult.ValidationType.CARDINALITY, "LcRenewal", path, "");
			})
			.collect(toList());
	}
