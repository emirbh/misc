		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasicAssetValuation", ValidationResult.ValidationType.TYPE_FORMAT, "BasicAssetValuation", path, "", res.getError());
				}
				return success("BasicAssetValuation", ValidationResult.ValidationType.TYPE_FORMAT, "BasicAssetValuation", path, "");
			})
			.collect(toList());
	}
