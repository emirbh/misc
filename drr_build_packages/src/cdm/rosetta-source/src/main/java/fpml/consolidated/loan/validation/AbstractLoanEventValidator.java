		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanEvent", path, "", res.getError());
				}
				return success("AbstractLoanEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanEvent", path, "");
			})
			.collect(toList());
	}
