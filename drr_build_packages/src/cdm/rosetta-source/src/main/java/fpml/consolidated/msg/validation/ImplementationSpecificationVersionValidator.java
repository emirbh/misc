		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ImplementationSpecificationVersion", ValidationResult.ValidationType.CARDINALITY, "ImplementationSpecificationVersion", path, "", res.getError());
				}
				return success("ImplementationSpecificationVersion", ValidationResult.ValidationType.CARDINALITY, "ImplementationSpecificationVersion", path, "");
			})
			.collect(toList());
	}
