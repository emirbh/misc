		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyMessageInformation", ValidationResult.ValidationType.TYPE_FORMAT, "PartyMessageInformation", path, "", res.getError());
				}
				return success("PartyMessageInformation", ValidationResult.ValidationType.TYPE_FORMAT, "PartyMessageInformation", path, "");
			})
			.collect(toList());
	}
