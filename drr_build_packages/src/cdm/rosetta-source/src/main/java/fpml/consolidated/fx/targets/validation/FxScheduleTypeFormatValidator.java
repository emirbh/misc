		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "FxSchedule", path, "", res.getError());
				}
				return success("FxSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "FxSchedule", path, "");
			})
			.collect(toList());
	}
