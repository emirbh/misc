		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxStrikeReference", ValidationResult.ValidationType.CARDINALITY, "FxStrikeReference", path, "", res.getError());
				}
				return success("FxStrikeReference", ValidationResult.ValidationType.CARDINALITY, "FxStrikeReference", path, "");
			})
			.collect(toList());
	}
