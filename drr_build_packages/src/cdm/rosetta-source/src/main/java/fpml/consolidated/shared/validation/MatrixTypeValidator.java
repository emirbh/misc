		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MatrixType", ValidationResult.ValidationType.CARDINALITY, "MatrixType", path, "", res.getError());
				}
				return success("MatrixType", ValidationResult.ValidationType.CARDINALITY, "MatrixType", path, "");
			})
			.collect(toList());
	}
