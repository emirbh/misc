		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityCommitment", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityCommitment", path, "", res.getError());
				}
				return success("FacilityCommitment", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityCommitment", path, "");
			})
			.collect(toList());
	}
