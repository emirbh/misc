...
public interface AllocationChoice extends Validator<Allocation> {
	...
	String DEFINITION = "optional choice allocatedFraction, allocatedNotional";
	...
	class Default implements AllocationChoice {
		...
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, Allocation allocation) {
			...
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Allocation", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "Allocation", path, DEFINITION, failureMessage));
		}
		
		private ComparisonResult executeDataRule(Allocation allocation) {
			try {
				return choice(MapperS.of(allocation), Arrays.asList("allocatedFraction", "allocatedNotional"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
