		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractTradingAccrual", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractTradingAccrual", path, "", res.getError());
				}
				return success("AbstractTradingAccrual", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractTradingAccrual", path, "");
			})
			.collect(toList());
	}
