		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VarianceSwap", ValidationResult.ValidationType.CARDINALITY, "VarianceSwap", path, "", res.getError());
				}
				return success("VarianceSwap", ValidationResult.ValidationType.CARDINALITY, "VarianceSwap", path, "");
			})
			.collect(toList());
	}
