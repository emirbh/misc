...
public class AllocationApprovedTypeFormatValidator implements Validator<AllocationApproved> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationApproved", ValidationResult.ValidationType.TYPE_FORMAT, "AllocationApproved", path, "", res.getError());
				}
				return success("AllocationApproved", ValidationResult.ValidationType.TYPE_FORMAT, "AllocationApproved", path, "");
			})
			.collect(toList());
	}
