		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeeChange", ValidationResult.ValidationType.CARDINALITY, "AccruingFeeChange", path, "", res.getError());
				}
				return success("AccruingFeeChange", ValidationResult.ValidationType.CARDINALITY, "AccruingFeeChange", path, "");
			})
			.collect(toList());
	}
