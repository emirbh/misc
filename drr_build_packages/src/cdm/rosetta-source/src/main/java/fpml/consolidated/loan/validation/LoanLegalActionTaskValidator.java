...
public class LoanLegalActionTaskValidator implements Validator<LoanLegalActionTask> {
	...
	private List<ComparisonResult> getComparisonResults(LoanLegalActionTask o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("type", (LoanLegalActionTaskType) o._getType() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanLegalActionTask", ValidationResult.ValidationType.CARDINALITY, "LoanLegalActionTask", path, "", res.getError());
				}
				return success("LoanLegalActionTask", ValidationResult.ValidationType.CARDINALITY, "LoanLegalActionTask", path, "");
			})
			.collect(toList());
	}
