 ...
 * @version 2.1.1
 */
@RosettaDataRule("TradeNovationContentChoice5")
@ImplementedBy(TradeNovationContentChoice5.Default.class)
public interface TradeNovationContentChoice5 extends Validator<TradeNovationContent> {
	
	String NAME = "TradeNovationContentChoice5";
	String DEFINITION = "if novationAmount exists then remainingAmount is absent and remainingNumberOfOptions is absent and remainingNumberOfUnits is absent and optional choice oldTradeIdentifier, oldTrade and if oldTradeIdentifier exists then optional choice feeTradeIdentifier, feeTrade and if oldTrade exists then optional choice feeTradeIdentifier, feeTrade and if oldTradeIdentifier is absent and oldTrade is absent then optional choice feeTradeIdentifier, feeTrade";
	
	class Default implements TradeNovationContentChoice5 {
		...
		private ComparisonResult executeDataRule(TradeNovationContent tradeNovationContent) {
			try {
				if (exists(MapperS.of(tradeNovationContent).<TradeLegSizeChange>mapC("getNovationAmount", _tradeNovationContent -> _tradeNovationContent.getNovationAmount())).getOrDefault(false)) {
	...
	class NoOp implements TradeNovationContentChoice5 {
