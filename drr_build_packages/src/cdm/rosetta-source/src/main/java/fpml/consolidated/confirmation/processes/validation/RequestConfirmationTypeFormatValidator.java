...
public class RequestConfirmationTypeFormatValidator implements Validator<RequestConfirmation> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConfirmation", ValidationResult.ValidationType.TYPE_FORMAT, "RequestConfirmation", path, "", res.getError());
				}
				return success("RequestConfirmation", ValidationResult.ValidationType.TYPE_FORMAT, "RequestConfirmation", path, "");
			})
			.collect(toList());
	}
