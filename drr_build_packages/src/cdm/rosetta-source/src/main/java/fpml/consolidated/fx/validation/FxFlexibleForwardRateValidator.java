		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFlexibleForwardRate", ValidationResult.ValidationType.CARDINALITY, "FxFlexibleForwardRate", path, "", res.getError());
				}
				return success("FxFlexibleForwardRate", ValidationResult.ValidationType.CARDINALITY, "FxFlexibleForwardRate", path, "");
			})
			.collect(toList());
	}
