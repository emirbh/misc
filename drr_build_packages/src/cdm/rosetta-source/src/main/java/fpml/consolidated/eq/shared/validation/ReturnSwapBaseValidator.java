...
public class ReturnSwapBaseValidator implements Validator<ReturnSwapBase> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnSwapBase", ValidationResult.ValidationType.CARDINALITY, "ReturnSwapBase", path, "", res.getError());
				}
				return success("ReturnSwapBase", ValidationResult.ValidationType.CARDINALITY, "ReturnSwapBase", path, "");
			})
			.collect(toList());
	}
