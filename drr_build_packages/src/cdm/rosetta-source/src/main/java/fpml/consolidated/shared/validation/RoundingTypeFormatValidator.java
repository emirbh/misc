		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Rounding", ValidationResult.ValidationType.TYPE_FORMAT, "Rounding", path, "", res.getError());
				}
				return success("Rounding", ValidationResult.ValidationType.TYPE_FORMAT, "Rounding", path, "");
			})
			.collect(toList());
	}
