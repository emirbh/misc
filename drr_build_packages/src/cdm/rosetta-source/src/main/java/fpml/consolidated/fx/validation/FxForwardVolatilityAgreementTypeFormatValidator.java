		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxForwardVolatilityAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "FxForwardVolatilityAgreement", path, "", res.getError());
				}
				return success("FxForwardVolatilityAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "FxForwardVolatilityAgreement", path, "");
			})
			.collect(toList());
	}
