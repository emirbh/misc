 ...
 * @version 2.1.1
 */
@RosettaDataRule("LcNotificationChoice0")
@ImplementedBy(LcNotificationChoice0.Default.class)
public interface LcNotificationChoice0 extends Validator<LcNotification> {
	
	String NAME = "LcNotificationChoice0";
	String DEFINITION = "optional choice dealIdentifier, dealSummary";
	
	class Default implements LcNotificationChoice0 {
		...
		private ComparisonResult executeDataRule(LcNotification lcNotification) {
			try {
				return choice(MapperS.of(lcNotification), Arrays.asList("dealIdentifier", "dealSummary"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements LcNotificationChoice0 {
