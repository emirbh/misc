		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeeType", ValidationResult.ValidationType.TYPE_FORMAT, "AccruingFeeType", path, "", res.getError());
				}
				return success("AccruingFeeType", ValidationResult.ValidationType.TYPE_FORMAT, "AccruingFeeType", path, "");
			})
			.collect(toList());
	}
