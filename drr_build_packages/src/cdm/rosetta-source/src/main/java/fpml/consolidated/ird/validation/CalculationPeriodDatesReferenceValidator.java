		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodDatesReference", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriodDatesReference", path, "", res.getError());
				}
				return success("CalculationPeriodDatesReference", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriodDatesReference", path, "");
			})
			.collect(toList());
	}
