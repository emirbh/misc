		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnSwapAmount", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnSwapAmount", path, "", res.getError());
				}
				return success("ReturnSwapAmount", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnSwapAmount", path, "");
			})
			.collect(toList());
	}
