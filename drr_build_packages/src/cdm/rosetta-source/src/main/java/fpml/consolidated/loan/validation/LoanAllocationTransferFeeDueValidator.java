...
public class LoanAllocationTransferFeeDueValidator implements Validator<LoanAllocationTransferFeeDue> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationTransferFeeDue", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationTransferFeeDue", path, "", res.getError());
				}
				return success("LoanAllocationTransferFeeDue", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationTransferFeeDue", path, "");
			})
			.collect(toList());
	}
