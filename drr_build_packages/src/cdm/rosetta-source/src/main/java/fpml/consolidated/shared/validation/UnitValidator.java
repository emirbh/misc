		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Unit", ValidationResult.ValidationType.CARDINALITY, "Unit", path, "", res.getError());
				}
				return success("Unit", ValidationResult.ValidationType.CARDINALITY, "Unit", path, "");
			})
			.collect(toList());
	}
