...
public interface CommodityPhysicalExerciseChoice extends Validator<CommodityPhysicalExercise> {
	...
	class Default implements CommodityPhysicalExerciseChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, CommodityPhysicalExercise commodityPhysicalExercise) {
			ComparisonResult result = executeDataRule(commodityPhysicalExercise);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "CommodityPhysicalExercise", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "CommodityPhysicalExercise", path, DEFINITION, failureMessage));
		}
		
		private ComparisonResult executeDataRule(CommodityPhysicalExercise commodityPhysicalExercise) {
			try {
				return choice(MapperS.of(commodityPhysicalExercise), Arrays.asList("americanExercise", "europeanExercise"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
