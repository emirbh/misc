		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestLegCalculationPeriodDatesReference", ValidationResult.ValidationType.CARDINALITY, "InterestLegCalculationPeriodDatesReference", path, "", res.getError());
				}
				return success("InterestLegCalculationPeriodDatesReference", ValidationResult.ValidationType.CARDINALITY, "InterestLegCalculationPeriodDatesReference", path, "");
			})
			.collect(toList());
	}
