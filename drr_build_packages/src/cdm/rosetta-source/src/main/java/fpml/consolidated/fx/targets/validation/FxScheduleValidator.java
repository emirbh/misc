...
public class FxScheduleValidator implements Validator<FxSchedule> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSchedule", ValidationResult.ValidationType.CARDINALITY, "FxSchedule", path, "", res.getError());
				}
				return success("FxSchedule", ValidationResult.ValidationType.CARDINALITY, "FxSchedule", path, "");
			})
			.collect(toList());
	}
