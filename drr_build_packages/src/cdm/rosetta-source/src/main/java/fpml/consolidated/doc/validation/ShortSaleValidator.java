		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ShortSale", ValidationResult.ValidationType.CARDINALITY, "ShortSale", path, "", res.getError());
				}
				return success("ShortSale", ValidationResult.ValidationType.CARDINALITY, "ShortSale", path, "");
			})
			.collect(toList());
	}
