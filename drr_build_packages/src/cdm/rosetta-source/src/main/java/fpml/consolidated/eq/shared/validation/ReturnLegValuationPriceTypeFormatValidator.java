		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnLegValuationPrice", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnLegValuationPrice", path, "", res.getError());
				}
				return success("ReturnLegValuationPrice", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnLegValuationPrice", path, "");
			})
			.collect(toList());
	}
