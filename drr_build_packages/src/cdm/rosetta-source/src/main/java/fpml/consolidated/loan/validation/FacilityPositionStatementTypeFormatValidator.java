...
public class FacilityPositionStatementTypeFormatValidator implements Validator<FacilityPositionStatement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityPositionStatement", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityPositionStatement", path, "", res.getError());
				}
				return success("FacilityPositionStatement", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityPositionStatement", path, "");
			})
			.collect(toList());
	}
