...
public class ExecutionAdviceExceptionOnlyExistsValidator implements ValidatorWithArg<ExecutionAdviceException, Set<String>> {

	/* Casting is required to ensure types are output to ensure recompilation in Rosetta */
	@Override
	public <T2 extends ExecutionAdviceException> ValidationResult<ExecutionAdviceException> validate(RosettaPath path, T2 o, Set<String> fields) {
		...
		if (setFields.equals(fields)) {
			return success("ExecutionAdviceException", ValidationResult.ValidationType.ONLY_EXISTS, "ExecutionAdviceException", path, "");
		}
		return failure("ExecutionAdviceException", ValidationResult.ValidationType.ONLY_EXISTS, "ExecutionAdviceException", path, "",
