		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Cashflows", ValidationResult.ValidationType.TYPE_FORMAT, "Cashflows", path, "", res.getError());
				}
				return success("Cashflows", ValidationResult.ValidationType.TYPE_FORMAT, "Cashflows", path, "");
			})
			.collect(toList());
	}
