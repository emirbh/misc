...
public class MandatoryCostRateChangeValidator implements Validator<MandatoryCostRateChange> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MandatoryCostRateChange", ValidationResult.ValidationType.CARDINALITY, "MandatoryCostRateChange", path, "", res.getError());
				}
				return success("MandatoryCostRateChange", ValidationResult.ValidationType.CARDINALITY, "MandatoryCostRateChange", path, "");
			})
			.collect(toList());
	}
