		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PGPData", ValidationResult.ValidationType.CARDINALITY, "PGPData", path, "", res.getError());
				}
				return success("PGPData", ValidationResult.ValidationType.CARDINALITY, "PGPData", path, "");
			})
			.collect(toList());
	}
