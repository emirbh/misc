		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentBase", ValidationResult.ValidationType.CARDINALITY, "PaymentBase", path, "", res.getError());
				}
				return success("PaymentBase", ValidationResult.ValidationType.CARDINALITY, "PaymentBase", path, "");
			})
			.collect(toList());
	}
