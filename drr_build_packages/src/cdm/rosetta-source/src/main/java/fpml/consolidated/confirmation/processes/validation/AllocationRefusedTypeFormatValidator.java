...
public class AllocationRefusedTypeFormatValidator implements Validator<AllocationRefused> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationRefused", ValidationResult.ValidationType.TYPE_FORMAT, "AllocationRefused", path, "", res.getError());
				}
				return success("AllocationRefused", ValidationResult.ValidationType.TYPE_FORMAT, "AllocationRefused", path, "");
			})
			.collect(toList());
	}
