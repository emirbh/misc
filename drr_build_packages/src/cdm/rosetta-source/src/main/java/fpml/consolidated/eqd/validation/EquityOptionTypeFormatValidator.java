...
public class EquityOptionTypeFormatValidator implements Validator<EquityOption> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityOption", ValidationResult.ValidationType.TYPE_FORMAT, "EquityOption", path, "", res.getError());
				}
				return success("EquityOption", ValidationResult.ValidationType.TYPE_FORMAT, "EquityOption", path, "");
			})
			.collect(toList());
	}
