...
public class BaseRateSetValidator implements Validator<BaseRateSet> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BaseRateSet", ValidationResult.ValidationType.CARDINALITY, "BaseRateSet", path, "", res.getError());
				}
				return success("BaseRateSet", ValidationResult.ValidationType.CARDINALITY, "BaseRateSet", path, "");
			})
			.collect(toList());
	}
