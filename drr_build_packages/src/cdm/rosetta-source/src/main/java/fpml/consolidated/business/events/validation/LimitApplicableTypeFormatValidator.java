...
public class LimitApplicableTypeFormatValidator implements Validator<LimitApplicable> {
	...
	private List<ComparisonResult> getComparisonResults(LimitApplicable o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkNumber("amountUtilized", o.getAmountUtilized(), empty(), of(0), empty(), empty()), 
				checkNumber("amountRemaining", o.getAmountRemaining(), empty(), of(0), empty(), empty())
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LimitApplicable", ValidationResult.ValidationType.TYPE_FORMAT, "LimitApplicable", path, "", res.getError());
				}
				return success("LimitApplicable", ValidationResult.ValidationType.TYPE_FORMAT, "LimitApplicable", path, "");
			})
			.collect(toList());
	}
