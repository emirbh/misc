...
public class RevolverValidator implements Validator<Revolver> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Revolver", ValidationResult.ValidationType.CARDINALITY, "Revolver", path, "", res.getError());
				}
				return success("Revolver", ValidationResult.ValidationType.CARDINALITY, "Revolver", path, "");
			})
			.collect(toList());
	}
