		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingPriceLeg", ValidationResult.ValidationType.CARDINALITY, "FloatingPriceLeg", path, "", res.getError());
				}
				return success("FloatingPriceLeg", ValidationResult.ValidationType.CARDINALITY, "FloatingPriceLeg", path, "");
			})
			.collect(toList());
	}
