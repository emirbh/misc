		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeSummary", ValidationResult.ValidationType.CARDINALITY, "LoanTradeSummary", path, "", res.getError());
				}
				return success("LoanTradeSummary", ValidationResult.ValidationType.CARDINALITY, "LoanTradeSummary", path, "");
			})
			.collect(toList());
	}
