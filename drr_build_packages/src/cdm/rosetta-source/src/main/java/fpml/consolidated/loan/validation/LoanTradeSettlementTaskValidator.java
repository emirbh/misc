...
public class LoanTradeSettlementTaskValidator implements Validator<LoanTradeSettlementTask> {
	...
	private List<ComparisonResult> getComparisonResults(LoanTradeSettlementTask o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("type", (LoanTradeSettlementTaskType) o._getType() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeSettlementTask", ValidationResult.ValidationType.CARDINALITY, "LoanTradeSettlementTask", path, "", res.getError());
				}
				return success("LoanTradeSettlementTask", ValidationResult.ValidationType.CARDINALITY, "LoanTradeSettlementTask", path, "");
			})
			.collect(toList());
	}
