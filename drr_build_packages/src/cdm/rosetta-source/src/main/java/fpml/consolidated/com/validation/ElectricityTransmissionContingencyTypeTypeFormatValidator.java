		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ElectricityTransmissionContingencyType", ValidationResult.ValidationType.TYPE_FORMAT, "ElectricityTransmissionContingencyType", path, "", res.getError());
				}
				return success("ElectricityTransmissionContingencyType", ValidationResult.ValidationType.TYPE_FORMAT, "ElectricityTransmissionContingencyType", path, "");
			})
			.collect(toList());
	}
