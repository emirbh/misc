...
public class LcIssuanceValidator implements Validator<LcIssuance> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcIssuance", ValidationResult.ValidationType.CARDINALITY, "LcIssuance", path, "", res.getError());
				}
				return success("LcIssuance", ValidationResult.ValidationType.CARDINALITY, "LcIssuance", path, "");
			})
			.collect(toList());
	}
