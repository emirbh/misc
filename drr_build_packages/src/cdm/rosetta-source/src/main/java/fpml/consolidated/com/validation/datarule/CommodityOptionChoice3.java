 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityOptionChoice3")
@ImplementedBy(CommodityOptionChoice3.Default.class)
public interface CommodityOptionChoice3 extends Validator<CommodityOption> {
	
	String NAME = "CommodityOptionChoice3";
	String DEFINITION = "if strikePricePerUnitSchedule exists then commodity exists and physicalExercise is absent and weatherNotionalAmount is absent and weatherIndexStrikeLevel is absent and calculation is absent and weatherIndexData is absent and commodityForward is absent and weatherCalculationPeriodsReference is absent and optional choice calculationPeriodsSchedule, calculationPeriods and if calculationPeriodsSchedule exists then optional choice totalNotionalQuantity, quantityReference and if quantityReference exists then exercise exists and pricingDates exists or notionalQuantitySchedule is absent and notionalQuantity is absent and settlementPeriodsNotionalQuantity is absent and if totalNotionalQuantity is absent and quantityReference is absent then exercise exists and pricingDates exists and (notionalQuantity exists or notionalQuantitySchedule exists or settlementPeriodsNotionalQuantity exists) and if calculationPeriods exists then optional choice totalNotionalQuantity, quantityReference and if quantityReference exists then exercise exists and pricingDates exists or notionalQuantitySchedule is absent and notionalQuantity is absent and settlementPeriodsNotionalQuantity is absent and if totalNotionalQuantity is absent and quantityReference is absent then exercise exists and pricingDates exists and (notionalQuantity exists or notionalQuantitySchedule exists or settlementPeriodsNotionalQuantity exists) and if calculationPeriodsSchedule is absent and calculationPeriods is absent then optional choice totalNotionalQuantity, quantityReference and if quantityReference exists then exercise exists and pricingDates is absent and averagingMethod is absent or notionalQuantitySchedule is absent and notionalQuantity is absent and settlementPeriodsNotionalQuantity is absent and if totalNotionalQuantity is absent and quantityReference is absent then exercise exists and pricingDates is absent and averagingMethod is absent and (notionalQuantity exists or notionalQuantitySchedule exists or settlementPeriodsNotionalQuantity exists)";
	
	class Default implements CommodityOptionChoice3 {
		...
		private ComparisonResult executeDataRule(CommodityOption commodityOption) {
			try {
				if (exists(MapperS.of(commodityOption).<CommodityStrikeSchedule>map("getStrikePricePerUnitSchedule", _commodityOption -> _commodityOption.getStrikePricePerUnitSchedule())).getOrDefault(false)) {
	...
	class NoOp implements CommodityOptionChoice3 {
