 ...
 * @version 2.1.1
 */
@RosettaDataRule("LoanAllocationNotificationChoice2")
@ImplementedBy(LoanAllocationNotificationChoice2.Default.class)
public interface LoanAllocationNotificationChoice2 extends Validator<LoanAllocationNotification> {
	
	String NAME = "LoanAllocationNotificationChoice2";
	String DEFINITION = "required choice facilityIdentifier, facilitySummary";
	
	class Default implements LoanAllocationNotificationChoice2 {
		...
		private ComparisonResult executeDataRule(LoanAllocationNotification loanAllocationNotification) {
			try {
				return choice(MapperS.of(loanAllocationNotification), Arrays.asList("facilityIdentifier", "facilitySummary"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements LoanAllocationNotificationChoice2 {
