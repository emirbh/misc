		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcRenewal", ValidationResult.ValidationType.TYPE_FORMAT, "LcRenewal", path, "", res.getError());
				}
				return success("LcRenewal", ValidationResult.ValidationType.TYPE_FORMAT, "LcRenewal", path, "");
			})
			.collect(toList());
	}
