		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualPayoffRegionReference", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualPayoffRegionReference", path, "", res.getError());
				}
				return success("FxAccrualPayoffRegionReference", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualPayoffRegionReference", path, "");
			})
			.collect(toList());
	}
