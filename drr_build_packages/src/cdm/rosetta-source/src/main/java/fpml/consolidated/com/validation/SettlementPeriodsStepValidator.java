		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPeriodsStep", ValidationResult.ValidationType.CARDINALITY, "SettlementPeriodsStep", path, "", res.getError());
				}
				return success("SettlementPeriodsStep", ValidationResult.ValidationType.CARDINALITY, "SettlementPeriodsStep", path, "");
			})
			.collect(toList());
	}
