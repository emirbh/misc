		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxDigitalAmericanExercise", ValidationResult.ValidationType.CARDINALITY, "FxDigitalAmericanExercise", path, "", res.getError());
				}
				return success("FxDigitalAmericanExercise", ValidationResult.ValidationType.CARDINALITY, "FxDigitalAmericanExercise", path, "");
			})
			.collect(toList());
	}
