		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OffsetPrevailingTime", ValidationResult.ValidationType.TYPE_FORMAT, "OffsetPrevailingTime", path, "", res.getError());
				}
				return success("OffsetPrevailingTime", ValidationResult.ValidationType.TYPE_FORMAT, "OffsetPrevailingTime", path, "");
			})
			.collect(toList());
	}
