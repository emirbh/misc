		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedRate", ValidationResult.ValidationType.CARDINALITY, "IdentifiedRate", path, "", res.getError());
				}
				return success("IdentifiedRate", ValidationResult.ValidationType.CARDINALITY, "IdentifiedRate", path, "");
			})
			.collect(toList());
	}
