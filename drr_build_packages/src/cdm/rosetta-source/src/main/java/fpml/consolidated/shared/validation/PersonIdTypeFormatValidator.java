		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PersonId", ValidationResult.ValidationType.TYPE_FORMAT, "PersonId", path, "", res.getError());
				}
				return success("PersonId", ValidationResult.ValidationType.TYPE_FORMAT, "PersonId", path, "");
			})
			.collect(toList());
	}
