...
public class GenericFrequencyTypeFormatValidator implements Validator<GenericFrequency> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericFrequency", ValidationResult.ValidationType.TYPE_FORMAT, "GenericFrequency", path, "", res.getError());
				}
				return success("GenericFrequency", ValidationResult.ValidationType.TYPE_FORMAT, "GenericFrequency", path, "");
			})
			.collect(toList());
	}
