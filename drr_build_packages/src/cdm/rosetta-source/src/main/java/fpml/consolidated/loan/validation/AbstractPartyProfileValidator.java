		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractPartyProfile", ValidationResult.ValidationType.CARDINALITY, "AbstractPartyProfile", path, "", res.getError());
				}
				return success("AbstractPartyProfile", ValidationResult.ValidationType.CARDINALITY, "AbstractPartyProfile", path, "");
			})
			.collect(toList());
	}
