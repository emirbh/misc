		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySettlementPeriodsNotionalQuantitySchedule", ValidationResult.ValidationType.TYPE_FORMAT, "CommoditySettlementPeriodsNotionalQuantitySchedule", path, "", res.getError());
				}
				return success("CommoditySettlementPeriodsNotionalQuantitySchedule", ValidationResult.ValidationType.TYPE_FORMAT, "CommoditySettlementPeriodsNotionalQuantitySchedule", path, "");
			})
			.collect(toList());
	}
