		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditDocument", ValidationResult.ValidationType.TYPE_FORMAT, "CreditDocument", path, "", res.getError());
				}
				return success("CreditDocument", ValidationResult.ValidationType.TYPE_FORMAT, "CreditDocument", path, "");
			})
			.collect(toList());
	}
