		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationSwap", ValidationResult.ValidationType.CARDINALITY, "CorrelationSwap", path, "", res.getError());
				}
				return success("CorrelationSwap", ValidationResult.ValidationType.CARDINALITY, "CorrelationSwap", path, "");
			})
			.collect(toList());
	}
