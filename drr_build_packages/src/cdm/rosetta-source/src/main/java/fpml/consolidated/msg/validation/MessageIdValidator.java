		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MessageId", ValidationResult.ValidationType.CARDINALITY, "MessageId", path, "", res.getError());
				}
				return success("MessageId", ValidationResult.ValidationType.CARDINALITY, "MessageId", path, "");
			})
			.collect(toList());
	}
