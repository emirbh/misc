		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Sensitivity", ValidationResult.ValidationType.CARDINALITY, "Sensitivity", path, "", res.getError());
				}
				return success("Sensitivity", ValidationResult.ValidationType.CARDINALITY, "Sensitivity", path, "");
			})
			.collect(toList());
	}
