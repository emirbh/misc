		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetMeasureType", ValidationResult.ValidationType.CARDINALITY, "AssetMeasureType", path, "", res.getError());
				}
				return success("AssetMeasureType", ValidationResult.ValidationType.CARDINALITY, "AssetMeasureType", path, "");
			})
			.collect(toList());
	}
