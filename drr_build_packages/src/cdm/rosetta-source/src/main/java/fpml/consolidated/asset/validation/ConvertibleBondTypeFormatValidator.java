...
public class ConvertibleBondTypeFormatValidator implements Validator<ConvertibleBond> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConvertibleBond", ValidationResult.ValidationType.TYPE_FORMAT, "ConvertibleBond", path, "", res.getError());
				}
				return success("ConvertibleBond", ValidationResult.ValidationType.TYPE_FORMAT, "ConvertibleBond", path, "");
			})
			.collect(toList());
	}
