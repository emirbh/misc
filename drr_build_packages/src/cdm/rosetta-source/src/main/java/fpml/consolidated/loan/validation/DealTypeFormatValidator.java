		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Deal", ValidationResult.ValidationType.TYPE_FORMAT, "Deal", path, "", res.getError());
				}
				return success("Deal", ValidationResult.ValidationType.TYPE_FORMAT, "Deal", path, "");
			})
			.collect(toList());
	}
