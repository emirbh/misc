...
public class CommodityPerformanceSwapValidator implements Validator<CommodityPerformanceSwap> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPerformanceSwap", ValidationResult.ValidationType.CARDINALITY, "CommodityPerformanceSwap", path, "", res.getError());
				}
				return success("CommodityPerformanceSwap", ValidationResult.ValidationType.CARDINALITY, "CommodityPerformanceSwap", path, "");
			})
			.collect(toList());
	}
