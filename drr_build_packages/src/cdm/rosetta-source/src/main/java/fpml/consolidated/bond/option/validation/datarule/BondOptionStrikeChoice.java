	...
	class Default implements BondOptionStrikeChoice {
		...
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, BondOptionStrike bondOptionStrike) {
			...
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "BondOptionStrike", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "BondOptionStrike", path, DEFINITION, failureMessage));
		}
