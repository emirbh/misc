...
public class AsianValidator implements Validator<Asian> {
	...
	private List<ComparisonResult> getComparisonResults(Asian o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("averagingInOut", (AveragingInOutEnum) o.getAveragingInOut() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Asian", ValidationResult.ValidationType.CARDINALITY, "Asian", path, "", res.getError());
				}
				return success("Asian", ValidationResult.ValidationType.CARDINALITY, "Asian", path, "");
			})
			.collect(toList());
	}
