		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationRejected", ValidationResult.ValidationType.CARDINALITY, "CollateralAllocationRejected", path, "", res.getError());
				}
				return success("CollateralAllocationRejected", ValidationResult.ValidationType.CARDINALITY, "CollateralAllocationRejected", path, "");
			})
			.collect(toList());
	}
