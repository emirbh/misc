		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxCounterCurrencyAmount", ValidationResult.ValidationType.TYPE_FORMAT, "FxCounterCurrencyAmount", path, "", res.getError());
				}
				return success("FxCounterCurrencyAmount", ValidationResult.ValidationType.TYPE_FORMAT, "FxCounterCurrencyAmount", path, "");
			})
			.collect(toList());
	}
