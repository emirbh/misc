		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CapFloor", ValidationResult.ValidationType.CARDINALITY, "CapFloor", path, "", res.getError());
				}
				return success("CapFloor", ValidationResult.ValidationType.CARDINALITY, "CapFloor", path, "");
			})
			.collect(toList());
	}
