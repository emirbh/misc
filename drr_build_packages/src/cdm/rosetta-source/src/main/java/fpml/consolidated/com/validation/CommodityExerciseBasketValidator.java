		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityExerciseBasket", ValidationResult.ValidationType.CARDINALITY, "CommodityExerciseBasket", path, "", res.getError());
				}
				return success("CommodityExerciseBasket", ValidationResult.ValidationType.CARDINALITY, "CommodityExerciseBasket", path, "");
			})
			.collect(toList());
	}
