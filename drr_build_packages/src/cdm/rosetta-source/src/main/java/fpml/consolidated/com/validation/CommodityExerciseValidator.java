		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityExercise", ValidationResult.ValidationType.CARDINALITY, "CommodityExercise", path, "", res.getError());
				}
				return success("CommodityExercise", ValidationResult.ValidationType.CARDINALITY, "CommodityExercise", path, "");
			})
			.collect(toList());
	}
