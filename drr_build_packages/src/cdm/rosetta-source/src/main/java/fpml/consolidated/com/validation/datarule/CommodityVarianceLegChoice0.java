 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityVarianceLegChoice0")
@ImplementedBy(CommodityVarianceLegChoice0.Default.class)
public interface CommodityVarianceLegChoice0 extends Validator<CommodityVarianceLeg> {
	
	String NAME = "CommodityVarianceLegChoice0";
	String DEFINITION = "required choice relativePaymentDates, paymentDates, masterAgreementPaymentDates";
	
	class Default implements CommodityVarianceLegChoice0 {
		...
		private ComparisonResult executeDataRule(CommodityVarianceLeg commodityVarianceLeg) {
			try {
				return choice(MapperS.of(commodityVarianceLeg), Arrays.asList("relativePaymentDates", "paymentDates", "masterAgreementPaymentDates"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements CommodityVarianceLegChoice0 {
