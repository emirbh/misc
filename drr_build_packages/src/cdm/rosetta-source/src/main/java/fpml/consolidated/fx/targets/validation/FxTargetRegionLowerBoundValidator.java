		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetRegionLowerBound", ValidationResult.ValidationType.CARDINALITY, "FxTargetRegionLowerBound", path, "", res.getError());
				}
				return success("FxTargetRegionLowerBound", ValidationResult.ValidationType.CARDINALITY, "FxTargetRegionLowerBound", path, "");
			})
			.collect(toList());
	}
