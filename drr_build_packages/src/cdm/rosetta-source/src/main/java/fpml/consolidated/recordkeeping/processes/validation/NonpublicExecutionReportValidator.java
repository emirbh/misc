...
public class NonpublicExecutionReportValidator implements Validator<NonpublicExecutionReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonpublicExecutionReport", ValidationResult.ValidationType.CARDINALITY, "NonpublicExecutionReport", path, "", res.getError());
				}
				return success("NonpublicExecutionReport", ValidationResult.ValidationType.CARDINALITY, "NonpublicExecutionReport", path, "");
			})
			.collect(toList());
	}
