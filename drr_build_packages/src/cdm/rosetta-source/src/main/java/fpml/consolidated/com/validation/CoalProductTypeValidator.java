		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalProductType", ValidationResult.ValidationType.CARDINALITY, "CoalProductType", path, "", res.getError());
				}
				return success("CoalProductType", ValidationResult.ValidationType.CARDINALITY, "CoalProductType", path, "");
			})
			.collect(toList());
	}
