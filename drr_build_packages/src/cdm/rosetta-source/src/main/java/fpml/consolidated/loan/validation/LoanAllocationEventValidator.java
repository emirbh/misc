		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationEvent", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationEvent", path, "", res.getError());
				}
				return success("LoanAllocationEvent", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationEvent", path, "");
			})
			.collect(toList());
	}
