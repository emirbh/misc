		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventPayment", ValidationResult.ValidationType.TYPE_FORMAT, "EventPayment", path, "", res.getError());
				}
				return success("EventPayment", ValidationResult.ValidationType.TYPE_FORMAT, "EventPayment", path, "");
			})
			.collect(toList());
	}
