		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResourceId", ValidationResult.ValidationType.CARDINALITY, "ResourceId", path, "", res.getError());
				}
				return success("ResourceId", ValidationResult.ValidationType.CARDINALITY, "ResourceId", path, "");
			})
			.collect(toList());
	}
