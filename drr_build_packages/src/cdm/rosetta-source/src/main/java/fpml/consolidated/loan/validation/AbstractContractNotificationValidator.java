...
public class AbstractContractNotificationValidator implements Validator<AbstractContractNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractContractNotification", ValidationResult.ValidationType.CARDINALITY, "AbstractContractNotification", path, "", res.getError());
				}
				return success("AbstractContractNotification", ValidationResult.ValidationType.CARDINALITY, "AbstractContractNotification", path, "");
			})
			.collect(toList());
	}
