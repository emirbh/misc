...
public class ExecutionExceptionOnlyExistsValidator implements ValidatorWithArg<ExecutionException, Set<String>> {

	/* Casting is required to ensure types are output to ensure recompilation in Rosetta */
	@Override
	public <T2 extends ExecutionException> ValidationResult<ExecutionException> validate(RosettaPath path, T2 o, Set<String> fields) {
		...
		if (setFields.equals(fields)) {
			return success("ExecutionException", ValidationResult.ValidationType.ONLY_EXISTS, "ExecutionException", path, "");
		}
		return failure("ExecutionException", ValidationResult.ValidationType.ONLY_EXISTS, "ExecutionException", path, "",
