		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContractId", ValidationResult.ValidationType.CARDINALITY, "ContractId", path, "", res.getError());
				}
				return success("ContractId", ValidationResult.ValidationType.CARDINALITY, "ContractId", path, "");
			})
			.collect(toList());
	}
