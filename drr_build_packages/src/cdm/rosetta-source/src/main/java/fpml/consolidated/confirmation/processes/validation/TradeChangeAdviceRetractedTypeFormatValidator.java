		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdviceRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdviceRetracted", path, "", res.getError());
				}
				return success("TradeChangeAdviceRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdviceRetracted", path, "");
			})
			.collect(toList());
	}
