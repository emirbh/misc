...
public class AllocationAcknowledgementValidator implements Validator<AllocationAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "AllocationAcknowledgement", path, "", res.getError());
				}
				return success("AllocationAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "AllocationAcknowledgement", path, "");
			})
			.collect(toList());
	}
