		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OrganizationCharacteristic", ValidationResult.ValidationType.TYPE_FORMAT, "OrganizationCharacteristic", path, "", res.getError());
				}
				return success("OrganizationCharacteristic", ValidationResult.ValidationType.TYPE_FORMAT, "OrganizationCharacteristic", path, "");
			})
			.collect(toList());
	}
