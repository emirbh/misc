		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProductType", ValidationResult.ValidationType.CARDINALITY, "ProductType", path, "", res.getError());
				}
				return success("ProductType", ValidationResult.ValidationType.CARDINALITY, "ProductType", path, "");
			})
			.collect(toList());
	}
