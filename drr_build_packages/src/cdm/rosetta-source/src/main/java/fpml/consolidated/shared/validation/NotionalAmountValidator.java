...
public class NotionalAmountValidator implements Validator<NotionalAmount> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotionalAmount", ValidationResult.ValidationType.CARDINALITY, "NotionalAmount", path, "", res.getError());
				}
				return success("NotionalAmount", ValidationResult.ValidationType.CARDINALITY, "NotionalAmount", path, "");
			})
			.collect(toList());
	}
