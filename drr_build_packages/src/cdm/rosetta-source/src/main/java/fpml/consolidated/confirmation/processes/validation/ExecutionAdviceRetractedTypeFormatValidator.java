...
public class ExecutionAdviceRetractedTypeFormatValidator implements Validator<ExecutionAdviceRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdviceRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionAdviceRetracted", path, "", res.getError());
				}
				return success("ExecutionAdviceRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionAdviceRetracted", path, "");
			})
			.collect(toList());
	}
