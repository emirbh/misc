		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityExercise", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityExercise", path, "", res.getError());
				}
				return success("CommodityExercise", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityExercise", path, "");
			})
			.collect(toList());
	}
