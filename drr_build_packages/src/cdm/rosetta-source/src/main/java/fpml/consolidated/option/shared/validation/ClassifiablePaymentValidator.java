		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClassifiablePayment", ValidationResult.ValidationType.CARDINALITY, "ClassifiablePayment", path, "", res.getError());
				}
				return success("ClassifiablePayment", ValidationResult.ValidationType.CARDINALITY, "ClassifiablePayment", path, "");
			})
			.collect(toList());
	}
