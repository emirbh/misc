		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasketChange", ValidationResult.ValidationType.CARDINALITY, "BasketChange", path, "", res.getError());
				}
				return success("BasketChange", ValidationResult.ValidationType.CARDINALITY, "BasketChange", path, "");
			})
			.collect(toList());
	}
