		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNotificationRetracted", ValidationResult.ValidationType.CARDINALITY, "CreditEventNotificationRetracted", path, "", res.getError());
				}
				return success("CreditEventNotificationRetracted", ValidationResult.ValidationType.CARDINALITY, "CreditEventNotificationRetracted", path, "");
			})
			.collect(toList());
	}
