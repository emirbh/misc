		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingStrikePrice", path, "", res.getError());
				}
				return success("FloatingStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingStrikePrice", path, "");
			})
			.collect(toList());
	}
