		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxCashSettlement", ValidationResult.ValidationType.CARDINALITY, "FxCashSettlement", path, "", res.getError());
				}
				return success("FxCashSettlement", ValidationResult.ValidationType.CARDINALITY, "FxCashSettlement", path, "");
			})
			.collect(toList());
	}
