		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Fra", ValidationResult.ValidationType.TYPE_FORMAT, "Fra", path, "", res.getError());
				}
				return success("Fra", ValidationResult.ValidationType.TYPE_FORMAT, "Fra", path, "");
			})
			.collect(toList());
	}
