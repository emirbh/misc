		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityReference", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityReference", path, "", res.getError());
				}
				return success("FacilityReference", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityReference", path, "");
			})
			.collect(toList());
	}
