		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdvice", ValidationResult.ValidationType.CARDINALITY, "TradeChangeAdvice", path, "", res.getError());
				}
				return success("TradeChangeAdvice", ValidationResult.ValidationType.CARDINALITY, "TradeChangeAdvice", path, "");
			})
			.collect(toList());
	}
