...
public class GenericProductExchangeRateTypeFormatValidator implements Validator<GenericProductExchangeRate> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericProductExchangeRate", ValidationResult.ValidationType.TYPE_FORMAT, "GenericProductExchangeRate", path, "", res.getError());
				}
				return success("GenericProductExchangeRate", ValidationResult.ValidationType.TYPE_FORMAT, "GenericProductExchangeRate", path, "");
			})
			.collect(toList());
	}
