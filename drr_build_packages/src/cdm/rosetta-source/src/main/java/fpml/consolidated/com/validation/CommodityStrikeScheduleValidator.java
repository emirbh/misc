		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityStrikeSchedule", ValidationResult.ValidationType.CARDINALITY, "CommodityStrikeSchedule", path, "", res.getError());
				}
				return success("CommodityStrikeSchedule", ValidationResult.ValidationType.CARDINALITY, "CommodityStrikeSchedule", path, "");
			})
			.collect(toList());
	}
