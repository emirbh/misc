		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashPriceMethod", ValidationResult.ValidationType.CARDINALITY, "CashPriceMethod", path, "", res.getError());
				}
				return success("CashPriceMethod", ValidationResult.ValidationType.CARDINALITY, "CashPriceMethod", path, "");
			})
			.collect(toList());
	}
