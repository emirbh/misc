		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResourceId", ValidationResult.ValidationType.TYPE_FORMAT, "ResourceId", path, "", res.getError());
				}
				return success("ResourceId", ValidationResult.ValidationType.TYPE_FORMAT, "ResourceId", path, "");
			})
			.collect(toList());
	}
