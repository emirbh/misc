...
public class AbstractLoanTradeNotificationValidator implements Validator<AbstractLoanTradeNotification> {

	private List<ComparisonResult> getComparisonResults(AbstractLoanTradeNotification o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, AbstractLoanTradeNotification o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanTradeNotification", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanTradeNotification", path, "", res.getError());
				}
				return success("AbstractLoanTradeNotification", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanTradeNotification", path, "");
			})
			.collect(toList());
	}
