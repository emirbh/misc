		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyRelationshipType", ValidationResult.ValidationType.TYPE_FORMAT, "PartyRelationshipType", path, "", res.getError());
				}
				return success("PartyRelationshipType", ValidationResult.ValidationType.TYPE_FORMAT, "PartyRelationshipType", path, "");
			})
			.collect(toList());
	}
