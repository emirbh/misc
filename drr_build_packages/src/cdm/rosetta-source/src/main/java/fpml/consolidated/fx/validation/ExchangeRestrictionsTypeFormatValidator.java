		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeRestrictions", ValidationResult.ValidationType.TYPE_FORMAT, "ExchangeRestrictions", path, "", res.getError());
				}
				return success("ExchangeRestrictions", ValidationResult.ValidationType.TYPE_FORMAT, "ExchangeRestrictions", path, "");
			})
			.collect(toList());
	}
