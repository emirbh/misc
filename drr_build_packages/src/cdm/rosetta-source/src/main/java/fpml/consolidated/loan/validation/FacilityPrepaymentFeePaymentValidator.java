...
public class FacilityPrepaymentFeePaymentValidator implements Validator<FacilityPrepaymentFeePayment> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityPrepaymentFeePayment", ValidationResult.ValidationType.CARDINALITY, "FacilityPrepaymentFeePayment", path, "", res.getError());
				}
				return success("FacilityPrepaymentFeePayment", ValidationResult.ValidationType.CARDINALITY, "FacilityPrepaymentFeePayment", path, "");
			})
			.collect(toList());
	}
