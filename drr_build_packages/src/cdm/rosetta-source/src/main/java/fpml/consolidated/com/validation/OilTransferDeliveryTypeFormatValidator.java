		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OilTransferDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "OilTransferDelivery", path, "", res.getError());
				}
				return success("OilTransferDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "OilTransferDelivery", path, "");
			})
			.collect(toList());
	}
