...
public class AbstractFacilityNotificationTypeFormatValidator implements Validator<AbstractFacilityNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityNotification", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractFacilityNotification", path, "", res.getError());
				}
				return success("AbstractFacilityNotification", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractFacilityNotification", path, "");
			})
			.collect(toList());
	}
