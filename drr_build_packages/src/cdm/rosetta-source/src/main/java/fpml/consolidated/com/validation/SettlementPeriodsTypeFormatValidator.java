		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPeriods", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPeriods", path, "", res.getError());
				}
				return success("SettlementPeriods", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPeriods", path, "");
			})
			.collect(toList());
	}
