...
public class DataDocumentTypeFormatValidator implements Validator<DataDocument> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DataDocument", ValidationResult.ValidationType.TYPE_FORMAT, "DataDocument", path, "", res.getError());
				}
				return success("DataDocument", ValidationResult.ValidationType.TYPE_FORMAT, "DataDocument", path, "");
			})
			.collect(toList());
	}
