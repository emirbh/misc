...
public class LcNotificationValidator implements Validator<LcNotification> {

	private List<ComparisonResult> getComparisonResults(LcNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("lcEventGroup", (AbstractLcEvent) o.getLcEventGroup() != null ? 1 : 0, 1, 1), 
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LcNotification o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcNotification", ValidationResult.ValidationType.CARDINALITY, "LcNotification", path, "", res.getError());
				}
				return success("LcNotification", ValidationResult.ValidationType.CARDINALITY, "LcNotification", path, "");
			})
			.collect(toList());
	}
