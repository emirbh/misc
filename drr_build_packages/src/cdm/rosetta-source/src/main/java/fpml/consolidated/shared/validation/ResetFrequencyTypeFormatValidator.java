...
public class ResetFrequencyTypeFormatValidator implements Validator<ResetFrequency> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResetFrequency", ValidationResult.ValidationType.TYPE_FORMAT, "ResetFrequency", path, "", res.getError());
				}
				return success("ResetFrequency", ValidationResult.ValidationType.TYPE_FORMAT, "ResetFrequency", path, "");
			})
			.collect(toList());
	}
