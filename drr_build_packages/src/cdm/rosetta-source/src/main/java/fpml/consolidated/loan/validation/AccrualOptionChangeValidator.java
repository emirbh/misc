...
public class AccrualOptionChangeValidator implements Validator<AccrualOptionChange> {

	private List<ComparisonResult> getComparisonResults(AccrualOptionChange o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, AccrualOptionChange o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccrualOptionChange", ValidationResult.ValidationType.CARDINALITY, "AccrualOptionChange", path, "", res.getError());
				}
				return success("AccrualOptionChange", ValidationResult.ValidationType.CARDINALITY, "AccrualOptionChange", path, "");
			})
			.collect(toList());
	}
