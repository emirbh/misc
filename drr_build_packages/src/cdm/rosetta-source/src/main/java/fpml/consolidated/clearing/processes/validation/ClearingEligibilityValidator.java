		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingEligibility", ValidationResult.ValidationType.CARDINALITY, "ClearingEligibility", path, "", res.getError());
				}
				return success("ClearingEligibility", ValidationResult.ValidationType.CARDINALITY, "ClearingEligibility", path, "");
			})
			.collect(toList());
	}
