...
public class CorrelationLegValidator implements Validator<CorrelationLeg> {
	...
	private List<ComparisonResult> getComparisonResults(CorrelationLeg o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("amount", (CorrelationAmount) o.getAmount() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationLeg", ValidationResult.ValidationType.CARDINALITY, "CorrelationLeg", path, "", res.getError());
				}
				return success("CorrelationLeg", ValidationResult.ValidationType.CARDINALITY, "CorrelationLeg", path, "");
			})
			.collect(toList());
	}
