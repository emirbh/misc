		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowType", ValidationResult.ValidationType.CARDINALITY, "CashflowType", path, "", res.getError());
				}
				return success("CashflowType", ValidationResult.ValidationType.CARDINALITY, "CashflowType", path, "");
			})
			.collect(toList());
	}
