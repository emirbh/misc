		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "FxOptionStrikePrice", path, "", res.getError());
				}
				return success("FxOptionStrikePrice", ValidationResult.ValidationType.TYPE_FORMAT, "FxOptionStrikePrice", path, "");
			})
			.collect(toList());
	}
