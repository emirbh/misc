		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityDeliveryPeriods", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityDeliveryPeriods", path, "", res.getError());
				}
				return success("CommodityDeliveryPeriods", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityDeliveryPeriods", path, "");
			})
			.collect(toList());
	}
