		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxComplexBarrierBase", ValidationResult.ValidationType.CARDINALITY, "FxComplexBarrierBase", path, "", res.getError());
				}
				return success("FxComplexBarrierBase", ValidationResult.ValidationType.CARDINALITY, "FxComplexBarrierBase", path, "");
			})
			.collect(toList());
	}
