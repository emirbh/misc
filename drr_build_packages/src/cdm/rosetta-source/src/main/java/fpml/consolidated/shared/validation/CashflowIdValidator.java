		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowId", ValidationResult.ValidationType.CARDINALITY, "CashflowId", path, "", res.getError());
				}
				return success("CashflowId", ValidationResult.ValidationType.CARDINALITY, "CashflowId", path, "");
			})
			.collect(toList());
	}
