		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPeriodsSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPeriodsSchedule", path, "", res.getError());
				}
				return success("SettlementPeriodsSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPeriodsSchedule", path, "");
			})
			.collect(toList());
	}
