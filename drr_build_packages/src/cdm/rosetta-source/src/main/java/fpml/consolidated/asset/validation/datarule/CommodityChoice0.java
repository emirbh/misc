 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityChoice0")
@ImplementedBy(CommodityChoice0.Default.class)
public interface CommodityChoice0 extends Validator<Commodity> {
	
	String NAME = "CommodityChoice0";
	String DEFINITION = "optional choice exchangeId, publication";
	
	class Default implements CommodityChoice0 {
		...
		private ComparisonResult executeDataRule(Commodity commodity) {
			try {
				return choice(MapperS.of(commodity), Arrays.asList("exchangeId", "publication"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements CommodityChoice0 {
