		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PerturbationType", ValidationResult.ValidationType.CARDINALITY, "PerturbationType", path, "", res.getError());
				}
				return success("PerturbationType", ValidationResult.ValidationType.CARDINALITY, "PerturbationType", path, "");
			})
			.collect(toList());
	}
