...
public class DefaultRateChangeValidator implements Validator<DefaultRateChange> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DefaultRateChange", ValidationResult.ValidationType.CARDINALITY, "DefaultRateChange", path, "", res.getError());
				}
				return success("DefaultRateChange", ValidationResult.ValidationType.CARDINALITY, "DefaultRateChange", path, "");
			})
			.collect(toList());
	}
