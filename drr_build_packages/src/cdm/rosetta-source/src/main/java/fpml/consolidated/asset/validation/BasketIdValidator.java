		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasketId", ValidationResult.ValidationType.CARDINALITY, "BasketId", path, "", res.getError());
				}
				return success("BasketId", ValidationResult.ValidationType.CARDINALITY, "BasketId", path, "");
			})
			.collect(toList());
	}
