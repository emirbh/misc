		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FundingFeePayment", ValidationResult.ValidationType.TYPE_FORMAT, "FundingFeePayment", path, "", res.getError());
				}
				return success("FundingFeePayment", ValidationResult.ValidationType.TYPE_FORMAT, "FundingFeePayment", path, "");
			})
			.collect(toList());
	}
