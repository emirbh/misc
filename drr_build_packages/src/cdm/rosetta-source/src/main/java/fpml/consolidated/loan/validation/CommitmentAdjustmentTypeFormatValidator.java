		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommitmentAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "CommitmentAdjustment", path, "", res.getError());
				}
				return success("CommitmentAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "CommitmentAdjustment", path, "");
			})
			.collect(toList());
	}
