		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Commission", ValidationResult.ValidationType.TYPE_FORMAT, "Commission", path, "", res.getError());
				}
				return success("Commission", ValidationResult.ValidationType.TYPE_FORMAT, "Commission", path, "");
			})
			.collect(toList());
	}
