		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ParametricAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "ParametricAdjustment", path, "", res.getError());
				}
				return success("ParametricAdjustment", ValidationResult.ValidationType.TYPE_FORMAT, "ParametricAdjustment", path, "");
			})
			.collect(toList());
	}
