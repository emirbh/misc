		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationScenarioReference", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationScenarioReference", path, "", res.getError());
				}
				return success("ValuationScenarioReference", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationScenarioReference", path, "");
			})
			.collect(toList());
	}
