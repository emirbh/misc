		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SecurityValuation", ValidationResult.ValidationType.TYPE_FORMAT, "SecurityValuation", path, "", res.getError());
				}
				return success("SecurityValuation", ValidationResult.ValidationType.TYPE_FORMAT, "SecurityValuation", path, "");
			})
			.collect(toList());
	}
