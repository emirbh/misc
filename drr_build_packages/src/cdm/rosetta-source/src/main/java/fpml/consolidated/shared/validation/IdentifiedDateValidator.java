		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedDate", ValidationResult.ValidationType.CARDINALITY, "IdentifiedDate", path, "", res.getError());
				}
				return success("IdentifiedDate", ValidationResult.ValidationType.CARDINALITY, "IdentifiedDate", path, "");
			})
			.collect(toList());
	}
