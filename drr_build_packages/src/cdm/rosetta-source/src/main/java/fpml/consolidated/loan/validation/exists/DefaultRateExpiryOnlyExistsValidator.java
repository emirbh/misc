...
public class DefaultRateExpiryOnlyExistsValidator implements ValidatorWithArg<DefaultRateExpiry, Set<String>> {

	/* Casting is required to ensure types are output to ensure recompilation in Rosetta */
	@Override
	public <T2 extends DefaultRateExpiry> ValidationResult<DefaultRateExpiry> validate(RosettaPath path, T2 o, Set<String> fields) {
		...
		if (setFields.equals(fields)) {
			return success("DefaultRateExpiry", ValidationResult.ValidationType.ONLY_EXISTS, "DefaultRateExpiry", path, "");
		}
		return failure("DefaultRateExpiry", ValidationResult.ValidationType.ONLY_EXISTS, "DefaultRateExpiry", path, "",
