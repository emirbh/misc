...
public class AbstractLoanServicingEventOnlyExistsValidator implements ValidatorWithArg<AbstractLoanServicingEvent, Set<String>> {

	/* Casting is required to ensure types are output to ensure recompilation in Rosetta */
	@Override
	public <T2 extends AbstractLoanServicingEvent> ValidationResult<AbstractLoanServicingEvent> validate(RosettaPath path, T2 o, Set<String> fields) {
		...
		if (setFields.equals(fields)) {
			return success("AbstractLoanServicingEvent", ValidationResult.ValidationType.ONLY_EXISTS, "AbstractLoanServicingEvent", path, "");
		}
		return failure("AbstractLoanServicingEvent", ValidationResult.ValidationType.ONLY_EXISTS, "AbstractLoanServicingEvent", path, "",
