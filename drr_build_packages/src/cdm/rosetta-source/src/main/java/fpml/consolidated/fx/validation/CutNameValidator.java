		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CutName", ValidationResult.ValidationType.CARDINALITY, "CutName", path, "", res.getError());
				}
				return success("CutName", ValidationResult.ValidationType.CARDINALITY, "CutName", path, "");
			})
			.collect(toList());
	}
