		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ApplicableTransactionType", ValidationResult.ValidationType.TYPE_FORMAT, "ApplicableTransactionType", path, "", res.getError());
				}
				return success("ApplicableTransactionType", ValidationResult.ValidationType.TYPE_FORMAT, "ApplicableTransactionType", path, "");
			})
			.collect(toList());
	}
