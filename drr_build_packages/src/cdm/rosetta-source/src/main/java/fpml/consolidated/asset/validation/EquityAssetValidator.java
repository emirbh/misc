		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityAsset", ValidationResult.ValidationType.CARDINALITY, "EquityAsset", path, "", res.getError());
				}
				return success("EquityAsset", ValidationResult.ValidationType.CARDINALITY, "EquityAsset", path, "");
			})
			.collect(toList());
	}
