		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AvailableInventory", ValidationResult.ValidationType.CARDINALITY, "AvailableInventory", path, "", res.getError());
				}
				return success("AvailableInventory", ValidationResult.ValidationType.CARDINALITY, "AvailableInventory", path, "");
			})
			.collect(toList());
	}
