		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallIssuance", ValidationResult.ValidationType.CARDINALITY, "MarginCallIssuance", path, "", res.getError());
				}
				return success("MarginCallIssuance", ValidationResult.ValidationType.CARDINALITY, "MarginCallIssuance", path, "");
			})
			.collect(toList());
	}
