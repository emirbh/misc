		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingAmountEvents", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingAmountEvents", path, "", res.getError());
				}
				return success("FloatingAmountEvents", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingAmountEvents", path, "");
			})
			.collect(toList());
	}
