		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustedRelativeDateOffset", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustedRelativeDateOffset", path, "", res.getError());
				}
				return success("AdjustedRelativeDateOffset", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustedRelativeDateOffset", path, "");
			})
			.collect(toList());
	}
