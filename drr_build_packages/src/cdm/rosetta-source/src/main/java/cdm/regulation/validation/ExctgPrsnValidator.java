		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExctgPrsn", ValidationResult.ValidationType.CARDINALITY, "ExctgPrsn", path, "", res.getError());
				}
				return success("ExctgPrsn", ValidationResult.ValidationType.CARDINALITY, "ExctgPrsn", path, "");
			})
			.collect(toList());
	}
