		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyReferencePayerReceiver", ValidationResult.ValidationType.CARDINALITY, "PartyReferencePayerReceiver", path, "", res.getError());
				}
				return success("PartyReferencePayerReceiver", ValidationResult.ValidationType.CARDINALITY, "PartyReferencePayerReceiver", path, "");
			})
			.collect(toList());
	}
