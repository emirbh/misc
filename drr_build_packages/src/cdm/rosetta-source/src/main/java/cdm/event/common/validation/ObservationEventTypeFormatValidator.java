		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationEvent", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationEvent", path, "", res.getError());
				}
				return success("ObservationEvent", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationEvent", path, "");
			})
			.collect(toList());
	}
