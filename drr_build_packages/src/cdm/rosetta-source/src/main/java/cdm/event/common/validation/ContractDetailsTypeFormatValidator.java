		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContractDetails", ValidationResult.ValidationType.TYPE_FORMAT, "ContractDetails", path, "", res.getError());
				}
				return success("ContractDetails", ValidationResult.ValidationType.TYPE_FORMAT, "ContractDetails", path, "");
			})
			.collect(toList());
	}
