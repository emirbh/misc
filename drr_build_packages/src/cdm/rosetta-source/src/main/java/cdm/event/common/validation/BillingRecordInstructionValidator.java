...
public class BillingRecordInstructionValidator implements Validator<BillingRecordInstruction> {
	...
	private List<ComparisonResult> getComparisonResults(BillingRecordInstruction o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("observation", (List<? extends Observation>) o.getObservation() == null ? 0 : o.getObservation().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingRecordInstruction", ValidationResult.ValidationType.CARDINALITY, "BillingRecordInstruction", path, "", res.getError());
				}
				return success("BillingRecordInstruction", ValidationResult.ValidationType.CARDINALITY, "BillingRecordInstruction", path, "");
			})
			.collect(toList());
	}
