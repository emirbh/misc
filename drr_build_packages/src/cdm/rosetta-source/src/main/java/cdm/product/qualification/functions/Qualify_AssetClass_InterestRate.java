	...
	*/
	...
	public static class Qualify_AssetClass_InterestRateDefault extends Qualify_AssetClass_InterestRate {
		...
		protected Boolean assignOutput(Boolean is_product, EconomicTerms economicTerms) {
			...
			is_product = ComparisonResult.ofNullSafe(MapperS.of(interestRatePayoutOnlyExists.evaluate(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).getMulti()))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(optionPayoutOnlyExists.evaluate(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).getMulti()))).andNullSafe(ComparisonResult.ofNullSafe(MapperS.of(observableQualification.evaluate((referenceWithMetaObservable == null ? null : referenceWithMetaObservable.getValue()), InstrumentTypeEnum.DEBT, AssetClassEnum.INTEREST_RATE))).orNullSafe(areEqual(optionUnderlier(economicTerms).<Product>map("getProduct", underlier -> underlier.getProduct()).<TransferableProduct>map("getTransferableProduct", product -> product.getTransferableProduct()).<Instrument>map("getInstrument", transferableProduct -> transferableProduct.getInstrument()).<InstrumentTypeEnum>map("chooseInstrumentType", instrument -> instrumentDeepPathUtil.chooseInstrumentType(instrument)), MapperS.of(InstrumentTypeEnum.DEBT), CardinalityOperator.All)).orNullSafe(ifThenElseResult0))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(settlementPayoutOnlyExists.evaluate(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).getMulti()))).orNullSafe(exists(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).<SettlementPayout>map("getSettlementPayout", payout -> payout.getSettlementPayout())).andNullSafe(exists(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).<InterestRatePayout>map("getInterestRatePayout", payout -> payout.getInterestRatePayout())))).andNullSafe(ComparisonResult.ofNullSafe(MapperS.of(underlierQualification.evaluate(settlementUnderlier(economicTerms).get(), InstrumentTypeEnum.DEBT, AssetClassEnum.INTEREST_RATE))).orNullSafe(ifThenElseResult1))).get();
