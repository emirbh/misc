		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PayoutBase", ValidationResult.ValidationType.TYPE_FORMAT, "PayoutBase", path, "", res.getError());
				}
				return success("PayoutBase", ValidationResult.ValidationType.TYPE_FORMAT, "PayoutBase", path, "");
			})
			.collect(toList());
	}
