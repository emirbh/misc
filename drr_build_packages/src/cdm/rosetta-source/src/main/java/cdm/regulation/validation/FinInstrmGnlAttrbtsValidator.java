		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FinInstrmGnlAttrbts", ValidationResult.ValidationType.CARDINALITY, "FinInstrmGnlAttrbts", path, "", res.getError());
				}
				return success("FinInstrmGnlAttrbts", ValidationResult.ValidationType.CARDINALITY, "FinInstrmGnlAttrbts", path, "");
			})
			.collect(toList());
	}
