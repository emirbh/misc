...
public abstract class Create_Reset implements RosettaFunction {
	...
	*/
	...
	protected abstract MapperC<? extends ReferenceWithMetaPayout> payout(ResetInstruction instruction, TradeState tradeState);
	...
	public static class Create_ResetDefault extends Create_Reset {
		...
		protected TradeState.TradeStateBuilder assignOutput(TradeState.TradeStateBuilder reset, ResetInstruction instruction, TradeState tradeState) {
			reset = toBuilder(tradeState, () -> TradeState.builder());
			
			final List<Reset> ifThenElseResult;
			if (areEqual(MapperS.of(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<PerformancePayout>map("getPerformancePayout", _payout -> _payout.getPerformancePayout()).resultCount()), MapperS.of(1), CardinalityOperator.All).getOrDefault(false)) {
				final Reset reset0 = resolvePerformanceReset.evaluate(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<PerformancePayout>map("getPerformancePayout", _payout -> _payout.getPerformancePayout()).get(), observation(instruction, tradeState).get(), MapperS.of(instruction).<Date>map("getResetDate", resetInstruction -> resetInstruction.getResetDate()).get());
				ifThenElseResult = reset0 == null ? Collections.<Reset>emptyList() : Collections.singletonList(reset0);
			} else if (exists(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<InterestRatePayout>map("getInterestRatePayout", _payout -> _payout.getInterestRatePayout())).getOrDefault(false)) {
				final Reset reset1 = resolveInterestRateReset.evaluate(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<InterestRatePayout>map("getInterestRatePayout", _payout -> _payout.getInterestRatePayout()).getMulti(), observation(instruction, tradeState).get(), MapperS.of(instruction).<Date>map("getResetDate", resetInstruction -> resetInstruction.getResetDate()).get(), MapperS.of(instruction).<Date>map("getRateRecordDate", resetInstruction -> resetInstruction.getRateRecordDate()).get());
		...
		@Override
		protected MapperC<? extends ReferenceWithMetaPayout> payout(ResetInstruction instruction, TradeState tradeState) {
			return MapperS.of(instruction).<ReferenceWithMetaPayout>mapC("getPayout", resetInstruction -> resetInstruction.getPayout());
		}
		...
		protected MapperS<? extends ObservationIdentifier> observationIdentifiers(ResetInstruction instruction, TradeState tradeState) {
			if (areEqual(MapperS.of(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<PerformancePayout>map("getPerformancePayout", _payout -> _payout.getPerformancePayout()).resultCount()), MapperS.of(1), CardinalityOperator.All).getOrDefault(false)) {
				return MapperS.of(resolvePerformanceObservationIdentifiers.evaluate(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<PerformancePayout>map("getPerformancePayout", _payout -> _payout.getPerformancePayout()).get(), MapperS.of(instruction).<Date>map("getResetDate", resetInstruction -> resetInstruction.getResetDate()).get()));
			}
			if (exists(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<InterestRatePayout>map("getInterestRatePayout", _payout -> _payout.getInterestRatePayout())).getOrDefault(false)) {
				return MapperS.of(resolveInterestRateObservationIdentifiers.evaluate(payout(instruction, tradeState).<Payout>map("Type coercion", referenceWithMetaPayout -> referenceWithMetaPayout.getValue()).<InterestRatePayout>map("getInterestRatePayout", _payout -> _payout.getInterestRatePayout()).get(), observationDate(instruction, tradeState).get()));
			}
