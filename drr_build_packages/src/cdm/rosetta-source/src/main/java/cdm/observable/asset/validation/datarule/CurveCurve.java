	...
	class Default implements CurveCurve {
		...
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, Curve curve) {
			...
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Curve", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "Curve", path, DEFINITION, failureMessage));
		}
