		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StandardizedScheduleInitialMargin", ValidationResult.ValidationType.CARDINALITY, "StandardizedScheduleInitialMargin", path, "", res.getError());
				}
				return success("StandardizedScheduleInitialMargin", ValidationResult.ValidationType.CARDINALITY, "StandardizedScheduleInitialMargin", path, "");
			})
			.collect(toList());
	}
