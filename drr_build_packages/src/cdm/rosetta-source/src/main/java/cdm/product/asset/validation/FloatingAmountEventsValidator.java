		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingAmountEvents", ValidationResult.ValidationType.CARDINALITY, "FloatingAmountEvents", path, "", res.getError());
				}
				return success("FloatingAmountEvents", ValidationResult.ValidationType.CARDINALITY, "FloatingAmountEvents", path, "");
			})
			.collect(toList());
	}
