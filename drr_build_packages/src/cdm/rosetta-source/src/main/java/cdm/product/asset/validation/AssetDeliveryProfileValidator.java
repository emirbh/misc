		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetDeliveryProfile", ValidationResult.ValidationType.CARDINALITY, "AssetDeliveryProfile", path, "", res.getError());
				}
				return success("AssetDeliveryProfile", ValidationResult.ValidationType.CARDINALITY, "AssetDeliveryProfile", path, "");
			})
			.collect(toList());
	}
