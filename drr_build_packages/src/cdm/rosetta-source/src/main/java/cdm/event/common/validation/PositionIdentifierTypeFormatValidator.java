		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PositionIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "PositionIdentifier", path, "", res.getError());
				}
				return success("PositionIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "PositionIdentifier", path, "");
			})
			.collect(toList());
	}
