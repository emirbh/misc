		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateIndex", ValidationResult.ValidationType.CARDINALITY, "FloatingRateIndex", path, "", res.getError());
				}
				return success("FloatingRateIndex", ValidationResult.ValidationType.CARDINALITY, "FloatingRateIndex", path, "");
			})
			.collect(toList());
	}
