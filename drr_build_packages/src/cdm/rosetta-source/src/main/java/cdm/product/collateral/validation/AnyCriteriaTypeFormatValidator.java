		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AnyCriteria", ValidationResult.ValidationType.TYPE_FORMAT, "AnyCriteria", path, "", res.getError());
				}
				return success("AnyCriteria", ValidationResult.ValidationType.TYPE_FORMAT, "AnyCriteria", path, "");
			})
			.collect(toList());
	}
