		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CustomisedWorkflow", ValidationResult.ValidationType.TYPE_FORMAT, "CustomisedWorkflow", path, "", res.getError());
				}
				return success("CustomisedWorkflow", ValidationResult.ValidationType.TYPE_FORMAT, "CustomisedWorkflow", path, "");
			})
			.collect(toList());
	}
