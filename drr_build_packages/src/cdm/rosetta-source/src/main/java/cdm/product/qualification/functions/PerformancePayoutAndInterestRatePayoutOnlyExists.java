	...
	*/
	...
	public static class PerformancePayoutAndInterestRatePayoutOnlyExistsDefault extends PerformancePayoutAndInterestRatePayoutOnlyExists {
		...
		protected Boolean assignOutput(Boolean result, List<? extends Payout> payouts) {
			result = exists(MapperC.<Payout>of(payouts).<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout())).andNullSafe(exists(MapperC.<Payout>of(payouts).<InterestRatePayout>map("getInterestRatePayout", payout -> payout.getInterestRatePayout()))).andNullSafe(areEqual(MapperC.<Payout>of(payouts)
				.mapItem(item -> exists(item.<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout())).orNullSafe(exists(item.<InterestRatePayout>map("getInterestRatePayout", payout -> payout.getInterestRatePayout()))).asMapper()), MapperS.of(true), CardinalityOperator.All)).get();
