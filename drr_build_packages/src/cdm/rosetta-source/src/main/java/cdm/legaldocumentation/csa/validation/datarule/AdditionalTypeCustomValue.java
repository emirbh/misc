...
@ImplementedBy(AdditionalTypeCustomValue.Default.class)
public interface AdditionalTypeCustomValue extends Validator<AdditionalType> {
	
	String NAME = "AdditionalTypeCustomValue";
	String DEFINITION = "if standardValue = AdditionalTypeEnum -> Other then customValue exists";
	
	class Default implements AdditionalTypeCustomValue {
		...
		private ComparisonResult executeDataRule(AdditionalType additionalType) {
			try {
				if (areEqual(MapperS.of(additionalType).<AdditionalTypeEnum>map("getStandardValue", _additionalType -> _additionalType.getStandardValue()), MapperS.of(AdditionalTypeEnum.OTHER), CardinalityOperator.All).getOrDefault(false)) {
	...
	class NoOp implements AdditionalTypeCustomValue {
