		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableRelativeOrPeriodicDates", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableRelativeOrPeriodicDates", path, "", res.getError());
				}
				return success("AdjustableRelativeOrPeriodicDates", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableRelativeOrPeriodicDates", path, "");
			})
			.collect(toList());
	}
