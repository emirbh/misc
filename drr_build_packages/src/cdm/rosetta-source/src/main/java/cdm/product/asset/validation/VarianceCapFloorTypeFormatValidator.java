		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VarianceCapFloor", ValidationResult.ValidationType.TYPE_FORMAT, "VarianceCapFloor", path, "", res.getError());
				}
				return success("VarianceCapFloor", ValidationResult.ValidationType.TYPE_FORMAT, "VarianceCapFloor", path, "");
			})
			.collect(toList());
	}
