		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationMethod", ValidationResult.ValidationType.CARDINALITY, "ValuationMethod", path, "", res.getError());
				}
				return success("ValuationMethod", ValidationResult.ValidationType.CARDINALITY, "ValuationMethod", path, "");
			})
			.collect(toList());
	}
