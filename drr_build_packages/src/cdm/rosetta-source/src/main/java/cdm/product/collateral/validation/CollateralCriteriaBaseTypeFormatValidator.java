		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralCriteriaBase", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralCriteriaBase", path, "", res.getError());
				}
				return success("CollateralCriteriaBase", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralCriteriaBase", path, "");
			})
			.collect(toList());
	}
