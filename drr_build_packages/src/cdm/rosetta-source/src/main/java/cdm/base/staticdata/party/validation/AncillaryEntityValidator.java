		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AncillaryEntity", ValidationResult.ValidationType.CARDINALITY, "AncillaryEntity", path, "", res.getError());
				}
				return success("AncillaryEntity", ValidationResult.ValidationType.CARDINALITY, "AncillaryEntity", path, "");
			})
			.collect(toList());
	}
