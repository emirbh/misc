		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationFrequency", ValidationResult.ValidationType.CARDINALITY, "CalculationFrequency", path, "", res.getError());
				}
				return success("CalculationFrequency", ValidationResult.ValidationType.CARDINALITY, "CalculationFrequency", path, "");
			})
			.collect(toList());
	}
