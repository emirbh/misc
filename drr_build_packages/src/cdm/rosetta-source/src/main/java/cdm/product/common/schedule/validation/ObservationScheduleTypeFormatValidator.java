		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationSchedule", path, "", res.getError());
				}
				return success("ObservationSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationSchedule", path, "");
			})
			.collect(toList());
	}
