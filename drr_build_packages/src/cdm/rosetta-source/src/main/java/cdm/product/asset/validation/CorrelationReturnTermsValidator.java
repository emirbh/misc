		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationReturnTerms", ValidationResult.ValidationType.CARDINALITY, "CorrelationReturnTerms", path, "", res.getError());
				}
				return success("CorrelationReturnTerms", ValidationResult.ValidationType.CARDINALITY, "CorrelationReturnTerms", path, "");
			})
			.collect(toList());
	}
