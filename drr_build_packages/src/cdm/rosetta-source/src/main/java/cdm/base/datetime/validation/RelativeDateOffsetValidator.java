		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RelativeDateOffset", ValidationResult.ValidationType.CARDINALITY, "RelativeDateOffset", path, "", res.getError());
				}
				return success("RelativeDateOffset", ValidationResult.ValidationType.CARDINALITY, "RelativeDateOffset", path, "");
			})
			.collect(toList());
	}
