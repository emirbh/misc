		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResolvablePriceQuantity", ValidationResult.ValidationType.CARDINALITY, "ResolvablePriceQuantity", path, "", res.getError());
				}
				return success("ResolvablePriceQuantity", ValidationResult.ValidationType.CARDINALITY, "ResolvablePriceQuantity", path, "");
			})
			.collect(toList());
	}
