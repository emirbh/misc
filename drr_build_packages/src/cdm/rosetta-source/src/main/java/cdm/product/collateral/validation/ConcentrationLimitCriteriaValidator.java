...
public class ConcentrationLimitCriteriaValidator implements Validator<ConcentrationLimitCriteria> {
	...
	private List<ComparisonResult> getComparisonResults(ConcentrationLimitCriteria o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("collateralCriteria", (CollateralCriteria) o.getCollateralCriteria() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConcentrationLimitCriteria", ValidationResult.ValidationType.CARDINALITY, "ConcentrationLimitCriteria", path, "", res.getError());
				}
				return success("ConcentrationLimitCriteria", ValidationResult.ValidationType.CARDINALITY, "ConcentrationLimitCriteria", path, "");
			})
			.collect(toList());
	}
