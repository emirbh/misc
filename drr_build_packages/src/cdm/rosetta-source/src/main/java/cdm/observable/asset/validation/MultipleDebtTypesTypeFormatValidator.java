		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MultipleDebtTypes", ValidationResult.ValidationType.TYPE_FORMAT, "MultipleDebtTypes", path, "", res.getError());
				}
				return success("MultipleDebtTypes", ValidationResult.ValidationType.TYPE_FORMAT, "MultipleDebtTypes", path, "");
			})
			.collect(toList());
	}
