		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DebtType", ValidationResult.ValidationType.CARDINALITY, "DebtType", path, "", res.getError());
				}
				return success("DebtType", ValidationResult.ValidationType.CARDINALITY, "DebtType", path, "");
			})
			.collect(toList());
	}
