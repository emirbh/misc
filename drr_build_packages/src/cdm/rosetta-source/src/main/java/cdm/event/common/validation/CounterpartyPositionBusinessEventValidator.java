		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CounterpartyPositionBusinessEvent", ValidationResult.ValidationType.CARDINALITY, "CounterpartyPositionBusinessEvent", path, "", res.getError());
				}
				return success("CounterpartyPositionBusinessEvent", ValidationResult.ValidationType.CARDINALITY, "CounterpartyPositionBusinessEvent", path, "");
			})
			.collect(toList());
	}
