		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProtectionTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ProtectionTerms", path, "", res.getError());
				}
				return success("ProtectionTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ProtectionTerms", path, "");
			})
			.collect(toList());
	}
