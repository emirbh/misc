...
public abstract class Create_Return implements RosettaFunction {
	...
	*/
	...
	public static class Create_ReturnDefault extends Create_Return {
		...
		protected BusinessEvent.BusinessEventBuilder assignOutput(BusinessEvent.BusinessEventBuilder returnEvent, TradeState tradeState, ReturnInstruction returnInstruction, Date returnDate) {
			final TradeState _tradeState = create_QuantityChange.evaluate(QuantityChangeInstruction.builder()
				.setChange(new ArrayList<>(changePriceQuantity(tradeState, returnInstruction, returnDate).getMulti()))
				.setDirection(QuantityChangeDirectionEnum.DECREASE)
				.setLotIdentifier(Collections.<Identifier>emptyList())
				.build(), tradeState);
			returnEvent
				.addAfter((_tradeState == null ? Collections.<TradeState>emptyList() : Collections.singletonList(_tradeState)));
		...
		protected MapperC<? extends NonNegativeQuantitySchedule> quantitySchedule(TradeState tradeState, ReturnInstruction returnInstruction, Date returnDate) {
			return MapperS.of(returnInstruction).<Quantity>mapC("getQuantity", _returnInstruction -> _returnInstruction.getQuantity())
				.mapItem(item -> MapperS.of(NonNegativeQuantitySchedule.builder()
					.setValue(item.<BigDecimal>map("getValue", quantity -> quantity.getValue()).get())
					.setUnit(item.<UnitType>map("getUnit", quantity -> quantity.getUnit()).get())
					.build()));
		}
