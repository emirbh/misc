		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventInstruction", ValidationResult.ValidationType.CARDINALITY, "EventInstruction", path, "", res.getError());
				}
				return success("EventInstruction", ValidationResult.ValidationType.CARDINALITY, "EventInstruction", path, "");
			})
			.collect(toList());
	}
