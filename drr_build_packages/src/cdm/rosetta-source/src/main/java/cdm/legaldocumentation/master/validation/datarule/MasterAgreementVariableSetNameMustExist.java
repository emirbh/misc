 ...
 * @version 6.23.0
 */
@RosettaDataRule("MasterAgreementVariableSetNameMustExist")
@ImplementedBy(MasterAgreementVariableSetNameMustExist.Default.class)
public interface MasterAgreementVariableSetNameMustExist extends Validator<MasterAgreementVariableSet> {
	
	String NAME = "MasterAgreementVariableSetNameMustExist";
	String DEFINITION = "if name exists then value exists";
	
	class Default implements MasterAgreementVariableSetNameMustExist {
		...
		private ComparisonResult executeDataRule(MasterAgreementVariableSet masterAgreementVariableSet) {
			try {
				if (exists(MapperS.of(masterAgreementVariableSet).<String>map("getName", _masterAgreementVariableSet -> _masterAgreementVariableSet.getName())).getOrDefault(false)) {
					return exists(MapperS.of(masterAgreementVariableSet).<String>map("getValue", _masterAgreementVariableSet -> _masterAgreementVariableSet.getValue()));
				}
	...
	class NoOp implements MasterAgreementVariableSetNameMustExist {
