		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralCriteria", ValidationResult.ValidationType.CARDINALITY, "CollateralCriteria", path, "", res.getError());
				}
				return success("CollateralCriteria", ValidationResult.ValidationType.CARDINALITY, "CollateralCriteria", path, "");
			})
			.collect(toList());
	}
