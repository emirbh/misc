		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VarianceReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "VarianceReturnTerms", path, "", res.getError());
				}
				return success("VarianceReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "VarianceReturnTerms", path, "");
			})
			.collect(toList());
	}
