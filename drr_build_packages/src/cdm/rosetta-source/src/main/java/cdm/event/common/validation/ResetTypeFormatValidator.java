		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Reset", ValidationResult.ValidationType.TYPE_FORMAT, "Reset", path, "", res.getError());
				}
				return success("Reset", ValidationResult.ValidationType.TYPE_FORMAT, "Reset", path, "");
			})
			.collect(toList());
	}
