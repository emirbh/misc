		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPriceReturnTerms", ValidationResult.ValidationType.CARDINALITY, "CommodityPriceReturnTerms", path, "", res.getError());
				}
				return success("CommodityPriceReturnTerms", ValidationResult.ValidationType.CARDINALITY, "CommodityPriceReturnTerms", path, "");
			})
			.collect(toList());
	}
