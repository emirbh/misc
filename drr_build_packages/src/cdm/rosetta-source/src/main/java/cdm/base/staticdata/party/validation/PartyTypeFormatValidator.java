		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Party", ValidationResult.ValidationType.TYPE_FORMAT, "Party", path, "", res.getError());
				}
				return success("Party", ValidationResult.ValidationType.TYPE_FORMAT, "Party", path, "");
			})
			.collect(toList());
	}
