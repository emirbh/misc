...
public class PerformancePayoutValidator implements Validator<PerformancePayout> {
	...
	private List<ComparisonResult> getComparisonResults(PerformancePayout o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("underlier", (Underlier) o.getUnderlier() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PerformancePayout", ValidationResult.ValidationType.CARDINALITY, "PerformancePayout", path, "", res.getError());
				}
				return success("PerformancePayout", ValidationResult.ValidationType.CARDINALITY, "PerformancePayout", path, "");
			})
			.collect(toList());
	}
