		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegalAgreement", ValidationResult.ValidationType.CARDINALITY, "LegalAgreement", path, "", res.getError());
				}
				return success("LegalAgreement", ValidationResult.ValidationType.CARDINALITY, "LegalAgreement", path, "");
			})
			.collect(toList());
	}
