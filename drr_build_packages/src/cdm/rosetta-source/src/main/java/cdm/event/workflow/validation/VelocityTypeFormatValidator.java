		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Velocity", ValidationResult.ValidationType.TYPE_FORMAT, "Velocity", path, "", res.getError());
				}
				return success("Velocity", ValidationResult.ValidationType.TYPE_FORMAT, "Velocity", path, "");
			})
			.collect(toList());
	}
