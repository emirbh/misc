		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyCustomisedWorkflow", ValidationResult.ValidationType.CARDINALITY, "PartyCustomisedWorkflow", path, "", res.getError());
				}
				return success("PartyCustomisedWorkflow", ValidationResult.ValidationType.CARDINALITY, "PartyCustomisedWorkflow", path, "");
			})
			.collect(toList());
	}
