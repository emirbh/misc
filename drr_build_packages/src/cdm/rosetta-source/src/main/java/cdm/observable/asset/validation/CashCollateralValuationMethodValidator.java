...
public class CashCollateralValuationMethodValidator implements Validator<CashCollateralValuationMethod> {
	...
	private List<ComparisonResult> getComparisonResults(CashCollateralValuationMethod o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("protectedParty", (List<PartyDeterminationEnum>) o.getProtectedParty() == null ? 0 : o.getProtectedParty().size(), 0, 2), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashCollateralValuationMethod", ValidationResult.ValidationType.CARDINALITY, "CashCollateralValuationMethod", path, "", res.getError());
				}
				return success("CashCollateralValuationMethod", ValidationResult.ValidationType.CARDINALITY, "CashCollateralValuationMethod", path, "");
			})
			.collect(toList());
	}
