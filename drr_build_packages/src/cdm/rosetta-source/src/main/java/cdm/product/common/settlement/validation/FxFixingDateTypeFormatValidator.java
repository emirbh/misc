		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFixingDate", ValidationResult.ValidationType.TYPE_FORMAT, "FxFixingDate", path, "", res.getError());
				}
				return success("FxFixingDate", ValidationResult.ValidationType.TYPE_FORMAT, "FxFixingDate", path, "");
			})
			.collect(toList());
	}
