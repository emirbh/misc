		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WorkflowStepApproval", ValidationResult.ValidationType.CARDINALITY, "WorkflowStepApproval", path, "", res.getError());
				}
				return success("WorkflowStepApproval", ValidationResult.ValidationType.CARDINALITY, "WorkflowStepApproval", path, "");
			})
			.collect(toList());
	}
