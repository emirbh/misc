		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Position", ValidationResult.ValidationType.CARDINALITY, "Position", path, "", res.getError());
				}
				return success("Position", ValidationResult.ValidationType.CARDINALITY, "Position", path, "");
			})
			.collect(toList());
	}
