		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExerciseTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ExerciseTerms", path, "", res.getError());
				}
				return success("ExerciseTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ExerciseTerms", path, "");
			})
			.collect(toList());
	}
