	...
	*/
	...
	public static class Qualify_CancellationDefault extends Qualify_Cancellation {
		...
		protected Boolean assignOutput(Boolean is_event, BusinessEvent businessEvent) {
			...
			if (exists(openEconomicTerms(businessEvent).<AdjustableOrRelativeDate>map("getTerminationDate", economicTerms -> economicTerms.getTerminationDate())).andNullSafe(exists(closedEconomicTerms(businessEvent).<AdjustableOrRelativeDate>map("getTerminationDate", economicTerms -> economicTerms.getTerminationDate()))).getOrDefault(false)) {
			...
			is_event = areEqual(MapperS.of(MapperS.of(businessEvent).<Instruction>mapC("getInstruction", _businessEvent -> _businessEvent.getInstruction()).<ReferenceWithMetaTradeState>map("getBefore", instruction -> instruction.getBefore()).resultCount()), MapperS.of(1), CardinalityOperator.All).andNullSafe(exists(closedEconomicTerms(businessEvent))).andNullSafe(exists(openEconomicTerms(businessEvent))).andNullSafe(ifThenElseResult).get();
