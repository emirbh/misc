		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FinInstrm", ValidationResult.ValidationType.TYPE_FORMAT, "FinInstrm", path, "", res.getError());
				}
				return success("FinInstrm", ValidationResult.ValidationType.TYPE_FORMAT, "FinInstrm", path, "");
			})
			.collect(toList());
	}
