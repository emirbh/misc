...
public class LegalAgreementBaseValidator implements Validator<LegalAgreementBase> {
	...
	private List<ComparisonResult> getComparisonResults(LegalAgreementBase o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("contractualParty", (List<? extends ReferenceWithMetaParty>) o.getContractualParty() == null ? 0 : o.getContractualParty().size(), 2, 2)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegalAgreementBase", ValidationResult.ValidationType.CARDINALITY, "LegalAgreementBase", path, "", res.getError());
				}
				return success("LegalAgreementBase", ValidationResult.ValidationType.CARDINALITY, "LegalAgreementBase", path, "");
			})
			.collect(toList());
	}
