		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Term", ValidationResult.ValidationType.TYPE_FORMAT, "Term", path, "", res.getError());
				}
				return success("Term", ValidationResult.ValidationType.TYPE_FORMAT, "Term", path, "");
			})
			.collect(toList());
	}
