...
public abstract class Create_AdjustmentPrimitiveInstruction implements RosettaFunction {
	...
	*/
	public PrimitiveInstruction evaluate(TradeState tradeState, BigDecimal newAllinPrice, BigDecimal newAssetQuantity, AdjustableOrRelativeDate effectiveRepriceDate) {
		// pre-conditions
		conditionValidator.validate(() -> areEqual(MapperS.of(MapperS.of(tradeState).<Trade>map("getTrade", _tradeState -> _tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).resultCount()), MapperS.of(1), CardinalityOperator.All),
	...
	protected abstract MapperS<? extends FieldWithMetaPriceSchedule> currentAssetPrice(TradeState tradeState, BigDecimal newAllinPrice, BigDecimal newAssetQuantity, AdjustableOrRelativeDate effectiveRepriceDate);
	...
	public static class Create_AdjustmentPrimitiveInstructionDefault extends Create_AdjustmentPrimitiveInstruction {
		...
		protected PrimitiveInstruction.PrimitiveInstructionBuilder assignOutput(PrimitiveInstruction.PrimitiveInstructionBuilder instruction, TradeState tradeState, BigDecimal newAllinPrice, BigDecimal newAssetQuantity, AdjustableOrRelativeDate effectiveRepriceDate) {
			instruction
				.getOrCreateSplit()
				.setBreakdown(MapperC.<PrimitiveInstruction>of(MapperS.of(create_TerminationInstruction.evaluate(tradeState, effectiveRepriceDate))).getMulti());
		...
		@Override
		protected MapperS<? extends FieldWithMetaPriceSchedule> currentAssetPrice(TradeState tradeState, BigDecimal newAllinPrice, BigDecimal newAssetQuantity, AdjustableOrRelativeDate effectiveRepriceDate) {
			final MapperListOfLists<FieldWithMetaPriceSchedule> thenArg0 = oldPriceQuantity(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate)
		...
		protected MapperS<? extends Price> newPrice(TradeState tradeState, BigDecimal newAllinPrice, BigDecimal newAssetQuantity, AdjustableOrRelativeDate effectiveRepriceDate) {
			return MapperS.of(Price.builder()
				...
				.setUnit(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule0 -> fieldWithMetaPriceSchedule0 == null ? null : fieldWithMetaPriceSchedule0.getValue()).<UnitType>map("getUnit", priceSchedule -> priceSchedule.getUnit()).get())
				.setPerUnitOf(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule1 -> fieldWithMetaPriceSchedule1 == null ? null : fieldWithMetaPriceSchedule1.getValue()).<UnitType>map("getPerUnitOf", priceSchedule -> priceSchedule.getPerUnitOf()).get())
				.setPriceType(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule2 -> fieldWithMetaPriceSchedule2 == null ? null : fieldWithMetaPriceSchedule2.getValue()).<PriceTypeEnum>map("getPriceType", priceSchedule -> priceSchedule.getPriceType()).get())
				.setPriceExpression(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule3 -> fieldWithMetaPriceSchedule3 == null ? null : fieldWithMetaPriceSchedule3.getValue()).<PriceExpressionEnum>map("getPriceExpression", priceSchedule -> priceSchedule.getPriceExpression()).get())
				.setComposite(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule4 -> fieldWithMetaPriceSchedule4 == null ? null : fieldWithMetaPriceSchedule4.getValue()).<PriceComposite>map("getComposite", priceSchedule -> priceSchedule.getComposite()).get())
				.setArithmeticOperator(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule5 -> fieldWithMetaPriceSchedule5 == null ? null : fieldWithMetaPriceSchedule5.getValue()).<ArithmeticOperationEnum>map("getArithmeticOperator", priceSchedule -> priceSchedule.getArithmeticOperator()).get())
				.setCashPrice(currentAssetPrice(tradeState, newAllinPrice, newAssetQuantity, effectiveRepriceDate).<PriceSchedule>map("Type coercion", fieldWithMetaPriceSchedule6 -> fieldWithMetaPriceSchedule6 == null ? null : fieldWithMetaPriceSchedule6.getValue()).<CashPrice>map("getCashPrice", priceSchedule -> priceSchedule.getCashPrice()).get())
