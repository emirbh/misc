		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeterminationMethodology", ValidationResult.ValidationType.TYPE_FORMAT, "DeterminationMethodology", path, "", res.getError());
				}
				return success("DeterminationMethodology", ValidationResult.ValidationType.TYPE_FORMAT, "DeterminationMethodology", path, "");
			})
			.collect(toList());
	}
