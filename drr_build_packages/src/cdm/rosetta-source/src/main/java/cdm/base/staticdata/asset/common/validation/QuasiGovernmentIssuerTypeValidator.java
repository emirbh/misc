		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuasiGovernmentIssuerType", ValidationResult.ValidationType.CARDINALITY, "QuasiGovernmentIssuerType", path, "", res.getError());
				}
				return success("QuasiGovernmentIssuerType", ValidationResult.ValidationType.CARDINALITY, "QuasiGovernmentIssuerType", path, "");
			})
			.collect(toList());
	}
