		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ForeignExchangeRateIndex", ValidationResult.ValidationType.CARDINALITY, "ForeignExchangeRateIndex", path, "", res.getError());
				}
				return success("ForeignExchangeRateIndex", ValidationResult.ValidationType.CARDINALITY, "ForeignExchangeRateIndex", path, "");
			})
			.collect(toList());
	}
