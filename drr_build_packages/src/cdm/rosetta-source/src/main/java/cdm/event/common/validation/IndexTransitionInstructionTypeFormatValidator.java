		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexTransitionInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "IndexTransitionInstruction", path, "", res.getError());
				}
				return success("IndexTransitionInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "IndexTransitionInstruction", path, "");
			})
			.collect(toList());
	}
