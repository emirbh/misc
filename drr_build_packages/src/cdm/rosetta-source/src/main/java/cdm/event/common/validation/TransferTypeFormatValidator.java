		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Transfer", ValidationResult.ValidationType.TYPE_FORMAT, "Transfer", path, "", res.getError());
				}
				return success("Transfer", ValidationResult.ValidationType.TYPE_FORMAT, "Transfer", path, "");
			})
			.collect(toList());
	}
