	...
	public static class CriteriaMatchesAssetTypeDefault extends CriteriaMatchesAssetType {
		...
		protected Boolean assignOutput(Boolean matches, CollateralCriteria inputCriteria, InstrumentTypeEnum assetType) {
			...
			if (switchArgument.get() == null) {
				...
				final MapperS<AssetType> _assetType = switchArgument.<AssetType>map("getAssetType", collateralCriteria -> collateralCriteria.getAssetType());
				matches = areEqual(_assetType.<InstrumentTypeEnum>map("getSecurityType", __assetType -> __assetType.getSecurityType()), MapperS.of(assetType), CardinalityOperator.All).get();
				...
				final MapperC<Boolean> thenArg1 = allCriteria.<CollateralCriteria>mapC("getAllCriteria", _allCriteria -> _allCriteria.getAllCriteria())
					.mapItem(item -> MapperS.of(criteriaMatchesAssetType.evaluate(item.get(), assetType)));
				...
				matches = areEqual(MapperS.of(criteriaMatchesAssetType.evaluate(negativeCriteria.<CollateralCriteria>map("getNegativeCriteria", _negativeCriteria -> _negativeCriteria.getNegativeCriteria()).get(), assetType)), MapperS.of(false), CardinalityOperator.All).get();
			} else {
				matches = false;
			}
