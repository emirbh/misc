		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationTerms", ValidationResult.ValidationType.CARDINALITY, "ValuationTerms", path, "", res.getError());
				}
				return success("ValuationTerms", ValidationResult.ValidationType.CARDINALITY, "ValuationTerms", path, "");
			})
			.collect(toList());
	}
