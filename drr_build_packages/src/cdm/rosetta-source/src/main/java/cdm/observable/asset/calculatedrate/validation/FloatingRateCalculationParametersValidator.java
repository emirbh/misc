		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateCalculationParameters", ValidationResult.ValidationType.CARDINALITY, "FloatingRateCalculationParameters", path, "", res.getError());
				}
				return success("FloatingRateCalculationParameters", ValidationResult.ValidationType.CARDINALITY, "FloatingRateCalculationParameters", path, "");
			})
			.collect(toList());
	}
