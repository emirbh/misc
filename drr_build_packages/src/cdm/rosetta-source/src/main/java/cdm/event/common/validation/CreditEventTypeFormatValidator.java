		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEvent", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEvent", path, "", res.getError());
				}
				return success("CreditEvent", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEvent", path, "");
			})
			.collect(toList());
	}
