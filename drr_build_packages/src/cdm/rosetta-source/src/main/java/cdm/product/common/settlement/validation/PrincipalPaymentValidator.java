		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrincipalPayment", ValidationResult.ValidationType.CARDINALITY, "PrincipalPayment", path, "", res.getError());
				}
				return success("PrincipalPayment", ValidationResult.ValidationType.CARDINALITY, "PrincipalPayment", path, "");
			})
			.collect(toList());
	}
