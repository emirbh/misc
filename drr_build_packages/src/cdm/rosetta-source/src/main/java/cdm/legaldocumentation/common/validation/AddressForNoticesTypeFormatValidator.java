		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AddressForNotices", ValidationResult.ValidationType.TYPE_FORMAT, "AddressForNotices", path, "", res.getError());
				}
				return success("AddressForNotices", ValidationResult.ValidationType.TYPE_FORMAT, "AddressForNotices", path, "");
			})
			.collect(toList());
	}
