		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SpecificAsset", ValidationResult.ValidationType.CARDINALITY, "SpecificAsset", path, "", res.getError());
				}
				return success("SpecificAsset", ValidationResult.ValidationType.CARDINALITY, "SpecificAsset", path, "");
			})
			.collect(toList());
	}
