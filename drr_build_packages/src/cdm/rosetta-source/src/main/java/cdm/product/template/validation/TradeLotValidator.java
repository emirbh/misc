		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeLot", ValidationResult.ValidationType.CARDINALITY, "TradeLot", path, "", res.getError());
				}
				return success("TradeLot", ValidationResult.ValidationType.CARDINALITY, "TradeLot", path, "");
			})
			.collect(toList());
	}
