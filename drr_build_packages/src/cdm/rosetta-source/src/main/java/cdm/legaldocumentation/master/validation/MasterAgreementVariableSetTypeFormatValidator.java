		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementVariableSet", ValidationResult.ValidationType.TYPE_FORMAT, "MasterAgreementVariableSet", path, "", res.getError());
				}
				return success("MasterAgreementVariableSet", ValidationResult.ValidationType.TYPE_FORMAT, "MasterAgreementVariableSet", path, "");
			})
			.collect(toList());
	}
