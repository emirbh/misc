		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaxonomyClassification", ValidationResult.ValidationType.TYPE_FORMAT, "TaxonomyClassification", path, "", res.getError());
				}
				return success("TaxonomyClassification", ValidationResult.ValidationType.TYPE_FORMAT, "TaxonomyClassification", path, "");
			})
			.collect(toList());
	}
