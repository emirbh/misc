		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculatedRateObservationDatesAndWeights", ValidationResult.ValidationType.TYPE_FORMAT, "CalculatedRateObservationDatesAndWeights", path, "", res.getError());
				}
				return success("CalculatedRateObservationDatesAndWeights", ValidationResult.ValidationType.TYPE_FORMAT, "CalculatedRateObservationDatesAndWeights", path, "");
			})
			.collect(toList());
	}
