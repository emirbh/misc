		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UnitType", ValidationResult.ValidationType.CARDINALITY, "UnitType", path, "", res.getError());
				}
				return success("UnitType", ValidationResult.ValidationType.CARDINALITY, "UnitType", path, "");
			})
			.collect(toList());
	}
