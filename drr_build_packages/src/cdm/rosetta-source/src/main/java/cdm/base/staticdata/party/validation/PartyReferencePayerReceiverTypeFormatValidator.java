		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyReferencePayerReceiver", ValidationResult.ValidationType.TYPE_FORMAT, "PartyReferencePayerReceiver", path, "", res.getError());
				}
				return success("PartyReferencePayerReceiver", ValidationResult.ValidationType.TYPE_FORMAT, "PartyReferencePayerReceiver", path, "");
			})
			.collect(toList());
	}
