	...
	*/
	...
	public static class Qualify_SecurityTransferDefault extends Qualify_SecurityTransfer {
		...
		protected Boolean assignOutput(Boolean is_event, BusinessEvent businessEvent) {
			is_event = exists(transfer(businessEvent).<Asset>map("getAsset", _transfer -> _transfer.getAsset()).<Instrument>map("getInstrument", asset -> asset.getInstrument()).<Security>map("getSecurity", instrument -> instrument.getSecurity())).andNullSafe(onlyExists(transfer(businessEvent).<NonNegativeQuantity>map("getQuantity", _transfer -> _transfer.getQuantity()).<UnitType>map("getUnit", nonNegativeQuantity -> nonNegativeQuantity.getUnit()), Arrays.asList("capacityUnit", "weatherUnit", "financialUnit", "currency"), Arrays.asList("financialUnit"))).get();
