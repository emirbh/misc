		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransferInstruction", ValidationResult.ValidationType.CARDINALITY, "TransferInstruction", path, "", res.getError());
				}
				return success("TransferInstruction", ValidationResult.ValidationType.CARDINALITY, "TransferInstruction", path, "");
			})
			.collect(toList());
	}
