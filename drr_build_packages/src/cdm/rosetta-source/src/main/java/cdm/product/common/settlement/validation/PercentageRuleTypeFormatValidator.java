		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PercentageRule", ValidationResult.ValidationType.TYPE_FORMAT, "PercentageRule", path, "", res.getError());
				}
				return success("PercentageRule", ValidationResult.ValidationType.TYPE_FORMAT, "PercentageRule", path, "");
			})
			.collect(toList());
	}
