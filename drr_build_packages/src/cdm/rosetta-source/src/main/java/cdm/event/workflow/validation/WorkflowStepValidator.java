...
public class WorkflowStepValidator implements Validator<WorkflowStep> {
	...
	private List<ComparisonResult> getComparisonResults(WorkflowStep o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("timestamp", (List<? extends EventTimestamp>) o.getTimestamp() == null ? 0 : o.getTimestamp().size(), 1, 0), 
				checkCardinality("eventIdentifier", (List<? extends Identifier>) o.getEventIdentifier() == null ? 0 : o.getEventIdentifier().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WorkflowStep", ValidationResult.ValidationType.CARDINALITY, "WorkflowStep", path, "", res.getError());
				}
				return success("WorkflowStep", ValidationResult.ValidationType.CARDINALITY, "WorkflowStep", path, "");
			})
			.collect(toList());
	}
