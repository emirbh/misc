		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DatedValue", ValidationResult.ValidationType.TYPE_FORMAT, "DatedValue", path, "", res.getError());
				}
				return success("DatedValue", ValidationResult.ValidationType.TYPE_FORMAT, "DatedValue", path, "");
			})
			.collect(toList());
	}
