...
public interface AssetTypeEquitySubType extends Validator<AssetType> {
	...
	String DEFINITION = "if securityType <> InstrumentTypeEnum -> Equity then equityType is absent";
	...
	class Default implements AssetTypeEquitySubType {
		...
		private ComparisonResult executeDataRule(AssetType assetType) {
			try {
				if (notEqual(MapperS.of(assetType).<InstrumentTypeEnum>map("getSecurityType", _assetType -> _assetType.getSecurityType()), MapperS.of(InstrumentTypeEnum.EQUITY), CardinalityOperator.Any).getOrDefault(false)) {
