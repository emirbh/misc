		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DerivInstrmAttrbts", ValidationResult.ValidationType.CARDINALITY, "DerivInstrmAttrbts", path, "", res.getError());
				}
				return success("DerivInstrmAttrbts", ValidationResult.ValidationType.CARDINALITY, "DerivInstrmAttrbts", path, "");
			})
			.collect(toList());
	}
