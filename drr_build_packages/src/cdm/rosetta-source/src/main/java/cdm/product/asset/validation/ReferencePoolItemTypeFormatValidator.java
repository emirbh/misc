		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReferencePoolItem", ValidationResult.ValidationType.TYPE_FORMAT, "ReferencePoolItem", path, "", res.getError());
				}
				return success("ReferencePoolItem", ValidationResult.ValidationType.TYPE_FORMAT, "ReferencePoolItem", path, "");
			})
			.collect(toList());
	}
