		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestAmountApplication", ValidationResult.ValidationType.CARDINALITY, "InterestAmountApplication", path, "", res.getError());
				}
				return success("InterestAmountApplication", ValidationResult.ValidationType.CARDINALITY, "InterestAmountApplication", path, "");
			})
			.collect(toList());
	}
