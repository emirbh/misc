	...
	*/
	...
	public static class CheckMaturityDefault extends CheckMaturity {
		...
		protected Boolean assignOutput(Boolean isEqual, AssetMaturity maturityRange, EligibilityQuery query) {
			isEqual = notExists(MapperS.of(maturityRange)).orNullSafe(ComparisonResult.ofNullSafe(upperBoundCheck(maturityRange, query)).andNullSafe(ComparisonResult.ofNullSafe(lowerBoundCheck(maturityRange, query)))).get();
		...
		protected MapperS<Boolean> upperBoundCheck(AssetMaturity maturityRange, EligibilityQuery query) {
			...
			if (ComparisonResult.ofNullSafe(MapperS.of(maturityRange).<PeriodRange>map("getMaturityRange", assetMaturity -> assetMaturity.getMaturityRange()).<PeriodBound>map("getUpperBound", periodRange -> periodRange.getUpperBound()).<Boolean>map("getInclusive", periodBound -> periodBound.getInclusive())).orNullSafe(notExists(MapperS.of(maturityRange).<PeriodRange>map("getMaturityRange", assetMaturity -> assetMaturity.getMaturityRange()).<PeriodBound>map("getUpperBound", periodRange -> periodRange.getUpperBound()).<Boolean>map("getInclusive", periodBound -> periodBound.getInclusive()))).getOrDefault(false)) {
		...
		protected MapperS<Boolean> lowerBoundCheck(AssetMaturity maturityRange, EligibilityQuery query) {
			...
			if (ComparisonResult.ofNullSafe(MapperS.of(maturityRange).<PeriodRange>map("getMaturityRange", assetMaturity -> assetMaturity.getMaturityRange()).<PeriodBound>map("getLowerBound", periodRange -> periodRange.getLowerBound()).<Boolean>map("getInclusive", periodBound -> periodBound.getInclusive())).orNullSafe(notExists(MapperS.of(maturityRange).<PeriodRange>map("getMaturityRange", assetMaturity -> assetMaturity.getMaturityRange()).<PeriodBound>map("getLowerBound", periodRange -> periodRange.getLowerBound()).<Boolean>map("getInclusive", periodBound -> periodBound.getInclusive()))).getOrDefault(false)) {
