...
@ImplementedBy(CollateralIssuerTypeQuasiGovernmentSubType.Default.class)
public interface CollateralIssuerTypeQuasiGovernmentSubType extends Validator<CollateralIssuerType> {
	
	String NAME = "CollateralIssuerTypeQuasiGovernmentSubType";
	String DEFINITION = "if issuerType <> IssuerTypeEnum -> QuasiGovernment then quasiGovernmentType is absent";
	
	class Default implements CollateralIssuerTypeQuasiGovernmentSubType {
		...
		private ComparisonResult executeDataRule(CollateralIssuerType collateralIssuerType) {
			try {
				if (notEqual(MapperS.of(collateralIssuerType).<IssuerTypeEnum>map("getIssuerType", _collateralIssuerType -> _collateralIssuerType.getIssuerType()), MapperS.of(IssuerTypeEnum.QUASI_GOVERNMENT), CardinalityOperator.Any).getOrDefault(false)) {
	...
	class NoOp implements CollateralIssuerTypeQuasiGovernmentSubType {
