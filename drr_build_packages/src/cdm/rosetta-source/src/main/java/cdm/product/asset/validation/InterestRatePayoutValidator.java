		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestRatePayout", ValidationResult.ValidationType.CARDINALITY, "InterestRatePayout", path, "", res.getError());
				}
				return success("InterestRatePayout", ValidationResult.ValidationType.CARDINALITY, "InterestRatePayout", path, "");
			})
			.collect(toList());
	}
