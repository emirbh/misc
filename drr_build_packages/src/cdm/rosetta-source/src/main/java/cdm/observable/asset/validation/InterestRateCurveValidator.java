		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestRateCurve", ValidationResult.ValidationType.CARDINALITY, "InterestRateCurve", path, "", res.getError());
				}
				return success("InterestRateCurve", ValidationResult.ValidationType.CARDINALITY, "InterestRateCurve", path, "");
			})
			.collect(toList());
	}
