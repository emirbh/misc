		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentRule", ValidationResult.ValidationType.CARDINALITY, "PaymentRule", path, "", res.getError());
				}
				return success("PaymentRule", ValidationResult.ValidationType.CARDINALITY, "PaymentRule", path, "");
			})
			.collect(toList());
	}
