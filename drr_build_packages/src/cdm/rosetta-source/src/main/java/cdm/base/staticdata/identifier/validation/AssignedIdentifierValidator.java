		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssignedIdentifier", ValidationResult.ValidationType.CARDINALITY, "AssignedIdentifier", path, "", res.getError());
				}
				return success("AssignedIdentifier", ValidationResult.ValidationType.CARDINALITY, "AssignedIdentifier", path, "");
			})
			.collect(toList());
	}
