	...
	*/
	...
	public static class Qualify_AssetClass_ForeignExchangeDefault extends Qualify_AssetClass_ForeignExchange {
		...
		protected Boolean assignOutput(Boolean is_product, EconomicTerms economicTerms) {
			...
			is_product = ComparisonResult.ofNullSafe(MapperS.of(settlementPayoutOnlyExists.evaluate(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).getMulti()))).andNullSafe(areEqual(settlementUnderliers(economicTerms)
				.mapItem(item -> exists(item.<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", _referenceWithMetaObservable -> _referenceWithMetaObservable == null ? null : _referenceWithMetaObservable.getValue()).<Asset>map("getAsset", observable -> observable.getAsset()).<Cash>map("getCash", asset -> asset.getCash())).asMapper()), MapperS.of(true), CardinalityOperator.All).orNullSafe(areEqual(settlementUnderliers(economicTerms)
				.mapItem(item -> MapperS.of(underlierQualification.evaluate(item.get(), null, AssetClassEnum.FOREIGN_EXCHANGE))), MapperS.of(true), CardinalityOperator.All))).orNullSafe(exists(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout()).<Underlier>map("getUnderlier", performancePayout -> performancePayout.getUnderlier()).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", _referenceWithMetaObservable -> _referenceWithMetaObservable.getValue()).<Index>map("getIndex", observable -> observable.getIndex()).<ForeignExchangeRateIndex>map("getForeignExchangeRateIndex", index -> index.getForeignExchangeRateIndex()))).orNullSafe(exists(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout()).<Underlier>map("getUnderlier", performancePayout -> performancePayout.getUnderlier()).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", _referenceWithMetaObservable -> _referenceWithMetaObservable.getValue()).<Basket>map("getBasket", observable -> observable.getBasket()).<FieldWithMetaBasketConstituent>mapC("getBasketConstituent", basket -> basket.getBasketConstituent()).<BasketConstituent>map("Type coercion", fieldWithMetaBasketConstituent -> fieldWithMetaBasketConstituent.getValue()).<Index>map("getIndex", basketConstituent -> basketConstituent.getIndex()).<ForeignExchangeRateIndex>map("getForeignExchangeRateIndex", index -> index.getForeignExchangeRateIndex()))).orNullSafe(exists(MapperS.of(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).get()).<OptionPayout>map("getOptionPayout", payout -> payout.getOptionPayout())).andNullSafe(exists(optionUnderlier(economicTerms).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", referenceWithMetaObservable -> referenceWithMetaObservable == null ? null : referenceWithMetaObservable.getValue()).<Asset>map("getAsset", observable -> observable.getAsset()).<Cash>map("getCash", asset -> asset.getCash())).orNullSafe(ifThenElseResult))).get();
