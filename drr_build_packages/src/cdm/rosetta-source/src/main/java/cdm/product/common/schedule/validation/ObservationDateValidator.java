		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationDate", ValidationResult.ValidationType.CARDINALITY, "ObservationDate", path, "", res.getError());
				}
				return success("ObservationDate", ValidationResult.ValidationType.CARDINALITY, "ObservationDate", path, "");
			})
			.collect(toList());
	}
