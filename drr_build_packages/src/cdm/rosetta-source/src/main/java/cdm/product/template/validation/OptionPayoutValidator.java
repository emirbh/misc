		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionPayout", ValidationResult.ValidationType.CARDINALITY, "OptionPayout", path, "", res.getError());
				}
				return success("OptionPayout", ValidationResult.ValidationType.CARDINALITY, "OptionPayout", path, "");
			})
			.collect(toList());
	}
