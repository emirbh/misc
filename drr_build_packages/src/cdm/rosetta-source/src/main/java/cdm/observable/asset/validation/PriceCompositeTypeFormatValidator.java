		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceComposite", ValidationResult.ValidationType.TYPE_FORMAT, "PriceComposite", path, "", res.getError());
				}
				return success("PriceComposite", ValidationResult.ValidationType.TYPE_FORMAT, "PriceComposite", path, "");
			})
			.collect(toList());
	}
