...
public class PartyChangeInstructionValidator implements Validator<PartyChangeInstruction> {
	...
	private List<ComparisonResult> getComparisonResults(PartyChangeInstruction o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("tradeId", (List<? extends TradeIdentifier>) o.getTradeId() == null ? 0 : o.getTradeId().size(), 1, 0)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyChangeInstruction", ValidationResult.ValidationType.CARDINALITY, "PartyChangeInstruction", path, "", res.getError());
				}
				return success("PartyChangeInstruction", ValidationResult.ValidationType.CARDINALITY, "PartyChangeInstruction", path, "");
			})
			.collect(toList());
	}
