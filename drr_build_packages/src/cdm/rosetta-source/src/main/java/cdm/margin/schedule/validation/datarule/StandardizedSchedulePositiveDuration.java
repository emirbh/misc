 ...
 * @version 6.23.0
 */
@RosettaDataRule("StandardizedSchedulePositiveDuration")
@ImplementedBy(StandardizedSchedulePositiveDuration.Default.class)
public interface StandardizedSchedulePositiveDuration extends Validator<StandardizedSchedule> {
	
	String NAME = "StandardizedSchedulePositiveDuration";
	String DEFINITION = "durationInYears > 0";
	
	class Default implements StandardizedSchedulePositiveDuration {
		...
		private ComparisonResult executeDataRule(StandardizedSchedule standardizedSchedule) {
			try {
				return greaterThan(MapperS.of(standardizedSchedule).<BigDecimal>map("getDurationInYears", _standardizedSchedule -> _standardizedSchedule.getDurationInYears()), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All);
			}
			catch (Exception ex) {
	...
	class NoOp implements StandardizedSchedulePositiveDuration {
