		...
		private ComparisonResult executeDataRule(AssetType assetType) {
			try {
				if (notEqual(MapperS.of(assetType).<AssetTypeEnum>map("getAssetType", _assetType -> _assetType.getAssetType()), MapperS.of(AssetTypeEnum.SECURITY), CardinalityOperator.Any).getOrDefault(false)) {
					return notExists(MapperS.of(assetType).<InstrumentTypeEnum>map("getSecurityType", _assetType -> _assetType.getSecurityType())).andNullSafe(notExists(MapperS.of(assetType).<DebtType>map("getDebtType", _assetType -> _assetType.getDebtType()))).andNullSafe(notExists(MapperS.of(assetType).<EquityTypeEnum>map("getEquityType", _assetType -> _assetType.getEquityType()))).andNullSafe(notExists(MapperS.of(assetType).<FundProductTypeEnum>map("getFundType", _assetType -> _assetType.getFundType())));
				}
