...
public class QuantityMultiplierValidator implements Validator<QuantityMultiplier> {
	...
	private List<ComparisonResult> getComparisonResults(QuantityMultiplier o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("multiplierValue", (BigDecimal) o.getMultiplierValue() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuantityMultiplier", ValidationResult.ValidationType.CARDINALITY, "QuantityMultiplier", path, "", res.getError());
				}
				return success("QuantityMultiplier", ValidationResult.ValidationType.CARDINALITY, "QuantityMultiplier", path, "");
			})
			.collect(toList());
	}
