		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeterminationMethodology", ValidationResult.ValidationType.CARDINALITY, "DeterminationMethodology", path, "", res.getError());
				}
				return success("DeterminationMethodology", ValidationResult.ValidationType.CARDINALITY, "DeterminationMethodology", path, "");
			})
			.collect(toList());
	}
