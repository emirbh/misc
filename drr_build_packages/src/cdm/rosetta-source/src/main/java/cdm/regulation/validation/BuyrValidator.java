		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Buyr", ValidationResult.ValidationType.CARDINALITY, "Buyr", path, "", res.getError());
				}
				return success("Buyr", ValidationResult.ValidationType.CARDINALITY, "Buyr", path, "");
			})
			.collect(toList());
	}
