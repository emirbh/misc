		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnTerms", path, "", res.getError());
				}
				return success("ReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnTerms", path, "");
			})
			.collect(toList());
	}
