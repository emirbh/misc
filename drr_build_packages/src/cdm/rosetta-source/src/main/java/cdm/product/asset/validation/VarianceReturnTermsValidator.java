...
public class VarianceReturnTermsValidator implements Validator<VarianceReturnTerms> {
	...
	private List<ComparisonResult> getComparisonResults(VarianceReturnTerms o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("exchangeTradedContractNearest", (ReferenceWithMetaObservable) o.getExchangeTradedContractNearest() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VarianceReturnTerms", ValidationResult.ValidationType.CARDINALITY, "VarianceReturnTerms", path, "", res.getError());
				}
				return success("VarianceReturnTerms", ValidationResult.ValidationType.CARDINALITY, "VarianceReturnTerms", path, "");
			})
			.collect(toList());
	}
