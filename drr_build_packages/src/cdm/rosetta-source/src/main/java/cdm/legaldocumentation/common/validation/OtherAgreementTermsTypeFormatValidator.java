		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OtherAgreementTerms", ValidationResult.ValidationType.TYPE_FORMAT, "OtherAgreementTerms", path, "", res.getError());
				}
				return success("OtherAgreementTerms", ValidationResult.ValidationType.TYPE_FORMAT, "OtherAgreementTerms", path, "");
			})
			.collect(toList());
	}
