...
public class AssetTypeValidator implements Validator<AssetType> {
	...
	private List<ComparisonResult> getComparisonResults(AssetType o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("securityType", (InstrumentTypeEnum) o.getSecurityType() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetType", ValidationResult.ValidationType.CARDINALITY, "AssetType", path, "", res.getError());
				}
				return success("AssetType", ValidationResult.ValidationType.CARDINALITY, "AssetType", path, "");
			})
			.collect(toList());
	}
