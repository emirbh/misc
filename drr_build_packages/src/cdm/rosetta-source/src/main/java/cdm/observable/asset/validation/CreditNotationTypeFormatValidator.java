		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditNotation", ValidationResult.ValidationType.TYPE_FORMAT, "CreditNotation", path, "", res.getError());
				}
				return success("CreditNotation", ValidationResult.ValidationType.TYPE_FORMAT, "CreditNotation", path, "");
			})
			.collect(toList());
	}
