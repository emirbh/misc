		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralInterestCalculationParameters", ValidationResult.ValidationType.CARDINALITY, "CollateralInterestCalculationParameters", path, "", res.getError());
				}
				return success("CollateralInterestCalculationParameters", ValidationResult.ValidationType.CARDINALITY, "CollateralInterestCalculationParameters", path, "");
			})
			.collect(toList());
	}
