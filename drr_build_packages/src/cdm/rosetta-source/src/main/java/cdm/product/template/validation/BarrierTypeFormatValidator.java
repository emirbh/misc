		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Barrier", ValidationResult.ValidationType.TYPE_FORMAT, "Barrier", path, "", res.getError());
				}
				return success("Barrier", ValidationResult.ValidationType.TYPE_FORMAT, "Barrier", path, "");
			})
			.collect(toList());
	}
