		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CounterpartyOwnIssuePermitted", ValidationResult.ValidationType.CARDINALITY, "CounterpartyOwnIssuePermitted", path, "", res.getError());
				}
				return success("CounterpartyOwnIssuePermitted", ValidationResult.ValidationType.CARDINALITY, "CounterpartyOwnIssuePermitted", path, "");
			})
			.collect(toList());
	}
