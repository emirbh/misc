		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IssuerName", ValidationResult.ValidationType.TYPE_FORMAT, "IssuerName", path, "", res.getError());
				}
				return success("IssuerName", ValidationResult.ValidationType.TYPE_FORMAT, "IssuerName", path, "");
			})
			.collect(toList());
	}
