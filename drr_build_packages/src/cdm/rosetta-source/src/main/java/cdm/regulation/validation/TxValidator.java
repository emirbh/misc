		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Tx", ValidationResult.ValidationType.CARDINALITY, "Tx", path, "", res.getError());
				}
				return success("Tx", ValidationResult.ValidationType.CARDINALITY, "Tx", path, "");
			})
			.collect(toList());
	}
