		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StandardizedScheduleTradeInfo", ValidationResult.ValidationType.CARDINALITY, "StandardizedScheduleTradeInfo", path, "", res.getError());
				}
				return success("StandardizedScheduleTradeInfo", ValidationResult.ValidationType.CARDINALITY, "StandardizedScheduleTradeInfo", path, "");
			})
			.collect(toList());
	}
