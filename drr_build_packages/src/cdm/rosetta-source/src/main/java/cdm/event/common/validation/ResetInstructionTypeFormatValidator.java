		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResetInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "ResetInstruction", path, "", res.getError());
				}
				return success("ResetInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "ResetInstruction", path, "");
			})
			.collect(toList());
	}
