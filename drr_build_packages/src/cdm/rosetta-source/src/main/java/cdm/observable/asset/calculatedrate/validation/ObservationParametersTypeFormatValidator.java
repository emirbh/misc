		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationParameters", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationParameters", path, "", res.getError());
				}
				return success("ObservationParameters", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationParameters", path, "");
			})
			.collect(toList());
	}
