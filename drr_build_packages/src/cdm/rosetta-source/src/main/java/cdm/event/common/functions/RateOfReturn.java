	...
	*/
	...
	public static class RateOfReturnDefault extends RateOfReturn {
		...
		protected BigDecimal assignOutput(BigDecimal rateOfReturn, PriceSchedule initialPrice, PriceSchedule finalPrice) {
			if (exists(finalPriceValue(initialPrice, finalPrice)).andNullSafe(exists(initialPriceValue(initialPrice, finalPrice))).andNullSafe(greaterThan(initialPriceValue(initialPrice, finalPrice), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All)).getOrDefault(false)) {
		...
		protected MapperS<BigDecimal> initialPriceValue(PriceSchedule initialPrice, PriceSchedule finalPrice) {
			return MapperS.of(initialPrice).<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue());
		}
		
		@Override
		protected MapperS<BigDecimal> finalPriceValue(PriceSchedule initialPrice, PriceSchedule finalPrice) {
			return MapperS.of(finalPrice).<BigDecimal>map("getValue", priceSchedule -> priceSchedule.getValue());
		}
	}
}
