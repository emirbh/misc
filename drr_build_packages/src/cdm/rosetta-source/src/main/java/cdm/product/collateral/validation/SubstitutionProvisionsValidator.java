		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SubstitutionProvisions", ValidationResult.ValidationType.CARDINALITY, "SubstitutionProvisions", path, "", res.getError());
				}
				return success("SubstitutionProvisions", ValidationResult.ValidationType.CARDINALITY, "SubstitutionProvisions", path, "");
			})
			.collect(toList());
	}
