		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "BillingInstruction", path, "", res.getError());
				}
				return success("BillingInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "BillingInstruction", path, "");
			})
			.collect(toList());
	}
