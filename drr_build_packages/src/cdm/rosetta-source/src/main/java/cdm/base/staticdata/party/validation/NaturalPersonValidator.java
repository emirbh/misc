		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NaturalPerson", ValidationResult.ValidationType.CARDINALITY, "NaturalPerson", path, "", res.getError());
				}
				return success("NaturalPerson", ValidationResult.ValidationType.CARDINALITY, "NaturalPerson", path, "");
			})
			.collect(toList());
	}
