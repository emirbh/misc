		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuantityChangeInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "QuantityChangeInstruction", path, "", res.getError());
				}
				return success("QuantityChangeInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "QuantityChangeInstruction", path, "");
			})
			.collect(toList());
	}
