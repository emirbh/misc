		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegalAgreementIdentification", ValidationResult.ValidationType.CARDINALITY, "LegalAgreementIdentification", path, "", res.getError());
				}
				return success("LegalAgreementIdentification", ValidationResult.ValidationType.CARDINALITY, "LegalAgreementIdentification", path, "");
			})
			.collect(toList());
	}
