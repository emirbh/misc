		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetType", ValidationResult.ValidationType.TYPE_FORMAT, "AssetType", path, "", res.getError());
				}
				return success("AssetType", ValidationResult.ValidationType.TYPE_FORMAT, "AssetType", path, "");
			})
			.collect(toList());
	}
