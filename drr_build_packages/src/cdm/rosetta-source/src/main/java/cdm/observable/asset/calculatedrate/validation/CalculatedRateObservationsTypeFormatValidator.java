		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculatedRateObservations", ValidationResult.ValidationType.TYPE_FORMAT, "CalculatedRateObservations", path, "", res.getError());
				}
				return success("CalculatedRateObservations", ValidationResult.ValidationType.TYPE_FORMAT, "CalculatedRateObservations", path, "");
			})
			.collect(toList());
	}
