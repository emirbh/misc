		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FutureValueAmount", ValidationResult.ValidationType.CARDINALITY, "FutureValueAmount", path, "", res.getError());
				}
				return success("FutureValueAmount", ValidationResult.ValidationType.CARDINALITY, "FutureValueAmount", path, "");
			})
			.collect(toList());
	}
