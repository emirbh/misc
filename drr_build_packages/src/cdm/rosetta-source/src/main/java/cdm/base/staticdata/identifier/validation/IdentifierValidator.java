		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Identifier", ValidationResult.ValidationType.CARDINALITY, "Identifier", path, "", res.getError());
				}
				return success("Identifier", ValidationResult.ValidationType.CARDINALITY, "Identifier", path, "");
			})
			.collect(toList());
	}
