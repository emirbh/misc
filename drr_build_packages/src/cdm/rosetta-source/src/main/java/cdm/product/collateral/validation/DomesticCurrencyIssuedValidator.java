		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DomesticCurrencyIssued", ValidationResult.ValidationType.CARDINALITY, "DomesticCurrencyIssued", path, "", res.getError());
				}
				return success("DomesticCurrencyIssued", ValidationResult.ValidationType.CARDINALITY, "DomesticCurrencyIssued", path, "");
			})
			.collect(toList());
	}
