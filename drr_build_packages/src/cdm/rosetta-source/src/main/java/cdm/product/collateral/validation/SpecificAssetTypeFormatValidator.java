		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SpecificAsset", ValidationResult.ValidationType.TYPE_FORMAT, "SpecificAsset", path, "", res.getError());
				}
				return success("SpecificAsset", ValidationResult.ValidationType.TYPE_FORMAT, "SpecificAsset", path, "");
			})
			.collect(toList());
	}
