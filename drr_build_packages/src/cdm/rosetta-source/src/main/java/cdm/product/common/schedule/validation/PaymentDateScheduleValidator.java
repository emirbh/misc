		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentDateSchedule", ValidationResult.ValidationType.CARDINALITY, "PaymentDateSchedule", path, "", res.getError());
				}
				return success("PaymentDateSchedule", ValidationResult.ValidationType.CARDINALITY, "PaymentDateSchedule", path, "");
			})
			.collect(toList());
	}
