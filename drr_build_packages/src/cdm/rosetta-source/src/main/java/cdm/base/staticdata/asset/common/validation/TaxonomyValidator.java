		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Taxonomy", ValidationResult.ValidationType.CARDINALITY, "Taxonomy", path, "", res.getError());
				}
				return success("Taxonomy", ValidationResult.ValidationType.CARDINALITY, "Taxonomy", path, "");
			})
			.collect(toList());
	}
