...
public abstract class Create_PartyChange implements RosettaFunction {
	...
	*/
	...
	protected abstract MapperS<? extends ReferenceWithMetaParty> partyToRemove(Counterparty counterparty, AncillaryParty ancillaryParty, PartyRole partyRole, List<? extends TradeIdentifier> tradeId, TradeState originalTrade);
	...
	public static class Create_PartyChangeDefault extends Create_PartyChange {
		...
		protected TradeState.TradeStateBuilder assignOutput(TradeState.TradeStateBuilder newTrade, Counterparty counterparty, AncillaryParty ancillaryParty, PartyRole partyRole, List<? extends TradeIdentifier> tradeId, TradeState originalTrade) {
			newTrade = toBuilder(originalTrade, () -> TradeState.builder());
			...
			final ReferenceWithMetaParty referenceWithMetaParty0 = partyToRemove(counterparty, ancillaryParty, partyRole, tradeId, originalTrade).get();
			final ReferenceWithMetaParty referenceWithMetaParty1 = MapperS.of(counterparty).<ReferenceWithMetaParty>map("getPartyReference", _counterparty -> _counterparty.getPartyReference()).get();
			newTrade
				.getOrCreateTrade()
				.setParty(replaceParty.evaluate(MapperS.of(originalTrade).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<Party>mapC("getParty", trade -> trade.getParty()).getMulti(), (referenceWithMetaParty0 == null ? null : referenceWithMetaParty0.getValue()), (referenceWithMetaParty1 == null ? null : referenceWithMetaParty1.getValue())));
			...
			newTrade
				.getOrCreateTrade()
				.addParty(MapperS.of(ancillaryParty).<ReferenceWithMetaParty>mapC("getPartyReference", __ancillaryParty -> __ancillaryParty.getPartyReference()).<Party>map("Type coercion", referenceWithMetaParty -> referenceWithMetaParty.getValue()).getMulti());
			...
			newTrade
				.getOrCreateTrade()
				.addParty(MapperS.of(partyRole).<ReferenceWithMetaParty>map("getPartyReference", __partyRole -> __partyRole.getPartyReference()).<Party>map("Type coercion", referenceWithMetaParty2 -> referenceWithMetaParty2 == null ? null : referenceWithMetaParty2.getValue()).getMulti());
		...
		protected MapperS<? extends Counterparty> counterparty1(Counterparty counterparty, AncillaryParty ancillaryParty, PartyRole partyRole, List<? extends TradeIdentifier> tradeId, TradeState originalTrade) {
			if (areEqual(MapperS.of(counterparty).<CounterpartyRoleEnum>map("getRole", _counterparty -> _counterparty.getRole()), MapperS.of(CounterpartyRoleEnum.PARTY_1), CardinalityOperator.All).getOrDefault(false)) {
				final ReferenceWithMetaParty referenceWithMetaParty = MapperS.of(counterparty).<ReferenceWithMetaParty>map("getPartyReference", _counterparty -> _counterparty.getPartyReference()).get();
				final Party partyReference = referenceWithMetaParty == null ? null : referenceWithMetaParty.getValue();
			...
			return MapperS.of(extractCounterpartyByRole.evaluate(MapperS.of(originalTrade).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<Counterparty>mapC("getCounterparty", trade -> trade.getCounterparty()).getMulti(), CounterpartyRoleEnum.PARTY_1));
		}
		...
		protected MapperS<? extends Counterparty> counterparty2(Counterparty counterparty, AncillaryParty ancillaryParty, PartyRole partyRole, List<? extends TradeIdentifier> tradeId, TradeState originalTrade) {
			...
			return MapperS.of(extractCounterpartyByRole.evaluate(MapperS.of(originalTrade).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<Counterparty>mapC("getCounterparty", trade -> trade.getCounterparty()).getMulti(), CounterpartyRoleEnum.PARTY_2));
		}
		
		@Override
		protected MapperS<? extends ReferenceWithMetaParty> partyToRemove(Counterparty counterparty, AncillaryParty ancillaryParty, PartyRole partyRole, List<? extends TradeIdentifier> tradeId, TradeState originalTrade) {
			return MapperS.of(extractCounterpartyByRole.evaluate(MapperS.of(originalTrade).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<Counterparty>mapC("getCounterparty", trade -> trade.getCounterparty()).getMulti(), MapperS.of(counterparty).<CounterpartyRoleEnum>map("getRole", _counterparty -> _counterparty.getRole()).get())).<ReferenceWithMetaParty>map("getPartyReference", _counterparty -> _counterparty.getPartyReference());
		}
	}
}
