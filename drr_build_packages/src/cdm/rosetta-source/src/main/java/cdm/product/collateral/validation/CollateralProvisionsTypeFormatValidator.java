		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralProvisions", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralProvisions", path, "", res.getError());
				}
				return success("CollateralProvisions", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralProvisions", path, "");
			})
			.collect(toList());
	}
