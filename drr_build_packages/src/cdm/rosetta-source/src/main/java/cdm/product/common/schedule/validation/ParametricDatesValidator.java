		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ParametricDates", ValidationResult.ValidationType.CARDINALITY, "ParametricDates", path, "", res.getError());
				}
				return success("ParametricDates", ValidationResult.ValidationType.CARDINALITY, "ParametricDates", path, "");
			})
			.collect(toList());
	}
