		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationDates", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationDates", path, "", res.getError());
				}
				return success("ObservationDates", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationDates", path, "");
			})
			.collect(toList());
	}
