		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAgreementFloatingRate", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralAgreementFloatingRate", path, "", res.getError());
				}
				return success("CollateralAgreementFloatingRate", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralAgreementFloatingRate", path, "");
			})
			.collect(toList());
	}
