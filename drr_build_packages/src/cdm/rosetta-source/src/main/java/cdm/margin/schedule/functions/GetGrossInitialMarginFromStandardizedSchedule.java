	...
	*/
	...
	public static class GetGrossInitialMarginFromStandardizedScheduleDefault extends GetGrossInitialMarginFromStandardizedSchedule {
		...
		protected Money.MoneyBuilder assignOutput(Money.MoneyBuilder grossInitialMargin, StandardizedSchedule standardizedSchedule) {
			...
			final String string = MapperS.of(standardizedSchedule).<String>map("getNotionalCurrency", _standardizedSchedule -> _standardizedSchedule.getNotionalCurrency()).get();
			grossInitialMargin
				.getOrCreateUnit()
				.setCurrency((string == null ? FieldWithMetaString.builder().build() : FieldWithMetaString.builder().setValue(string).build()));
