		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Underlier", ValidationResult.ValidationType.CARDINALITY, "Underlier", path, "", res.getError());
				}
				return success("Underlier", ValidationResult.ValidationType.CARDINALITY, "Underlier", path, "");
			})
			.collect(toList());
	}
