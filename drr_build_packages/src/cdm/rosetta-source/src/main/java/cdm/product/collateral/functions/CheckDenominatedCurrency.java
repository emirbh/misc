	...
	*/
	...
	public static class CheckDenominatedCurrencyDefault extends CheckDenominatedCurrency {
		...
		protected Boolean assignOutput(Boolean isEqual, CurrencyCodeEnum denominatedCurrency, EligibilityQuery query) {
			isEqual = notExists(MapperS.of(denominatedCurrency)).orNullSafe(areEqual(MapperS.of(denominatedCurrency), MapperS.of(query).<CurrencyCodeEnum>map("getDenominatedCurrency", eligibilityQuery -> eligibilityQuery.getDenominatedCurrency()), CardinalityOperator.All)).get();
