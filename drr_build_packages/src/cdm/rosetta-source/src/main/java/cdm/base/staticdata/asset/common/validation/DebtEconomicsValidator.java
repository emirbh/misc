		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DebtEconomics", ValidationResult.ValidationType.CARDINALITY, "DebtEconomics", path, "", res.getError());
				}
				return success("DebtEconomics", ValidationResult.ValidationType.CARDINALITY, "DebtEconomics", path, "");
			})
			.collect(toList());
	}
