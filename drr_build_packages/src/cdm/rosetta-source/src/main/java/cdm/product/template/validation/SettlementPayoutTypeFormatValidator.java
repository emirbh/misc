		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPayout", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPayout", path, "", res.getError());
				}
				return success("SettlementPayout", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPayout", path, "");
			})
			.collect(toList());
	}
