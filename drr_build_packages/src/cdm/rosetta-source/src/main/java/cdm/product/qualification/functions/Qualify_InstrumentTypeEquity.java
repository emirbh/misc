	...
	*/
	...
	public static class Qualify_InstrumentTypeEquityDefault extends Qualify_InstrumentTypeEquity {
		...
		protected Boolean assignOutput(Boolean is_equity, Instrument instrument) {
			is_equity = areEqual(instrumentType(instrument), MapperS.of(InstrumentTypeEnum.EQUITY), CardinalityOperator.All).orNullSafe(areEqual(instrumentType(instrument), MapperS.of(InstrumentTypeEnum.FUND), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(instrument).<Security>map("getSecurity", _instrument -> _instrument.getSecurity()).<FundProductTypeEnum>map("getFundType", security -> security.getFundType()), MapperS.of(FundProductTypeEnum.EXCHANGE_TRADED_FUND), CardinalityOperator.All))).orNullSafe(areEqual(instrumentType(instrument), MapperS.of(InstrumentTypeEnum.FUND), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(instrument).<Security>map("getSecurity", _instrument -> _instrument.getSecurity()).<FundProductTypeEnum>map("getFundType", security -> security.getFundType()), MapperS.of(FundProductTypeEnum.MUTUAL_FUND), CardinalityOperator.All))).orNullSafe(areEqual(instrumentType(instrument), MapperS.of(InstrumentTypeEnum.WARRANT), CardinalityOperator.All)).get();
