		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConcentrationLimit", ValidationResult.ValidationType.TYPE_FORMAT, "ConcentrationLimit", path, "", res.getError());
				}
				return success("ConcentrationLimit", ValidationResult.ValidationType.TYPE_FORMAT, "ConcentrationLimit", path, "");
			})
			.collect(toList());
	}
