		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SchedulePeriod", ValidationResult.ValidationType.CARDINALITY, "SchedulePeriod", path, "", res.getError());
				}
				return success("SchedulePeriod", ValidationResult.ValidationType.CARDINALITY, "SchedulePeriod", path, "");
			})
			.collect(toList());
	}
