		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationTerms", path, "", res.getError());
				}
				return success("ObservationTerms", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationTerms", path, "");
			})
			.collect(toList());
	}
