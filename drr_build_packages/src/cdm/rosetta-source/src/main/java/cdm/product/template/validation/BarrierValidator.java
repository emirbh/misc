		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Barrier", ValidationResult.ValidationType.CARDINALITY, "Barrier", path, "", res.getError());
				}
				return success("Barrier", ValidationResult.ValidationType.CARDINALITY, "Barrier", path, "");
			})
			.collect(toList());
	}
