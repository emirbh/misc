		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyContactInformation", ValidationResult.ValidationType.TYPE_FORMAT, "PartyContactInformation", path, "", res.getError());
				}
				return success("PartyContactInformation", ValidationResult.ValidationType.TYPE_FORMAT, "PartyContactInformation", path, "");
			})
			.collect(toList());
	}
