		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransferableProduct", ValidationResult.ValidationType.CARDINALITY, "TransferableProduct", path, "", res.getError());
				}
				return success("TransferableProduct", ValidationResult.ValidationType.CARDINALITY, "TransferableProduct", path, "");
			})
			.collect(toList());
	}
