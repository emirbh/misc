		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IssuerAgencyRating", ValidationResult.ValidationType.CARDINALITY, "IssuerAgencyRating", path, "", res.getError());
				}
				return success("IssuerAgencyRating", ValidationResult.ValidationType.CARDINALITY, "IssuerAgencyRating", path, "");
			})
			.collect(toList());
	}
