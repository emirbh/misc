		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingObservationList", ValidationResult.ValidationType.CARDINALITY, "AveragingObservationList", path, "", res.getError());
				}
				return success("AveragingObservationList", ValidationResult.ValidationType.CARDINALITY, "AveragingObservationList", path, "");
			})
			.collect(toList());
	}
