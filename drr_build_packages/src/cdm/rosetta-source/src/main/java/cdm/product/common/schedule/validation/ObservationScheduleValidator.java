		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationSchedule", ValidationResult.ValidationType.CARDINALITY, "ObservationSchedule", path, "", res.getError());
				}
				return success("ObservationSchedule", ValidationResult.ValidationType.CARDINALITY, "ObservationSchedule", path, "");
			})
			.collect(toList());
	}
