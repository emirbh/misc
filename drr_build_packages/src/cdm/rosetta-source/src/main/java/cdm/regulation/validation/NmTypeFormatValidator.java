		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Nm", ValidationResult.ValidationType.TYPE_FORMAT, "Nm", path, "", res.getError());
				}
				return success("Nm", ValidationResult.ValidationType.TYPE_FORMAT, "Nm", path, "");
			})
			.collect(toList());
	}
