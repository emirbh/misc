		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetMaturity", ValidationResult.ValidationType.CARDINALITY, "AssetMaturity", path, "", res.getError());
				}
				return success("AssetMaturity", ValidationResult.ValidationType.CARDINALITY, "AssetMaturity", path, "");
			})
			.collect(toList());
	}
