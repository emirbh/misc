		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LocationIdentifier", ValidationResult.ValidationType.CARDINALITY, "LocationIdentifier", path, "", res.getError());
				}
				return success("LocationIdentifier", ValidationResult.ValidationType.CARDINALITY, "LocationIdentifier", path, "");
			})
			.collect(toList());
	}
