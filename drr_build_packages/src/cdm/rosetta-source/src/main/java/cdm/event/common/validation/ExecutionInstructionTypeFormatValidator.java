		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionInstruction", path, "", res.getError());
				}
				return success("ExecutionInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionInstruction", path, "");
			})
			.collect(toList());
	}
