		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AncillaryEntity", ValidationResult.ValidationType.TYPE_FORMAT, "AncillaryEntity", path, "", res.getError());
				}
				return success("AncillaryEntity", ValidationResult.ValidationType.TYPE_FORMAT, "AncillaryEntity", path, "");
			})
			.collect(toList());
	}
