		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexTransitionInstruction", ValidationResult.ValidationType.CARDINALITY, "IndexTransitionInstruction", path, "", res.getError());
				}
				return success("IndexTransitionInstruction", ValidationResult.ValidationType.CARDINALITY, "IndexTransitionInstruction", path, "");
			})
			.collect(toList());
	}
