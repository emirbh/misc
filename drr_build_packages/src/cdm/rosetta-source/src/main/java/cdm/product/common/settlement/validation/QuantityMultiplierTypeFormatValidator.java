		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuantityMultiplier", ValidationResult.ValidationType.TYPE_FORMAT, "QuantityMultiplier", path, "", res.getError());
				}
				return success("QuantityMultiplier", ValidationResult.ValidationType.TYPE_FORMAT, "QuantityMultiplier", path, "");
			})
			.collect(toList());
	}
