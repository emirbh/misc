		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnTermsBase", ValidationResult.ValidationType.CARDINALITY, "ReturnTermsBase", path, "", res.getError());
				}
				return success("ReturnTermsBase", ValidationResult.ValidationType.CARDINALITY, "ReturnTermsBase", path, "");
			})
			.collect(toList());
	}
