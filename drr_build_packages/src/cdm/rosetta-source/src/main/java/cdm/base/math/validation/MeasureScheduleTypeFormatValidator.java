		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MeasureSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "MeasureSchedule", path, "", res.getError());
				}
				return success("MeasureSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "MeasureSchedule", path, "");
			})
			.collect(toList());
	}
