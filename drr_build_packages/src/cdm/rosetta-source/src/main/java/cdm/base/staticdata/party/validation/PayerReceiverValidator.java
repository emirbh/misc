		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PayerReceiver", ValidationResult.ValidationType.CARDINALITY, "PayerReceiver", path, "", res.getError());
				}
				return success("PayerReceiver", ValidationResult.ValidationType.CARDINALITY, "PayerReceiver", path, "");
			})
			.collect(toList());
	}
