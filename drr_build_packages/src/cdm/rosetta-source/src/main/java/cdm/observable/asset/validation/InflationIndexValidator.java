		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InflationIndex", ValidationResult.ValidationType.CARDINALITY, "InflationIndex", path, "", res.getError());
				}
				return success("InflationIndex", ValidationResult.ValidationType.CARDINALITY, "InflationIndex", path, "");
			})
			.collect(toList());
	}
