		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("New", ValidationResult.ValidationType.CARDINALITY, "New", path, "", res.getError());
				}
				return success("New", ValidationResult.ValidationType.CARDINALITY, "New", path, "");
			})
			.collect(toList());
	}
