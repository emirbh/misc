		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CancelableProvision", ValidationResult.ValidationType.TYPE_FORMAT, "CancelableProvision", path, "", res.getError());
				}
				return success("CancelableProvision", ValidationResult.ValidationType.TYPE_FORMAT, "CancelableProvision", path, "");
			})
			.collect(toList());
	}
