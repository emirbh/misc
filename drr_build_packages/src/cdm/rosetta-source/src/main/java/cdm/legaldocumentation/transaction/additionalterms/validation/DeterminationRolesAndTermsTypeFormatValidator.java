		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeterminationRolesAndTerms", ValidationResult.ValidationType.TYPE_FORMAT, "DeterminationRolesAndTerms", path, "", res.getError());
				}
				return success("DeterminationRolesAndTerms", ValidationResult.ValidationType.TYPE_FORMAT, "DeterminationRolesAndTerms", path, "");
			})
			.collect(toList());
	}
