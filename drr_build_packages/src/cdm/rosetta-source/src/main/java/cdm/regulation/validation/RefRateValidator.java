		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RefRate", ValidationResult.ValidationType.CARDINALITY, "RefRate", path, "", res.getError());
				}
				return success("RefRate", ValidationResult.ValidationType.CARDINALITY, "RefRate", path, "");
			})
			.collect(toList());
	}
