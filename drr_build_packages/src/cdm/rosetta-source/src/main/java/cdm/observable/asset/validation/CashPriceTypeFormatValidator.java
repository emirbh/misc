		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashPrice", ValidationResult.ValidationType.TYPE_FORMAT, "CashPrice", path, "", res.getError());
				}
				return success("CashPrice", ValidationResult.ValidationType.TYPE_FORMAT, "CashPrice", path, "");
			})
			.collect(toList());
	}
