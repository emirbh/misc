	...
	*/
	...
	public static class Qualify_StockSplitDefault extends Qualify_StockSplit {
		...
		protected Boolean assignOutput(Boolean is_event, BusinessEvent businessEvent) {
			is_event = areEqual(currencyAmountUnchanged(businessEvent), MapperS.of(true), CardinalityOperator.All).andNullSafe(areEqual(noOfUnitsChanged(businessEvent), MapperS.of(true), CardinalityOperator.All)).andNullSafe(areEqual(cashPriceChanged(businessEvent), MapperS.of(true), CardinalityOperator.All)).andNullSafe(areEqual(adjustmentRatioMatches(businessEvent), MapperS.of(true), CardinalityOperator.All)).get();
		...
		protected MapperS<Boolean> currencyAmountUnchanged(BusinessEvent businessEvent) {
			if (exists(beforeCurrencyAmount(businessEvent)).andNullSafe(exists(afterCurrencyAmount(businessEvent))).getOrDefault(false)) {
		...
		protected MapperS<Boolean> noOfUnitsChanged(BusinessEvent businessEvent) {
			if (exists(beforeNoOfUnits(businessEvent)).andNullSafe(exists(afterNoOfUnits(businessEvent))).getOrDefault(false)) {
		...
		protected MapperS<Boolean> cashPriceChanged(BusinessEvent businessEvent) {
			if (exists(beforePrice(businessEvent)).andNullSafe(exists(afterPrice(businessEvent))).getOrDefault(false)) {
		...
		protected MapperS<Boolean> adjustmentRatioMatches(BusinessEvent businessEvent) {
			if (exists(beforeNoOfUnits(businessEvent)).andNullSafe(greaterThan(beforeNoOfUnits(businessEvent), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All)).andNullSafe(exists(afterNoOfUnits(businessEvent))).andNullSafe(exists(beforePrice(businessEvent))).andNullSafe(exists(afterPrice(businessEvent))).andNullSafe(greaterThan(afterPrice(businessEvent), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All)).getOrDefault(false)) {
