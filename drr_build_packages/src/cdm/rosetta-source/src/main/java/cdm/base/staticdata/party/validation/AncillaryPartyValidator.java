...
public class AncillaryPartyValidator implements Validator<AncillaryParty> {
	...
	private List<ComparisonResult> getComparisonResults(AncillaryParty o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("partyReference", (List<? extends ReferenceWithMetaParty>) o.getPartyReference() == null ? 0 : o.getPartyReference().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AncillaryParty", ValidationResult.ValidationType.CARDINALITY, "AncillaryParty", path, "", res.getError());
				}
				return success("AncillaryParty", ValidationResult.ValidationType.CARDINALITY, "AncillaryParty", path, "");
			})
			.collect(toList());
	}
