		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodFrequency", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriodFrequency", path, "", res.getError());
				}
				return success("CalculationPeriodFrequency", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriodFrequency", path, "");
			})
			.collect(toList());
	}
