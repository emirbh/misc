...
public class ObservationTermsValidator implements Validator<ObservationTerms> {
	...
	private List<ComparisonResult> getComparisonResults(ObservationTerms o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("observationTimeType", (TimeTypeEnum) o.getObservationTimeType() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationTerms", ValidationResult.ValidationType.CARDINALITY, "ObservationTerms", path, "", res.getError());
				}
				return success("ObservationTerms", ValidationResult.ValidationType.CARDINALITY, "ObservationTerms", path, "");
			})
			.collect(toList());
	}
