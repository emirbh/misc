		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StandardizedSchedule", ValidationResult.ValidationType.CARDINALITY, "StandardizedSchedule", path, "", res.getError());
				}
				return success("StandardizedSchedule", ValidationResult.ValidationType.CARDINALITY, "StandardizedSchedule", path, "");
			})
			.collect(toList());
	}
