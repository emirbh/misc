		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RollFeature", ValidationResult.ValidationType.CARDINALITY, "RollFeature", path, "", res.getError());
				}
				return success("RollFeature", ValidationResult.ValidationType.CARDINALITY, "RollFeature", path, "");
			})
			.collect(toList());
	}
