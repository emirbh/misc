		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuotedCurrencyPair", ValidationResult.ValidationType.TYPE_FORMAT, "QuotedCurrencyPair", path, "", res.getError());
				}
				return success("QuotedCurrencyPair", ValidationResult.ValidationType.TYPE_FORMAT, "QuotedCurrencyPair", path, "");
			})
			.collect(toList());
	}
