...
@ImplementedBy(SimmVersionVersionSpecified.Default.class)
public interface SimmVersionVersionSpecified extends Validator<SimmVersion> {
	
	String NAME = "SimmVersionVersionSpecified";
	String DEFINITION = "if isSpecified = True then partyVersion exists";
	
	class Default implements SimmVersionVersionSpecified {
		...
		private ComparisonResult executeDataRule(SimmVersion simmVersion) {
			try {
				if (areEqual(MapperS.of(simmVersion).<Boolean>map("getIsSpecified", _simmVersion -> _simmVersion.getIsSpecified()), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
	...
	class NoOp implements SimmVersionVersionSpecified {
