...
@ImplementedBy(CommodityPayoutOnlyExists.CommodityPayoutOnlyExistsDefault.class)
public abstract class CommodityPayoutOnlyExists implements RosettaFunction {
	...
	*/
	...
	public static class CommodityPayoutOnlyExistsDefault extends CommodityPayoutOnlyExists {
		@Override
		...
		protected Boolean assignOutput(Boolean result, List<? extends Payout> payouts) {
			final MapperC<Boolean> thenArg0 = MapperC.<Payout>of(payouts)
				.mapItem(item -> exists(item.<CommodityPayout>map("getCommodityPayout", payout -> payout.getCommodityPayout())).asMapper());
