		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AverageTradingVolume", ValidationResult.ValidationType.TYPE_FORMAT, "AverageTradingVolume", path, "", res.getError());
				}
				return success("AverageTradingVolume", ValidationResult.ValidationType.TYPE_FORMAT, "AverageTradingVolume", path, "");
			})
			.collect(toList());
	}
