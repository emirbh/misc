		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Cashflow", ValidationResult.ValidationType.CARDINALITY, "Cashflow", path, "", res.getError());
				}
				return success("Cashflow", ValidationResult.ValidationType.CARDINALITY, "Cashflow", path, "");
			})
			.collect(toList());
	}
