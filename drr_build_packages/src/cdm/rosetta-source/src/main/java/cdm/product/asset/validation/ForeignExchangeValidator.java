		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ForeignExchange", ValidationResult.ValidationType.CARDINALITY, "ForeignExchange", path, "", res.getError());
				}
				return success("ForeignExchange", ValidationResult.ValidationType.CARDINALITY, "ForeignExchange", path, "");
			})
			.collect(toList());
	}
