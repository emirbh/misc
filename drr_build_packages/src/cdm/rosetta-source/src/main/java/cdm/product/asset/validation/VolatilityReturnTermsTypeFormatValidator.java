		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "VolatilityReturnTerms", path, "", res.getError());
				}
				return success("VolatilityReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "VolatilityReturnTerms", path, "");
			})
			.collect(toList());
	}
