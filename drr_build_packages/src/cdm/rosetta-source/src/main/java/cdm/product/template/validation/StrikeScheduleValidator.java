		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StrikeSchedule", ValidationResult.ValidationType.CARDINALITY, "StrikeSchedule", path, "", res.getError());
				}
				return success("StrikeSchedule", ValidationResult.ValidationType.CARDINALITY, "StrikeSchedule", path, "");
			})
			.collect(toList());
	}
