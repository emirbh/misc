		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexBase", ValidationResult.ValidationType.CARDINALITY, "IndexBase", path, "", res.getError());
				}
				return success("IndexBase", ValidationResult.ValidationType.CARDINALITY, "IndexBase", path, "");
			})
			.collect(toList());
	}
