		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralProvisions", ValidationResult.ValidationType.CARDINALITY, "CollateralProvisions", path, "", res.getError());
				}
				return success("CollateralProvisions", ValidationResult.ValidationType.CARDINALITY, "CollateralProvisions", path, "");
			})
			.collect(toList());
	}
