		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateProcessingParameters", ValidationResult.ValidationType.CARDINALITY, "FloatingRateProcessingParameters", path, "", res.getError());
				}
				return success("FloatingRateProcessingParameters", ValidationResult.ValidationType.CARDINALITY, "FloatingRateProcessingParameters", path, "");
			})
			.collect(toList());
	}
