...
public class AgreementNameValidator implements Validator<AgreementName> {
	...
	private List<ComparisonResult> getComparisonResults(AgreementName o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("brokerConfirmationType", (BrokerConfirmationTypeEnum) o.getBrokerConfirmationType() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AgreementName", ValidationResult.ValidationType.CARDINALITY, "AgreementName", path, "", res.getError());
				}
				return success("AgreementName", ValidationResult.ValidationType.CARDINALITY, "AgreementName", path, "");
			})
			.collect(toList());
	}
