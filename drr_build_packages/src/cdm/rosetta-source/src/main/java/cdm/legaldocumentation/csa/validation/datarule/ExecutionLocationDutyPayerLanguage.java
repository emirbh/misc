...
@ImplementedBy(ExecutionLocationDutyPayerLanguage.Default.class)
public interface ExecutionLocationDutyPayerLanguage extends Validator<ExecutionLocation> {
	
	String NAME = "ExecutionLocationDutyPayerLanguage";
	String DEFINITION = "if dutyPayer = \"Specify\" then dutyPayerLanguage exists";
	
	class Default implements ExecutionLocationDutyPayerLanguage {
		...
		private ComparisonResult executeDataRule(ExecutionLocation executionLocation) {
			try {
				if (areEqual(MapperS.of(executionLocation).<String>map("getDutyPayer", _executionLocation -> _executionLocation.getDutyPayer()), MapperS.of("Specify"), CardinalityOperator.All).getOrDefault(false)) {
	...
	class NoOp implements ExecutionLocationDutyPayerLanguage {
