	...
	*/
	...
	public static class ToMoneyDefault extends ToMoney {
		...
		protected Money.MoneyBuilder assignOutput(Money.MoneyBuilder money, Quantity quantity) {
			money
				.setValue(MapperS.of(quantity).<BigDecimal>map("getValue", _quantity -> _quantity.getValue()).get());
			
			money
				.getOrCreateUnit()
				.setCurrency(MapperS.of(quantity).<UnitType>map("getUnit", _quantity -> _quantity.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).get());
