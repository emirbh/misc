		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AcctOwnr", ValidationResult.ValidationType.CARDINALITY, "AcctOwnr", path, "", res.getError());
				}
				return success("AcctOwnr", ValidationResult.ValidationType.CARDINALITY, "AcctOwnr", path, "");
			})
			.collect(toList());
	}
