		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuantitySchedule", ValidationResult.ValidationType.CARDINALITY, "QuantitySchedule", path, "", res.getError());
				}
				return success("QuantitySchedule", ValidationResult.ValidationType.CARDINALITY, "QuantitySchedule", path, "");
			})
			.collect(toList());
	}
