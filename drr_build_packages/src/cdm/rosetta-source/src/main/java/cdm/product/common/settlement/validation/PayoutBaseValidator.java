		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PayoutBase", ValidationResult.ValidationType.CARDINALITY, "PayoutBase", path, "", res.getError());
				}
				return success("PayoutBase", ValidationResult.ValidationType.CARDINALITY, "PayoutBase", path, "");
			})
			.collect(toList());
	}
