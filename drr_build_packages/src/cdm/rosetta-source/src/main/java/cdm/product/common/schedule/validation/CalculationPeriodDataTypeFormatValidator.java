		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodData", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodData", path, "", res.getError());
				}
				return success("CalculationPeriodData", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodData", path, "");
			})
			.collect(toList());
	}
