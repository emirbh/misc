		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AgreementTerms", ValidationResult.ValidationType.CARDINALITY, "AgreementTerms", path, "", res.getError());
				}
				return success("AgreementTerms", ValidationResult.ValidationType.CARDINALITY, "AgreementTerms", path, "");
			})
			.collect(toList());
	}
