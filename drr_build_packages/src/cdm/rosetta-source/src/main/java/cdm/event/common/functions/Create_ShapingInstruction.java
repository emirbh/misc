	...
	*/
	...
	public static class Create_ShapingInstructionDefault extends Create_ShapingInstruction {
		...
		protected PrimitiveInstruction.PrimitiveInstructionBuilder assignOutput(PrimitiveInstruction.PrimitiveInstructionBuilder instruction, TradeState tradeState, List<? extends TradeLot> tradeLots, Identifier shapeIdentifier) {
			instruction
				.getOrCreateSplit()
				.setBreakdown(MapperC.<PrimitiveInstruction>of(MapperS.of(create_TerminationInstruction.evaluate(tradeState, null))).getMulti());
			
			final MapperListOfLists<PriceQuantity> thenArg = MapperC.<TradeLot>of(tradeLots)
				.mapItemToList(item -> item.<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()));
			instruction
				.getOrCreateSplit()
				.addBreakdown(thenArg
					.mapListToItem(item -> {
						...
						return MapperS.of(PrimitiveInstruction.builder()
							...
							.setExecution(ExecutionInstruction.builder()
								.setProduct(MapperS.of(tradeState).<Trade>map("getTrade", _tradeState -> _tradeState.getTrade()).<NonTransferableProduct>map("getProduct", trade -> trade.getProduct()).get())
								.setPriceQuantity(MapperS.of(MapperS.of(tradeState).<Trade>map("getTrade", _tradeState -> _tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()).getMulti())
								...
								.setAncillaryParty(MapperS.of(tradeState).<Trade>map("getTrade", _tradeState -> _tradeState.getTrade()).<AncillaryParty>mapC("getAncillaryParty", trade -> trade.getAncillaryParty()).getMulti())
								...
								.setTradeDate(MapperS.of(tradeState).<Trade>map("getTrade", _tradeState -> _tradeState.getTrade()).<FieldWithMetaDate>map("getTradeDate", trade -> trade.getTradeDate()).get())
