...
public class EconomicTermsValidator implements Validator<EconomicTerms> {
	...
	private List<ComparisonResult> getComparisonResults(EconomicTerms o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("payout", (List<? extends Payout>) o.getPayout() == null ? 0 : o.getPayout().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EconomicTerms", ValidationResult.ValidationType.CARDINALITY, "EconomicTerms", path, "", res.getError());
				}
				return success("EconomicTerms", ValidationResult.ValidationType.CARDINALITY, "EconomicTerms", path, "");
			})
			.collect(toList());
	}
