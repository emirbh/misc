...
public interface ExtendibleProvisionExtendibleProvisionExerciseNoticeReceiverParty extends Validator<ExtendibleProvision> {
	...
	class Default implements ExtendibleProvisionExtendibleProvisionExerciseNoticeReceiverParty {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, ExtendibleProvision extendibleProvision) {
			ComparisonResult result = executeDataRule(extendibleProvision);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "ExtendibleProvision", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "ExtendibleProvision", path, DEFINITION, failureMessage));
		}
