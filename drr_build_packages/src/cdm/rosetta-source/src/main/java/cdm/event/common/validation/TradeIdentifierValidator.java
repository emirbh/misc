		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeIdentifier", ValidationResult.ValidationType.CARDINALITY, "TradeIdentifier", path, "", res.getError());
				}
				return success("TradeIdentifier", ValidationResult.ValidationType.CARDINALITY, "TradeIdentifier", path, "");
			})
			.collect(toList());
	}
