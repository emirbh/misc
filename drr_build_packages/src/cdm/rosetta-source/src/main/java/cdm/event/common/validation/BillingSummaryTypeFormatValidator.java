		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingSummary", ValidationResult.ValidationType.TYPE_FORMAT, "BillingSummary", path, "", res.getError());
				}
				return success("BillingSummary", ValidationResult.ValidationType.TYPE_FORMAT, "BillingSummary", path, "");
			})
			.collect(toList());
	}
