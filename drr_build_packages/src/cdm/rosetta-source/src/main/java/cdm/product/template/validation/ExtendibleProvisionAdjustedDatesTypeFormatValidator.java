		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExtendibleProvisionAdjustedDates", ValidationResult.ValidationType.TYPE_FORMAT, "ExtendibleProvisionAdjustedDates", path, "", res.getError());
				}
				return success("ExtendibleProvisionAdjustedDates", ValidationResult.ValidationType.TYPE_FORMAT, "ExtendibleProvisionAdjustedDates", path, "");
			})
			.collect(toList());
	}
