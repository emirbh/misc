		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Nm", ValidationResult.ValidationType.CARDINALITY, "Nm", path, "", res.getError());
				}
				return success("Nm", ValidationResult.ValidationType.CARDINALITY, "Nm", path, "");
			})
			.collect(toList());
	}
