	...
	*/
	...
	public static class PerformancePayoutAndFixedPricePayoutOnlyExistsDefault extends PerformancePayoutAndFixedPricePayoutOnlyExists {
		...
		protected Boolean assignOutput(Boolean result, List<? extends Payout> payouts) {
			result = exists(MapperC.<Payout>of(payouts).<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout())).andNullSafe(exists(MapperC.<Payout>of(payouts).<FixedPricePayout>map("getFixedPricePayout", payout -> payout.getFixedPricePayout()))).andNullSafe(areEqual(MapperC.<Payout>of(payouts)
				.mapItem(item -> exists(item.<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout())).orNullSafe(exists(item.<FixedPricePayout>map("getFixedPricePayout", payout -> payout.getFixedPricePayout()))).asMapper()), MapperS.of(true), CardinalityOperator.All)).get();
