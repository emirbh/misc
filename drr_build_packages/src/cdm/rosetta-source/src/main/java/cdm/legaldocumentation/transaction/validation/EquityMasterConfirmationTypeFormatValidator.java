		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityMasterConfirmation", ValidationResult.ValidationType.TYPE_FORMAT, "EquityMasterConfirmation", path, "", res.getError());
				}
				return success("EquityMasterConfirmation", ValidationResult.ValidationType.TYPE_FORMAT, "EquityMasterConfirmation", path, "");
			})
			.collect(toList());
	}
