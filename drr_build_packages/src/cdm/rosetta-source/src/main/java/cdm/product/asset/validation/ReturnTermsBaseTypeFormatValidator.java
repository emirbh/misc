		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnTermsBase", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnTermsBase", path, "", res.getError());
				}
				return success("ReturnTermsBase", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnTermsBase", path, "");
			})
			.collect(toList());
	}
