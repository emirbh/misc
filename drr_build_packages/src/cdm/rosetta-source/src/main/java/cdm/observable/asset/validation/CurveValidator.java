		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Curve", ValidationResult.ValidationType.CARDINALITY, "Curve", path, "", res.getError());
				}
				return success("Curve", ValidationResult.ValidationType.CARDINALITY, "Curve", path, "");
			})
			.collect(toList());
	}
