		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Valuation", ValidationResult.ValidationType.TYPE_FORMAT, "Valuation", path, "", res.getError());
				}
				return success("Valuation", ValidationResult.ValidationType.TYPE_FORMAT, "Valuation", path, "");
			})
			.collect(toList());
	}
