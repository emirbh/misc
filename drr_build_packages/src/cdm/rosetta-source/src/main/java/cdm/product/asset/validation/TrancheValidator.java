		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Tranche", ValidationResult.ValidationType.CARDINALITY, "Tranche", path, "", res.getError());
				}
				return success("Tranche", ValidationResult.ValidationType.CARDINALITY, "Tranche", path, "");
			})
			.collect(toList());
	}
