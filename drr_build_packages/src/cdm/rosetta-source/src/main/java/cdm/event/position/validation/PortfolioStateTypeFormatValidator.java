		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PortfolioState", ValidationResult.ValidationType.TYPE_FORMAT, "PortfolioState", path, "", res.getError());
				}
				return success("PortfolioState", ValidationResult.ValidationType.TYPE_FORMAT, "PortfolioState", path, "");
			})
			.collect(toList());
	}
