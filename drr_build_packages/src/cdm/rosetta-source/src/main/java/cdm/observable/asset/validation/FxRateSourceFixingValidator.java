		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxRateSourceFixing", ValidationResult.ValidationType.CARDINALITY, "FxRateSourceFixing", path, "", res.getError());
				}
				return success("FxRateSourceFixing", ValidationResult.ValidationType.CARDINALITY, "FxRateSourceFixing", path, "");
			})
			.collect(toList());
	}
