		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingRecord", ValidationResult.ValidationType.TYPE_FORMAT, "BillingRecord", path, "", res.getError());
				}
				return success("BillingRecord", ValidationResult.ValidationType.TYPE_FORMAT, "BillingRecord", path, "");
			})
			.collect(toList());
	}
