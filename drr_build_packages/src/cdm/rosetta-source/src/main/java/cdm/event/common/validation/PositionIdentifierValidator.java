		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PositionIdentifier", ValidationResult.ValidationType.CARDINALITY, "PositionIdentifier", path, "", res.getError());
				}
				return success("PositionIdentifier", ValidationResult.ValidationType.CARDINALITY, "PositionIdentifier", path, "");
			})
			.collect(toList());
	}
