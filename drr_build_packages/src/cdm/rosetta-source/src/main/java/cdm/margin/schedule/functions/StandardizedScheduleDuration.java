	...
	*/
	...
	public static class StandardizedScheduleDurationDefault extends StandardizedScheduleDuration {
		...
		protected MapperS<BigDecimal> transactionTenorInYears(Trade trade, StandardizedScheduleAssetClassEnum assetClass, StandardizedScheduleProductClassEnum productClass) {
			if (exists(transactionEffectiveDate(trade, assetClass, productClass)).andNullSafe(exists(transactionTerminationDate(trade, assetClass, productClass))).getOrDefault(false)) {
		...
		protected MapperS<BigDecimal> underlyingTransactionTenorInYears(Trade trade, StandardizedScheduleAssetClassEnum assetClass, StandardizedScheduleProductClassEnum productClass) {
			if (exists(underlyingTransactionEffectiveDate(trade, assetClass, productClass)).andNullSafe(exists(underlyingTransactionTerminationDate(trade, assetClass, productClass))).getOrDefault(false)) {
		...
		protected MapperS<BigDecimal> timeToOptionExpiryInYears(Trade trade, StandardizedScheduleAssetClassEnum assetClass, StandardizedScheduleProductClassEnum productClass) {
			if (exists(transactionEffectiveDate(trade, assetClass, productClass)).andNullSafe(exists(optionExpiry(trade, assetClass, productClass))).getOrDefault(false)) {
		...
		protected MapperS<BigDecimal> genericDurationInYears(Trade trade, StandardizedScheduleAssetClassEnum assetClass, StandardizedScheduleProductClassEnum productClass) {
			if (areEqual(MapperS.of(assetClass), MapperS.of(StandardizedScheduleAssetClassEnum.INTEREST_RATES), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(assetClass), MapperS.of(StandardizedScheduleAssetClassEnum.CREDIT), CardinalityOperator.All)).getOrDefault(false)) {
				if (areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.SWAP), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.SWAP_WITH_CALLABLE_BERMUDAN_RIGHT_TO_ENTER_EXIT_SWAPS), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.CROSS_CURRENCY_SWAP), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.FORWARD_RATE_AGREEMENT), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.SINGLE_NAME_CREDIT_DEFAULT_SWAP), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.INDEX_CDS), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.INDEX_TRANCHE), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.CREDIT_NTH_TO_DEFAULT), CardinalityOperator.All)).getOrDefault(false)) {
					return transactionTenorInYears(trade, assetClass, productClass);
				}
				if (areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.SWAPTION), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(productClass), MapperS.of(StandardizedScheduleProductClassEnum.SWAPTION_STRADDLE), CardinalityOperator.All)).getOrDefault(false)) {
					if (exists(timeToOptionExpiryInYears(trade, assetClass, productClass)).andNullSafe(exists(underlyingTransactionTenorInYears(trade, assetClass, productClass))).getOrDefault(false)) {
