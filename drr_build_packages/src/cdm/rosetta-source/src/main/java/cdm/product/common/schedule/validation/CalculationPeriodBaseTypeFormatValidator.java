		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodBase", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodBase", path, "", res.getError());
				}
				return success("CalculationPeriodBase", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodBase", path, "");
			})
			.collect(toList());
	}
