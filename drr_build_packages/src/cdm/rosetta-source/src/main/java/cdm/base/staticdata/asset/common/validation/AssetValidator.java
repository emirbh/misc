		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Asset", ValidationResult.ValidationType.CARDINALITY, "Asset", path, "", res.getError());
				}
				return success("Asset", ValidationResult.ValidationType.CARDINALITY, "Asset", path, "");
			})
			.collect(toList());
	}
