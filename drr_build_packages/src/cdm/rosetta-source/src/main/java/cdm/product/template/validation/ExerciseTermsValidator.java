		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExerciseTerms", ValidationResult.ValidationType.CARDINALITY, "ExerciseTerms", path, "", res.getError());
				}
				return success("ExerciseTerms", ValidationResult.ValidationType.CARDINALITY, "ExerciseTerms", path, "");
			})
			.collect(toList());
	}
