		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallBase", ValidationResult.ValidationType.CARDINALITY, "MarginCallBase", path, "", res.getError());
				}
				return success("MarginCallBase", ValidationResult.ValidationType.CARDINALITY, "MarginCallBase", path, "");
			})
			.collect(toList());
	}
