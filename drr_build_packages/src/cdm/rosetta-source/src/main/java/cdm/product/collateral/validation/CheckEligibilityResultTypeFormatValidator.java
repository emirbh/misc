		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CheckEligibilityResult", ValidationResult.ValidationType.TYPE_FORMAT, "CheckEligibilityResult", path, "", res.getError());
				}
				return success("CheckEligibilityResult", ValidationResult.ValidationType.TYPE_FORMAT, "CheckEligibilityResult", path, "");
			})
			.collect(toList());
	}
