		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FallbackRateParameters", ValidationResult.ValidationType.CARDINALITY, "FallbackRateParameters", path, "", res.getError());
				}
				return success("FallbackRateParameters", ValidationResult.ValidationType.CARDINALITY, "FallbackRateParameters", path, "");
			})
			.collect(toList());
	}
