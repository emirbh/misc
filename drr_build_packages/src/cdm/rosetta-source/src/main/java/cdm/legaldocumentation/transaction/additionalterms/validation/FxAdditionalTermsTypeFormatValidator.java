		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAdditionalTerms", ValidationResult.ValidationType.TYPE_FORMAT, "FxAdditionalTerms", path, "", res.getError());
				}
				return success("FxAdditionalTerms", ValidationResult.ValidationType.TYPE_FORMAT, "FxAdditionalTerms", path, "");
			})
			.collect(toList());
	}
