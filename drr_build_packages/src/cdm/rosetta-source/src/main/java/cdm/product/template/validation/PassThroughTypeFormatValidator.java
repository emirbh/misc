		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PassThrough", ValidationResult.ValidationType.TYPE_FORMAT, "PassThrough", path, "", res.getError());
				}
				return success("PassThrough", ValidationResult.ValidationType.TYPE_FORMAT, "PassThrough", path, "");
			})
			.collect(toList());
	}
