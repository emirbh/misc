		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralIssuerType", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralIssuerType", path, "", res.getError());
				}
				return success("CollateralIssuerType", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralIssuerType", path, "");
			})
			.collect(toList());
	}
