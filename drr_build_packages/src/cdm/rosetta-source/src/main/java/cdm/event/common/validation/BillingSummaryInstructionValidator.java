		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingSummaryInstruction", ValidationResult.ValidationType.CARDINALITY, "BillingSummaryInstruction", path, "", res.getError());
				}
				return success("BillingSummaryInstruction", ValidationResult.ValidationType.CARDINALITY, "BillingSummaryInstruction", path, "");
			})
			.collect(toList());
	}
