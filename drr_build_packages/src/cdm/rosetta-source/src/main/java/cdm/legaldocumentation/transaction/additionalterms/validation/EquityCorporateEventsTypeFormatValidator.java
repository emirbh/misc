		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityCorporateEvents", ValidationResult.ValidationType.TYPE_FORMAT, "EquityCorporateEvents", path, "", res.getError());
				}
				return success("EquityCorporateEvents", ValidationResult.ValidationType.TYPE_FORMAT, "EquityCorporateEvents", path, "");
			})
			.collect(toList());
	}
