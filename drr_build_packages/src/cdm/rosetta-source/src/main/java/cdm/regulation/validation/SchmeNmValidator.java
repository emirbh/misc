		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SchmeNm", ValidationResult.ValidationType.CARDINALITY, "SchmeNm", path, "", res.getError());
				}
				return success("SchmeNm", ValidationResult.ValidationType.CARDINALITY, "SchmeNm", path, "");
			})
			.collect(toList());
	}
