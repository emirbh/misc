		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralIssuerType", ValidationResult.ValidationType.CARDINALITY, "CollateralIssuerType", path, "", res.getError());
				}
				return success("CollateralIssuerType", ValidationResult.ValidationType.CARDINALITY, "CollateralIssuerType", path, "");
			})
			.collect(toList());
	}
