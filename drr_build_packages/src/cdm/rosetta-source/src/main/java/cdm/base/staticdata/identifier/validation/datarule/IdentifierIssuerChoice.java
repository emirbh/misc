	...
	class Default implements IdentifierIssuerChoice {
		...
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, Identifier identifier) {
			...
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Identifier", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "Identifier", path, DEFINITION, failureMessage));
		}
