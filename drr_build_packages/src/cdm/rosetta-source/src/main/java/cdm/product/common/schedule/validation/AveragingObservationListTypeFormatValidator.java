		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingObservationList", ValidationResult.ValidationType.TYPE_FORMAT, "AveragingObservationList", path, "", res.getError());
				}
				return success("AveragingObservationList", ValidationResult.ValidationType.TYPE_FORMAT, "AveragingObservationList", path, "");
			})
			.collect(toList());
	}
