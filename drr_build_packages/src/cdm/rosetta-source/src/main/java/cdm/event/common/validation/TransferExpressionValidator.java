		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransferExpression", ValidationResult.ValidationType.CARDINALITY, "TransferExpression", path, "", res.getError());
				}
				return success("TransferExpression", ValidationResult.ValidationType.CARDINALITY, "TransferExpression", path, "");
			})
			.collect(toList());
	}
