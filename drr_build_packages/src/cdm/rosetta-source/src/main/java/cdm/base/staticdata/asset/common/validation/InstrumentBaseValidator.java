...
public class InstrumentBaseValidator implements Validator<InstrumentBase> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentBase", ValidationResult.ValidationType.CARDINALITY, "InstrumentBase", path, "", res.getError());
				}
				return success("InstrumentBase", ValidationResult.ValidationType.CARDINALITY, "InstrumentBase", path, "");
			})
			.collect(toList());
	}
