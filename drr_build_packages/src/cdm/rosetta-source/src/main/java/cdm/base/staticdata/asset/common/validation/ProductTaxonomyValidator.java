		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProductTaxonomy", ValidationResult.ValidationType.CARDINALITY, "ProductTaxonomy", path, "", res.getError());
				}
				return success("ProductTaxonomy", ValidationResult.ValidationType.CARDINALITY, "ProductTaxonomy", path, "");
			})
			.collect(toList());
	}
