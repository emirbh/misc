		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DigitalAsset", ValidationResult.ValidationType.TYPE_FORMAT, "DigitalAsset", path, "", res.getError());
				}
				return success("DigitalAsset", ValidationResult.ValidationType.TYPE_FORMAT, "DigitalAsset", path, "");
			})
			.collect(toList());
	}
