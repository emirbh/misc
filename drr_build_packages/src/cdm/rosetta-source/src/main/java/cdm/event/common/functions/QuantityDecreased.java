	...
	public static class QuantityDecreasedDefault extends QuantityDecreased {
		...
		protected Boolean assignOutput(Boolean result, TradeState before, List<? extends TradeState> after) {
			result = areEqual(MapperC.<TradeState>of(after)
				.mapItem(item -> areEqual(MapperS.of(compareTradeLot.evaluate(item.<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get(), CompareOp.LESS_THAN, MapperS.of(before).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get())), MapperS.of(true), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(compareTradeLotToAmount.evaluate(item.<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get(), CompareOp.GREATER_THAN, new BigDecimal("0.0"))), MapperS.of(true), CardinalityOperator.All)).asMapper()), MapperS.of(true), CardinalityOperator.All).get();
