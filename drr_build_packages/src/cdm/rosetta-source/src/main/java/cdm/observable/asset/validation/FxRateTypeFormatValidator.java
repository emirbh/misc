		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxRate", ValidationResult.ValidationType.TYPE_FORMAT, "FxRate", path, "", res.getError());
				}
				return success("FxRate", ValidationResult.ValidationType.TYPE_FORMAT, "FxRate", path, "");
			})
			.collect(toList());
	}
