	...
	*/
	...
	public static class Qualify_UnderlierObservable_EquityDefault extends Qualify_UnderlierObservable_Equity {
		...
		protected Boolean assignOutput(Boolean is_product, Observable observable) {
			is_product = ComparisonResult.ofNullSafe(MapperS.of(qualify_InstrumentTypeEquity.evaluate(MapperS.of(observable).<Asset>map("getAsset", _observable -> _observable.getAsset()).<Instrument>map("getInstrument", asset -> asset.getInstrument()).get()))).orNullSafe(areEqual(MapperS.of(observable).<Index>map("getIndex", _observable -> _observable.getIndex()).<AssetClassEnum>map("chooseAssetClass", index -> indexDeepPathUtil.chooseAssetClass(index)), MapperS.of(AssetClassEnum.EQUITY), CardinalityOperator.All)).orNullSafe(exists(MapperS.of(observable).<Basket>map("getBasket", _observable -> _observable.getBasket())).andNullSafe(areEqual(MapperS.of(observable).<Basket>map("getBasket", _observable -> _observable.getBasket()).<FieldWithMetaBasketConstituent>mapC("getBasketConstituent", basket -> basket.getBasketConstituent())
				.mapItem(item -> {
