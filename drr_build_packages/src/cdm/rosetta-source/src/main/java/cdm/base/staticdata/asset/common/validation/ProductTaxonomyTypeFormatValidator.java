		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProductTaxonomy", ValidationResult.ValidationType.TYPE_FORMAT, "ProductTaxonomy", path, "", res.getError());
				}
				return success("ProductTaxonomy", ValidationResult.ValidationType.TYPE_FORMAT, "ProductTaxonomy", path, "");
			})
			.collect(toList());
	}
