	...
	*/
	...
	public static class Qualify_ForeignExchange_ParameterReturnCorrelationDefault extends Qualify_ForeignExchange_ParameterReturnCorrelation {
		...
		protected Boolean assignOutput(Boolean is_product, EconomicTerms economicTerms) {
			is_product = areEqual(MapperS.of(qualify_AssetClass_ForeignExchange.evaluate(economicTerms)), MapperS.of(true), CardinalityOperator.All).andNullSafe(exists(MapperS.of(MapperS.of(economicTerms).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout()).get()).<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout()))).andNullSafe(onlyExists(performancePayout(economicTerms).<ReturnTerms>map("getReturnTerms", _performancePayout -> _performancePayout.getReturnTerms()), Arrays.asList("priceReturnTerms", "dividendReturnTerms", "varianceReturnTerms", "volatilityReturnTerms", "correlationReturnTerms"), Arrays.asList("correlationReturnTerms"))).andNullSafe(exists(performancePayout(economicTerms).<Underlier>map("getUnderlier", _performancePayout -> _performancePayout.getUnderlier()).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", referenceWithMetaObservable0 -> referenceWithMetaObservable0 == null ? null : referenceWithMetaObservable0.getValue()).<Basket>map("getBasket", observable -> observable.getBasket()))).andNullSafe(areEqual(performancePayout(economicTerms).<Underlier>map("getUnderlier", _performancePayout -> _performancePayout.getUnderlier()).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", referenceWithMetaObservable1 -> referenceWithMetaObservable1 == null ? null : referenceWithMetaObservable1.getValue()).<Basket>map("getBasket", observable -> observable.getBasket()).<FieldWithMetaBasketConstituent>mapC("getBasketConstituent", basket -> basket.getBasketConstituent())
				.mapItem(item -> exists(item.<BasketConstituent>map("Type coercion", fieldWithMetaBasketConstituent -> fieldWithMetaBasketConstituent == null ? null : fieldWithMetaBasketConstituent.getValue()).<Index>map("getIndex", basketConstituent -> basketConstituent.getIndex()).<ForeignExchangeRateIndex>map("getForeignExchangeRateIndex", index -> index.getForeignExchangeRateIndex())).asMapper()), MapperS.of(true), CardinalityOperator.All)).get();
