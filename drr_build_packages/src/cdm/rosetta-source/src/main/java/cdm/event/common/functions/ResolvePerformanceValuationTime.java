...
public abstract class ResolvePerformanceValuationTime implements RosettaFunction {
	...
	* @param assetIdentifier Specifies the asset identifier, along with the source, which should be used to determine the correct valuation time i.e. close times are different across exchanges.
	...
	*/
	public TimeZone evaluate(BusinessCenterTime valuationTime, TimeTypeEnum valuationTimeType, AssetIdentifier assetIdentifier, DeterminationMethodEnum determinationMethod) {
		TimeZone.TimeZoneBuilder timeBuilder = doEvaluate(valuationTime, valuationTimeType, assetIdentifier, determinationMethod);
	...
	protected abstract TimeZone.TimeZoneBuilder doEvaluate(BusinessCenterTime valuationTime, TimeTypeEnum valuationTimeType, AssetIdentifier assetIdentifier, DeterminationMethodEnum determinationMethod);
	...
	public static class ResolvePerformanceValuationTimeDefault extends ResolvePerformanceValuationTime {
		...
		protected TimeZone.TimeZoneBuilder doEvaluate(BusinessCenterTime valuationTime, TimeTypeEnum valuationTimeType, AssetIdentifier assetIdentifier, DeterminationMethodEnum determinationMethod) {
			...
			return assignOutput(time, valuationTime, valuationTimeType, assetIdentifier, determinationMethod);
		}
		...
		protected TimeZone.TimeZoneBuilder assignOutput(TimeZone.TimeZoneBuilder time, BusinessCenterTime valuationTime, TimeTypeEnum valuationTimeType, AssetIdentifier assetIdentifier, DeterminationMethodEnum determinationMethod) {
			...
			if (exists(MapperS.of(valuationTimeType)).getOrDefault(false)) {
				time = toBuilder(resolveTimeZoneFromTimeType.evaluate(assetIdentifier, valuationTimeType, determinationMethod));
			} else {
				time = null;
			}
