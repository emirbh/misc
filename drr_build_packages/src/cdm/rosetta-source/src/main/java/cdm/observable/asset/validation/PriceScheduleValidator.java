...
public class PriceScheduleValidator implements Validator<PriceSchedule> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceSchedule", ValidationResult.ValidationType.CARDINALITY, "PriceSchedule", path, "", res.getError());
				}
				return success("PriceSchedule", ValidationResult.ValidationType.CARDINALITY, "PriceSchedule", path, "");
			})
			.collect(toList());
	}
