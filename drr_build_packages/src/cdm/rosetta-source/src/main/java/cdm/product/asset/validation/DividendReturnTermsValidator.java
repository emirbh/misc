		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DividendReturnTerms", ValidationResult.ValidationType.CARDINALITY, "DividendReturnTerms", path, "", res.getError());
				}
				return success("DividendReturnTerms", ValidationResult.ValidationType.CARDINALITY, "DividendReturnTerms", path, "");
			})
			.collect(toList());
	}
