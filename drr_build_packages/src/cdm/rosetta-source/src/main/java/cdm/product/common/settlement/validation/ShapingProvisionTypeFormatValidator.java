		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ShapingProvision", ValidationResult.ValidationType.TYPE_FORMAT, "ShapingProvision", path, "", res.getError());
				}
				return success("ShapingProvision", ValidationResult.ValidationType.TYPE_FORMAT, "ShapingProvision", path, "");
			})
			.collect(toList());
	}
