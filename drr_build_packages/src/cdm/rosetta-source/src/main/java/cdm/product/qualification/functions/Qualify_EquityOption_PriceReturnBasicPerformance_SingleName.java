	...
	*/
	...
	public static class Qualify_EquityOption_PriceReturnBasicPerformance_SingleNameDefault extends Qualify_EquityOption_PriceReturnBasicPerformance_SingleName {
		...
		protected Boolean assignOutput(Boolean is_product, EconomicTerms economicTerms) {
			...
			is_product = ComparisonResult.ofNullSafe(MapperS.of(qualify_AssetClass_Equity.evaluate(economicTerms))).andNullSafe(exists(optionPayout(economicTerms))).andNullSafe(exists(optionUnderlier(economicTerms).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", referenceWithMetaObservable0 -> referenceWithMetaObservable0 == null ? null : referenceWithMetaObservable0.getValue()).<Asset>map("getAsset", observable -> observable.getAsset()))).andNullSafe(exists(optionUnderlier(economicTerms).<ReferenceWithMetaObservable>map("getObservable", underlier -> underlier.getObservable()).<Observable>map("Type coercion", referenceWithMetaObservable1 -> referenceWithMetaObservable1 == null ? null : referenceWithMetaObservable1.getValue()).<Asset>map("getAsset", observable -> observable.getAsset()).<Instrument>map("getInstrument", asset -> asset.getInstrument()).<Security>map("getSecurity", instrument -> instrument.getSecurity()))).andNullSafe(notExists(optionPayout(economicTerms).<OptionFeature>map("getFeature", _optionPayout -> _optionPayout.getFeature())).orNullSafe(onlyExists(optionPayout(economicTerms).<OptionFeature>map("getFeature", _optionPayout -> _optionPayout.getFeature()), Arrays.asList("fxFeature", "strategyFeature", "averagingFeature", "barrier", "knock", "passThrough"), Arrays.asList("averagingFeature"))).orNullSafe(exists(MapperS.of(optionPayout(economicTerms).<OptionFeature>map("getFeature", _optionPayout -> _optionPayout.getFeature()).<FxFeature>mapC("getFxFeature", optionFeature -> optionFeature.getFxFeature()).get()).<Quanto>map("getQuanto", fxFeature -> fxFeature.getQuanto()))).orNullSafe(exists(MapperS.of(optionPayout(economicTerms).<OptionFeature>map("getFeature", _optionPayout -> _optionPayout.getFeature()).<FxFeature>mapC("getFxFeature", optionFeature -> optionFeature.getFxFeature()).get()).<Composite>map("getComposite", fxFeature -> fxFeature.getComposite())))).andNullSafe(ifThenElseResult).get();
