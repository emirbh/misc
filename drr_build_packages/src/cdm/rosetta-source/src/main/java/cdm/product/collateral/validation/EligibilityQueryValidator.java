		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EligibilityQuery", ValidationResult.ValidationType.CARDINALITY, "EligibilityQuery", path, "", res.getError());
				}
				return success("EligibilityQuery", ValidationResult.ValidationType.CARDINALITY, "EligibilityQuery", path, "");
			})
			.collect(toList());
	}
