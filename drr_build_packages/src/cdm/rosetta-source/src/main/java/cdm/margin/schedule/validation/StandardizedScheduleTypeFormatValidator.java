		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StandardizedSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "StandardizedSchedule", path, "", res.getError());
				}
				return success("StandardizedSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "StandardizedSchedule", path, "");
			})
			.collect(toList());
	}
