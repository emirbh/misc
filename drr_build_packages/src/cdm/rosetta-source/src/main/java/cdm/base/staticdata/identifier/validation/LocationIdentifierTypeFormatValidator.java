		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LocationIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "LocationIdentifier", path, "", res.getError());
				}
				return success("LocationIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "LocationIdentifier", path, "");
			})
			.collect(toList());
	}
