		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradePricingReport", ValidationResult.ValidationType.CARDINALITY, "TradePricingReport", path, "", res.getError());
				}
				return success("TradePricingReport", ValidationResult.ValidationType.CARDINALITY, "TradePricingReport", path, "");
			})
			.collect(toList());
	}
