		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StubPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "StubPeriod", path, "", res.getError());
				}
				return success("StubPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "StubPeriod", path, "");
			})
			.collect(toList());
	}
