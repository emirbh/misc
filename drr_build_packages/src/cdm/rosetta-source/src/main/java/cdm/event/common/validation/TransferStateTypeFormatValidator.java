		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransferState", ValidationResult.ValidationType.TYPE_FORMAT, "TransferState", path, "", res.getError());
				}
				return success("TransferState", ValidationResult.ValidationType.TYPE_FORMAT, "TransferState", path, "");
			})
			.collect(toList());
	}
