...
public interface PrincipalPaymentPrincipalAmount extends Validator<PrincipalPayment> {
	...
	String DEFINITION = "optional choice principalAmount, presentValuePrincipalAmount";
	...
	class Default implements PrincipalPaymentPrincipalAmount {
		...
		private ComparisonResult executeDataRule(PrincipalPayment principalPayment) {
			try {
				return choice(MapperS.of(principalPayment), Arrays.asList("principalAmount", "presentValuePrincipalAmount"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
