		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EligibilityQuery", ValidationResult.ValidationType.TYPE_FORMAT, "EligibilityQuery", path, "", res.getError());
				}
				return success("EligibilityQuery", ValidationResult.ValidationType.TYPE_FORMAT, "EligibilityQuery", path, "");
			})
			.collect(toList());
	}
