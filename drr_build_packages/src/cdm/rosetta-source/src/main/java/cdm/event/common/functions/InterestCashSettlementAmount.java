	...
	public static class InterestCashSettlementAmountDefault extends InterestCashSettlementAmount {
		...
		protected Transfer.TransferBuilder assignOutput(Transfer.TransferBuilder interestCashSettlementAmount, TradeState tradeState, Payout payout, List<? extends Reset> resets, Date date) {
			...
			interestCashSettlementAmount
				.getOrCreateQuantity()
				.getOrCreateUnit()
				.setCurrency(interestRatePayout(tradeState, payout, resets, date).<ResolvablePriceQuantity>map("getPriceQuantity", _interestRatePayout -> _interestRatePayout.getPriceQuantity()).<ReferenceWithMetaNonNegativeQuantitySchedule>map("getQuantitySchedule", resolvablePriceQuantity -> resolvablePriceQuantity.getQuantitySchedule()).<NonNegativeQuantitySchedule>map("Type coercion", referenceWithMetaNonNegativeQuantitySchedule -> referenceWithMetaNonNegativeQuantitySchedule == null ? null : referenceWithMetaNonNegativeQuantitySchedule.getValue()).<UnitType>map("getUnit", nonNegativeQuantitySchedule -> nonNegativeQuantitySchedule.getUnit()).<FieldWithMetaString>map("getCurrency", unitType -> unitType.getCurrency()).get());
			
			final ReferenceWithMetaParty ifThenElseResult0;
			if (greaterThanEquals(performance(tradeState, payout, resets, date), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All).getOrDefault(false)) {
				ifThenElseResult0 = payer(tradeState, payout, resets, date).get();
			} else {
				ifThenElseResult0 = receiver(tradeState, payout, resets, date).get();
			}
			interestCashSettlementAmount
				.getOrCreatePayerReceiver()
				.setPayerPartyReference(ifThenElseResult0);
			
			final ReferenceWithMetaParty ifThenElseResult1;
			if (greaterThanEquals(performance(tradeState, payout, resets, date), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All).getOrDefault(false)) {
				ifThenElseResult1 = receiver(tradeState, payout, resets, date).get();
			} else {
				ifThenElseResult1 = payer(tradeState, payout, resets, date).get();
			}
			interestCashSettlementAmount
				.getOrCreatePayerReceiver()
				.setReceiverPartyReference(ifThenElseResult1);
