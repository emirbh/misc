	...
	*/
	...
	public static class ObservableQualificationDefault extends ObservableQualification {
		...
		protected Boolean assignOutput(Boolean qualifies, Observable observable, InstrumentTypeEnum securityType, AssetClassEnum assetClass) {
			...
			qualifies = exists(MapperS.of(securityType)).andNullSafe(areEqual(MapperS.of(observable).<Asset>map("getAsset", _observable -> _observable.getAsset()).<Instrument>map("getInstrument", asset -> asset.getInstrument()).<Security>map("getSecurity", instrument -> instrument.getSecurity()).<InstrumentTypeEnum>map("getInstrumentType", security -> security.getInstrumentType()), MapperS.of(securityType), CardinalityOperator.All)).orNullSafe(exists(MapperS.of(assetClass)).andNullSafe(areEqual(MapperS.of(observable).<Index>map("getIndex", _observable -> _observable.getIndex()).<AssetClassEnum>map("chooseAssetClass", index -> indexDeepPathUtil.chooseAssetClass(index)), MapperS.of(assetClass), CardinalityOperator.All))).orNullSafe(ifThenElseResult).get();
