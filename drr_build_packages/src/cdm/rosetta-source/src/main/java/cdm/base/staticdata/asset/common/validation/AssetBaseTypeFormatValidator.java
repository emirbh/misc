		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetBase", ValidationResult.ValidationType.TYPE_FORMAT, "AssetBase", path, "", res.getError());
				}
				return success("AssetBase", ValidationResult.ValidationType.TYPE_FORMAT, "AssetBase", path, "");
			})
			.collect(toList());
	}
