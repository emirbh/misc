		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrimitiveInstruction", ValidationResult.ValidationType.CARDINALITY, "PrimitiveInstruction", path, "", res.getError());
				}
				return success("PrimitiveInstruction", ValidationResult.ValidationType.CARDINALITY, "PrimitiveInstruction", path, "");
			})
			.collect(toList());
	}
