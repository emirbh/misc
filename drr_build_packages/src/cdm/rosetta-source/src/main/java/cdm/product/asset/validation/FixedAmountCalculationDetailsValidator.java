		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedAmountCalculationDetails", ValidationResult.ValidationType.CARDINALITY, "FixedAmountCalculationDetails", path, "", res.getError());
				}
				return success("FixedAmountCalculationDetails", ValidationResult.ValidationType.CARDINALITY, "FixedAmountCalculationDetails", path, "");
			})
			.collect(toList());
	}
