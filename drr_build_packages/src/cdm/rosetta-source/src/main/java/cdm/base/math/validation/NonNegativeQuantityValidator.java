		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonNegativeQuantity", ValidationResult.ValidationType.CARDINALITY, "NonNegativeQuantity", path, "", res.getError());
				}
				return success("NonNegativeQuantity", ValidationResult.ValidationType.CARDINALITY, "NonNegativeQuantity", path, "");
			})
			.collect(toList());
	}
