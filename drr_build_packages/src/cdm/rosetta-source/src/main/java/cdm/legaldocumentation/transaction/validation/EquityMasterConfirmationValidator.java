		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityMasterConfirmation", ValidationResult.ValidationType.CARDINALITY, "EquityMasterConfirmation", path, "", res.getError());
				}
				return success("EquityMasterConfirmation", ValidationResult.ValidationType.CARDINALITY, "EquityMasterConfirmation", path, "");
			})
			.collect(toList());
	}
