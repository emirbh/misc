		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementSchedule", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementSchedule", path, "", res.getError());
				}
				return success("MasterAgreementSchedule", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementSchedule", path, "");
			})
			.collect(toList());
	}
