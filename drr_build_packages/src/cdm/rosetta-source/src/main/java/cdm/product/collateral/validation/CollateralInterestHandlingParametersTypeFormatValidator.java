		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralInterestHandlingParameters", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralInterestHandlingParameters", path, "", res.getError());
				}
				return success("CollateralInterestHandlingParameters", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralInterestHandlingParameters", path, "");
			})
			.collect(toList());
	}
