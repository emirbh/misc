		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPayout", ValidationResult.ValidationType.CARDINALITY, "SettlementPayout", path, "", res.getError());
				}
				return success("SettlementPayout", ValidationResult.ValidationType.CARDINALITY, "SettlementPayout", path, "");
			})
			.collect(toList());
	}
