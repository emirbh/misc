		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PersonIdentifier", ValidationResult.ValidationType.CARDINALITY, "PersonIdentifier", path, "", res.getError());
				}
				return success("PersonIdentifier", ValidationResult.ValidationType.CARDINALITY, "PersonIdentifier", path, "");
			})
			.collect(toList());
	}
