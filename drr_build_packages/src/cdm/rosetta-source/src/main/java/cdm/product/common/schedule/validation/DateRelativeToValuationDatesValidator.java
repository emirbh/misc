		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateRelativeToValuationDates", ValidationResult.ValidationType.CARDINALITY, "DateRelativeToValuationDates", path, "", res.getError());
				}
				return success("DateRelativeToValuationDates", ValidationResult.ValidationType.CARDINALITY, "DateRelativeToValuationDates", path, "");
			})
			.collect(toList());
	}
