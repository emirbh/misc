		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IssuerCountryOfOrigin", ValidationResult.ValidationType.TYPE_FORMAT, "IssuerCountryOfOrigin", path, "", res.getError());
				}
				return success("IssuerCountryOfOrigin", ValidationResult.ValidationType.TYPE_FORMAT, "IssuerCountryOfOrigin", path, "");
			})
			.collect(toList());
	}
