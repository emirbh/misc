...
public class PortfolioReturnTermsValidator implements Validator<PortfolioReturnTerms> {
	...
	private List<ComparisonResult> getComparisonResults(PortfolioReturnTerms o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("underlier", (ReferenceWithMetaObservable) o.getUnderlier() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PortfolioReturnTerms", ValidationResult.ValidationType.CARDINALITY, "PortfolioReturnTerms", path, "", res.getError());
				}
				return success("PortfolioReturnTerms", ValidationResult.ValidationType.CARDINALITY, "PortfolioReturnTerms", path, "");
			})
			.collect(toList());
	}
