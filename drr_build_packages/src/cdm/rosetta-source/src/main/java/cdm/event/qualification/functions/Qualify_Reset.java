	...
	*/
	...
	public static class Qualify_ResetDefault extends Qualify_Reset {
		...
		protected Boolean assignOutput(Boolean is_event, BusinessEvent businessEvent) {
			is_event = areEqual(MapperS.of(MapperS.of(businessEvent).<TradeState>mapC("getAfter", _businessEvent -> _businessEvent.getAfter()).resultCount()), MapperS.of(1), CardinalityOperator.All).andNullSafe(areEqual(beforeTradeState(businessEvent).<TradeState>map("Type coercion", referenceWithMetaTradeState0 -> referenceWithMetaTradeState0 == null ? null : referenceWithMetaTradeState0.getValue()).<Trade>map("getTrade", tradeState -> tradeState.getTrade()), afterTradeState(businessEvent).<Trade>map("getTrade", tradeState -> tradeState.getTrade()), CardinalityOperator.All)).andNullSafe(areEqual(MapperMaths.<Integer, Integer, Integer>add(MapperS.of(beforeTradeState(businessEvent).<TradeState>map("Type coercion", referenceWithMetaTradeState1 -> referenceWithMetaTradeState1 == null ? null : referenceWithMetaTradeState1.getValue()).<Reset>mapC("getResetHistory", tradeState -> tradeState.getResetHistory()).resultCount()), MapperS.of(1)), MapperS.of(afterTradeState(businessEvent).<Reset>mapC("getResetHistory", tradeState -> tradeState.getResetHistory()).resultCount()), CardinalityOperator.All)).get();
