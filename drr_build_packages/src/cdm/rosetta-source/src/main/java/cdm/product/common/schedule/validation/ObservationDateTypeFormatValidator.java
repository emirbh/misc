		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationDate", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationDate", path, "", res.getError());
				}
				return success("ObservationDate", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationDate", path, "");
			})
			.collect(toList());
	}
