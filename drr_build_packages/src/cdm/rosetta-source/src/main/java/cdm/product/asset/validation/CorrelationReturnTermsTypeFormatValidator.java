		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "CorrelationReturnTerms", path, "", res.getError());
				}
				return success("CorrelationReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "CorrelationReturnTerms", path, "");
			})
			.collect(toList());
	}
