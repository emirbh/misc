		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaxonomyClassification", ValidationResult.ValidationType.CARDINALITY, "TaxonomyClassification", path, "", res.getError());
				}
				return success("TaxonomyClassification", ValidationResult.ValidationType.CARDINALITY, "TaxonomyClassification", path, "");
			})
			.collect(toList());
	}
