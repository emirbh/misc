		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MeasureBase", ValidationResult.ValidationType.CARDINALITY, "MeasureBase", path, "", res.getError());
				}
				return success("MeasureBase", ValidationResult.ValidationType.CARDINALITY, "MeasureBase", path, "");
			})
			.collect(toList());
	}
