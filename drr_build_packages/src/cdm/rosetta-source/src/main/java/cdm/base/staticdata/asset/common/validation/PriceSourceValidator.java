		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceSource", ValidationResult.ValidationType.CARDINALITY, "PriceSource", path, "", res.getError());
				}
				return success("PriceSource", ValidationResult.ValidationType.CARDINALITY, "PriceSource", path, "");
			})
			.collect(toList());
	}
