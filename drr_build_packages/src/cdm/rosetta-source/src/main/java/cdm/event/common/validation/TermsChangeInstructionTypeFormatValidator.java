		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TermsChangeInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "TermsChangeInstruction", path, "", res.getError());
				}
				return success("TermsChangeInstruction", ValidationResult.ValidationType.TYPE_FORMAT, "TermsChangeInstruction", path, "");
			})
			.collect(toList());
	}
