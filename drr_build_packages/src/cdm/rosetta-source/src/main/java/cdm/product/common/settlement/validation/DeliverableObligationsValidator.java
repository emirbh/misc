		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DeliverableObligations", ValidationResult.ValidationType.CARDINALITY, "DeliverableObligations", path, "", res.getError());
				}
				return success("DeliverableObligations", ValidationResult.ValidationType.CARDINALITY, "DeliverableObligations", path, "");
			})
			.collect(toList());
	}
