		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyRole", ValidationResult.ValidationType.CARDINALITY, "PartyRole", path, "", res.getError());
				}
				return success("PartyRole", ValidationResult.ValidationType.CARDINALITY, "PartyRole", path, "");
			})
			.collect(toList());
	}
