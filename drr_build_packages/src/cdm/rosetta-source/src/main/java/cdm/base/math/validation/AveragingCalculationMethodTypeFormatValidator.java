		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingCalculationMethod", ValidationResult.ValidationType.TYPE_FORMAT, "AveragingCalculationMethod", path, "", res.getError());
				}
				return success("AveragingCalculationMethod", ValidationResult.ValidationType.TYPE_FORMAT, "AveragingCalculationMethod", path, "");
			})
			.collect(toList());
	}
