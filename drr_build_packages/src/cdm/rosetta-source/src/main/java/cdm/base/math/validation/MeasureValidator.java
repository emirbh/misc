		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Measure", ValidationResult.ValidationType.CARDINALITY, "Measure", path, "", res.getError());
				}
				return success("Measure", ValidationResult.ValidationType.CARDINALITY, "Measure", path, "");
			})
			.collect(toList());
	}
