		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExerciseFee", ValidationResult.ValidationType.TYPE_FORMAT, "ExerciseFee", path, "", res.getError());
				}
				return success("ExerciseFee", ValidationResult.ValidationType.TYPE_FORMAT, "ExerciseFee", path, "");
			})
			.collect(toList());
	}
