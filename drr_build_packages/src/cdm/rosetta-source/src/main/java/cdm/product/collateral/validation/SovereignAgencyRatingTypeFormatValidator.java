		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SovereignAgencyRating", ValidationResult.ValidationType.TYPE_FORMAT, "SovereignAgencyRating", path, "", res.getError());
				}
				return success("SovereignAgencyRating", ValidationResult.ValidationType.TYPE_FORMAT, "SovereignAgencyRating", path, "");
			})
			.collect(toList());
	}
