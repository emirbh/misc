		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionalEarlyTermination", ValidationResult.ValidationType.TYPE_FORMAT, "OptionalEarlyTermination", path, "", res.getError());
				}
				return success("OptionalEarlyTermination", ValidationResult.ValidationType.TYPE_FORMAT, "OptionalEarlyTermination", path, "");
			})
			.collect(toList());
	}
