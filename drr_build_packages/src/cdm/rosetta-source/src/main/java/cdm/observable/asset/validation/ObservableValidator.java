		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Observable", ValidationResult.ValidationType.CARDINALITY, "Observable", path, "", res.getError());
				}
				return success("Observable", ValidationResult.ValidationType.CARDINALITY, "Observable", path, "");
			})
			.collect(toList());
	}
