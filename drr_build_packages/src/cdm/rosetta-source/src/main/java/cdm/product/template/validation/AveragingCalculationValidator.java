		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingCalculation", ValidationResult.ValidationType.CARDINALITY, "AveragingCalculation", path, "", res.getError());
				}
				return success("AveragingCalculation", ValidationResult.ValidationType.CARDINALITY, "AveragingCalculation", path, "");
			})
			.collect(toList());
	}
