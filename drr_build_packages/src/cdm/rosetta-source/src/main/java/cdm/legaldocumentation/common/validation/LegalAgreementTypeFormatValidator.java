		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegalAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "LegalAgreement", path, "", res.getError());
				}
				return success("LegalAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "LegalAgreement", path, "");
			})
			.collect(toList());
	}
