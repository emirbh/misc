...
public class GeneralNonTransferabilityValidator implements Validator<GeneralNonTransferability> {

	private List<ComparisonResult> getComparisonResults(GeneralNonTransferability o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("generalNonTransferabilityIsApplicable", (Boolean) o.getGeneralNonTransferabilityIsApplicable() != null ? 1 : 0, 1, 1), 
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, GeneralNonTransferability o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GeneralNonTransferability", ValidationResult.ValidationType.CARDINALITY, "GeneralNonTransferability", path, "", res.getError());
				}
				return success("GeneralNonTransferability", ValidationResult.ValidationType.CARDINALITY, "GeneralNonTransferability", path, "");
			})
			.collect(toList());
	}
