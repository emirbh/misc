		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ListedDerivative", ValidationResult.ValidationType.CARDINALITY, "ListedDerivative", path, "", res.getError());
				}
				return success("ListedDerivative", ValidationResult.ValidationType.CARDINALITY, "ListedDerivative", path, "");
			})
			.collect(toList());
	}
