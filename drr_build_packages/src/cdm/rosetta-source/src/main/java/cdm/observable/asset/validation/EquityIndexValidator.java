...
public class EquityIndexValidator implements Validator<EquityIndex> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityIndex", ValidationResult.ValidationType.CARDINALITY, "EquityIndex", path, "", res.getError());
				}
				return success("EquityIndex", ValidationResult.ValidationType.CARDINALITY, "EquityIndex", path, "");
			})
			.collect(toList());
	}
