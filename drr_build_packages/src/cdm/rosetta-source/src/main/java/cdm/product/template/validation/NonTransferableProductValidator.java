		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonTransferableProduct", ValidationResult.ValidationType.CARDINALITY, "NonTransferableProduct", path, "", res.getError());
				}
				return success("NonTransferableProduct", ValidationResult.ValidationType.CARDINALITY, "NonTransferableProduct", path, "");
			})
			.collect(toList());
	}
