...
public class AgencyRatingCriteriaValidator implements Validator<AgencyRatingCriteria> {
	...
	private List<ComparisonResult> getComparisonResults(AgencyRatingCriteria o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("creditNotation", (CreditNotation) o.getCreditNotation() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AgencyRatingCriteria", ValidationResult.ValidationType.CARDINALITY, "AgencyRatingCriteria", path, "", res.getError());
				}
				return success("AgencyRatingCriteria", ValidationResult.ValidationType.CARDINALITY, "AgencyRatingCriteria", path, "");
			})
			.collect(toList());
	}
