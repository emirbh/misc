		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceComposite", ValidationResult.ValidationType.CARDINALITY, "PriceComposite", path, "", res.getError());
				}
				return success("PriceComposite", ValidationResult.ValidationType.CARDINALITY, "PriceComposite", path, "");
			})
			.collect(toList());
	}
