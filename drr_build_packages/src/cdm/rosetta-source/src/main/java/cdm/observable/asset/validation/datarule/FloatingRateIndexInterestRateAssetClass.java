...
public interface FloatingRateIndexInterestRateAssetClass extends Validator<FloatingRateIndex> {
	...
	class Default implements FloatingRateIndexInterestRateAssetClass {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, FloatingRateIndex floatingRateIndex) {
			ComparisonResult result = executeDataRule(floatingRateIndex);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "FloatingRateIndex", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "FloatingRateIndex", path, DEFINITION, failureMessage));
		}
