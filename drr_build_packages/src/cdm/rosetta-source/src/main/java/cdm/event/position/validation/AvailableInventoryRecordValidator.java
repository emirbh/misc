		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AvailableInventoryRecord", ValidationResult.ValidationType.CARDINALITY, "AvailableInventoryRecord", path, "", res.getError());
				}
				return success("AvailableInventoryRecord", ValidationResult.ValidationType.CARDINALITY, "AvailableInventoryRecord", path, "");
			})
			.collect(toList());
	}
