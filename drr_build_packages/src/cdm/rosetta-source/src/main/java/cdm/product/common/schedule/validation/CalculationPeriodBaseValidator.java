		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodBase", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriodBase", path, "", res.getError());
				}
				return success("CalculationPeriodBase", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriodBase", path, "");
			})
			.collect(toList());
	}
