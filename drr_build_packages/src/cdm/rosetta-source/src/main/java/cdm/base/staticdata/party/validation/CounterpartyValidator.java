		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Counterparty", ValidationResult.ValidationType.CARDINALITY, "Counterparty", path, "", res.getError());
				}
				return success("Counterparty", ValidationResult.ValidationType.CARDINALITY, "Counterparty", path, "");
			})
			.collect(toList());
	}
