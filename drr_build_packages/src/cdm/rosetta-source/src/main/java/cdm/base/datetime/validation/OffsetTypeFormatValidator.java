		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Offset", ValidationResult.ValidationType.TYPE_FORMAT, "Offset", path, "", res.getError());
				}
				return success("Offset", ValidationResult.ValidationType.TYPE_FORMAT, "Offset", path, "");
			})
			.collect(toList());
	}
