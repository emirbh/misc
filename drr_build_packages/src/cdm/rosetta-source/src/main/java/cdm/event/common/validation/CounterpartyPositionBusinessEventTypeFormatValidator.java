		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CounterpartyPositionBusinessEvent", ValidationResult.ValidationType.TYPE_FORMAT, "CounterpartyPositionBusinessEvent", path, "", res.getError());
				}
				return success("CounterpartyPositionBusinessEvent", ValidationResult.ValidationType.TYPE_FORMAT, "CounterpartyPositionBusinessEvent", path, "");
			})
			.collect(toList());
	}
