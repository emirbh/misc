...
public class CancelableProvisionAdjustedDatesValidator implements Validator<CancelableProvisionAdjustedDates> {
	...
	private List<ComparisonResult> getComparisonResults(CancelableProvisionAdjustedDates o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("cancellationEvent", (List<? extends CancellationEvent>) o.getCancellationEvent() == null ? 0 : o.getCancellationEvent().size(), 1, 0)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CancelableProvisionAdjustedDates", ValidationResult.ValidationType.CARDINALITY, "CancelableProvisionAdjustedDates", path, "", res.getError());
				}
				return success("CancelableProvisionAdjustedDates", ValidationResult.ValidationType.CARDINALITY, "CancelableProvisionAdjustedDates", path, "");
			})
			.collect(toList());
	}
