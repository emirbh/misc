...
public class AssetPayoutValidator implements Validator<AssetPayout> {
	...
	private List<ComparisonResult> getComparisonResults(AssetPayout o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("assetLeg", (List<? extends AssetLeg>) o.getAssetLeg() == null ? 0 : o.getAssetLeg().size(), 1, 0), 
				checkCardinality("underlier", (Asset) o.getUnderlier() != null ? 1 : 0, 1, 1), 
				...
				checkCardinality("tradeType", (AssetPayoutTradeTypeEnum) o.getTradeType() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetPayout", ValidationResult.ValidationType.CARDINALITY, "AssetPayout", path, "", res.getError());
				}
				return success("AssetPayout", ValidationResult.ValidationType.CARDINALITY, "AssetPayout", path, "");
			})
			.collect(toList());
	}
