		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Qty", ValidationResult.ValidationType.CARDINALITY, "Qty", path, "", res.getError());
				}
				return success("Qty", ValidationResult.ValidationType.CARDINALITY, "Qty", path, "");
			})
			.collect(toList());
	}
