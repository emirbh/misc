		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OtherAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "OtherAgreement", path, "", res.getError());
				}
				return success("OtherAgreement", ValidationResult.ValidationType.TYPE_FORMAT, "OtherAgreement", path, "");
			})
			.collect(toList());
	}
