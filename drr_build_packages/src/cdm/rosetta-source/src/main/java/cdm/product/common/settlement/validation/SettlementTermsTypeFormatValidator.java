		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementTerms", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementTerms", path, "", res.getError());
				}
				return success("SettlementTerms", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementTerms", path, "");
			})
			.collect(toList());
	}
