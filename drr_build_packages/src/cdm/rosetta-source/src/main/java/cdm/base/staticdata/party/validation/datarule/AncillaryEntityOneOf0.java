...
public interface AncillaryEntityOneOf0 extends Validator<AncillaryEntity> {
	...
	class Default implements AncillaryEntityOneOf0 {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, AncillaryEntity ancillaryEntity) {
			ComparisonResult result = executeDataRule(ancillaryEntity);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "AncillaryEntity", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "AncillaryEntity", path, DEFINITION, failureMessage));
		}
