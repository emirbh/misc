		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Valuation", ValidationResult.ValidationType.CARDINALITY, "Valuation", path, "", res.getError());
				}
				return success("Valuation", ValidationResult.ValidationType.CARDINALITY, "Valuation", path, "");
			})
			.collect(toList());
	}
