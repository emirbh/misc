		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateIndexCalculationDefaults", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingRateIndexCalculationDefaults", path, "", res.getError());
				}
				return success("FloatingRateIndexCalculationDefaults", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingRateIndexCalculationDefaults", path, "");
			})
			.collect(toList());
	}
