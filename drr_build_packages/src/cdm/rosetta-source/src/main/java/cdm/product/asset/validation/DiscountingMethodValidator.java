		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DiscountingMethod", ValidationResult.ValidationType.CARDINALITY, "DiscountingMethod", path, "", res.getError());
				}
				return success("DiscountingMethod", ValidationResult.ValidationType.CARDINALITY, "DiscountingMethod", path, "");
			})
			.collect(toList());
	}
