		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingRecord", ValidationResult.ValidationType.CARDINALITY, "BillingRecord", path, "", res.getError());
				}
				return success("BillingRecord", ValidationResult.ValidationType.CARDINALITY, "BillingRecord", path, "");
			})
			.collect(toList());
	}
