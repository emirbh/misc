	...
	*/
	public WorkflowStep evaluate(MessageInformation messageInformation, List<? extends EventTimestamp> timestamp, List<? extends Identifier> eventIdentifier, List<? extends Party> party, List<? extends Account> account, WorkflowStep previousWorkflowStep, ActionEnum action, EventInstruction proposedEvent, List<? extends WorkflowStepApproval> approval) {
		...
		conditionValidator.validate(() -> {
			if (exists(MapperS.of(previousWorkflowStep).<EventInstruction>map("getProposedEvent", workflowStep -> workflowStep.getProposedEvent())).andNullSafe(areEqual(MapperS.of(previousWorkflowStep).<ActionEnum>map("getAction", workflowStep -> workflowStep.getAction()), MapperS.of(ActionEnum.NEW), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(previousWorkflowStep).<ActionEnum>map("getAction", workflowStep -> workflowStep.getAction()), MapperS.of(ActionEnum.CORRECT), CardinalityOperator.All))).getOrDefault(false)) {
		...
		conditionValidator.validate(() -> {
			if (notExists(MapperS.of(previousWorkflowStep)).orNullSafe(exists(MapperS.of(previousWorkflowStep).<BusinessEvent>map("getBusinessEvent", workflowStep -> workflowStep.getBusinessEvent()))).getOrDefault(false)) {
