		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Clause", ValidationResult.ValidationType.CARDINALITY, "Clause", path, "", res.getError());
				}
				return success("Clause", ValidationResult.ValidationType.CARDINALITY, "Clause", path, "");
			})
			.collect(toList());
	}
