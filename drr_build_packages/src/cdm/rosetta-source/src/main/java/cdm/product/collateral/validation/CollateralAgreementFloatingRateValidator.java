		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAgreementFloatingRate", ValidationResult.ValidationType.CARDINALITY, "CollateralAgreementFloatingRate", path, "", res.getError());
				}
				return success("CollateralAgreementFloatingRate", ValidationResult.ValidationType.CARDINALITY, "CollateralAgreementFloatingRate", path, "");
			})
			.collect(toList());
	}
