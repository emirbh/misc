		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BrokerConfirmation", ValidationResult.ValidationType.TYPE_FORMAT, "BrokerConfirmation", path, "", res.getError());
				}
				return success("BrokerConfirmation", ValidationResult.ValidationType.TYPE_FORMAT, "BrokerConfirmation", path, "");
			})
			.collect(toList());
	}
