		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Quantity", ValidationResult.ValidationType.CARDINALITY, "Quantity", path, "", res.getError());
				}
				return success("Quantity", ValidationResult.ValidationType.CARDINALITY, "Quantity", path, "");
			})
			.collect(toList());
	}
