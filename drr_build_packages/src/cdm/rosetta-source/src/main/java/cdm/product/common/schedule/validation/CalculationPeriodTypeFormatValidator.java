		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriod", path, "", res.getError());
				}
				return success("CalculationPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriod", path, "");
			})
			.collect(toList());
	}
