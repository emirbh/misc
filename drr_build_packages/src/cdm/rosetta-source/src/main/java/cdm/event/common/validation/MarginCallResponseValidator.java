...
public class MarginCallResponseValidator implements Validator<MarginCallResponse> {
	...
	private List<ComparisonResult> getComparisonResults(MarginCallResponse o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("marginCallResponseAction", (List<? extends MarginCallResponseAction>) o.getMarginCallResponseAction() == null ? 0 : o.getMarginCallResponseAction().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallResponse", ValidationResult.ValidationType.CARDINALITY, "MarginCallResponse", path, "", res.getError());
				}
				return success("MarginCallResponse", ValidationResult.ValidationType.CARDINALITY, "MarginCallResponse", path, "");
			})
			.collect(toList());
	}
