		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Prsn", ValidationResult.ValidationType.TYPE_FORMAT, "Prsn", path, "", res.getError());
				}
				return success("Prsn", ValidationResult.ValidationType.TYPE_FORMAT, "Prsn", path, "");
			})
			.collect(toList());
	}
