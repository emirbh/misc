		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetLeg", ValidationResult.ValidationType.TYPE_FORMAT, "AssetLeg", path, "", res.getError());
				}
				return success("AssetLeg", ValidationResult.ValidationType.TYPE_FORMAT, "AssetLeg", path, "");
			})
			.collect(toList());
	}
