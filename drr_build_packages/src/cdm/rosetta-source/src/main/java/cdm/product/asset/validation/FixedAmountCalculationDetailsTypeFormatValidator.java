		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedAmountCalculationDetails", ValidationResult.ValidationType.TYPE_FORMAT, "FixedAmountCalculationDetails", path, "", res.getError());
				}
				return success("FixedAmountCalculationDetails", ValidationResult.ValidationType.TYPE_FORMAT, "FixedAmountCalculationDetails", path, "");
			})
			.collect(toList());
	}
