		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BusinessDateRange", ValidationResult.ValidationType.TYPE_FORMAT, "BusinessDateRange", path, "", res.getError());
				}
				return success("BusinessDateRange", ValidationResult.ValidationType.TYPE_FORMAT, "BusinessDateRange", path, "");
			})
			.collect(toList());
	}
