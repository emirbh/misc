		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowRepresentation", ValidationResult.ValidationType.TYPE_FORMAT, "CashflowRepresentation", path, "", res.getError());
				}
				return success("CashflowRepresentation", ValidationResult.ValidationType.TYPE_FORMAT, "CashflowRepresentation", path, "");
			})
			.collect(toList());
	}
