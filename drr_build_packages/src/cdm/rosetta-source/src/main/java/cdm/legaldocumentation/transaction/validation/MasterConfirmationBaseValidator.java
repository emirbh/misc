		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterConfirmationBase", ValidationResult.ValidationType.CARDINALITY, "MasterConfirmationBase", path, "", res.getError());
				}
				return success("MasterConfirmationBase", ValidationResult.ValidationType.CARDINALITY, "MasterConfirmationBase", path, "");
			})
			.collect(toList());
	}
