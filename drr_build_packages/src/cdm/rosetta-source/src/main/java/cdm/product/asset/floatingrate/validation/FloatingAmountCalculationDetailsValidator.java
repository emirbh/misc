		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingAmountCalculationDetails", ValidationResult.ValidationType.CARDINALITY, "FloatingAmountCalculationDetails", path, "", res.getError());
				}
				return success("FloatingAmountCalculationDetails", ValidationResult.ValidationType.CARDINALITY, "FloatingAmountCalculationDetails", path, "");
			})
			.collect(toList());
	}
