...
public class BillingInstructionValidator implements Validator<BillingInstruction> {
	...
	private List<ComparisonResult> getComparisonResults(BillingInstruction o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("billingRecordInstruction", (List<? extends BillingRecordInstruction>) o.getBillingRecordInstruction() == null ? 0 : o.getBillingRecordInstruction().size(), 1, 0)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingInstruction", ValidationResult.ValidationType.CARDINALITY, "BillingInstruction", path, "", res.getError());
				}
				return success("BillingInstruction", ValidationResult.ValidationType.CARDINALITY, "BillingInstruction", path, "");
			})
			.collect(toList());
	}
