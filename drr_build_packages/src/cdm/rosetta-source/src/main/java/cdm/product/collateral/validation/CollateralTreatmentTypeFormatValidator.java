		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralTreatment", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralTreatment", path, "", res.getError());
				}
				return success("CollateralTreatment", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralTreatment", path, "");
			})
			.collect(toList());
	}
