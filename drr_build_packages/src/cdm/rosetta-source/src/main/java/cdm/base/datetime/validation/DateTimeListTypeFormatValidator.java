		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateTimeList", ValidationResult.ValidationType.TYPE_FORMAT, "DateTimeList", path, "", res.getError());
				}
				return success("DateTimeList", ValidationResult.ValidationType.TYPE_FORMAT, "DateTimeList", path, "");
			})
			.collect(toList());
	}
