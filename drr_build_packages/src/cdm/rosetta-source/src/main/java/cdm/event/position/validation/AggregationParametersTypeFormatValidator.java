		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AggregationParameters", ValidationResult.ValidationType.TYPE_FORMAT, "AggregationParameters", path, "", res.getError());
				}
				return success("AggregationParameters", ValidationResult.ValidationType.TYPE_FORMAT, "AggregationParameters", path, "");
			})
			.collect(toList());
	}
