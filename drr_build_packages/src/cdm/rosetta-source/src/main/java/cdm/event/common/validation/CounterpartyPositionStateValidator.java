		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CounterpartyPositionState", ValidationResult.ValidationType.CARDINALITY, "CounterpartyPositionState", path, "", res.getError());
				}
				return success("CounterpartyPositionState", ValidationResult.ValidationType.CARDINALITY, "CounterpartyPositionState", path, "");
			})
			.collect(toList());
	}
