		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityProductDefinition", ValidationResult.ValidationType.CARDINALITY, "CommodityProductDefinition", path, "", res.getError());
				}
				return success("CommodityProductDefinition", ValidationResult.ValidationType.CARDINALITY, "CommodityProductDefinition", path, "");
			})
			.collect(toList());
	}
