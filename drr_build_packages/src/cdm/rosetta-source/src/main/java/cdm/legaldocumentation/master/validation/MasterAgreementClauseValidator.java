...
public class MasterAgreementClauseValidator implements Validator<MasterAgreementClause> {
	...
	private List<ComparisonResult> getComparisonResults(MasterAgreementClause o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("variant", (List<? extends MasterAgreementClauseVariant>) o.getVariant() == null ? 0 : o.getVariant().size(), 1, 0)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementClause", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementClause", path, "", res.getError());
				}
				return success("MasterAgreementClause", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementClause", path, "");
			})
			.collect(toList());
	}
