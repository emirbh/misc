		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Sngl", ValidationResult.ValidationType.TYPE_FORMAT, "Sngl", path, "", res.getError());
				}
				return success("Sngl", ValidationResult.ValidationType.TYPE_FORMAT, "Sngl", path, "");
			})
			.collect(toList());
	}
