...
public interface UnderlierChoice extends Validator<Underlier> {
	...
	class Default implements UnderlierChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, Underlier underlier) {
			ComparisonResult result = executeDataRule(underlier);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "Underlier", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "Underlier", path, DEFINITION, failureMessage));
		}
