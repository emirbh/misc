		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PortfolioReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "PortfolioReturnTerms", path, "", res.getError());
				}
				return success("PortfolioReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "PortfolioReturnTerms", path, "");
			})
			.collect(toList());
	}
