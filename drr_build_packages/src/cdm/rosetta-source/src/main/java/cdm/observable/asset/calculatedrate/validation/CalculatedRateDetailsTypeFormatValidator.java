		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculatedRateDetails", ValidationResult.ValidationType.TYPE_FORMAT, "CalculatedRateDetails", path, "", res.getError());
				}
				return success("CalculatedRateDetails", ValidationResult.ValidationType.TYPE_FORMAT, "CalculatedRateDetails", path, "");
			})
			.collect(toList());
	}
