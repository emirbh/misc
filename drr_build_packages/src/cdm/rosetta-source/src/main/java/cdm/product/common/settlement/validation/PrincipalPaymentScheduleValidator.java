		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrincipalPaymentSchedule", ValidationResult.ValidationType.CARDINALITY, "PrincipalPaymentSchedule", path, "", res.getError());
				}
				return success("PrincipalPaymentSchedule", ValidationResult.ValidationType.CARDINALITY, "PrincipalPaymentSchedule", path, "");
			})
			.collect(toList());
	}
