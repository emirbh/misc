		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFeature", ValidationResult.ValidationType.TYPE_FORMAT, "FxFeature", path, "", res.getError());
				}
				return success("FxFeature", ValidationResult.ValidationType.TYPE_FORMAT, "FxFeature", path, "");
			})
			.collect(toList());
	}
