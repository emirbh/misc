	...
	public static class Create_ExposureFromTradesDefault extends Create_ExposureFromTrades {
		...
		protected Exposure.ExposureBuilder assignOutput(Exposure.ExposureBuilder exposure, List<? extends TradeState> trades) {
			exposure
				...
				.getOrCreateValue().addPositions(MapperC.<TradeState>of(trades)
					.mapItem(item -> MapperS.of(Position.builder()
						.setCashBalance(null)
						.setPriceQuantity(item.<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradeLot>mapC("getTradeLot", _trade -> _trade.getTradeLot()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity()).getMulti())
						.setTradeReferenceValue(null)
						.setProduct(Product.builder()
							.setNonTransferableProduct(item.<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get())
							.build())
						.build())).getMulti());
			...
			exposure
				...
				.getOrCreateLineage()
				.setTradeReference(MapperC.<TradeState>of(trades).<Trade>map("getTrade", tradeState -> tradeState.getTrade())
					.first().<ReferenceWithMetaTrade>map("Type coercion", trade -> trade == null ? ReferenceWithMetaTrade.builder().build() : ReferenceWithMetaTrade.builder().setValue(trade).build()).getMulti());
