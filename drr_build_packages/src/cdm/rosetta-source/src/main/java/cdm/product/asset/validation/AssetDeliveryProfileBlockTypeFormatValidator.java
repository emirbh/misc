		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetDeliveryProfileBlock", ValidationResult.ValidationType.TYPE_FORMAT, "AssetDeliveryProfileBlock", path, "", res.getError());
				}
				return success("AssetDeliveryProfileBlock", ValidationResult.ValidationType.TYPE_FORMAT, "AssetDeliveryProfileBlock", path, "");
			})
			.collect(toList());
	}
