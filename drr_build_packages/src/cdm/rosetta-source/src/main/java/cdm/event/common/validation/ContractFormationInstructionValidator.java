		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContractFormationInstruction", ValidationResult.ValidationType.CARDINALITY, "ContractFormationInstruction", path, "", res.getError());
				}
				return success("ContractFormationInstruction", ValidationResult.ValidationType.CARDINALITY, "ContractFormationInstruction", path, "");
			})
			.collect(toList());
	}
