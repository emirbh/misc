...
public abstract class Create_TradeState implements RosettaFunction {
	...
	*/
	...
	protected abstract MapperS<Date> terminationDate(PrimitiveInstruction primitiveInstruction, TradeState before);
	...
	public static class Create_TradeStateDefault extends Create_TradeState {
		...
		protected TradeState.TradeStateBuilder assignOutput(TradeState.TradeStateBuilder after, PrimitiveInstruction primitiveInstruction, TradeState before) {
			after = toBuilder(stockSplit(primitiveInstruction, before).get(), () -> TradeState.builder());
		...
		protected MapperS<? extends TradeState> contractFormation(PrimitiveInstruction primitiveInstruction, TradeState before) {
			if (exists(MapperS.of(primitiveInstruction)).andNullSafe(notExists(MapperS.of(primitiveInstruction).<ContractFormationInstruction>map("getContractFormation", _primitiveInstruction -> _primitiveInstruction.getContractFormation()))).getOrDefault(false)) {
		...
		@Override
		protected MapperS<Date> terminationDate(PrimitiveInstruction primitiveInstruction, TradeState before) {
			if (exists(MapperS.of(primitiveInstruction).<QuantityChangeInstruction>map("getQuantityChange", _primitiveInstruction -> _primitiveInstruction.getQuantityChange()).<PriceQuantity>mapC("getChange", quantityChangeInstruction -> quantityChangeInstruction.getChange()).<AdjustableOrRelativeDate>map("getEffectiveDate", priceQuantity -> priceQuantity.getEffectiveDate())).getOrDefault(false)) {
				return MapperS.of(resolveAdjustableDate.evaluate(MapperS.of(primitiveInstruction).<QuantityChangeInstruction>map("getQuantityChange", _primitiveInstruction -> _primitiveInstruction.getQuantityChange()).<PriceQuantity>mapC("getChange", quantityChangeInstruction -> quantityChangeInstruction.getChange()).<AdjustableOrRelativeDate>map("getEffectiveDate", priceQuantity -> priceQuantity.getEffectiveDate())
					.first().get()));
			}
