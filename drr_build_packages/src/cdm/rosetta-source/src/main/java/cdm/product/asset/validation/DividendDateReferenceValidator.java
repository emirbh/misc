		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DividendDateReference", ValidationResult.ValidationType.CARDINALITY, "DividendDateReference", path, "", res.getError());
				}
				return success("DividendDateReference", ValidationResult.ValidationType.CARDINALITY, "DividendDateReference", path, "");
			})
			.collect(toList());
	}
