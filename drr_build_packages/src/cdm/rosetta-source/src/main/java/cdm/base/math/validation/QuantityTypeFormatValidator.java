		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Quantity", ValidationResult.ValidationType.TYPE_FORMAT, "Quantity", path, "", res.getError());
				}
				return success("Quantity", ValidationResult.ValidationType.TYPE_FORMAT, "Quantity", path, "");
			})
			.collect(toList());
	}
