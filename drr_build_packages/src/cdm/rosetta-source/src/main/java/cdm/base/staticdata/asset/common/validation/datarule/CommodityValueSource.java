...
public interface CommodityValueSource extends Validator<Commodity> {
	...
	String DEFINITION = "if taxonomy exists then (taxonomy -> source exists and taxonomy -> value exists)";
	...
	class Default implements CommodityValueSource {
		...
		private ComparisonResult executeDataRule(Commodity commodity) {
			try {
				if (exists(MapperS.of(commodity).<Taxonomy>mapC("getTaxonomy", _commodity -> _commodity.getTaxonomy())).getOrDefault(false)) {
					return exists(MapperS.of(commodity).<Taxonomy>mapC("getTaxonomy", _commodity -> _commodity.getTaxonomy()).<TaxonomySourceEnum>map("getSource", taxonomy -> taxonomy.getSource())).andNullSafe(exists(MapperS.of(commodity).<Taxonomy>mapC("getTaxonomy", _commodity -> _commodity.getTaxonomy()).<TaxonomyValue>map("getValue", taxonomy -> taxonomy.getValue())));
				}
