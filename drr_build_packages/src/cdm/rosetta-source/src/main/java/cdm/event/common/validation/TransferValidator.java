...
public class TransferValidator implements Validator<Transfer> {
	...
	private List<ComparisonResult> getComparisonResults(Transfer o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("settlementOrigin", (ReferenceWithMetaPayout) o.getSettlementOrigin() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Transfer", ValidationResult.ValidationType.CARDINALITY, "Transfer", path, "", res.getError());
				}
				return success("Transfer", ValidationResult.ValidationType.CARDINALITY, "Transfer", path, "");
			})
			.collect(toList());
	}
