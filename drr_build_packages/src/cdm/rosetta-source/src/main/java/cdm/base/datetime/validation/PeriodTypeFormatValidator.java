		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Period", ValidationResult.ValidationType.TYPE_FORMAT, "Period", path, "", res.getError());
				}
				return success("Period", ValidationResult.ValidationType.TYPE_FORMAT, "Period", path, "");
			})
			.collect(toList());
	}
