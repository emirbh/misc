		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TerminationProvision", ValidationResult.ValidationType.CARDINALITY, "TerminationProvision", path, "", res.getError());
				}
				return success("TerminationProvision", ValidationResult.ValidationType.CARDINALITY, "TerminationProvision", path, "");
			})
			.collect(toList());
	}
