		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NumberRange", ValidationResult.ValidationType.TYPE_FORMAT, "NumberRange", path, "", res.getError());
				}
				return success("NumberRange", ValidationResult.ValidationType.TYPE_FORMAT, "NumberRange", path, "");
			})
			.collect(toList());
	}
