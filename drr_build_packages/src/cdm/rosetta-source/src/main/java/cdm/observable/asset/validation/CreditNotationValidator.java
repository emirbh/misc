		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditNotation", ValidationResult.ValidationType.CARDINALITY, "CreditNotation", path, "", res.getError());
				}
				return success("CreditNotation", ValidationResult.ValidationType.CARDINALITY, "CreditNotation", path, "");
			})
			.collect(toList());
	}
