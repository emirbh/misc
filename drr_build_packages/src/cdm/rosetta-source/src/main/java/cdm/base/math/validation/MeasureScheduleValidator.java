		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MeasureSchedule", ValidationResult.ValidationType.CARDINALITY, "MeasureSchedule", path, "", res.getError());
				}
				return success("MeasureSchedule", ValidationResult.ValidationType.CARDINALITY, "MeasureSchedule", path, "");
			})
			.collect(toList());
	}
