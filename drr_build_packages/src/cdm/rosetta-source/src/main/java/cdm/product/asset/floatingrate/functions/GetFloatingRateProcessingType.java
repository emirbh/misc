	...
	*/
	...
	public static class GetFloatingRateProcessingTypeDefault extends GetFloatingRateProcessingType {
		...
		protected MapperS<Boolean> isCalculatedRate(FloatingRateSpecification rateDef) {
			return exists(MapperS.of(rateDef).<FloatingRateCalculationParameters>map("getCalculationParameters", floatingRateSpecification -> floatingRateSpecification.getCalculationParameters())).asMapper();
		}
		
		@Override
		protected MapperS<? extends FloatingRateIndexDefinition> floatingRateDefinition(FloatingRateSpecification rateDef) {
			final FieldWithMetaFloatingRateIndexEnum fieldWithMetaFloatingRateIndexEnum = MapperS.of(rateDef).<ReferenceWithMetaInterestRateIndex>map("getRateOption", floatingRateSpecification -> floatingRateSpecification.getRateOption()).<InterestRateIndex>map("Type coercion", referenceWithMetaInterestRateIndex -> referenceWithMetaInterestRateIndex == null ? null : referenceWithMetaInterestRateIndex.getValue()).<FloatingRateIndex>map("getFloatingRateIndex", interestRateIndex -> interestRateIndex.getFloatingRateIndex()).<FieldWithMetaFloatingRateIndexEnum>map("getFloatingRateIndex", floatingRateIndex -> floatingRateIndex.getFloatingRateIndex()).get();
		...
		protected MapperS<FloatingRateIndexProcessingTypeEnum> calcProcessingType(FloatingRateSpecification rateDef) {
			if (areEqual(idxStyle(rateDef), MapperS.of(FloatingRateIndexStyleEnum.COMPOUNDED_FRO), CardinalityOperator.All).andNullSafe(areEqual(method(rateDef), MapperS.of(FloatingRateIndexCalculationMethodEnum.OIS_COMPOUND), CardinalityOperator.All)).getOrDefault(false)) {
				return MapperS.of(FloatingRateIndexProcessingTypeEnum.OIS);
			}
			if (areEqual(idxStyle(rateDef), MapperS.of(FloatingRateIndexStyleEnum.AVERAGE_FRO), CardinalityOperator.All).andNullSafe(areEqual(method(rateDef), MapperS.of(FloatingRateIndexCalculationMethodEnum.AVERAGE), CardinalityOperator.All)).getOrDefault(false)) {
