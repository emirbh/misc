...
public class PriceQuantityValidator implements Validator<PriceQuantity> {
	...
	private List<ComparisonResult> getComparisonResults(PriceQuantity o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("observable", (FieldWithMetaObservable) o.getObservable() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceQuantity", ValidationResult.ValidationType.CARDINALITY, "PriceQuantity", path, "", res.getError());
				}
				return success("PriceQuantity", ValidationResult.ValidationType.CARDINALITY, "PriceQuantity", path, "");
			})
			.collect(toList());
	}
