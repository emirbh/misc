...
public class ProcessAgentValidator implements Validator<ProcessAgent> {
	...
	private List<ComparisonResult> getComparisonResults(ProcessAgent o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("partyElection", (List<? extends ProcessAgentElection>) o.getPartyElection() == null ? 0 : o.getPartyElection().size(), 2, 2)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProcessAgent", ValidationResult.ValidationType.CARDINALITY, "ProcessAgent", path, "", res.getError());
				}
				return success("ProcessAgent", ValidationResult.ValidationType.CARDINALITY, "ProcessAgent", path, "");
			})
			.collect(toList());
	}
