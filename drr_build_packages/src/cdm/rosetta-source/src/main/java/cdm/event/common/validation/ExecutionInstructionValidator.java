...
public class ExecutionInstructionValidator implements Validator<ExecutionInstruction> {
	...
	private List<ComparisonResult> getComparisonResults(ExecutionInstruction o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("parties", (List<? extends Party>) o.getParties() == null ? 0 : o.getParties().size(), 2, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionInstruction", ValidationResult.ValidationType.CARDINALITY, "ExecutionInstruction", path, "", res.getError());
				}
				return success("ExecutionInstruction", ValidationResult.ValidationType.CARDINALITY, "ExecutionInstruction", path, "");
			})
			.collect(toList());
	}
