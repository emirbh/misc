		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Id", ValidationResult.ValidationType.CARDINALITY, "Id", path, "", res.getError());
				}
				return success("Id", ValidationResult.ValidationType.CARDINALITY, "Id", path, "");
			})
			.collect(toList());
	}
