		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PeriodRange", ValidationResult.ValidationType.CARDINALITY, "PeriodRange", path, "", res.getError());
				}
				return success("PeriodRange", ValidationResult.ValidationType.CARDINALITY, "PeriodRange", path, "");
			})
			.collect(toList());
	}
