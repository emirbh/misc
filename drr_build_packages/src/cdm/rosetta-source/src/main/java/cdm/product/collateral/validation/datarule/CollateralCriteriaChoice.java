...
public interface CollateralCriteriaChoice extends Validator<CollateralCriteria> {
	...
	class Default implements CollateralCriteriaChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, CollateralCriteria collateralCriteria) {
			ComparisonResult result = executeDataRule(collateralCriteria);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "CollateralCriteria", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "CollateralCriteria", path, DEFINITION, failureMessage));
		}
