		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationSource", ValidationResult.ValidationType.CARDINALITY, "ValuationSource", path, "", res.getError());
				}
				return success("ValuationSource", ValidationResult.ValidationType.CARDINALITY, "ValuationSource", path, "");
			})
			.collect(toList());
	}
