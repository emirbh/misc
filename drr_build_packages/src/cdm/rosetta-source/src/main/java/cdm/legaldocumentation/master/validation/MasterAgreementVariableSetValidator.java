		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementVariableSet", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementVariableSet", path, "", res.getError());
				}
				return success("MasterAgreementVariableSet", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementVariableSet", path, "");
			})
			.collect(toList());
	}
