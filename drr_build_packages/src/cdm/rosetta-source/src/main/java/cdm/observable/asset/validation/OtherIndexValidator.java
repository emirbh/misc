		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OtherIndex", ValidationResult.ValidationType.CARDINALITY, "OtherIndex", path, "", res.getError());
				}
				return success("OtherIndex", ValidationResult.ValidationType.CARDINALITY, "OtherIndex", path, "");
			})
			.collect(toList());
	}
