...
public interface ContractFormationInstructionExecutedAgreements extends Validator<ContractFormationInstruction> {
	...
	class Default implements ContractFormationInstructionExecutedAgreements {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, ContractFormationInstruction contractFormationInstruction) {
			ComparisonResult result = executeDataRule(contractFormationInstruction);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "ContractFormationInstruction", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "ContractFormationInstruction", path, DEFINITION, failureMessage));
		}
		...
		private ComparisonResult executeDataRule(ContractFormationInstruction contractFormationInstruction) {
			try {
				if (exists(MapperS.of(contractFormationInstruction).<LegalAgreement>mapC("getLegalAgreement", _contractFormationInstruction -> _contractFormationInstruction.getLegalAgreement())).getOrDefault(false)) {
					return exists(MapperS.of(contractFormationInstruction).<LegalAgreement>mapC("getLegalAgreement", _contractFormationInstruction -> _contractFormationInstruction.getLegalAgreement()).<Date>map("getAgreementDate", legalAgreement -> legalAgreement.getAgreementDate()));
				}
