	...
	*/
	...
	public static class EvaluateScreenRateDefault extends EvaluateScreenRate {
		...
		protected MapperS<BigDecimal> observedRate(FloatingRate rateDef, ResetDates resetDates, CalculationPeriodBase calculationPeriod) {
			final ReferenceWithMetaInterestRateIndex referenceWithMetaInterestRateIndex = MapperS.of(rateDef).<ReferenceWithMetaInterestRateIndex>map("getRateOption", floatingRate -> floatingRate.getRateOption()).get();
			return MapperS.of(indexValueObservation.evaluate(fixingDate(rateDef, resetDates, calculationPeriod).get(), (referenceWithMetaInterestRateIndex == null ? null : referenceWithMetaInterestRateIndex.getValue())));
		}
	}
}
