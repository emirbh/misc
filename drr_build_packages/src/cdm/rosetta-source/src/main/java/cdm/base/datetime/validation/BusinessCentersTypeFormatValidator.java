		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BusinessCenters", ValidationResult.ValidationType.TYPE_FORMAT, "BusinessCenters", path, "", res.getError());
				}
				return success("BusinessCenters", ValidationResult.ValidationType.TYPE_FORMAT, "BusinessCenters", path, "");
			})
			.collect(toList());
	}
