		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementDate", ValidationResult.ValidationType.CARDINALITY, "SettlementDate", path, "", res.getError());
				}
				return success("SettlementDate", ValidationResult.ValidationType.CARDINALITY, "SettlementDate", path, "");
			})
			.collect(toList());
	}
