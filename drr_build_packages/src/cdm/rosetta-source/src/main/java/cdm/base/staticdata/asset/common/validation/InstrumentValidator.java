		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Instrument", ValidationResult.ValidationType.CARDINALITY, "Instrument", path, "", res.getError());
				}
				return success("Instrument", ValidationResult.ValidationType.CARDINALITY, "Instrument", path, "");
			})
			.collect(toList());
	}
