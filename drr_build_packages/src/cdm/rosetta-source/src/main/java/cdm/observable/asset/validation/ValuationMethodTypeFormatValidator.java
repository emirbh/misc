		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationMethod", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationMethod", path, "", res.getError());
				}
				return success("ValuationMethod", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationMethod", path, "");
			})
			.collect(toList());
	}
