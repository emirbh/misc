		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationIdentifier", ValidationResult.ValidationType.CARDINALITY, "ObservationIdentifier", path, "", res.getError());
				}
				return success("ObservationIdentifier", ValidationResult.ValidationType.CARDINALITY, "ObservationIdentifier", path, "");
			})
			.collect(toList());
	}
