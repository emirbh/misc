	...
	*/
	...
	public static class ResolveInterestRateObservationIdentifiersDefault extends ResolveInterestRateObservationIdentifiers {
		...
		protected ObservationIdentifier.ObservationIdentifierBuilder assignOutput(ObservationIdentifier.ObservationIdentifierBuilder identifiers, InterestRatePayout payout, Date date) {
			...
			final FieldWithMetaInterestRateIndex ifThenElseResult;
			if (referenceWithMetaInterestRateIndex == null) {
				ifThenElseResult = FieldWithMetaInterestRateIndex.builder().build();
			} else {
				final InterestRateIndex interestRateIndex = referenceWithMetaInterestRateIndex.getValue();
				ifThenElseResult = interestRateIndex == null ? FieldWithMetaInterestRateIndex.builder().build() : FieldWithMetaInterestRateIndex.builder().setValue(interestRateIndex).build();
			}
			identifiers
				.getOrCreateObservable()
				.getOrCreateIndex()
				.setInterestRateIndex(ifThenElseResult);
