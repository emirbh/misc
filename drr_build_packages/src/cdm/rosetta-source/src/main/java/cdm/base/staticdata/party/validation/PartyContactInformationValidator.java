		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyContactInformation", ValidationResult.ValidationType.CARDINALITY, "PartyContactInformation", path, "", res.getError());
				}
				return success("PartyContactInformation", ValidationResult.ValidationType.CARDINALITY, "PartyContactInformation", path, "");
			})
			.collect(toList());
	}
