		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OffsetCalculation", ValidationResult.ValidationType.TYPE_FORMAT, "OffsetCalculation", path, "", res.getError());
				}
				return success("OffsetCalculation", ValidationResult.ValidationType.TYPE_FORMAT, "OffsetCalculation", path, "");
			})
			.collect(toList());
	}
