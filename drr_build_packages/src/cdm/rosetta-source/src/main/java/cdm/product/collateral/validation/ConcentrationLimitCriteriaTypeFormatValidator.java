		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConcentrationLimitCriteria", ValidationResult.ValidationType.TYPE_FORMAT, "ConcentrationLimitCriteria", path, "", res.getError());
				}
				return success("ConcentrationLimitCriteria", ValidationResult.ValidationType.TYPE_FORMAT, "ConcentrationLimitCriteria", path, "");
			})
			.collect(toList());
	}
