		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SingleValuationDate", ValidationResult.ValidationType.TYPE_FORMAT, "SingleValuationDate", path, "", res.getError());
				}
				return success("SingleValuationDate", ValidationResult.ValidationType.TYPE_FORMAT, "SingleValuationDate", path, "");
			})
			.collect(toList());
	}
