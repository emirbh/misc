		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TerminationProvision", ValidationResult.ValidationType.TYPE_FORMAT, "TerminationProvision", path, "", res.getError());
				}
				return success("TerminationProvision", ValidationResult.ValidationType.TYPE_FORMAT, "TerminationProvision", path, "");
			})
			.collect(toList());
	}
