		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralInterestNotification", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralInterestNotification", path, "", res.getError());
				}
				return success("CollateralInterestNotification", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralInterestNotification", path, "");
			})
			.collect(toList());
	}
