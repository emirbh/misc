		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OrdrTrnsmssn", ValidationResult.ValidationType.CARDINALITY, "OrdrTrnsmssn", path, "", res.getError());
				}
				return success("OrdrTrnsmssn", ValidationResult.ValidationType.CARDINALITY, "OrdrTrnsmssn", path, "");
			})
			.collect(toList());
	}
