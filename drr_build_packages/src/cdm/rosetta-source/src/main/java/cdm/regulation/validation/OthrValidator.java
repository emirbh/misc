		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Othr", ValidationResult.ValidationType.CARDINALITY, "Othr", path, "", res.getError());
				}
				return success("Othr", ValidationResult.ValidationType.CARDINALITY, "Othr", path, "");
			})
			.collect(toList());
	}
