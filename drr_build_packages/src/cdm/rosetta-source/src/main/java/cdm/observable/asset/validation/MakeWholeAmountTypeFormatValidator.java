		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MakeWholeAmount", ValidationResult.ValidationType.TYPE_FORMAT, "MakeWholeAmount", path, "", res.getError());
				}
				return success("MakeWholeAmount", ValidationResult.ValidationType.TYPE_FORMAT, "MakeWholeAmount", path, "");
			})
			.collect(toList());
	}
