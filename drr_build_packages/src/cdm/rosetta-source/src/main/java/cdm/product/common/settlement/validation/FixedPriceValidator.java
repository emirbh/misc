		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedPrice", ValidationResult.ValidationType.CARDINALITY, "FixedPrice", path, "", res.getError());
				}
				return success("FixedPrice", ValidationResult.ValidationType.CARDINALITY, "FixedPrice", path, "");
			})
			.collect(toList());
	}
