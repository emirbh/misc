		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculateTransferInstruction", ValidationResult.ValidationType.CARDINALITY, "CalculateTransferInstruction", path, "", res.getError());
				}
				return success("CalculateTransferInstruction", ValidationResult.ValidationType.CARDINALITY, "CalculateTransferInstruction", path, "");
			})
			.collect(toList());
	}
