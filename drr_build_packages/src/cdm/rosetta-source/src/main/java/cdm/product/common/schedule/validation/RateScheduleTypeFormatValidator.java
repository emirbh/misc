		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "RateSchedule", path, "", res.getError());
				}
				return success("RateSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "RateSchedule", path, "");
			})
			.collect(toList());
	}
