		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AddtlAttrbts", ValidationResult.ValidationType.CARDINALITY, "AddtlAttrbts", path, "", res.getError());
				}
				return success("AddtlAttrbts", ValidationResult.ValidationType.CARDINALITY, "AddtlAttrbts", path, "");
			})
			.collect(toList());
	}
