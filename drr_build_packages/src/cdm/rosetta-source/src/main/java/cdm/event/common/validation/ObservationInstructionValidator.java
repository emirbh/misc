		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationInstruction", ValidationResult.ValidationType.CARDINALITY, "ObservationInstruction", path, "", res.getError());
				}
				return success("ObservationInstruction", ValidationResult.ValidationType.CARDINALITY, "ObservationInstruction", path, "");
			})
			.collect(toList());
	}
