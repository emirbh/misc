		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityUnderlierProvisions", ValidationResult.ValidationType.CARDINALITY, "EquityUnderlierProvisions", path, "", res.getError());
				}
				return success("EquityUnderlierProvisions", ValidationResult.ValidationType.CARDINALITY, "EquityUnderlierProvisions", path, "");
			})
			.collect(toList());
	}
