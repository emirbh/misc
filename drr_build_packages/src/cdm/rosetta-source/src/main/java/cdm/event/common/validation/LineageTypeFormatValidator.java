		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Lineage", ValidationResult.ValidationType.TYPE_FORMAT, "Lineage", path, "", res.getError());
				}
				return success("Lineage", ValidationResult.ValidationType.TYPE_FORMAT, "Lineage", path, "");
			})
			.collect(toList());
	}
