		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditIndex", ValidationResult.ValidationType.CARDINALITY, "CreditIndex", path, "", res.getError());
				}
				return success("CreditIndex", ValidationResult.ValidationType.CARDINALITY, "CreditIndex", path, "");
			})
			.collect(toList());
	}
