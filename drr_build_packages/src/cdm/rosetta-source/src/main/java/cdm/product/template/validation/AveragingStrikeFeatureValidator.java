		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingStrikeFeature", ValidationResult.ValidationType.CARDINALITY, "AveragingStrikeFeature", path, "", res.getError());
				}
				return success("AveragingStrikeFeature", ValidationResult.ValidationType.CARDINALITY, "AveragingStrikeFeature", path, "");
			})
			.collect(toList());
	}
