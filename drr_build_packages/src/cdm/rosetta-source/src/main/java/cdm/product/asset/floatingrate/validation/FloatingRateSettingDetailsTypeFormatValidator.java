		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateSettingDetails", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingRateSettingDetails", path, "", res.getError());
				}
				return success("FloatingRateSettingDetails", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingRateSettingDetails", path, "");
			})
			.collect(toList());
	}
