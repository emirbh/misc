		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingAmountCalculationDetails", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingAmountCalculationDetails", path, "", res.getError());
				}
				return success("FloatingAmountCalculationDetails", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingAmountCalculationDetails", path, "");
			})
			.collect(toList());
	}
