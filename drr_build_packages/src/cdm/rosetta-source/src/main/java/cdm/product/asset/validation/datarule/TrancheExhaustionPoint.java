...
public interface TrancheExhaustionPoint extends Validator<Tranche> {
	...
	class Default implements TrancheExhaustionPoint {
		...
		private ComparisonResult executeDataRule(Tranche tranche) {
			try {
				return greaterThanEquals(MapperS.of(tranche).<BigDecimal>map("getExhaustionPoint", _tranche -> _tranche.getExhaustionPoint()), MapperS.of(new BigDecimal("0.0")), CardinalityOperator.All).andNullSafe(lessThanEquals(MapperS.of(tranche).<BigDecimal>map("getExhaustionPoint", _tranche -> _tranche.getExhaustionPoint()), MapperS.of(new BigDecimal("1.0")), CardinalityOperator.All));
			}
			catch (Exception ex) {
