		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaxonomyValue", ValidationResult.ValidationType.TYPE_FORMAT, "TaxonomyValue", path, "", res.getError());
				}
				return success("TaxonomyValue", ValidationResult.ValidationType.TYPE_FORMAT, "TaxonomyValue", path, "");
			})
			.collect(toList());
	}
