		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Prsn", ValidationResult.ValidationType.CARDINALITY, "Prsn", path, "", res.getError());
				}
				return success("Prsn", ValidationResult.ValidationType.CARDINALITY, "Prsn", path, "");
			})
			.collect(toList());
	}
