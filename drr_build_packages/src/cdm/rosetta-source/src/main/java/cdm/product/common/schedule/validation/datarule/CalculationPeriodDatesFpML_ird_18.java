...
@ImplementedBy(CalculationPeriodDatesFpML_ird_18.Default.class)
public interface CalculationPeriodDatesFpML_ird_18 extends Validator<CalculationPeriodDates> {
	
	String NAME = "CalculationPeriodDatesFpML_ird_18";
	String DEFINITION = "if firstRegularPeriodStartDate exists and lastRegularPeriodEndDate exists then lastRegularPeriodEndDate > firstRegularPeriodStartDate";
	
	class Default implements CalculationPeriodDatesFpML_ird_18 {
		...
		private ComparisonResult executeDataRule(CalculationPeriodDates calculationPeriodDates) {
			try {
				if (exists(MapperS.of(calculationPeriodDates).<Date>map("getFirstRegularPeriodStartDate", _calculationPeriodDates -> _calculationPeriodDates.getFirstRegularPeriodStartDate())).andNullSafe(exists(MapperS.of(calculationPeriodDates).<Date>map("getLastRegularPeriodEndDate", _calculationPeriodDates -> _calculationPeriodDates.getLastRegularPeriodEndDate()))).getOrDefault(false)) {
	...
	class NoOp implements CalculationPeriodDatesFpML_ird_18 {
