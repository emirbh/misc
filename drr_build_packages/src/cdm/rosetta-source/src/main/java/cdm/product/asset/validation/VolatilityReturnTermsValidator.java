...
public class VolatilityReturnTermsValidator implements Validator<VolatilityReturnTerms> {
	...
	private List<ComparisonResult> getComparisonResults(VolatilityReturnTerms o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("exchangeTradedContractNearest", (ListedDerivative) o.getExchangeTradedContractNearest() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityReturnTerms", ValidationResult.ValidationType.CARDINALITY, "VolatilityReturnTerms", path, "", res.getError());
				}
				return success("VolatilityReturnTerms", ValidationResult.ValidationType.CARDINALITY, "VolatilityReturnTerms", path, "");
			})
			.collect(toList());
	}
