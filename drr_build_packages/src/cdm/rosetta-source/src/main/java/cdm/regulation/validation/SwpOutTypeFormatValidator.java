		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SwpOut", ValidationResult.ValidationType.TYPE_FORMAT, "SwpOut", path, "", res.getError());
				}
				return success("SwpOut", ValidationResult.ValidationType.TYPE_FORMAT, "SwpOut", path, "");
			})
			.collect(toList());
	}
