		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "PriceReturnTerms", path, "", res.getError());
				}
				return success("PriceReturnTerms", ValidationResult.ValidationType.TYPE_FORMAT, "PriceReturnTerms", path, "");
			})
			.collect(toList());
	}
