		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Term", ValidationResult.ValidationType.CARDINALITY, "Term", path, "", res.getError());
				}
				return success("Term", ValidationResult.ValidationType.CARDINALITY, "Term", path, "");
			})
			.collect(toList());
	}
