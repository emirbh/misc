		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AncillaryParty", ValidationResult.ValidationType.TYPE_FORMAT, "AncillaryParty", path, "", res.getError());
				}
				return success("AncillaryParty", ValidationResult.ValidationType.TYPE_FORMAT, "AncillaryParty", path, "");
			})
			.collect(toList());
	}
