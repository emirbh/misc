		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExerciseFeeSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "ExerciseFeeSchedule", path, "", res.getError());
				}
				return success("ExerciseFeeSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "ExerciseFeeSchedule", path, "");
			})
			.collect(toList());
	}
