		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallResponseAction", ValidationResult.ValidationType.CARDINALITY, "MarginCallResponseAction", path, "", res.getError());
				}
				return success("MarginCallResponseAction", ValidationResult.ValidationType.CARDINALITY, "MarginCallResponseAction", path, "");
			})
			.collect(toList());
	}
