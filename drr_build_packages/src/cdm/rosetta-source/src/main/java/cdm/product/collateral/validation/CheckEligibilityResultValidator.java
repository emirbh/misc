...
public class CheckEligibilityResultValidator implements Validator<CheckEligibilityResult> {
	...
	private List<ComparisonResult> getComparisonResults(CheckEligibilityResult o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("eligibilityQuery", (EligibilityQuery) o.getEligibilityQuery() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CheckEligibilityResult", ValidationResult.ValidationType.CARDINALITY, "CheckEligibilityResult", path, "", res.getError());
				}
				return success("CheckEligibilityResult", ValidationResult.ValidationType.CARDINALITY, "CheckEligibilityResult", path, "");
			})
			.collect(toList());
	}
