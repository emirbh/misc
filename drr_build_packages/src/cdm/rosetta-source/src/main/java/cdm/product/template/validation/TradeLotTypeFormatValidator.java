		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeLot", ValidationResult.ValidationType.TYPE_FORMAT, "TradeLot", path, "", res.getError());
				}
				return success("TradeLot", ValidationResult.ValidationType.TYPE_FORMAT, "TradeLot", path, "");
			})
			.collect(toList());
	}
