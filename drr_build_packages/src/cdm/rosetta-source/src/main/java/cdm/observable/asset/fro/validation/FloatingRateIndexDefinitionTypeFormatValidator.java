		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateIndexDefinition", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingRateIndexDefinition", path, "", res.getError());
				}
				return success("FloatingRateIndexDefinition", ValidationResult.ValidationType.TYPE_FORMAT, "FloatingRateIndexDefinition", path, "");
			})
			.collect(toList());
	}
