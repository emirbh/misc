	...
	*/
	...
	public static class UnderlierQualificationDefault extends UnderlierQualification {
		...
		protected Boolean assignOutput(Boolean qualifies, Underlier underlier, InstrumentTypeEnum securityType, AssetClassEnum assetClass) {
			...
			qualifies = ComparisonResult.ofNullSafe(MapperS.of(observableQualification.evaluate((referenceWithMetaObservable == null ? null : referenceWithMetaObservable.getValue()), securityType, assetClass))).orNullSafe(exists(MapperS.of(securityType)).andNullSafe(areEqual(MapperS.of(underlier).<Product>map("getProduct", _underlier -> _underlier.getProduct()).<TransferableProduct>map("getTransferableProduct", product -> product.getTransferableProduct()).<Instrument>map("getInstrument", transferableProduct -> transferableProduct.getInstrument()).<Security>map("getSecurity", instrument -> instrument.getSecurity()).<InstrumentTypeEnum>map("getInstrumentType", security -> security.getInstrumentType()), MapperS.of(securityType), CardinalityOperator.All))).get();
