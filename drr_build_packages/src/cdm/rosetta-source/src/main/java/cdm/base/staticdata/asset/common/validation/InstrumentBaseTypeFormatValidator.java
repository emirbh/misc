		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentBase", ValidationResult.ValidationType.TYPE_FORMAT, "InstrumentBase", path, "", res.getError());
				}
				return success("InstrumentBase", ValidationResult.ValidationType.TYPE_FORMAT, "InstrumentBase", path, "");
			})
			.collect(toList());
	}
