		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditLimitUtilisationPosition", ValidationResult.ValidationType.CARDINALITY, "CreditLimitUtilisationPosition", path, "", res.getError());
				}
				return success("CreditLimitUtilisationPosition", ValidationResult.ValidationType.CARDINALITY, "CreditLimitUtilisationPosition", path, "");
			})
			.collect(toList());
	}
