		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateRange", ValidationResult.ValidationType.TYPE_FORMAT, "DateRange", path, "", res.getError());
				}
				return success("DateRange", ValidationResult.ValidationType.TYPE_FORMAT, "DateRange", path, "");
			})
			.collect(toList());
	}
