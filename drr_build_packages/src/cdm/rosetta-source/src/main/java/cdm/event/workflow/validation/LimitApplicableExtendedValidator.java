		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LimitApplicableExtended", ValidationResult.ValidationType.CARDINALITY, "LimitApplicableExtended", path, "", res.getError());
				}
				return success("LimitApplicableExtended", ValidationResult.ValidationType.CARDINALITY, "LimitApplicableExtended", path, "");
			})
			.collect(toList());
	}
