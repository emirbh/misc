		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashflowRepresentation", ValidationResult.ValidationType.CARDINALITY, "CashflowRepresentation", path, "", res.getError());
				}
				return success("CashflowRepresentation", ValidationResult.ValidationType.CARDINALITY, "CashflowRepresentation", path, "");
			})
			.collect(toList());
	}
