		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegionalGovernmentIssuerType", ValidationResult.ValidationType.TYPE_FORMAT, "RegionalGovernmentIssuerType", path, "", res.getError());
				}
				return success("RegionalGovernmentIssuerType", ValidationResult.ValidationType.TYPE_FORMAT, "RegionalGovernmentIssuerType", path, "");
			})
			.collect(toList());
	}
