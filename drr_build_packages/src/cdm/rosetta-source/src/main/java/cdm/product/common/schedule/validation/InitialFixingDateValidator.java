		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InitialFixingDate", ValidationResult.ValidationType.CARDINALITY, "InitialFixingDate", path, "", res.getError());
				}
				return success("InitialFixingDate", ValidationResult.ValidationType.CARDINALITY, "InitialFixingDate", path, "");
			})
			.collect(toList());
	}
