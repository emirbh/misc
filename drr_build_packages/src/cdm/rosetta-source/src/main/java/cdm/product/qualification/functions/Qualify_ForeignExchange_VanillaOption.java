	...
	*/
	...
	public static class Qualify_ForeignExchange_VanillaOptionDefault extends Qualify_ForeignExchange_VanillaOption {
		...
		protected Boolean assignOutput(Boolean is_product, EconomicTerms economicTerms) {
			is_product = ComparisonResult.ofNullSafe(MapperS.of(qualify_AssetClass_ForeignExchange.evaluate(economicTerms))).andNullSafe(exists(optionPayout(economicTerms))).andNullSafe(notEqual(optionPayout(economicTerms).<ExerciseTerms>map("getExerciseTerms", _optionPayout -> _optionPayout.getExerciseTerms()).<OptionExerciseStyleEnum>map("getStyle", exerciseTerms -> exerciseTerms.getStyle()), MapperS.of(OptionExerciseStyleEnum.BERMUDA), CardinalityOperator.Any)).andNullSafe(notExists(optionPayout(economicTerms).<OptionFeature>map("getFeature", _optionPayout -> _optionPayout.getFeature())).orNullSafe(onlyExists(optionPayout(economicTerms).<OptionFeature>map("getFeature", _optionPayout -> _optionPayout.getFeature()), Arrays.asList("fxFeature", "strategyFeature", "averagingFeature", "barrier", "knock", "passThrough"), Arrays.asList("averagingFeature")))).get();
