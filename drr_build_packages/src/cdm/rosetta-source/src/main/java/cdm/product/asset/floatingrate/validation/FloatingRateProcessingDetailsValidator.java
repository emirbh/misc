		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateProcessingDetails", ValidationResult.ValidationType.CARDINALITY, "FloatingRateProcessingDetails", path, "", res.getError());
				}
				return success("FloatingRateProcessingDetails", ValidationResult.ValidationType.CARDINALITY, "FloatingRateProcessingDetails", path, "");
			})
			.collect(toList());
	}
