		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralInterestParameters", ValidationResult.ValidationType.CARDINALITY, "CollateralInterestParameters", path, "", res.getError());
				}
				return success("CollateralInterestParameters", ValidationResult.ValidationType.CARDINALITY, "CollateralInterestParameters", path, "");
			})
			.collect(toList());
	}
