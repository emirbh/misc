		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyIdentifier", ValidationResult.ValidationType.CARDINALITY, "PartyIdentifier", path, "", res.getError());
				}
				return success("PartyIdentifier", ValidationResult.ValidationType.CARDINALITY, "PartyIdentifier", path, "");
			})
			.collect(toList());
	}
