	...
	*/
	...
	public static class QuantityDecreasedToZeroDefault extends QuantityDecreasedToZero {
		...
		protected Boolean assignOutput(Boolean result, List<? extends TradeState> before, List<? extends TradeState> after) {
			result = areEqual(MapperS.of(compareTradeLotToAmount.evaluate(MapperC.<TradeState>of(before).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get(), CompareOp.GREATER_THAN, new BigDecimal("0.0"))), MapperS.of(true), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(compareTradeStatesToAmount.evaluate(after, CompareOp.EQUALS, new BigDecimal("0.0"))), MapperS.of(true), CardinalityOperator.All)).get();
