...
public abstract class Create_WorkflowStep implements RosettaFunction {
	...
	*/
	public WorkflowStep evaluate(MessageInformation messageInformation, List<? extends EventTimestamp> timestamp, List<? extends Identifier> eventIdentifier, List<? extends Party> party, List<? extends Account> account, WorkflowStep previousWorkflowStep, ActionEnum action, BusinessEvent businessEvent) {
		...
		conditionValidator.validate(() -> {
			if (areEqual(MapperS.of(previousWorkflowStep).<ActionEnum>map("getAction", _workflowStep -> _workflowStep.getAction()), MapperS.of(ActionEnum.NEW), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(previousWorkflowStep).<ActionEnum>map("getAction", _workflowStep -> _workflowStep.getAction()), MapperS.of(ActionEnum.CORRECT), CardinalityOperator.All)).getOrDefault(false)) {
				return areEqual(MapperS.of(action), MapperS.of(ActionEnum.NEW), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(action), MapperS.of(ActionEnum.CORRECT), CardinalityOperator.All)).orNullSafe(areEqual(MapperS.of(action), MapperS.of(ActionEnum.CANCEL), CardinalityOperator.All));
			}
			return ComparisonResult.ofEmpty();
		},
		...
		conditionValidator.validate(() -> {
			if (notExists(MapperS.of(previousWorkflowStep)).orNullSafe(notExists(MapperS.of(previousWorkflowStep).<ActionEnum>map("getAction", _workflowStep -> _workflowStep.getAction()))).getOrDefault(false)) {
