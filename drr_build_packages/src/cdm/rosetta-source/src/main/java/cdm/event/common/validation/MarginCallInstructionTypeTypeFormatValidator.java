		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallInstructionType", ValidationResult.ValidationType.TYPE_FORMAT, "MarginCallInstructionType", path, "", res.getError());
				}
				return success("MarginCallInstructionType", ValidationResult.ValidationType.TYPE_FORMAT, "MarginCallInstructionType", path, "");
			})
			.collect(toList());
	}
