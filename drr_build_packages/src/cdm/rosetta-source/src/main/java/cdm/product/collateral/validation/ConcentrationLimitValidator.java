...
public class ConcentrationLimitValidator implements Validator<ConcentrationLimit> {
	...
	private List<ComparisonResult> getComparisonResults(ConcentrationLimit o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("concentrationLimitCriteria", (ConcentrationLimitCriteria) o.getConcentrationLimitCriteria() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConcentrationLimit", ValidationResult.ValidationType.CARDINALITY, "ConcentrationLimit", path, "", res.getError());
				}
				return success("ConcentrationLimit", ValidationResult.ValidationType.CARDINALITY, "ConcentrationLimit", path, "");
			})
			.collect(toList());
	}
