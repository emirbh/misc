		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Cashflow", ValidationResult.ValidationType.TYPE_FORMAT, "Cashflow", path, "", res.getError());
				}
				return success("Cashflow", ValidationResult.ValidationType.TYPE_FORMAT, "Cashflow", path, "");
			})
			.collect(toList());
	}
