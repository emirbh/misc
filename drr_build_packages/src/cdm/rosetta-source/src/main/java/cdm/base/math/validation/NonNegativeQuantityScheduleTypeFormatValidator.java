		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonNegativeQuantitySchedule", ValidationResult.ValidationType.TYPE_FORMAT, "NonNegativeQuantitySchedule", path, "", res.getError());
				}
				return success("NonNegativeQuantitySchedule", ValidationResult.ValidationType.TYPE_FORMAT, "NonNegativeQuantitySchedule", path, "");
			})
			.collect(toList());
	}
