		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DividendTerms", ValidationResult.ValidationType.CARDINALITY, "DividendTerms", path, "", res.getError());
				}
				return success("DividendTerms", ValidationResult.ValidationType.CARDINALITY, "DividendTerms", path, "");
			})
			.collect(toList());
	}
