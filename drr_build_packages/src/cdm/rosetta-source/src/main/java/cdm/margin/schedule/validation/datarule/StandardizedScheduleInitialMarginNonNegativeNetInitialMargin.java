...
public interface StandardizedScheduleInitialMarginNonNegativeNetInitialMargin extends Validator<StandardizedScheduleInitialMargin> {
	...
	class Default implements StandardizedScheduleInitialMarginNonNegativeNetInitialMargin {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, StandardizedScheduleInitialMargin standardizedScheduleInitialMargin) {
			ComparisonResult result = executeDataRule(standardizedScheduleInitialMargin);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "StandardizedScheduleInitialMargin", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "StandardizedScheduleInitialMargin", path, DEFINITION, failureMessage));
		}
