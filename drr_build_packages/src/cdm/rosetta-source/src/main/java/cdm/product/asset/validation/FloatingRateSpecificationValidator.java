		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateSpecification", ValidationResult.ValidationType.CARDINALITY, "FloatingRateSpecification", path, "", res.getError());
				}
				return success("FloatingRateSpecification", ValidationResult.ValidationType.CARDINALITY, "FloatingRateSpecification", path, "");
			})
			.collect(toList());
	}
