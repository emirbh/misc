...
public interface TrancheAttachmentPoint extends Validator<Tranche> {
	...
	class Default implements TrancheAttachmentPoint {
		...
		private ComparisonResult executeDataRule(Tranche tranche) {
			try {
				return greaterThanEquals(MapperS.of(tranche).<BigDecimal>map("getAttachmentPoint", _tranche -> _tranche.getAttachmentPoint()), MapperS.of(new BigDecimal("0.0")), CardinalityOperator.All).andNullSafe(lessThanEquals(MapperS.of(tranche).<BigDecimal>map("getAttachmentPoint", _tranche -> _tranche.getAttachmentPoint()), MapperS.of(new BigDecimal("1.0")), CardinalityOperator.All));
			}
			catch (Exception ex) {
