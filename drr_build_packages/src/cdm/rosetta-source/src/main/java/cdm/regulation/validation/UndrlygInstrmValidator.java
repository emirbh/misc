		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UndrlygInstrm", ValidationResult.ValidationType.CARDINALITY, "UndrlygInstrm", path, "", res.getError());
				}
				return success("UndrlygInstrm", ValidationResult.ValidationType.CARDINALITY, "UndrlygInstrm", path, "");
			})
			.collect(toList());
	}
