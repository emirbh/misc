		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SwapCurveValuation", ValidationResult.ValidationType.CARDINALITY, "SwapCurveValuation", path, "", res.getError());
				}
				return success("SwapCurveValuation", ValidationResult.ValidationType.CARDINALITY, "SwapCurveValuation", path, "");
			})
			.collect(toList());
	}
