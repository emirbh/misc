		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralCriteriaBase", ValidationResult.ValidationType.CARDINALITY, "CollateralCriteriaBase", path, "", res.getError());
				}
				return success("CollateralCriteriaBase", ValidationResult.ValidationType.CARDINALITY, "CollateralCriteriaBase", path, "");
			})
			.collect(toList());
	}
