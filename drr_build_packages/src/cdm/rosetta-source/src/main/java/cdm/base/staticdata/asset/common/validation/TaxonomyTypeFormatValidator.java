		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Taxonomy", ValidationResult.ValidationType.TYPE_FORMAT, "Taxonomy", path, "", res.getError());
				}
				return success("Taxonomy", ValidationResult.ValidationType.TYPE_FORMAT, "Taxonomy", path, "");
			})
			.collect(toList());
	}
