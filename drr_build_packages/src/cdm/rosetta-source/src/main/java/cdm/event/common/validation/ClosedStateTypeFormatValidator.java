		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClosedState", ValidationResult.ValidationType.TYPE_FORMAT, "ClosedState", path, "", res.getError());
				}
				return success("ClosedState", ValidationResult.ValidationType.TYPE_FORMAT, "ClosedState", path, "");
			})
			.collect(toList());
	}
