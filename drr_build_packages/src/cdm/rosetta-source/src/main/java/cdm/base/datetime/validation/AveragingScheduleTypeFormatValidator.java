		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "AveragingSchedule", path, "", res.getError());
				}
				return success("AveragingSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "AveragingSchedule", path, "");
			})
			.collect(toList());
	}
