		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegionalGovernmentIssuerType", ValidationResult.ValidationType.CARDINALITY, "RegionalGovernmentIssuerType", path, "", res.getError());
				}
				return success("RegionalGovernmentIssuerType", ValidationResult.ValidationType.CARDINALITY, "RegionalGovernmentIssuerType", path, "");
			})
			.collect(toList());
	}
