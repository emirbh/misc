		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradableProduct", ValidationResult.ValidationType.CARDINALITY, "TradableProduct", path, "", res.getError());
				}
				return success("TradableProduct", ValidationResult.ValidationType.CARDINALITY, "TradableProduct", path, "");
			})
			.collect(toList());
	}
