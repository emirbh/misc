		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BillingSummary", ValidationResult.ValidationType.CARDINALITY, "BillingSummary", path, "", res.getError());
				}
				return success("BillingSummary", ValidationResult.ValidationType.CARDINALITY, "BillingSummary", path, "");
			})
			.collect(toList());
	}
