		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Payout", ValidationResult.ValidationType.CARDINALITY, "Payout", path, "", res.getError());
				}
				return success("Payout", ValidationResult.ValidationType.CARDINALITY, "Payout", path, "");
			})
			.collect(toList());
	}
