		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaymentDateSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "PaymentDateSchedule", path, "", res.getError());
				}
				return success("PaymentDateSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "PaymentDateSchedule", path, "");
			})
			.collect(toList());
	}
