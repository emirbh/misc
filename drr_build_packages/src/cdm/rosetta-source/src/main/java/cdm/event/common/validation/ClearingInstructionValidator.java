		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingInstruction", ValidationResult.ValidationType.CARDINALITY, "ClearingInstruction", path, "", res.getError());
				}
				return success("ClearingInstruction", ValidationResult.ValidationType.CARDINALITY, "ClearingInstruction", path, "");
			})
			.collect(toList());
	}
