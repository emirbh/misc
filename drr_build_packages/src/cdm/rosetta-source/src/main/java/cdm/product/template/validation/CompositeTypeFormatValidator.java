		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Composite", ValidationResult.ValidationType.TYPE_FORMAT, "Composite", path, "", res.getError());
				}
				return success("Composite", ValidationResult.ValidationType.TYPE_FORMAT, "Composite", path, "");
			})
			.collect(toList());
	}
