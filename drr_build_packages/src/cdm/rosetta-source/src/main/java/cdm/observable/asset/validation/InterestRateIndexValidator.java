		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestRateIndex", ValidationResult.ValidationType.CARDINALITY, "InterestRateIndex", path, "", res.getError());
				}
				return success("InterestRateIndex", ValidationResult.ValidationType.CARDINALITY, "InterestRateIndex", path, "");
			})
			.collect(toList());
	}
