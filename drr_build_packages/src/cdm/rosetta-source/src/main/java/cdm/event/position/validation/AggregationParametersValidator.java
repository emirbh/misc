		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AggregationParameters", ValidationResult.ValidationType.CARDINALITY, "AggregationParameters", path, "", res.getError());
				}
				return success("AggregationParameters", ValidationResult.ValidationType.CARDINALITY, "AggregationParameters", path, "");
			})
			.collect(toList());
	}
