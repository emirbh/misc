		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BusinessEvent", ValidationResult.ValidationType.CARDINALITY, "BusinessEvent", path, "", res.getError());
				}
				return success("BusinessEvent", ValidationResult.ValidationType.CARDINALITY, "BusinessEvent", path, "");
			})
			.collect(toList());
	}
