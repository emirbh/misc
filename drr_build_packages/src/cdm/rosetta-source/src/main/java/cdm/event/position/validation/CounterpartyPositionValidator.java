		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CounterpartyPosition", ValidationResult.ValidationType.CARDINALITY, "CounterpartyPosition", path, "", res.getError());
				}
				return success("CounterpartyPosition", ValidationResult.ValidationType.CARDINALITY, "CounterpartyPosition", path, "");
			})
			.collect(toList());
	}
