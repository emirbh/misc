		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AgreementTerms", ValidationResult.ValidationType.TYPE_FORMAT, "AgreementTerms", path, "", res.getError());
				}
				return success("AgreementTerms", ValidationResult.ValidationType.TYPE_FORMAT, "AgreementTerms", path, "");
			})
			.collect(toList());
	}
