		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetDeliveryProfileBlock", ValidationResult.ValidationType.CARDINALITY, "AssetDeliveryProfileBlock", path, "", res.getError());
				}
				return success("AssetDeliveryProfileBlock", ValidationResult.ValidationType.CARDINALITY, "AssetDeliveryProfileBlock", path, "");
			})
			.collect(toList());
	}
