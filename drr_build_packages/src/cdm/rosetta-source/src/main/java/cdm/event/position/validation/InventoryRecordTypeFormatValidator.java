		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InventoryRecord", ValidationResult.ValidationType.TYPE_FORMAT, "InventoryRecord", path, "", res.getError());
				}
				return success("InventoryRecord", ValidationResult.ValidationType.TYPE_FORMAT, "InventoryRecord", path, "");
			})
			.collect(toList());
	}
