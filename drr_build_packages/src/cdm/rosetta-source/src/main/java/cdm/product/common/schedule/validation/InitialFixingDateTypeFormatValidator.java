		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InitialFixingDate", ValidationResult.ValidationType.TYPE_FORMAT, "InitialFixingDate", path, "", res.getError());
				}
				return success("InitialFixingDate", ValidationResult.ValidationType.TYPE_FORMAT, "InitialFixingDate", path, "");
			})
			.collect(toList());
	}
