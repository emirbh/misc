...
public class PriceMaterialityTypeFormatValidator implements Validator<PriceMateriality> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceMateriality", ValidationResult.ValidationType.TYPE_FORMAT, "PriceMateriality", path, "", res.getError());
				}
				return success("PriceMateriality", ValidationResult.ValidationType.TYPE_FORMAT, "PriceMateriality", path, "");
			})
			.collect(toList());
	}
