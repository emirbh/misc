...
public class FloatingRateIndexCalculationDefaultsValidator implements Validator<FloatingRateIndexCalculationDefaults> {
	...
	private List<ComparisonResult> getComparisonResults(FloatingRateIndexCalculationDefaults o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("dayCountFraction", (DayCountFractionEnum) o.getDayCountFraction() != null ? 1 : 0, 0, 1), 
				checkCardinality("applicableBusinessDays", (BusinessCenters) o.getApplicableBusinessDays() != null ? 1 : 0, 0, 1), 
				checkCardinality("publicationCalendar", (BusinessCenterEnum) o.getPublicationCalendar() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateIndexCalculationDefaults", ValidationResult.ValidationType.CARDINALITY, "FloatingRateIndexCalculationDefaults", path, "", res.getError());
				}
				return success("FloatingRateIndexCalculationDefaults", ValidationResult.ValidationType.CARDINALITY, "FloatingRateIndexCalculationDefaults", path, "");
			})
			.collect(toList());
	}
