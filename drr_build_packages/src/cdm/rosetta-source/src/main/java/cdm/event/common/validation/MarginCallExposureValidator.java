		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallExposure", ValidationResult.ValidationType.CARDINALITY, "MarginCallExposure", path, "", res.getError());
				}
				return success("MarginCallExposure", ValidationResult.ValidationType.CARDINALITY, "MarginCallExposure", path, "");
			})
			.collect(toList());
	}
