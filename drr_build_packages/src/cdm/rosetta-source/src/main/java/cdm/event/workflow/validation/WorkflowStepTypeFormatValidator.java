		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WorkflowStep", ValidationResult.ValidationType.TYPE_FORMAT, "WorkflowStep", path, "", res.getError());
				}
				return success("WorkflowStep", ValidationResult.ValidationType.TYPE_FORMAT, "WorkflowStep", path, "");
			})
			.collect(toList());
	}
