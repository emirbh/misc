		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnTerms", ValidationResult.ValidationType.CARDINALITY, "ReturnTerms", path, "", res.getError());
				}
				return success("ReturnTerms", ValidationResult.ValidationType.CARDINALITY, "ReturnTerms", path, "");
			})
			.collect(toList());
	}
