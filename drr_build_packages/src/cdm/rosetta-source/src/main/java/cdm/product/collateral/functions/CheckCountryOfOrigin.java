	...
	*/
	...
	public static class CheckCountryOfOriginDefault extends CheckCountryOfOrigin {
		...
		protected Boolean assignOutput(Boolean isEqual, ISOCountryCodeEnum countryOfOrigin, EligibilityQuery query) {
			isEqual = notExists(MapperS.of(countryOfOrigin)).orNullSafe(areEqual(MapperS.of(countryOfOrigin), MapperS.of(query).<ISOCountryCodeEnum>map("getAssetCountryOfOrigin", eligibilityQuery -> eligibilityQuery.getAssetCountryOfOrigin()), CardinalityOperator.All)).get();
