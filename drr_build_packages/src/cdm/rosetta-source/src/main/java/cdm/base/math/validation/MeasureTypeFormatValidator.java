		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Measure", ValidationResult.ValidationType.TYPE_FORMAT, "Measure", path, "", res.getError());
				}
				return success("Measure", ValidationResult.ValidationType.TYPE_FORMAT, "Measure", path, "");
			})
			.collect(toList());
	}
