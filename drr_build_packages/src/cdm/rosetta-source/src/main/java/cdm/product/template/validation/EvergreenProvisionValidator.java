		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EvergreenProvision", ValidationResult.ValidationType.CARDINALITY, "EvergreenProvision", path, "", res.getError());
				}
				return success("EvergreenProvision", ValidationResult.ValidationType.CARDINALITY, "EvergreenProvision", path, "");
			})
			.collect(toList());
	}
