		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationSource", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationSource", path, "", res.getError());
				}
				return success("ValuationSource", ValidationResult.ValidationType.TYPE_FORMAT, "ValuationSource", path, "");
			})
			.collect(toList());
	}
