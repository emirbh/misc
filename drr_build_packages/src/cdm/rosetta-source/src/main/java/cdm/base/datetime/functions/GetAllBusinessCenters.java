	...
	*/
	...
	public static class GetAllBusinessCentersDefault extends GetAllBusinessCenters {
		...
		protected List<BusinessCenterEnum> assignOutput(List<BusinessCenterEnum> businessCenterEnums, BusinessCenters businessCenters) {
			businessCenterEnums.addAll(MapperS.of(businessCenters).<FieldWithMetaBusinessCenterEnum>mapC("getBusinessCenter", _businessCenters -> _businessCenters.getBusinessCenter()).<BusinessCenterEnum>map("Type coercion", fieldWithMetaBusinessCenterEnum -> fieldWithMetaBusinessCenterEnum.getValue()).getMulti());
			
			if (exists(MapperS.of(businessCenters).<ReferenceWithMetaBusinessCenters>map("getBusinessCentersReference", _businessCenters -> _businessCenters.getBusinessCentersReference())).getOrDefault(false)) {
				final ReferenceWithMetaBusinessCenters referenceWithMetaBusinessCenters = MapperS.of(businessCenters).<ReferenceWithMetaBusinessCenters>map("getBusinessCentersReference", _businessCenters -> _businessCenters.getBusinessCentersReference()).get();
				businessCenterEnums.addAll(getAllBusinessCenters.evaluate((referenceWithMetaBusinessCenters == null ? null : referenceWithMetaBusinessCenters.getValue())));
			} else {
