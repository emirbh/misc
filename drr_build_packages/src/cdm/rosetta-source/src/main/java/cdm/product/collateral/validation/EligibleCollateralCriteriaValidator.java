		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EligibleCollateralCriteria", ValidationResult.ValidationType.CARDINALITY, "EligibleCollateralCriteria", path, "", res.getError());
				}
				return success("EligibleCollateralCriteria", ValidationResult.ValidationType.CARDINALITY, "EligibleCollateralCriteria", path, "");
			})
			.collect(toList());
	}
