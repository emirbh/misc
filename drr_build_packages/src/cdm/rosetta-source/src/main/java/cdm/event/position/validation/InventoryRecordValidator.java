		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InventoryRecord", ValidationResult.ValidationType.CARDINALITY, "InventoryRecord", path, "", res.getError());
				}
				return success("InventoryRecord", ValidationResult.ValidationType.CARDINALITY, "InventoryRecord", path, "");
			})
			.collect(toList());
	}
