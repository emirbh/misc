		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceQuantity", ValidationResult.ValidationType.TYPE_FORMAT, "PriceQuantity", path, "", res.getError());
				}
				return success("PriceQuantity", ValidationResult.ValidationType.TYPE_FORMAT, "PriceQuantity", path, "");
			})
			.collect(toList());
	}
