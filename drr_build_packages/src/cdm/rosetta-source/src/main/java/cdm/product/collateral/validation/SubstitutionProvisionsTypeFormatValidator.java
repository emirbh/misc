		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SubstitutionProvisions", ValidationResult.ValidationType.TYPE_FORMAT, "SubstitutionProvisions", path, "", res.getError());
				}
				return success("SubstitutionProvisions", ValidationResult.ValidationType.TYPE_FORMAT, "SubstitutionProvisions", path, "");
			})
			.collect(toList());
	}
