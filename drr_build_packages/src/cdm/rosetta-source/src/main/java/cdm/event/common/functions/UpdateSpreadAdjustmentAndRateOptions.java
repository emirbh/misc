	...
	*/
	...
	public static class UpdateSpreadAdjustmentAndRateOptionsDefault extends UpdateSpreadAdjustmentAndRateOptions {
		...
		protected TradeState.TradeStateBuilder assignOutput(TradeState.TradeStateBuilder updatedTradeState, TradeState tradeState, List<? extends PriceQuantity> instructions) {
			updatedTradeState = toBuilder(tradeState, () -> TradeState.builder());
			
			updatedTradeState
				.getOrCreateTrade()
				.getOrCreateTradeLot(0)
				.setPriceQuantity(MapperS.of(MapperS.of(tradeState).<Trade>map("getTrade", _tradeState -> _tradeState.getTrade()).<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get()).<PriceQuantity>mapC("getPriceQuantity", tradeLot -> tradeLot.getPriceQuantity())
