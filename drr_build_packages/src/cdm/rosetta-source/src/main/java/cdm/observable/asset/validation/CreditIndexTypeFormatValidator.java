		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditIndex", ValidationResult.ValidationType.TYPE_FORMAT, "CreditIndex", path, "", res.getError());
				}
				return success("CreditIndex", ValidationResult.ValidationType.TYPE_FORMAT, "CreditIndex", path, "");
			})
			.collect(toList());
	}
