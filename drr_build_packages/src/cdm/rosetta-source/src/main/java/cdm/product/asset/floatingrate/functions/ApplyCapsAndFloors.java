	...
	*/
	...
	public static class ApplyCapsAndFloorsDefault extends ApplyCapsAndFloors {
		...
		protected MapperS<BigDecimal> cappedRate(FloatingRateProcessingParameters processing, BigDecimal inputRate) {
			if (exists(cap(processing, inputRate)).andNullSafe(greaterThan(MapperS.of(inputRate), cap(processing, inputRate), CardinalityOperator.All)).getOrDefault(false)) {
		...
		protected MapperS<BigDecimal> flooredRate(FloatingRateProcessingParameters processing, BigDecimal inputRate) {
			if (exists(floor(processing, inputRate)).andNullSafe(lessThan(cappedRate(processing, inputRate), floor(processing, inputRate), CardinalityOperator.All)).getOrDefault(false)) {
