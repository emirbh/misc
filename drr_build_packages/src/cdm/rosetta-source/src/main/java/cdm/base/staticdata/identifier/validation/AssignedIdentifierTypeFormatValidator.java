		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssignedIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "AssignedIdentifier", path, "", res.getError());
				}
				return success("AssignedIdentifier", ValidationResult.ValidationType.TYPE_FORMAT, "AssignedIdentifier", path, "");
			})
			.collect(toList());
	}
