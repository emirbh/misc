		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AddressForNotices", ValidationResult.ValidationType.CARDINALITY, "AddressForNotices", path, "", res.getError());
				}
				return success("AddressForNotices", ValidationResult.ValidationType.CARDINALITY, "AddressForNotices", path, "");
			})
			.collect(toList());
	}
