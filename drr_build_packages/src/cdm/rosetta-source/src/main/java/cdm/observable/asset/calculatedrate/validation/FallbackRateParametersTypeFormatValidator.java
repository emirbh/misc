		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FallbackRateParameters", ValidationResult.ValidationType.TYPE_FORMAT, "FallbackRateParameters", path, "", res.getError());
				}
				return success("FallbackRateParameters", ValidationResult.ValidationType.TYPE_FORMAT, "FallbackRateParameters", path, "");
			})
			.collect(toList());
	}
