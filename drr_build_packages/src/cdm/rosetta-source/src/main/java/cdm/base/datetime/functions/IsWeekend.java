	...
	*/
	...
	public static class IsWeekendDefault extends IsWeekend {
		...
		protected Boolean assignOutput(Boolean isWeekend, Date date, List<BusinessCenterEnum> businessCenters) {
			isWeekend = areEqual(dayOfWeek1(date, businessCenters), MapperS.of(DayOfWeekEnum.SAT), CardinalityOperator.All).orNullSafe(areEqual(dayOfWeek1(date, businessCenters), MapperS.of(DayOfWeekEnum.SUN), CardinalityOperator.All)).get();
