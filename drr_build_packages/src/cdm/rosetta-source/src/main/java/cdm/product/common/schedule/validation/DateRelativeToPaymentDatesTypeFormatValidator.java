		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateRelativeToPaymentDates", ValidationResult.ValidationType.TYPE_FORMAT, "DateRelativeToPaymentDates", path, "", res.getError());
				}
				return success("DateRelativeToPaymentDates", ValidationResult.ValidationType.TYPE_FORMAT, "DateRelativeToPaymentDates", path, "");
			})
			.collect(toList());
	}
