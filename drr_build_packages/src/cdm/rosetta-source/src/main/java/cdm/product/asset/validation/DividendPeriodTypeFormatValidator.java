		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DividendPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "DividendPeriod", path, "", res.getError());
				}
				return success("DividendPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "DividendPeriod", path, "");
			})
			.collect(toList());
	}
