		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ListingSector", ValidationResult.ValidationType.TYPE_FORMAT, "ListingSector", path, "", res.getError());
				}
				return success("ListingSector", ValidationResult.ValidationType.TYPE_FORMAT, "ListingSector", path, "");
			})
			.collect(toList());
	}
