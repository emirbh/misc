		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OneWayProvisions", ValidationResult.ValidationType.CARDINALITY, "OneWayProvisions", path, "", res.getError());
				}
				return success("OneWayProvisions", ValidationResult.ValidationType.CARDINALITY, "OneWayProvisions", path, "");
			})
			.collect(toList());
	}
