		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PerformanceValuationDates", ValidationResult.ValidationType.CARDINALITY, "PerformanceValuationDates", path, "", res.getError());
				}
				return success("PerformanceValuationDates", ValidationResult.ValidationType.CARDINALITY, "PerformanceValuationDates", path, "");
			})
			.collect(toList());
	}
