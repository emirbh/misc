		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObservationShiftCalculation", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationShiftCalculation", path, "", res.getError());
				}
				return success("ObservationShiftCalculation", ValidationResult.ValidationType.TYPE_FORMAT, "ObservationShiftCalculation", path, "");
			})
			.collect(toList());
	}
