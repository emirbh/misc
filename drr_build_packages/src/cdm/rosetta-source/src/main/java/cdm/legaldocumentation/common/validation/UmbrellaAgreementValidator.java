		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UmbrellaAgreement", ValidationResult.ValidationType.CARDINALITY, "UmbrellaAgreement", path, "", res.getError());
				}
				return success("UmbrellaAgreement", ValidationResult.ValidationType.CARDINALITY, "UmbrellaAgreement", path, "");
			})
			.collect(toList());
	}
