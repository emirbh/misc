	...
	*/
	...
	public static class CheckIssuerNameDefault extends CheckIssuerName {
		...
		protected Boolean assignOutput(Boolean isEqual, IssuerName issuerName, EligibilityQuery query) {
			isEqual = notExists(MapperS.of(issuerName)).orNullSafe(areEqual(MapperS.of(issuerName).<LegalEntity>map("getIssuerName", _issuerName -> _issuerName.getIssuerName()), MapperS.of(query).<LegalEntity>map("getIssuerName", eligibilityQuery -> eligibilityQuery.getIssuerName()), CardinalityOperator.All)).get();
