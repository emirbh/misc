...
public class SimpleFraValidator implements Validator<SimpleFra> {
	...
	private List<ComparisonResult> getComparisonResults(SimpleFra o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("startTerm", (Period) o.getStartTerm() != null ? 1 : 0, 0, 1), 
				checkCardinality("endTerm", (Period) o.getEndTerm() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SimpleFra", ValidationResult.ValidationType.CARDINALITY, "SimpleFra", path, "", res.getError());
				}
				return success("SimpleFra", ValidationResult.ValidationType.CARDINALITY, "SimpleFra", path, "");
			})
			.collect(toList());
	}
