		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingInputReplacement", ValidationResult.ValidationType.TYPE_FORMAT, "PricingInputReplacement", path, "", res.getError());
				}
				return success("PricingInputReplacement", ValidationResult.ValidationType.TYPE_FORMAT, "PricingInputReplacement", path, "");
			})
			.collect(toList());
	}
