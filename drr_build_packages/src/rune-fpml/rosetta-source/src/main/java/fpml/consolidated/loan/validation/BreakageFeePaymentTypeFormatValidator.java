		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BreakageFeePayment", ValidationResult.ValidationType.TYPE_FORMAT, "BreakageFeePayment", path, "", res.getError());
				}
				return success("BreakageFeePayment", ValidationResult.ValidationType.TYPE_FORMAT, "BreakageFeePayment", path, "");
			})
			.collect(toList());
	}
