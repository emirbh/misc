		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEvents", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEvents", path, "", res.getError());
				}
				return success("CreditEvents", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEvents", path, "");
			})
			.collect(toList());
	}
