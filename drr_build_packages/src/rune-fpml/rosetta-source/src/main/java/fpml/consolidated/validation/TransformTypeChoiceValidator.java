		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TransformTypeChoice", ValidationResult.ValidationType.CARDINALITY, "TransformTypeChoice", path, "", res.getError());
				}
				return success("TransformTypeChoice", ValidationResult.ValidationType.CARDINALITY, "TransformTypeChoice", path, "");
			})
			.collect(toList());
	}
