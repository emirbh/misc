		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedRateOption", ValidationResult.ValidationType.CARDINALITY, "FixedRateOption", path, "", res.getError());
				}
				return success("FixedRateOption", ValidationResult.ValidationType.CARDINALITY, "FixedRateOption", path, "");
			})
			.collect(toList());
	}
