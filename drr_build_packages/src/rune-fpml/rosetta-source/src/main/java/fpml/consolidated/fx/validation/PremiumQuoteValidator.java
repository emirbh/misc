		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PremiumQuote", ValidationResult.ValidationType.CARDINALITY, "PremiumQuote", path, "", res.getError());
				}
				return success("PremiumQuote", ValidationResult.ValidationType.CARDINALITY, "PremiumQuote", path, "");
			})
			.collect(toList());
	}
