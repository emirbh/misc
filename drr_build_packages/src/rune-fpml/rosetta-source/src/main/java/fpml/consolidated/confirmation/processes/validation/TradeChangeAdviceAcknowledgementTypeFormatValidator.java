...
public class TradeChangeAdviceAcknowledgementTypeFormatValidator implements Validator<TradeChangeAdviceAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdviceAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdviceAcknowledgement", path, "", res.getError());
				}
				return success("TradeChangeAdviceAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdviceAcknowledgement", path, "");
			})
			.collect(toList());
	}
