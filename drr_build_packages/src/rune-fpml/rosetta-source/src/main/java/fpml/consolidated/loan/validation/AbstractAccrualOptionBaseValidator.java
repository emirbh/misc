...
public class AbstractAccrualOptionBaseValidator implements Validator<AbstractAccrualOptionBase> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractAccrualOptionBase", ValidationResult.ValidationType.CARDINALITY, "AbstractAccrualOptionBase", path, "", res.getError());
				}
				return success("AbstractAccrualOptionBase", ValidationResult.ValidationType.CARDINALITY, "AbstractAccrualOptionBase", path, "");
			})
			.collect(toList());
	}
