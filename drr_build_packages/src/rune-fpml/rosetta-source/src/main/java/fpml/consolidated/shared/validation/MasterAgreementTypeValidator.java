		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementType", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementType", path, "", res.getError());
				}
				return success("MasterAgreementType", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementType", path, "");
			})
			.collect(toList());
	}
