 ...
 * @version 2.1.1
 */
@RosettaDataRule("FxFixingScheduleChoice4")
@ImplementedBy(FxFixingScheduleChoice4.Default.class)
public interface FxFixingScheduleChoice4 extends Validator<FxFixingSchedule> {
	
	String NAME = "FxFixingScheduleChoice4";
	String DEFINITION = "if businessCenters exists then dayType exists and (startDate exists or endDate exists) and optional choice fixingDate, numberOfFixings";
	
	class Default implements FxFixingScheduleChoice4 {
		...
		private ComparisonResult executeDataRule(FxFixingSchedule fxFixingSchedule) {
			try {
				if (exists(MapperS.of(fxFixingSchedule).<BusinessCenters>map("getBusinessCenters", _fxFixingSchedule -> _fxFixingSchedule.getBusinessCenters())).getOrDefault(false)) {
	...
	class NoOp implements FxFixingScheduleChoice4 {
