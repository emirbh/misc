...
public class LcTerminationValidator implements Validator<LcTermination> {
	...
	private List<ComparisonResult> getComparisonResults(LcTermination o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("expiry", (Boolean) o.getExpiry() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcTermination", ValidationResult.ValidationType.CARDINALITY, "LcTermination", path, "", res.getError());
				}
				return success("LcTermination", ValidationResult.ValidationType.CARDINALITY, "LcTermination", path, "");
			})
			.collect(toList());
	}
