...
public interface FxKnockoutLevelChoice extends Validator<FxKnockoutLevel> {
	...
	class Default implements FxKnockoutLevelChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, FxKnockoutLevel fxKnockoutLevel) {
			ComparisonResult result = executeDataRule(fxKnockoutLevel);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "FxKnockoutLevel", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "FxKnockoutLevel", path, DEFINITION, failureMessage));
		}
