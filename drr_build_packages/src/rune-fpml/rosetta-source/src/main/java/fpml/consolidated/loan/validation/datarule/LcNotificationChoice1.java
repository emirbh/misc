 ...
 * @version 2.1.1
 */
@RosettaDataRule("LcNotificationChoice1")
@ImplementedBy(LcNotificationChoice1.Default.class)
public interface LcNotificationChoice1 extends Validator<LcNotification> {
	
	String NAME = "LcNotificationChoice1";
	String DEFINITION = "required choice facilityIdentifier, facilitySummary";
	
	class Default implements LcNotificationChoice1 {
		...
		private ComparisonResult executeDataRule(LcNotification lcNotification) {
			try {
				return choice(MapperS.of(lcNotification), Arrays.asList("facilityIdentifier", "facilitySummary"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements LcNotificationChoice1 {
