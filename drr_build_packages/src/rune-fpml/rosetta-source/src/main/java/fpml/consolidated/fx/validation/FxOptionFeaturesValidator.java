...
public class FxOptionFeaturesValidator implements Validator<FxOptionFeatures> {
	...
	private List<ComparisonResult> getComparisonResults(FxOptionFeatures o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("asian", (FxAsianFeature) o.getAsian() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionFeatures", ValidationResult.ValidationType.CARDINALITY, "FxOptionFeatures", path, "", res.getError());
				}
				return success("FxOptionFeatures", ValidationResult.ValidationType.CARDINALITY, "FxOptionFeatures", path, "");
			})
			.collect(toList());
	}
