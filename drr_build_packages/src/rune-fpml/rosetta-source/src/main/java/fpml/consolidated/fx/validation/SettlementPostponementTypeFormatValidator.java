		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPostponement", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPostponement", path, "", res.getError());
				}
				return success("SettlementPostponement", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPostponement", path, "");
			})
			.collect(toList());
	}
