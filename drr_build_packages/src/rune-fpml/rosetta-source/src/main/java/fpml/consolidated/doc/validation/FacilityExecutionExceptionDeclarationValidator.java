		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityExecutionExceptionDeclaration", ValidationResult.ValidationType.CARDINALITY, "FacilityExecutionExceptionDeclaration", path, "", res.getError());
				}
				return success("FacilityExecutionExceptionDeclaration", ValidationResult.ValidationType.CARDINALITY, "FacilityExecutionExceptionDeclaration", path, "");
			})
			.collect(toList());
	}
