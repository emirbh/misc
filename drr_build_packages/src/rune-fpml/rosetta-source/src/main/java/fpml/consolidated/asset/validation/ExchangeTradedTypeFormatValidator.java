		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeTraded", ValidationResult.ValidationType.TYPE_FORMAT, "ExchangeTraded", path, "", res.getError());
				}
				return success("ExchangeTraded", ValidationResult.ValidationType.TYPE_FORMAT, "ExchangeTraded", path, "");
			})
			.collect(toList());
	}
