		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureMethodType", ValidationResult.ValidationType.CARDINALITY, "SignatureMethodType", path, "", res.getError());
				}
				return success("SignatureMethodType", ValidationResult.ValidationType.CARDINALITY, "SignatureMethodType", path, "");
			})
			.collect(toList());
	}
