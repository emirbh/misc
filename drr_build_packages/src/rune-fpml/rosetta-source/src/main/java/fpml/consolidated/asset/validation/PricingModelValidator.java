		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingModel", ValidationResult.ValidationType.CARDINALITY, "PricingModel", path, "", res.getError());
				}
				return success("PricingModel", ValidationResult.ValidationType.CARDINALITY, "PricingModel", path, "");
			})
			.collect(toList());
	}
