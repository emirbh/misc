		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentTradePrincipal", ValidationResult.ValidationType.TYPE_FORMAT, "InstrumentTradePrincipal", path, "", res.getError());
				}
				return success("InstrumentTradePrincipal", ValidationResult.ValidationType.TYPE_FORMAT, "InstrumentTradePrincipal", path, "");
			})
			.collect(toList());
	}
