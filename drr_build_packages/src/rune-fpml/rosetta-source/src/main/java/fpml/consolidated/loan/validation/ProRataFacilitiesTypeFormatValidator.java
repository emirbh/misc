		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ProRataFacilities", ValidationResult.ValidationType.TYPE_FORMAT, "ProRataFacilities", path, "", res.getError());
				}
				return success("ProRataFacilities", ValidationResult.ValidationType.TYPE_FORMAT, "ProRataFacilities", path, "");
			})
			.collect(toList());
	}
