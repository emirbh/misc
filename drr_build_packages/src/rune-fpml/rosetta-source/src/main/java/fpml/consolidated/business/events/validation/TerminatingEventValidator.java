		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TerminatingEvent", ValidationResult.ValidationType.CARDINALITY, "TerminatingEvent", path, "", res.getError());
				}
				return success("TerminatingEvent", ValidationResult.ValidationType.CARDINALITY, "TerminatingEvent", path, "");
			})
			.collect(toList());
	}
