		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyTradeIdentifier", ValidationResult.ValidationType.CARDINALITY, "PartyTradeIdentifier", path, "", res.getError());
				}
				return success("PartyTradeIdentifier", ValidationResult.ValidationType.CARDINALITY, "PartyTradeIdentifier", path, "");
			})
			.collect(toList());
	}
