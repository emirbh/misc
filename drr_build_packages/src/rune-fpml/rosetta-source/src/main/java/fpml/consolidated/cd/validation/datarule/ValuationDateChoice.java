 ...
 * @version 2.1.1
 */
@RosettaDataRule("ValuationDateChoice")
@ImplementedBy(ValuationDateChoice.Default.class)
public interface ValuationDateChoice extends Validator<ValuationDate> {
	
	String NAME = "ValuationDateChoice";
	String DEFINITION = "one-of";
	
	class Default implements ValuationDateChoice {
		...
		private ComparisonResult executeDataRule(ValuationDate valuationDate) {
			try {
				return choice(MapperS.of(valuationDate), Arrays.asList("singleValuationDate", "multipleValuationDates"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements ValuationDateChoice {
