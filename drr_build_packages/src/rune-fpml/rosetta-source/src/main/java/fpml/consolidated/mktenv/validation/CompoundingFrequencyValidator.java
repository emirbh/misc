		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CompoundingFrequency", ValidationResult.ValidationType.CARDINALITY, "CompoundingFrequency", path, "", res.getError());
				}
				return success("CompoundingFrequency", ValidationResult.ValidationType.CARDINALITY, "CompoundingFrequency", path, "");
			})
			.collect(toList());
	}
