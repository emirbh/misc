		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BullionPhysicalLeg", ValidationResult.ValidationType.TYPE_FORMAT, "BullionPhysicalLeg", path, "", res.getError());
				}
				return success("BullionPhysicalLeg", ValidationResult.ValidationType.TYPE_FORMAT, "BullionPhysicalLeg", path, "");
			})
			.collect(toList());
	}
