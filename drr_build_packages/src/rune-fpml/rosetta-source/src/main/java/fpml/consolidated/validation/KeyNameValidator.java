		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("KeyName", ValidationResult.ValidationType.CARDINALITY, "KeyName", path, "", res.getError());
				}
				return success("KeyName", ValidationResult.ValidationType.CARDINALITY, "KeyName", path, "");
			})
			.collect(toList());
	}
