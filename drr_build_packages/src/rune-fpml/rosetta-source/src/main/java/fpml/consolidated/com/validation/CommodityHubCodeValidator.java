		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityHubCode", ValidationResult.ValidationType.CARDINALITY, "CommodityHubCode", path, "", res.getError());
				}
				return success("CommodityHubCode", ValidationResult.ValidationType.CARDINALITY, "CommodityHubCode", path, "");
			})
			.collect(toList());
	}
