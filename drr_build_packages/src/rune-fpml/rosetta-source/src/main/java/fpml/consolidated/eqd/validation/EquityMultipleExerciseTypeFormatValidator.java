		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityMultipleExercise", ValidationResult.ValidationType.TYPE_FORMAT, "EquityMultipleExercise", path, "", res.getError());
				}
				return success("EquityMultipleExercise", ValidationResult.ValidationType.TYPE_FORMAT, "EquityMultipleExercise", path, "");
			})
			.collect(toList());
	}
