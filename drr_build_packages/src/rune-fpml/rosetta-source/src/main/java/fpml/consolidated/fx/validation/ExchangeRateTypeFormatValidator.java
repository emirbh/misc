		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeRate", ValidationResult.ValidationType.TYPE_FORMAT, "ExchangeRate", path, "", res.getError());
				}
				return success("ExchangeRate", ValidationResult.ValidationType.TYPE_FORMAT, "ExchangeRate", path, "");
			})
			.collect(toList());
	}
