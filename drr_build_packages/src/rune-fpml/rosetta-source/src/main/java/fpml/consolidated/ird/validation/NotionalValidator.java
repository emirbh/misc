		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Notional", ValidationResult.ValidationType.CARDINALITY, "Notional", path, "", res.getError());
				}
				return success("Notional", ValidationResult.ValidationType.CARDINALITY, "Notional", path, "");
			})
			.collect(toList());
	}
