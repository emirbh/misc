		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityExerciseValuationSettlement", ValidationResult.ValidationType.TYPE_FORMAT, "EquityExerciseValuationSettlement", path, "", res.getError());
				}
				return success("EquityExerciseValuationSettlement", ValidationResult.ValidationType.TYPE_FORMAT, "EquityExerciseValuationSettlement", path, "");
			})
			.collect(toList());
	}
