...
public class RequestEventStatusTypeFormatValidator implements Validator<RequestEventStatus> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestEventStatus", ValidationResult.ValidationType.TYPE_FORMAT, "RequestEventStatus", path, "", res.getError());
				}
				return success("RequestEventStatus", ValidationResult.ValidationType.TYPE_FORMAT, "RequestEventStatus", path, "");
			})
			.collect(toList());
	}
