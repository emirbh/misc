		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityEvent", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractFacilityEvent", path, "", res.getError());
				}
				return success("AbstractFacilityEvent", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractFacilityEvent", path, "");
			})
			.collect(toList());
	}
