...
public class AcknowledgementTypeFormatValidator implements Validator<Acknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Acknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "Acknowledgement", path, "", res.getError());
				}
				return success("Acknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "Acknowledgement", path, "");
			})
			.collect(toList());
	}
