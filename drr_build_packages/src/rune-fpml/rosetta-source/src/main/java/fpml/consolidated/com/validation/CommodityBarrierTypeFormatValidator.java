		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBarrier", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBarrier", path, "", res.getError());
				}
				return success("CommodityBarrier", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBarrier", path, "");
			})
			.collect(toList());
	}
