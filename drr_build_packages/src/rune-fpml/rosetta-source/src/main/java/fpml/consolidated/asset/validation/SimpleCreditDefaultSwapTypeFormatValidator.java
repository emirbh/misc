		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SimpleCreditDefaultSwap", ValidationResult.ValidationType.TYPE_FORMAT, "SimpleCreditDefaultSwap", path, "", res.getError());
				}
				return success("SimpleCreditDefaultSwap", ValidationResult.ValidationType.TYPE_FORMAT, "SimpleCreditDefaultSwap", path, "");
			})
			.collect(toList());
	}
