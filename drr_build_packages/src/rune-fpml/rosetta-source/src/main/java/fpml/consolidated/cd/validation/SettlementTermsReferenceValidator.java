		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementTermsReference", ValidationResult.ValidationType.CARDINALITY, "SettlementTermsReference", path, "", res.getError());
				}
				return success("SettlementTermsReference", ValidationResult.ValidationType.CARDINALITY, "SettlementTermsReference", path, "");
			})
			.collect(toList());
	}
