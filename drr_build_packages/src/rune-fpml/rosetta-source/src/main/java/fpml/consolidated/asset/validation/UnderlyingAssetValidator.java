...
public class UnderlyingAssetValidator implements Validator<UnderlyingAsset> {

	private List<ComparisonResult> getComparisonResults(UnderlyingAsset o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, UnderlyingAsset o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UnderlyingAsset", ValidationResult.ValidationType.CARDINALITY, "UnderlyingAsset", path, "", res.getError());
				}
				return success("UnderlyingAsset", ValidationResult.ValidationType.CARDINALITY, "UnderlyingAsset", path, "");
			})
			.collect(toList());
	}
