		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateDefinition", ValidationResult.ValidationType.CARDINALITY, "FloatingRateDefinition", path, "", res.getError());
				}
				return success("FloatingRateDefinition", ValidationResult.ValidationType.CARDINALITY, "FloatingRateDefinition", path, "");
			})
			.collect(toList());
	}
