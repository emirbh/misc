		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdvice", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionAdvice", path, "", res.getError());
				}
				return success("ExecutionAdvice", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionAdvice", path, "");
			})
			.collect(toList());
	}
