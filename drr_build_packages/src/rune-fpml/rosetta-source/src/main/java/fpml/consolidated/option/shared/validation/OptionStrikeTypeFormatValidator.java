		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionStrike", ValidationResult.ValidationType.TYPE_FORMAT, "OptionStrike", path, "", res.getError());
				}
				return success("OptionStrike", ValidationResult.ValidationType.TYPE_FORMAT, "OptionStrike", path, "");
			})
			.collect(toList());
	}
