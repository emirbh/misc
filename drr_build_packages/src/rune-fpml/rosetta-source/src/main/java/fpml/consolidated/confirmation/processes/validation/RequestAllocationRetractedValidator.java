		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestAllocationRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestAllocationRetracted", path, "", res.getError());
				}
				return success("RequestAllocationRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestAllocationRetracted", path, "");
			})
			.collect(toList());
	}
