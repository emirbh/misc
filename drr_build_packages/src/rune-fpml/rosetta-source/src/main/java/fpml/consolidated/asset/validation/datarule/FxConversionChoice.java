...
public interface FxConversionChoice extends Validator<FxConversion> {
	...
	class Default implements FxConversionChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, FxConversion fxConversion) {
			ComparisonResult result = executeDataRule(fxConversion);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "FxConversion", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "FxConversion", path, DEFINITION, failureMessage));
		}
