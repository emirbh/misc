...
public class CorrectableRequestMessageValidator implements Validator<CorrectableRequestMessage> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrectableRequestMessage", ValidationResult.ValidationType.CARDINALITY, "CorrectableRequestMessage", path, "", res.getError());
				}
				return success("CorrectableRequestMessage", ValidationResult.ValidationType.CARDINALITY, "CorrectableRequestMessage", path, "");
			})
			.collect(toList());
	}
