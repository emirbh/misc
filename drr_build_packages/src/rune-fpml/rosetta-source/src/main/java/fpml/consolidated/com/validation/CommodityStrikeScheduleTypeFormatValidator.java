		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityStrikeSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityStrikeSchedule", path, "", res.getError());
				}
				return success("CommodityStrikeSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityStrikeSchedule", path, "");
			})
			.collect(toList());
	}
