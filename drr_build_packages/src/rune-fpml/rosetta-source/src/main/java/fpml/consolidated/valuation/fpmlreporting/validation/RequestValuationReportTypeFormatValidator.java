...
public class RequestValuationReportTypeFormatValidator implements Validator<RequestValuationReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "RequestValuationReport", path, "", res.getError());
				}
				return success("RequestValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "RequestValuationReport", path, "");
			})
			.collect(toList());
	}
