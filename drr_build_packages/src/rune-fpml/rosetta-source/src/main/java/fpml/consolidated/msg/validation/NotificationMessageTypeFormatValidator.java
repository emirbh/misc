		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotificationMessage", ValidationResult.ValidationType.TYPE_FORMAT, "NotificationMessage", path, "", res.getError());
				}
				return success("NotificationMessage", ValidationResult.ValidationType.TYPE_FORMAT, "NotificationMessage", path, "");
			})
			.collect(toList());
	}
