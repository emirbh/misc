		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MaturityException", ValidationResult.ValidationType.CARDINALITY, "MaturityException", path, "", res.getError());
				}
				return success("MaturityException", ValidationResult.ValidationType.CARDINALITY, "MaturityException", path, "");
			})
			.collect(toList());
	}
