		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BaseRateSet", ValidationResult.ValidationType.TYPE_FORMAT, "BaseRateSet", path, "", res.getError());
				}
				return success("BaseRateSet", ValidationResult.ValidationType.TYPE_FORMAT, "BaseRateSet", path, "");
			})
			.collect(toList());
	}
