		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ServiceProcessingEvent", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceProcessingEvent", path, "", res.getError());
				}
				return success("ServiceProcessingEvent", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceProcessingEvent", path, "");
			})
			.collect(toList());
	}
