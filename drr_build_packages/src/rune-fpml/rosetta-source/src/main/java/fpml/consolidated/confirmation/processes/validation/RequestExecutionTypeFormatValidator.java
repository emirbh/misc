		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestExecution", ValidationResult.ValidationType.TYPE_FORMAT, "RequestExecution", path, "", res.getError());
				}
				return success("RequestExecution", ValidationResult.ValidationType.TYPE_FORMAT, "RequestExecution", path, "");
			})
			.collect(toList());
	}
