		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractServicingNotification", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractServicingNotification", path, "", res.getError());
				}
				return success("AbstractServicingNotification", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractServicingNotification", path, "");
			})
			.collect(toList());
	}
