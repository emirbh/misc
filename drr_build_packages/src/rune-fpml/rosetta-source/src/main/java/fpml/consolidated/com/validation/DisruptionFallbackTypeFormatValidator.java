		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DisruptionFallback", ValidationResult.ValidationType.TYPE_FORMAT, "DisruptionFallback", path, "", res.getError());
				}
				return success("DisruptionFallback", ValidationResult.ValidationType.TYPE_FORMAT, "DisruptionFallback", path, "");
			})
			.collect(toList());
	}
