		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("GenericProductFeature", ValidationResult.ValidationType.TYPE_FORMAT, "GenericProductFeature", path, "", res.getError());
				}
				return success("GenericProductFeature", ValidationResult.ValidationType.TYPE_FORMAT, "GenericProductFeature", path, "");
			})
			.collect(toList());
	}
