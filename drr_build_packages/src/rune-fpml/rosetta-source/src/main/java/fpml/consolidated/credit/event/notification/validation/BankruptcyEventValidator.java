		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BankruptcyEvent", ValidationResult.ValidationType.CARDINALITY, "BankruptcyEvent", path, "", res.getError());
				}
				return success("BankruptcyEvent", ValidationResult.ValidationType.CARDINALITY, "BankruptcyEvent", path, "");
			})
			.collect(toList());
	}
