		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdviceAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "TradeChangeAdviceAcknowledgement", path, "", res.getError());
				}
				return success("TradeChangeAdviceAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "TradeChangeAdviceAcknowledgement", path, "");
			})
			.collect(toList());
	}
