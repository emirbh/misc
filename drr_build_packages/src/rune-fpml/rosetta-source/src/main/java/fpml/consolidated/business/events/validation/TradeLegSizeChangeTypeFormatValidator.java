		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeLegSizeChange", ValidationResult.ValidationType.TYPE_FORMAT, "TradeLegSizeChange", path, "", res.getError());
				}
				return success("TradeLegSizeChange", ValidationResult.ValidationType.TYPE_FORMAT, "TradeLegSizeChange", path, "");
			})
			.collect(toList());
	}
