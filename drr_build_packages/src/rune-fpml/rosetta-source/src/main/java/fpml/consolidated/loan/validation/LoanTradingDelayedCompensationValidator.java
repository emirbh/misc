...
public class LoanTradingDelayedCompensationValidator implements Validator<LoanTradingDelayedCompensation> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradingDelayedCompensation", ValidationResult.ValidationType.CARDINALITY, "LoanTradingDelayedCompensation", path, "", res.getError());
				}
				return success("LoanTradingDelayedCompensation", ValidationResult.ValidationType.CARDINALITY, "LoanTradingDelayedCompensation", path, "");
			})
			.collect(toList());
	}
