		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeEvent", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeEvent", path, "", res.getError());
				}
				return success("LoanTradeEvent", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeEvent", path, "");
			})
			.collect(toList());
	}
