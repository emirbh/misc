		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BullionDeliveryLocation", ValidationResult.ValidationType.CARDINALITY, "BullionDeliveryLocation", path, "", res.getError());
				}
				return success("BullionDeliveryLocation", ValidationResult.ValidationType.CARDINALITY, "BullionDeliveryLocation", path, "");
			})
			.collect(toList());
	}
