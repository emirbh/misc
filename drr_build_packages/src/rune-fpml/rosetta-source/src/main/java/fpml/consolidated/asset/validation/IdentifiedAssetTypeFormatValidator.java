...
public class IdentifiedAssetTypeFormatValidator implements Validator<IdentifiedAsset> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedAsset", ValidationResult.ValidationType.TYPE_FORMAT, "IdentifiedAsset", path, "", res.getError());
				}
				return success("IdentifiedAsset", ValidationResult.ValidationType.TYPE_FORMAT, "IdentifiedAsset", path, "");
			})
			.collect(toList());
	}
