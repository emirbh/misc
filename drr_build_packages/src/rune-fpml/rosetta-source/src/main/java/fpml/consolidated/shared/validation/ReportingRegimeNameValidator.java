		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingRegimeName", ValidationResult.ValidationType.CARDINALITY, "ReportingRegimeName", path, "", res.getError());
				}
				return success("ReportingRegimeName", ValidationResult.ValidationType.CARDINALITY, "ReportingRegimeName", path, "");
			})
			.collect(toList());
	}
