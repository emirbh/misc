		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Warrant", ValidationResult.ValidationType.CARDINALITY, "Warrant", path, "", res.getError());
				}
				return success("Warrant", ValidationResult.ValidationType.CARDINALITY, "Warrant", path, "");
			})
			.collect(toList());
	}
