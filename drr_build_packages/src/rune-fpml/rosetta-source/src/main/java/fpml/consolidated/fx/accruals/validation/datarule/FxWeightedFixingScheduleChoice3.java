 ...
 * @version 2.1.1
 */
@RosettaDataRule("FxWeightedFixingScheduleChoice3")
@ImplementedBy(FxWeightedFixingScheduleChoice3.Default.class)
public interface FxWeightedFixingScheduleChoice3 extends Validator<FxWeightedFixingSchedule> {
	
	String NAME = "FxWeightedFixingScheduleChoice3";
	String DEFINITION = "optional choice observationFrequency, businessCenters";
	
	class Default implements FxWeightedFixingScheduleChoice3 {
		...
		private ComparisonResult executeDataRule(FxWeightedFixingSchedule fxWeightedFixingSchedule) {
			try {
				return choice(MapperS.of(fxWeightedFixingSchedule), Arrays.asList("observationFrequency", "businessCenters"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements FxWeightedFixingScheduleChoice3 {
