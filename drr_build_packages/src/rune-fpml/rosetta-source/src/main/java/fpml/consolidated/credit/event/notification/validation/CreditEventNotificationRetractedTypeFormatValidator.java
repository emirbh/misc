...
public class CreditEventNotificationRetractedTypeFormatValidator implements Validator<CreditEventNotificationRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNotificationRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventNotificationRetracted", path, "", res.getError());
				}
				return success("CreditEventNotificationRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventNotificationRetracted", path, "");
			})
			.collect(toList());
	}
