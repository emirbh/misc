		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DirectionalLegUnderlyerValuation", ValidationResult.ValidationType.CARDINALITY, "DirectionalLegUnderlyerValuation", path, "", res.getError());
				}
				return success("DirectionalLegUnderlyerValuation", ValidationResult.ValidationType.CARDINALITY, "DirectionalLegUnderlyerValuation", path, "");
			})
			.collect(toList());
	}
