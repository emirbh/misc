...
public class ResponseMessageTypeFormatValidator implements Validator<ResponseMessage> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResponseMessage", ValidationResult.ValidationType.TYPE_FORMAT, "ResponseMessage", path, "", res.getError());
				}
				return success("ResponseMessage", ValidationResult.ValidationType.TYPE_FORMAT, "ResponseMessage", path, "");
			})
			.collect(toList());
	}
