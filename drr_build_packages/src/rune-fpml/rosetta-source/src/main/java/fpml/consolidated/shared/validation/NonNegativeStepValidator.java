...
public class NonNegativeStepValidator implements Validator<NonNegativeStep> {
	...
	private List<ComparisonResult> getComparisonResults(NonNegativeStep o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("stepValue", (BigDecimal) o.getStepValue() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonNegativeStep", ValidationResult.ValidationType.CARDINALITY, "NonNegativeStep", path, "", res.getError());
				}
				return success("NonNegativeStep", ValidationResult.ValidationType.CARDINALITY, "NonNegativeStep", path, "");
			})
			.collect(toList());
	}
