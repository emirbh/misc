		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingStructureValuation", ValidationResult.ValidationType.TYPE_FORMAT, "PricingStructureValuation", path, "", res.getError());
				}
				return success("PricingStructureValuation", ValidationResult.ValidationType.TYPE_FORMAT, "PricingStructureValuation", path, "");
			})
			.collect(toList());
	}
