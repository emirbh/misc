		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxCashSettlementSimple", ValidationResult.ValidationType.CARDINALITY, "FxCashSettlementSimple", path, "", res.getError());
				}
				return success("FxCashSettlementSimple", ValidationResult.ValidationType.CARDINALITY, "FxCashSettlementSimple", path, "");
			})
			.collect(toList());
	}
