...
public class RequestAllocationTypeFormatValidator implements Validator<RequestAllocation> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestAllocation", ValidationResult.ValidationType.TYPE_FORMAT, "RequestAllocation", path, "", res.getError());
				}
				return success("RequestAllocation", ValidationResult.ValidationType.TYPE_FORMAT, "RequestAllocation", path, "");
			})
			.collect(toList());
	}
