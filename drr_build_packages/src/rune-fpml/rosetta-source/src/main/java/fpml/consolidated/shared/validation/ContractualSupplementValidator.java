		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContractualSupplement", ValidationResult.ValidationType.CARDINALITY, "ContractualSupplement", path, "", res.getError());
				}
				return success("ContractualSupplement", ValidationResult.ValidationType.CARDINALITY, "ContractualSupplement", path, "");
			})
			.collect(toList());
	}
