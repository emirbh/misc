		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableDate2", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableDate2", path, "", res.getError());
				}
				return success("AdjustableDate2", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableDate2", path, "");
			})
			.collect(toList());
	}
