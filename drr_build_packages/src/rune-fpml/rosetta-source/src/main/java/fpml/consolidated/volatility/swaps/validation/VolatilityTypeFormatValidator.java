		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Volatility", ValidationResult.ValidationType.TYPE_FORMAT, "Volatility", path, "", res.getError());
				}
				return success("Volatility", ValidationResult.ValidationType.TYPE_FORMAT, "Volatility", path, "");
			})
			.collect(toList());
	}
