		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeEvent", ValidationResult.ValidationType.CARDINALITY, "LoanTradeEvent", path, "", res.getError());
				}
				return success("LoanTradeEvent", ValidationResult.ValidationType.CARDINALITY, "LoanTradeEvent", path, "");
			})
			.collect(toList());
	}
