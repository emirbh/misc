...
public class AbstractFacilityContractEventValidator implements Validator<AbstractFacilityContractEvent> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityContractEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityContractEvent", path, "", res.getError());
				}
				return success("AbstractFacilityContractEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityContractEvent", path, "");
			})
			.collect(toList());
	}
