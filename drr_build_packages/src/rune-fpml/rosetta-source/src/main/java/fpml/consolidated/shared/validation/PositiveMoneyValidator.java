		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PositiveMoney", ValidationResult.ValidationType.CARDINALITY, "PositiveMoney", path, "", res.getError());
				}
				return success("PositiveMoney", ValidationResult.ValidationType.CARDINALITY, "PositiveMoney", path, "");
			})
			.collect(toList());
	}
