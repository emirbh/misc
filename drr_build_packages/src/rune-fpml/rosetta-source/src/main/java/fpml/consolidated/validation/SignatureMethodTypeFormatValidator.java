...
public class SignatureMethodTypeFormatValidator implements Validator<SignatureMethod> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureMethod", ValidationResult.ValidationType.TYPE_FORMAT, "SignatureMethod", path, "", res.getError());
				}
				return success("SignatureMethod", ValidationResult.ValidationType.TYPE_FORMAT, "SignatureMethod", path, "");
			})
			.collect(toList());
	}
