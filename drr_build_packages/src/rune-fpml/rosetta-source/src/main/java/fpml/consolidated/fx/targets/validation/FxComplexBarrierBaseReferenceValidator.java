		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxComplexBarrierBaseReference", ValidationResult.ValidationType.CARDINALITY, "FxComplexBarrierBaseReference", path, "", res.getError());
				}
				return success("FxComplexBarrierBaseReference", ValidationResult.ValidationType.CARDINALITY, "FxComplexBarrierBaseReference", path, "");
			})
			.collect(toList());
	}
