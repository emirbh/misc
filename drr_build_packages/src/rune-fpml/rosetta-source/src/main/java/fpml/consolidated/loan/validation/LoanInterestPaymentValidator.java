		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanInterestPayment", ValidationResult.ValidationType.CARDINALITY, "LoanInterestPayment", path, "", res.getError());
				}
				return success("LoanInterestPayment", ValidationResult.ValidationType.CARDINALITY, "LoanInterestPayment", path, "");
			})
			.collect(toList());
	}
