		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Beneficiary", ValidationResult.ValidationType.CARDINALITY, "Beneficiary", path, "", res.getError());
				}
				return success("Beneficiary", ValidationResult.ValidationType.CARDINALITY, "Beneficiary", path, "");
			})
			.collect(toList());
	}
