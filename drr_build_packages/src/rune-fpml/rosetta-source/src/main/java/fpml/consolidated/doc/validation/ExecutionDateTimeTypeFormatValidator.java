		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionDateTime", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionDateTime", path, "", res.getError());
				}
				return success("ExecutionDateTime", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionDateTime", path, "");
			})
			.collect(toList());
	}
