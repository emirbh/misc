		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingBoolean", ValidationResult.ValidationType.CARDINALITY, "ReportingBoolean", path, "", res.getError());
				}
				return success("ReportingBoolean", ValidationResult.ValidationType.CARDINALITY, "ReportingBoolean", path, "");
			})
			.collect(toList());
	}
