		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeSettlementTaskType", ValidationResult.ValidationType.CARDINALITY, "LoanTradeSettlementTaskType", path, "", res.getError());
				}
				return success("LoanTradeSettlementTaskType", ValidationResult.ValidationType.CARDINALITY, "LoanTradeSettlementTaskType", path, "");
			})
			.collect(toList());
	}
