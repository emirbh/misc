		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ActualPrice", ValidationResult.ValidationType.CARDINALITY, "ActualPrice", path, "", res.getError());
				}
				return success("ActualPrice", ValidationResult.ValidationType.CARDINALITY, "ActualPrice", path, "");
			})
			.collect(toList());
	}
