		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotificationMessageHeader", ValidationResult.ValidationType.CARDINALITY, "NotificationMessageHeader", path, "", res.getError());
				}
				return success("NotificationMessageHeader", ValidationResult.ValidationType.CARDINALITY, "NotificationMessageHeader", path, "");
			})
			.collect(toList());
	}
