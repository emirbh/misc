		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeHeader", ValidationResult.ValidationType.TYPE_FORMAT, "TradeHeader", path, "", res.getError());
				}
				return success("TradeHeader", ValidationResult.ValidationType.TYPE_FORMAT, "TradeHeader", path, "");
			})
			.collect(toList());
	}
