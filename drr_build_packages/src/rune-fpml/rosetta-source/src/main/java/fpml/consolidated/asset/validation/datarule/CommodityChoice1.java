 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityChoice1")
@ImplementedBy(CommodityChoice1.Default.class)
public interface CommodityChoice1 extends Validator<Commodity> {
	
	String NAME = "CommodityChoice1";
	String DEFINITION = "optional choice deliveryDates, deliveryNearby, deliveryDate, deliveryDateYearMonth";
	
	class Default implements CommodityChoice1 {
		...
		private ComparisonResult executeDataRule(Commodity commodity) {
			try {
				return choice(MapperS.of(commodity), Arrays.asList("deliveryDates", "deliveryNearby", "deliveryDate", "deliveryDateYearMonth"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements CommodityChoice1 {
