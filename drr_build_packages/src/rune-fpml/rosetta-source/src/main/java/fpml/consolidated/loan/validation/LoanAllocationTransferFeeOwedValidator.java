...
public class LoanAllocationTransferFeeOwedValidator implements Validator<LoanAllocationTransferFeeOwed> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationTransferFeeOwed", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationTransferFeeOwed", path, "", res.getError());
				}
				return success("LoanAllocationTransferFeeOwed", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationTransferFeeOwed", path, "");
			})
			.collect(toList());
	}
