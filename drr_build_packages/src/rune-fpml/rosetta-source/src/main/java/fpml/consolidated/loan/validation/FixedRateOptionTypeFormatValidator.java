		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedRateOption", ValidationResult.ValidationType.TYPE_FORMAT, "FixedRateOption", path, "", res.getError());
				}
				return success("FixedRateOption", ValidationResult.ValidationType.TYPE_FORMAT, "FixedRateOption", path, "");
			})
			.collect(toList());
	}
