...
public class AbstractLoanServicingEventValidator implements Validator<AbstractLoanServicingEvent> {

	private List<ComparisonResult> getComparisonResults(AbstractLoanServicingEvent o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, AbstractLoanServicingEvent o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanServicingEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanServicingEvent", path, "", res.getError());
				}
				return success("AbstractLoanServicingEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanServicingEvent", path, "");
			})
			.collect(toList());
	}
