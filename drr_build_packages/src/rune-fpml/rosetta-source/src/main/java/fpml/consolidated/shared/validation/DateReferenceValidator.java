		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateReference", ValidationResult.ValidationType.CARDINALITY, "DateReference", path, "", res.getError());
				}
				return success("DateReference", ValidationResult.ValidationType.CARDINALITY, "DateReference", path, "");
			})
			.collect(toList());
	}
