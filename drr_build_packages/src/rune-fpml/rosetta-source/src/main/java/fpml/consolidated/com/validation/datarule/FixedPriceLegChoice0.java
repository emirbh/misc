 ...
 * @version 2.1.1
 */
@RosettaDataRule("FixedPriceLegChoice0")
@ImplementedBy(FixedPriceLegChoice0.Default.class)
public interface FixedPriceLegChoice0 extends Validator<FixedPriceLeg> {
	
	String NAME = "FixedPriceLegChoice0";
	String DEFINITION = "required choice relativePaymentDates, paymentDates, masterAgreementPaymentDates";
	
	class Default implements FixedPriceLegChoice0 {
		...
		private ComparisonResult executeDataRule(FixedPriceLeg fixedPriceLeg) {
			try {
				return choice(MapperS.of(fixedPriceLeg), Arrays.asList("relativePaymentDates", "paymentDates", "masterAgreementPaymentDates"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements FixedPriceLegChoice0 {
