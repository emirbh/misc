		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityTrigger", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityTrigger", path, "", res.getError());
				}
				return success("CommodityTrigger", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityTrigger", path, "");
			})
			.collect(toList());
	}
