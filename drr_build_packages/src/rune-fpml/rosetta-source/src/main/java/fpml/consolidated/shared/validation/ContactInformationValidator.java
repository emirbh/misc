...
public class ContactInformationValidator implements Validator<ContactInformation> {
	...
	private List<ComparisonResult> getComparisonResults(ContactInformation o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("address", (Address) o.getAddress() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContactInformation", ValidationResult.ValidationType.CARDINALITY, "ContactInformation", path, "", res.getError());
				}
				return success("ContactInformation", ValidationResult.ValidationType.CARDINALITY, "ContactInformation", path, "");
			})
			.collect(toList());
	}
