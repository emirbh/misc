		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IdentifiedCurrencyReference", ValidationResult.ValidationType.TYPE_FORMAT, "IdentifiedCurrencyReference", path, "", res.getError());
				}
				return success("IdentifiedCurrencyReference", ValidationResult.ValidationType.TYPE_FORMAT, "IdentifiedCurrencyReference", path, "");
			})
			.collect(toList());
	}
