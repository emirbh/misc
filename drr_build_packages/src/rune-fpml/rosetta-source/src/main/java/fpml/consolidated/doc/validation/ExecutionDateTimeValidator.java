		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionDateTime", ValidationResult.ValidationType.CARDINALITY, "ExecutionDateTime", path, "", res.getError());
				}
				return success("ExecutionDateTime", ValidationResult.ValidationType.CARDINALITY, "ExecutionDateTime", path, "");
			})
			.collect(toList());
	}
