		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxRateAsset", ValidationResult.ValidationType.CARDINALITY, "FxRateAsset", path, "", res.getError());
				}
				return success("FxRateAsset", ValidationResult.ValidationType.CARDINALITY, "FxRateAsset", path, "");
			})
			.collect(toList());
	}
