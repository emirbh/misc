		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestAccrualsMethod", ValidationResult.ValidationType.TYPE_FORMAT, "InterestAccrualsMethod", path, "", res.getError());
				}
				return success("InterestAccrualsMethod", ValidationResult.ValidationType.TYPE_FORMAT, "InterestAccrualsMethod", path, "");
			})
			.collect(toList());
	}
