		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationRefused", ValidationResult.ValidationType.CARDINALITY, "AllocationRefused", path, "", res.getError());
				}
				return success("AllocationRefused", ValidationResult.ValidationType.CARDINALITY, "AllocationRefused", path, "");
			})
			.collect(toList());
	}
