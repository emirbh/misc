...
public class LoanTradeFeeDueValidator implements Validator<LoanTradeFeeDue> {

	private List<ComparisonResult> getComparisonResults(LoanTradeFeeDue o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanTradeFeeDue o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeFeeDue", ValidationResult.ValidationType.CARDINALITY, "LoanTradeFeeDue", path, "", res.getError());
				}
				return success("LoanTradeFeeDue", ValidationResult.ValidationType.CARDINALITY, "LoanTradeFeeDue", path, "");
			})
			.collect(toList());
	}
