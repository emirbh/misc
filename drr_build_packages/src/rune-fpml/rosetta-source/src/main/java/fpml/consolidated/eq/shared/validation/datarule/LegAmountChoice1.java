 ...
 * @version 2.1.1
 */
@RosettaDataRule("LegAmountChoice1")
@ImplementedBy(LegAmountChoice1.Default.class)
public interface LegAmountChoice1 extends Validator<LegAmount> {
	
	String NAME = "LegAmountChoice1";
	String DEFINITION = "optional choice referenceAmount, formula, encodedDescription";
	
	class Default implements LegAmountChoice1 {
		...
		private ComparisonResult executeDataRule(LegAmount legAmount) {
			try {
				return choice(MapperS.of(legAmount), Arrays.asList("referenceAmount", "formula", "encodedDescription"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements LegAmountChoice1 {
