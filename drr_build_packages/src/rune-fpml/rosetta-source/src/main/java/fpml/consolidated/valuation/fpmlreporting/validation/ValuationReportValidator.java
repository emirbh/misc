...
public class ValuationReportValidator implements Validator<ValuationReport> {

	private List<ComparisonResult> getComparisonResults(ValuationReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("generatedDateTime", (ZonedDateTime) o.getGeneratedDateTime() != null ? 1 : 0, 0, 1), 
				checkCardinality("market", (Market) o.getMarket() != null ? 1 : 0, 0, 1)
			);
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, ValuationReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationReport", ValidationResult.ValidationType.CARDINALITY, "ValuationReport", path, "", res.getError());
				}
				return success("ValuationReport", ValidationResult.ValidationType.CARDINALITY, "ValuationReport", path, "");
			})
			.collect(toList());
	}
