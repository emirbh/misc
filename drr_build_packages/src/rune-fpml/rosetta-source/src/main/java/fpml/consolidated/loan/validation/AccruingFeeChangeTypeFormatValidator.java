		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeeChange", ValidationResult.ValidationType.TYPE_FORMAT, "AccruingFeeChange", path, "", res.getError());
				}
				return success("AccruingFeeChange", ValidationResult.ValidationType.TYPE_FORMAT, "AccruingFeeChange", path, "");
			})
			.collect(toList());
	}
