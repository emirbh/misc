		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "CollateralAllocationAcknowledgement", path, "", res.getError());
				}
				return success("CollateralAllocationAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "CollateralAllocationAcknowledgement", path, "");
			})
			.collect(toList());
	}
