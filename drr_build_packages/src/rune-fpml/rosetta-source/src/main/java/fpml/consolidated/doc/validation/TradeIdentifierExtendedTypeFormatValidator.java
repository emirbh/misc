		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeIdentifierExtended", ValidationResult.ValidationType.TYPE_FORMAT, "TradeIdentifierExtended", path, "", res.getError());
				}
				return success("TradeIdentifierExtended", ValidationResult.ValidationType.TYPE_FORMAT, "TradeIdentifierExtended", path, "");
			})
			.collect(toList());
	}
