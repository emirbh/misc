		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashSettlement", ValidationResult.ValidationType.TYPE_FORMAT, "CashSettlement", path, "", res.getError());
				}
				return success("CashSettlement", ValidationResult.ValidationType.TYPE_FORMAT, "CashSettlement", path, "");
			})
			.collect(toList());
	}
