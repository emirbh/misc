		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrincipalExchanges", ValidationResult.ValidationType.TYPE_FORMAT, "PrincipalExchanges", path, "", res.getError());
				}
				return success("PrincipalExchanges", ValidationResult.ValidationType.TYPE_FORMAT, "PrincipalExchanges", path, "");
			})
			.collect(toList());
	}
