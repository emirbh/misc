...
public class FormulaComponentTypeFormatValidator implements Validator<FormulaComponent> {
	...
	private List<ComparisonResult> getComparisonResults(FormulaComponent o) {
		return Lists.<ComparisonResult>newArrayList(
				checkString("componentDescription", o.getComponentDescription(), 1, empty(), empty())
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FormulaComponent", ValidationResult.ValidationType.TYPE_FORMAT, "FormulaComponent", path, "", res.getError());
				}
				return success("FormulaComponent", ValidationResult.ValidationType.TYPE_FORMAT, "FormulaComponent", path, "");
			})
			.collect(toList());
	}
