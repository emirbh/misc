...
public class AbsoluteToleranceValidator implements Validator<AbsoluteTolerance> {
	...
	private List<ComparisonResult> getComparisonResults(AbsoluteTolerance o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("positive", (BigDecimal) o.getPositive() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbsoluteTolerance", ValidationResult.ValidationType.CARDINALITY, "AbsoluteTolerance", path, "", res.getError());
				}
				return success("AbsoluteTolerance", ValidationResult.ValidationType.CARDINALITY, "AbsoluteTolerance", path, "");
			})
			.collect(toList());
	}
