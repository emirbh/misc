		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ApprovalType", ValidationResult.ValidationType.TYPE_FORMAT, "ApprovalType", path, "", res.getError());
				}
				return success("ApprovalType", ValidationResult.ValidationType.TYPE_FORMAT, "ApprovalType", path, "");
			})
			.collect(toList());
	}
