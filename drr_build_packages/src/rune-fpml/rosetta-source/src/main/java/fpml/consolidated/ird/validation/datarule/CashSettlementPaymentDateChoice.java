...
public interface CashSettlementPaymentDateChoice extends Validator<CashSettlementPaymentDate> {
	...
	class Default implements CashSettlementPaymentDateChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, CashSettlementPaymentDate cashSettlementPaymentDate) {
			ComparisonResult result = executeDataRule(cashSettlementPaymentDate);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "CashSettlementPaymentDate", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "CashSettlementPaymentDate", path, DEFINITION, failureMessage));
		}
