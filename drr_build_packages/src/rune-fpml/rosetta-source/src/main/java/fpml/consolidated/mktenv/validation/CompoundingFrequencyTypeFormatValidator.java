		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CompoundingFrequency", ValidationResult.ValidationType.TYPE_FORMAT, "CompoundingFrequency", path, "", res.getError());
				}
				return success("CompoundingFrequency", ValidationResult.ValidationType.TYPE_FORMAT, "CompoundingFrequency", path, "");
			})
			.collect(toList());
	}
