		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityProductGrade", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityProductGrade", path, "", res.getError());
				}
				return success("CommodityProductGrade", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityProductGrade", path, "");
			})
			.collect(toList());
	}
