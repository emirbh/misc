...
public class LoanLegalActionStatementValidator implements Validator<LoanLegalActionStatement> {

	private List<ComparisonResult> getComparisonResults(LoanLegalActionStatement o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("legalAction", (LoanLegalAction) o.getLegalAction() != null ? 1 : 0, 1, 1), 
				...
				checkCardinality("deal", (Deal) o.getDeal() != null ? 1 : 0, 0, 1)
			);
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanLegalActionStatement o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanLegalActionStatement", ValidationResult.ValidationType.CARDINALITY, "LoanLegalActionStatement", path, "", res.getError());
				}
				return success("LoanLegalActionStatement", ValidationResult.ValidationType.CARDINALITY, "LoanLegalActionStatement", path, "");
			})
			.collect(toList());
	}
