...
public class ApprovalStatusNotificationTypeFormatValidator implements Validator<ApprovalStatusNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ApprovalStatusNotification", ValidationResult.ValidationType.TYPE_FORMAT, "ApprovalStatusNotification", path, "", res.getError());
				}
				return success("ApprovalStatusNotification", ValidationResult.ValidationType.TYPE_FORMAT, "ApprovalStatusNotification", path, "");
			})
			.collect(toList());
	}
