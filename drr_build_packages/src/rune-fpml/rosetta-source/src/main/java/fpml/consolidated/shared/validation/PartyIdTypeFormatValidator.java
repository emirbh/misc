		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyId", ValidationResult.ValidationType.TYPE_FORMAT, "PartyId", path, "", res.getError());
				}
				return success("PartyId", ValidationResult.ValidationType.TYPE_FORMAT, "PartyId", path, "");
			})
			.collect(toList());
	}
