...
public class MandatoryCostRateChangeOnlyExistsValidator implements ValidatorWithArg<MandatoryCostRateChange, Set<String>> {

	/* Casting is required to ensure types are output to ensure recompilation in Rosetta */
	@Override
	public <T2 extends MandatoryCostRateChange> ValidationResult<MandatoryCostRateChange> validate(RosettaPath path, T2 o, Set<String> fields) {
		...
		if (setFields.equals(fields)) {
			return success("MandatoryCostRateChange", ValidationResult.ValidationType.ONLY_EXISTS, "MandatoryCostRateChange", path, "");
		}
		return failure("MandatoryCostRateChange", ValidationResult.ValidationType.ONLY_EXISTS, "MandatoryCostRateChange", path, "",
