		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonNegativeAmountSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "NonNegativeAmountSchedule", path, "", res.getError());
				}
				return success("NonNegativeAmountSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "NonNegativeAmountSchedule", path, "");
			})
			.collect(toList());
	}
