		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionPremium", ValidationResult.ValidationType.CARDINALITY, "FxOptionPremium", path, "", res.getError());
				}
				return success("FxOptionPremium", ValidationResult.ValidationType.CARDINALITY, "FxOptionPremium", path, "");
			})
			.collect(toList());
	}
