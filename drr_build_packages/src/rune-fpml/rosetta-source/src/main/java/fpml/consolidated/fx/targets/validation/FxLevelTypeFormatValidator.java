		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxLevel", ValidationResult.ValidationType.TYPE_FORMAT, "FxLevel", path, "", res.getError());
				}
				return success("FxLevel", ValidationResult.ValidationType.TYPE_FORMAT, "FxLevel", path, "");
			})
			.collect(toList());
	}
