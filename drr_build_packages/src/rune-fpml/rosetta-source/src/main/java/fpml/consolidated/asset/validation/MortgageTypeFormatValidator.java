...
public class MortgageTypeFormatValidator implements Validator<Mortgage> {
	...
	private List<ComparisonResult> getComparisonResults(Mortgage o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkString("tranche", o.getTranche(), 1, empty(), empty())
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Mortgage", ValidationResult.ValidationType.TYPE_FORMAT, "Mortgage", path, "", res.getError());
				}
				return success("Mortgage", ValidationResult.ValidationType.TYPE_FORMAT, "Mortgage", path, "");
			})
			.collect(toList());
	}
