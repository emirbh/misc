		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettledEntityMatrix", ValidationResult.ValidationType.TYPE_FORMAT, "SettledEntityMatrix", path, "", res.getError());
				}
				return success("SettledEntityMatrix", ValidationResult.ValidationType.TYPE_FORMAT, "SettledEntityMatrix", path, "");
			})
			.collect(toList());
	}
