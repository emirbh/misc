		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommoditySwap", ValidationResult.ValidationType.TYPE_FORMAT, "CommoditySwap", path, "", res.getError());
				}
				return success("CommoditySwap", ValidationResult.ValidationType.TYPE_FORMAT, "CommoditySwap", path, "");
			})
			.collect(toList());
	}
