		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexName", ValidationResult.ValidationType.TYPE_FORMAT, "IndexName", path, "", res.getError());
				}
				return success("IndexName", ValidationResult.ValidationType.TYPE_FORMAT, "IndexName", path, "");
			})
			.collect(toList());
	}
