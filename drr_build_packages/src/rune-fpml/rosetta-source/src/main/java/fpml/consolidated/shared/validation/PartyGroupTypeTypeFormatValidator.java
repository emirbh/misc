		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyGroupType", ValidationResult.ValidationType.TYPE_FORMAT, "PartyGroupType", path, "", res.getError());
				}
				return success("PartyGroupType", ValidationResult.ValidationType.TYPE_FORMAT, "PartyGroupType", path, "");
			})
			.collect(toList());
	}
