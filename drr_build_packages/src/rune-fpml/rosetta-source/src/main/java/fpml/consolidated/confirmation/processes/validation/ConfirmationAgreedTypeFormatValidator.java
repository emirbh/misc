		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationAgreed", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationAgreed", path, "", res.getError());
				}
				return success("ConfirmationAgreed", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationAgreed", path, "");
			})
			.collect(toList());
	}
