		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MakeWholeProvisions", ValidationResult.ValidationType.TYPE_FORMAT, "MakeWholeProvisions", path, "", res.getError());
				}
				return success("MakeWholeProvisions", ValidationResult.ValidationType.TYPE_FORMAT, "MakeWholeProvisions", path, "");
			})
			.collect(toList());
	}
