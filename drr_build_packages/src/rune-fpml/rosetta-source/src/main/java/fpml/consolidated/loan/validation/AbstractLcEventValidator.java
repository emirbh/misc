...
public class AbstractLcEventValidator implements Validator<AbstractLcEvent> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLcEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLcEvent", path, "", res.getError());
				}
				return success("AbstractLcEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractLcEvent", path, "");
			})
			.collect(toList());
	}
