		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalDeliveryPoint", ValidationResult.ValidationType.TYPE_FORMAT, "CoalDeliveryPoint", path, "", res.getError());
				}
				return success("CoalDeliveryPoint", ValidationResult.ValidationType.TYPE_FORMAT, "CoalDeliveryPoint", path, "");
			})
			.collect(toList());
	}
