...
public class RequestAllocationRetractedTypeFormatValidator implements Validator<RequestAllocationRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestAllocationRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestAllocationRetracted", path, "", res.getError());
				}
				return success("RequestAllocationRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestAllocationRetracted", path, "");
			})
			.collect(toList());
	}
