...
public class ClearingExceptionTypeFormatValidator implements Validator<ClearingException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingException", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingException", path, "", res.getError());
				}
				return success("ClearingException", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingException", path, "");
			})
			.collect(toList());
	}
