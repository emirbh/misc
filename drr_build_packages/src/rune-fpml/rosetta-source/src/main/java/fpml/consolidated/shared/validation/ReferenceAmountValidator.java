		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReferenceAmount", ValidationResult.ValidationType.CARDINALITY, "ReferenceAmount", path, "", res.getError());
				}
				return success("ReferenceAmount", ValidationResult.ValidationType.CARDINALITY, "ReferenceAmount", path, "");
			})
			.collect(toList());
	}
