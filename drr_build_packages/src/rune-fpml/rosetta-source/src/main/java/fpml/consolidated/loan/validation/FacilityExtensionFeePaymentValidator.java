...
public class FacilityExtensionFeePaymentValidator implements Validator<FacilityExtensionFeePayment> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityExtensionFeePayment", ValidationResult.ValidationType.CARDINALITY, "FacilityExtensionFeePayment", path, "", res.getError());
				}
				return success("FacilityExtensionFeePayment", ValidationResult.ValidationType.CARDINALITY, "FacilityExtensionFeePayment", path, "");
			})
			.collect(toList());
	}
