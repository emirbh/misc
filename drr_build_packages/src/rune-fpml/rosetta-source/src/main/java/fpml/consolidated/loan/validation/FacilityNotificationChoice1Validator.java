...
public class FacilityNotificationChoice1Validator implements Validator<FacilityNotificationChoice1> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityNotificationChoice1", ValidationResult.ValidationType.CARDINALITY, "FacilityNotificationChoice1", path, "", res.getError());
				}
				return success("FacilityNotificationChoice1", ValidationResult.ValidationType.CARDINALITY, "FacilityNotificationChoice1", path, "");
			})
			.collect(toList());
	}
