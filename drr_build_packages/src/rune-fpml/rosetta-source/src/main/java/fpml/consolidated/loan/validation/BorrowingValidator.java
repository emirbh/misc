		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Borrowing", ValidationResult.ValidationType.CARDINALITY, "Borrowing", path, "", res.getError());
				}
				return success("Borrowing", ValidationResult.ValidationType.CARDINALITY, "Borrowing", path, "");
			})
			.collect(toList());
	}
