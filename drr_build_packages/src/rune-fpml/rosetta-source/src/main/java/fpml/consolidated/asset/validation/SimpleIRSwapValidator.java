		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SimpleIRSwap", ValidationResult.ValidationType.CARDINALITY, "SimpleIRSwap", path, "", res.getError());
				}
				return success("SimpleIRSwap", ValidationResult.ValidationType.CARDINALITY, "SimpleIRSwap", path, "");
			})
			.collect(toList());
	}
