		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResetDatesReference", ValidationResult.ValidationType.TYPE_FORMAT, "ResetDatesReference", path, "", res.getError());
				}
				return success("ResetDatesReference", ValidationResult.ValidationType.TYPE_FORMAT, "ResetDatesReference", path, "");
			})
			.collect(toList());
	}
