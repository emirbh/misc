...
public class LoanAllocationSettlementValidator implements Validator<LoanAllocationSettlement> {

	private List<ComparisonResult> getComparisonResults(LoanAllocationSettlement o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanAllocationSettlement o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationSettlement", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationSettlement", path, "", res.getError());
				}
				return success("LoanAllocationSettlement", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationSettlement", path, "");
			})
			.collect(toList());
	}
