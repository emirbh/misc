		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PrincipalExchangeFeatures", ValidationResult.ValidationType.CARDINALITY, "PrincipalExchangeFeatures", path, "", res.getError());
				}
				return success("PrincipalExchangeFeatures", ValidationResult.ValidationType.CARDINALITY, "PrincipalExchangeFeatures", path, "");
			})
			.collect(toList());
	}
