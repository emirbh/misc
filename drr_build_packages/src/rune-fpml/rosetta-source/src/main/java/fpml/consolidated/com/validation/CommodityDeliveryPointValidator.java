		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityDeliveryPoint", ValidationResult.ValidationType.CARDINALITY, "CommodityDeliveryPoint", path, "", res.getError());
				}
				return success("CommodityDeliveryPoint", ValidationResult.ValidationType.CARDINALITY, "CommodityDeliveryPoint", path, "");
			})
			.collect(toList());
	}
