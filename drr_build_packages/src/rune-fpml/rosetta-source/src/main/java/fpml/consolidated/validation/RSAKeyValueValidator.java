		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RSAKeyValue", ValidationResult.ValidationType.CARDINALITY, "RSAKeyValue", path, "", res.getError());
				}
				return success("RSAKeyValue", ValidationResult.ValidationType.CARDINALITY, "RSAKeyValue", path, "");
			})
			.collect(toList());
	}
