		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EnvironmentalTrackingSystem", ValidationResult.ValidationType.TYPE_FORMAT, "EnvironmentalTrackingSystem", path, "", res.getError());
				}
				return success("EnvironmentalTrackingSystem", ValidationResult.ValidationType.TYPE_FORMAT, "EnvironmentalTrackingSystem", path, "");
			})
			.collect(toList());
	}
