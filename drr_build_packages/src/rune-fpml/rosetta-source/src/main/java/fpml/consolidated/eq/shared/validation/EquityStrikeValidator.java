		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityStrike", ValidationResult.ValidationType.CARDINALITY, "EquityStrike", path, "", res.getError());
				}
				return success("EquityStrike", ValidationResult.ValidationType.CARDINALITY, "EquityStrike", path, "");
			})
			.collect(toList());
	}
