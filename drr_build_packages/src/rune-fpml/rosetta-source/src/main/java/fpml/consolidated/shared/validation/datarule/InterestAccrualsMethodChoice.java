...
public interface InterestAccrualsMethodChoice extends Validator<InterestAccrualsMethod> {
	...
	class Default implements InterestAccrualsMethodChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, InterestAccrualsMethod interestAccrualsMethod) {
			ComparisonResult result = executeDataRule(interestAccrualsMethod);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "InterestAccrualsMethod", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "InterestAccrualsMethod", path, DEFINITION, failureMessage));
		}
