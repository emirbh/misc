...
public class RequestConfirmationRetractedValidator implements Validator<RequestConfirmationRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConfirmationRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestConfirmationRetracted", path, "", res.getError());
				}
				return success("RequestConfirmationRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestConfirmationRetracted", path, "");
			})
			.collect(toList());
	}
