...
public class RegulatoryReportingExceptionTypeFormatValidator implements Validator<RegulatoryReportingException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegulatoryReportingException", ValidationResult.ValidationType.TYPE_FORMAT, "RegulatoryReportingException", path, "", res.getError());
				}
				return success("RegulatoryReportingException", ValidationResult.ValidationType.TYPE_FORMAT, "RegulatoryReportingException", path, "");
			})
			.collect(toList());
	}
