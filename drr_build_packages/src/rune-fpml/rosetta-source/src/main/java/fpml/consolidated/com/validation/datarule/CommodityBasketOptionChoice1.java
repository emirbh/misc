 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityBasketOptionChoice1")
@ImplementedBy(CommodityBasketOptionChoice1.Default.class)
public interface CommodityBasketOptionChoice1 extends Validator<CommodityBasketOption> {
	
	String NAME = "CommodityBasketOptionChoice1";
	String DEFINITION = "required choice notionalQuantityBasket, notionalAmountBasket";
	
	class Default implements CommodityBasketOptionChoice1 {
		...
		private ComparisonResult executeDataRule(CommodityBasketOption commodityBasketOption) {
			try {
				return choice(MapperS.of(commodityBasketOption), Arrays.asList("notionalQuantityBasket", "notionalAmountBasket"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements CommodityBasketOptionChoice1 {
