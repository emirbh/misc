		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionFeatures", ValidationResult.ValidationType.TYPE_FORMAT, "OptionFeatures", path, "", res.getError());
				}
				return success("OptionFeatures", ValidationResult.ValidationType.TYPE_FORMAT, "OptionFeatures", path, "");
			})
			.collect(toList());
	}
