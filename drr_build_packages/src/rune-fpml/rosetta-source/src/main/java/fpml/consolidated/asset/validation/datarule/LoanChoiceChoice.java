	...
	class Default implements LoanChoiceChoice {
		...
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanChoice loanChoice) {
			...
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "LoanChoice", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "LoanChoice", path, DEFINITION, failureMessage));
		}
