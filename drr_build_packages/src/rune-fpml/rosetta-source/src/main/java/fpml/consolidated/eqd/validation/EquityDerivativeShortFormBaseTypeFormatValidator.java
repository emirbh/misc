		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityDerivativeShortFormBase", ValidationResult.ValidationType.TYPE_FORMAT, "EquityDerivativeShortFormBase", path, "", res.getError());
				}
				return success("EquityDerivativeShortFormBase", ValidationResult.ValidationType.TYPE_FORMAT, "EquityDerivativeShortFormBase", path, "");
			})
			.collect(toList());
	}
