		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFixingSchedule", ValidationResult.ValidationType.CARDINALITY, "FxFixingSchedule", path, "", res.getError());
				}
				return success("FxFixingSchedule", ValidationResult.ValidationType.CARDINALITY, "FxFixingSchedule", path, "");
			})
			.collect(toList());
	}
