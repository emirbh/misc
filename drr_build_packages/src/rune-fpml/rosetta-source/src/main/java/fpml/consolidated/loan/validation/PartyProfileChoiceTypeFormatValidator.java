		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyProfileChoice", ValidationResult.ValidationType.TYPE_FORMAT, "PartyProfileChoice", path, "", res.getError());
				}
				return success("PartyProfileChoice", ValidationResult.ValidationType.TYPE_FORMAT, "PartyProfileChoice", path, "");
			})
			.collect(toList());
	}
