		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StrikePriceUnderlyingReference", ValidationResult.ValidationType.TYPE_FORMAT, "StrikePriceUnderlyingReference", path, "", res.getError());
				}
				return success("StrikePriceUnderlyingReference", ValidationResult.ValidationType.TYPE_FORMAT, "StrikePriceUnderlyingReference", path, "");
			})
			.collect(toList());
	}
