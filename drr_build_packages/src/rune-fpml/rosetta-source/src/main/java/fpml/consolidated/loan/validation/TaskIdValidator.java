		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TaskId", ValidationResult.ValidationType.CARDINALITY, "TaskId", path, "", res.getError());
				}
				return success("TaskId", ValidationResult.ValidationType.CARDINALITY, "TaskId", path, "");
			})
			.collect(toList());
	}
