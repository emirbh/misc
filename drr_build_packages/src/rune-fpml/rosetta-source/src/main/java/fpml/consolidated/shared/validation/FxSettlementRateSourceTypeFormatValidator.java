		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSettlementRateSource", ValidationResult.ValidationType.TYPE_FORMAT, "FxSettlementRateSource", path, "", res.getError());
				}
				return success("FxSettlementRateSource", ValidationResult.ValidationType.TYPE_FORMAT, "FxSettlementRateSource", path, "");
			})
			.collect(toList());
	}
