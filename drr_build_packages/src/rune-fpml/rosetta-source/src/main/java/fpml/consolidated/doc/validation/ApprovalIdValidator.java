		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ApprovalId", ValidationResult.ValidationType.CARDINALITY, "ApprovalId", path, "", res.getError());
				}
				return success("ApprovalId", ValidationResult.ValidationType.CARDINALITY, "ApprovalId", path, "");
			})
			.collect(toList());
	}
