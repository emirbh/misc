		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragePriceLeg", ValidationResult.ValidationType.TYPE_FORMAT, "AveragePriceLeg", path, "", res.getError());
				}
				return success("AveragePriceLeg", ValidationResult.ValidationType.TYPE_FORMAT, "AveragePriceLeg", path, "");
			})
			.collect(toList());
	}
