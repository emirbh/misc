		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationId", ValidationResult.ValidationType.CARDINALITY, "CorrelationId", path, "", res.getError());
				}
				return success("CorrelationId", ValidationResult.ValidationType.CARDINALITY, "CorrelationId", path, "");
			})
			.collect(toList());
	}
