		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralType", ValidationResult.ValidationType.CARDINALITY, "CollateralType", path, "", res.getError());
				}
				return success("CollateralType", ValidationResult.ValidationType.CARDINALITY, "CollateralType", path, "");
			})
			.collect(toList());
	}
