		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingPikPayment", ValidationResult.ValidationType.TYPE_FORMAT, "AccruingPikPayment", path, "", res.getError());
				}
				return success("AccruingPikPayment", ValidationResult.ValidationType.TYPE_FORMAT, "AccruingPikPayment", path, "");
			})
			.collect(toList());
	}
