		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NettedSwapBase", ValidationResult.ValidationType.CARDINALITY, "NettedSwapBase", path, "", res.getError());
				}
				return success("NettedSwapBase", ValidationResult.ValidationType.CARDINALITY, "NettedSwapBase", path, "");
			})
			.collect(toList());
	}
