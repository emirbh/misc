		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SwapCurveValuation", ValidationResult.ValidationType.TYPE_FORMAT, "SwapCurveValuation", path, "", res.getError());
				}
				return success("SwapCurveValuation", ValidationResult.ValidationType.TYPE_FORMAT, "SwapCurveValuation", path, "");
			})
			.collect(toList());
	}
