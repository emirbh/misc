...
public class ServiceNotificationExceptionValidator implements Validator<ServiceNotificationException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ServiceNotificationException", ValidationResult.ValidationType.CARDINALITY, "ServiceNotificationException", path, "", res.getError());
				}
				return success("ServiceNotificationException", ValidationResult.ValidationType.CARDINALITY, "ServiceNotificationException", path, "");
			})
			.collect(toList());
	}
