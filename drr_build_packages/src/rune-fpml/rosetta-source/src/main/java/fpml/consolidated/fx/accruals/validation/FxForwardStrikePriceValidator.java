		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxForwardStrikePrice", ValidationResult.ValidationType.CARDINALITY, "FxForwardStrikePrice", path, "", res.getError());
				}
				return success("FxForwardStrikePrice", ValidationResult.ValidationType.CARDINALITY, "FxForwardStrikePrice", path, "");
			})
			.collect(toList());
	}
