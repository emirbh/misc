		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SPKIData", ValidationResult.ValidationType.CARDINALITY, "SPKIData", path, "", res.getError());
				}
				return success("SPKIData", ValidationResult.ValidationType.CARDINALITY, "SPKIData", path, "");
			})
			.collect(toList());
	}
