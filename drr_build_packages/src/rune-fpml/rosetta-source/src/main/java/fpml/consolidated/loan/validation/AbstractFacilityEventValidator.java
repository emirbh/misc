...
public class AbstractFacilityEventValidator implements Validator<AbstractFacilityEvent> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityEvent", path, "", res.getError());
				}
				return success("AbstractFacilityEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityEvent", path, "");
			})
			.collect(toList());
	}
