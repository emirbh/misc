		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestExecutionRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestExecutionRetracted", path, "", res.getError());
				}
				return success("RequestExecutionRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestExecutionRetracted", path, "");
			})
			.collect(toList());
	}
