		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestedAction", ValidationResult.ValidationType.CARDINALITY, "RequestedAction", path, "", res.getError());
				}
				return success("RequestedAction", ValidationResult.ValidationType.CARDINALITY, "RequestedAction", path, "");
			})
			.collect(toList());
	}
