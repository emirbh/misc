		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InitialPayment", ValidationResult.ValidationType.CARDINALITY, "InitialPayment", path, "", res.getError());
				}
				return success("InitialPayment", ValidationResult.ValidationType.CARDINALITY, "InitialPayment", path, "");
			})
			.collect(toList());
	}
