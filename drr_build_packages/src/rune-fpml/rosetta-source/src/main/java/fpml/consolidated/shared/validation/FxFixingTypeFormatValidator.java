		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFixing", ValidationResult.ValidationType.TYPE_FORMAT, "FxFixing", path, "", res.getError());
				}
				return success("FxFixing", ValidationResult.ValidationType.TYPE_FORMAT, "FxFixing", path, "");
			})
			.collect(toList());
	}
