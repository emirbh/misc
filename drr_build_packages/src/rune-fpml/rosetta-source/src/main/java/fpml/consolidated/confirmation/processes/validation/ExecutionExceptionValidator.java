...
public class ExecutionExceptionValidator implements Validator<ExecutionException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionException", ValidationResult.ValidationType.CARDINALITY, "ExecutionException", path, "", res.getError());
				}
				return success("ExecutionException", ValidationResult.ValidationType.CARDINALITY, "ExecutionException", path, "");
			})
			.collect(toList());
	}
