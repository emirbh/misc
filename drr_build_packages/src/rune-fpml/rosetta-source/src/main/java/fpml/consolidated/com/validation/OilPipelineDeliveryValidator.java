...
public class OilPipelineDeliveryValidator implements Validator<OilPipelineDelivery> {
	...
	private List<ComparisonResult> getComparisonResults(OilPipelineDelivery o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("pipelineName", (CommodityPipeline) o.getPipelineName() != null ? 1 : 0, 0, 1), 
				...
				checkCardinality("deliverableByBarge", (Boolean) o.getDeliverableByBarge() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OilPipelineDelivery", ValidationResult.ValidationType.CARDINALITY, "OilPipelineDelivery", path, "", res.getError());
				}
				return success("OilPipelineDelivery", ValidationResult.ValidationType.CARDINALITY, "OilPipelineDelivery", path, "");
			})
			.collect(toList());
	}
