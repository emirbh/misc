		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("KeyInfo", ValidationResult.ValidationType.CARDINALITY, "KeyInfo", path, "", res.getError());
				}
				return success("KeyInfo", ValidationResult.ValidationType.CARDINALITY, "KeyInfo", path, "");
			})
			.collect(toList());
	}
