		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Commodity", ValidationResult.ValidationType.TYPE_FORMAT, "Commodity", path, "", res.getError());
				}
				return success("Commodity", ValidationResult.ValidationType.TYPE_FORMAT, "Commodity", path, "");
			})
			.collect(toList());
	}
