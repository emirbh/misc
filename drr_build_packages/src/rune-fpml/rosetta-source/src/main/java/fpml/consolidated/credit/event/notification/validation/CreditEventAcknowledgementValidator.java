...
public class CreditEventAcknowledgementValidator implements Validator<CreditEventAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "CreditEventAcknowledgement", path, "", res.getError());
				}
				return success("CreditEventAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "CreditEventAcknowledgement", path, "");
			})
			.collect(toList());
	}
