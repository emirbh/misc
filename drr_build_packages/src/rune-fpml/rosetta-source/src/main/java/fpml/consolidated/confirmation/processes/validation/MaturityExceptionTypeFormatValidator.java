		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MaturityException", ValidationResult.ValidationType.TYPE_FORMAT, "MaturityException", path, "", res.getError());
				}
				return success("MaturityException", ValidationResult.ValidationType.TYPE_FORMAT, "MaturityException", path, "");
			})
			.collect(toList());
	}
