		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualPayoffRegion", ValidationResult.ValidationType.CARDINALITY, "FxAccrualPayoffRegion", path, "", res.getError());
				}
				return success("FxAccrualPayoffRegion", ValidationResult.ValidationType.CARDINALITY, "FxAccrualPayoffRegion", path, "");
			})
			.collect(toList());
	}
