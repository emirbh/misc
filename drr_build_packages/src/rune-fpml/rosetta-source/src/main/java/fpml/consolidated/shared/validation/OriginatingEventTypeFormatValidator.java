		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OriginatingEvent", ValidationResult.ValidationType.TYPE_FORMAT, "OriginatingEvent", path, "", res.getError());
				}
				return success("OriginatingEvent", ValidationResult.ValidationType.TYPE_FORMAT, "OriginatingEvent", path, "");
			})
			.collect(toList());
	}
