		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RepoNearLeg", ValidationResult.ValidationType.TYPE_FORMAT, "RepoNearLeg", path, "", res.getError());
				}
				return success("RepoNearLeg", ValidationResult.ValidationType.TYPE_FORMAT, "RepoNearLeg", path, "");
			})
			.collect(toList());
	}
