		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxValuationDateOffset", ValidationResult.ValidationType.CARDINALITY, "FxValuationDateOffset", path, "", res.getError());
				}
				return success("FxValuationDateOffset", ValidationResult.ValidationType.CARDINALITY, "FxValuationDateOffset", path, "");
			})
			.collect(toList());
	}
