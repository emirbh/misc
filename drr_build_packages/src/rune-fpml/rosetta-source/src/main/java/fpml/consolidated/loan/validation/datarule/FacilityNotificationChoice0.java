 ...
 * @version 2.1.1
 */
@RosettaDataRule("FacilityNotificationChoice0")
@ImplementedBy(FacilityNotificationChoice0.Default.class)
public interface FacilityNotificationChoice0 extends Validator<FacilityNotification> {
	
	String NAME = "FacilityNotificationChoice0";
	String DEFINITION = "optional choice dealIdentifier, dealSummary";
	
	class Default implements FacilityNotificationChoice0 {
		...
		private ComparisonResult executeDataRule(FacilityNotification facilityNotification) {
			try {
				return choice(MapperS.of(facilityNotification), Arrays.asList("dealIdentifier", "dealSummary"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements FacilityNotificationChoice0 {
