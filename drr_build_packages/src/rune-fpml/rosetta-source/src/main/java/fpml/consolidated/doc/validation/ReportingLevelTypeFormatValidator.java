		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportingLevel", ValidationResult.ValidationType.TYPE_FORMAT, "ReportingLevel", path, "", res.getError());
				}
				return success("ReportingLevel", ValidationResult.ValidationType.TYPE_FORMAT, "ReportingLevel", path, "");
			})
			.collect(toList());
	}
