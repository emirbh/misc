...
public class RegulatoryClassificationMAS_2013Validator implements Validator<RegulatoryClassificationMAS_2013> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegulatoryClassificationMAS_2013", ValidationResult.ValidationType.CARDINALITY, "RegulatoryClassificationMAS_2013", path, "", res.getError());
				}
				return success("RegulatoryClassificationMAS_2013", ValidationResult.ValidationType.CARDINALITY, "RegulatoryClassificationMAS_2013", path, "");
			})
			.collect(toList());
	}
