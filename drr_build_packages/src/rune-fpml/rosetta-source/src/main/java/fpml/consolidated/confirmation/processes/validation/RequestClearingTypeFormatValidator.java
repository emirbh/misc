		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestClearing", ValidationResult.ValidationType.TYPE_FORMAT, "RequestClearing", path, "", res.getError());
				}
				return success("RequestClearing", ValidationResult.ValidationType.TYPE_FORMAT, "RequestClearing", path, "");
			})
			.collect(toList());
	}
