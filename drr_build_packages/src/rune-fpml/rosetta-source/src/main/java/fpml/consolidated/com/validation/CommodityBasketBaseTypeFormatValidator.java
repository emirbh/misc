		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasketBase", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasketBase", path, "", res.getError());
				}
				return success("CommodityBasketBase", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasketBase", path, "");
			})
			.collect(toList());
	}
