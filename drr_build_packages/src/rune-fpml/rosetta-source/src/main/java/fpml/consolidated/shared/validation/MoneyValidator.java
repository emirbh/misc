		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Money", ValidationResult.ValidationType.CARDINALITY, "Money", path, "", res.getError());
				}
				return success("Money", ValidationResult.ValidationType.CARDINALITY, "Money", path, "");
			})
			.collect(toList());
	}
