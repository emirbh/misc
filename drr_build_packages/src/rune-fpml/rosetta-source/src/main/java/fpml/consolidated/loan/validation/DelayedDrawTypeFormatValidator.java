		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DelayedDraw", ValidationResult.ValidationType.TYPE_FORMAT, "DelayedDraw", path, "", res.getError());
				}
				return success("DelayedDraw", ValidationResult.ValidationType.TYPE_FORMAT, "DelayedDraw", path, "");
			})
			.collect(toList());
	}
