		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocation", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocation", path, "", res.getError());
				}
				return success("LoanAllocation", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocation", path, "");
			})
			.collect(toList());
	}
