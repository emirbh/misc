		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityMetalShape", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityMetalShape", path, "", res.getError());
				}
				return success("CommodityMetalShape", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityMetalShape", path, "");
			})
			.collect(toList());
	}
