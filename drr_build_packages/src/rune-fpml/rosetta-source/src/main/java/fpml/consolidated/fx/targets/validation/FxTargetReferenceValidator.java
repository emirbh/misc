		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetReference", ValidationResult.ValidationType.CARDINALITY, "FxTargetReference", path, "", res.getError());
				}
				return success("FxTargetReference", ValidationResult.ValidationType.CARDINALITY, "FxTargetReference", path, "");
			})
			.collect(toList());
	}
