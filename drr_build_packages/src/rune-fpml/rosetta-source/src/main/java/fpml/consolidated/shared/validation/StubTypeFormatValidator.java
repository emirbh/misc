		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Stub", ValidationResult.ValidationType.TYPE_FORMAT, "Stub", path, "", res.getError());
				}
				return success("Stub", ValidationResult.ValidationType.TYPE_FORMAT, "Stub", path, "");
			})
			.collect(toList());
	}
