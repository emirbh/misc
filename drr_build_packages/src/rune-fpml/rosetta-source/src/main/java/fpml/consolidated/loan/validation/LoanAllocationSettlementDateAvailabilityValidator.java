...
public class LoanAllocationSettlementDateAvailabilityValidator implements Validator<LoanAllocationSettlementDateAvailability> {
	...
	private List<ComparisonResult> getComparisonResults(LoanAllocationSettlementDateAvailability o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("onOrAfterDate", (ZonedDateTime) o.getOnOrAfterDate() != null ? 1 : 0, 0, 1), 
				checkCardinality("leadDays", (Integer) o.getLeadDays() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationSettlementDateAvailability", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationSettlementDateAvailability", path, "", res.getError());
				}
				return success("LoanAllocationSettlementDateAvailability", ValidationResult.ValidationType.CARDINALITY, "LoanAllocationSettlementDateAvailability", path, "");
			})
			.collect(toList());
	}
