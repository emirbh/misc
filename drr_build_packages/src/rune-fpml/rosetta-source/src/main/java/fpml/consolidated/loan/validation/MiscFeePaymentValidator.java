...
public class MiscFeePaymentValidator implements Validator<MiscFeePayment> {
	...
	private List<ComparisonResult> getComparisonResults(MiscFeePayment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("type", (MiscFeeType) o._getType() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MiscFeePayment", ValidationResult.ValidationType.CARDINALITY, "MiscFeePayment", path, "", res.getError());
				}
				return success("MiscFeePayment", ValidationResult.ValidationType.CARDINALITY, "MiscFeePayment", path, "");
			})
			.collect(toList());
	}
