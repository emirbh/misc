		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityNotification", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityNotification", path, "", res.getError());
				}
				return success("FacilityNotification", ValidationResult.ValidationType.TYPE_FORMAT, "FacilityNotification", path, "");
			})
			.collect(toList());
	}
