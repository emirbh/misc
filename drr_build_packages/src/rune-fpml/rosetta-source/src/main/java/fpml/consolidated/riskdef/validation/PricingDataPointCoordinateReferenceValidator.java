		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingDataPointCoordinateReference", ValidationResult.ValidationType.CARDINALITY, "PricingDataPointCoordinateReference", path, "", res.getError());
				}
				return success("PricingDataPointCoordinateReference", ValidationResult.ValidationType.CARDINALITY, "PricingDataPointCoordinateReference", path, "");
			})
			.collect(toList());
	}
