		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentTradeQuantity", ValidationResult.ValidationType.CARDINALITY, "InstrumentTradeQuantity", path, "", res.getError());
				}
				return success("InstrumentTradeQuantity", ValidationResult.ValidationType.CARDINALITY, "InstrumentTradeQuantity", path, "");
			})
			.collect(toList());
	}
