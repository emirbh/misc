...
public class MultipleValuationDatesTypeFormatValidator implements Validator<MultipleValuationDates> {
	...
	private List<ComparisonResult> getComparisonResults(MultipleValuationDates o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkNumber("businessDaysThereafter", o.getBusinessDaysThereafter(), empty(), of(0), of(new BigDecimal("1")), empty()), 
				checkNumber("numberValuationDates", o.getNumberValuationDates(), empty(), of(0), of(new BigDecimal("1")), empty())
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MultipleValuationDates", ValidationResult.ValidationType.TYPE_FORMAT, "MultipleValuationDates", path, "", res.getError());
				}
				return success("MultipleValuationDates", ValidationResult.ValidationType.TYPE_FORMAT, "MultipleValuationDates", path, "");
			})
			.collect(toList());
	}
