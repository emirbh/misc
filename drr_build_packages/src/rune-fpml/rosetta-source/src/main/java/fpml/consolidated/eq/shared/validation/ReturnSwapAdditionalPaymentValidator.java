...
public class ReturnSwapAdditionalPaymentValidator implements Validator<ReturnSwapAdditionalPayment> {
	...
	private List<ComparisonResult> getComparisonResults(ReturnSwapAdditionalPayment o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("additionalPaymentAmount", (AdditionalPaymentAmount) o.getAdditionalPaymentAmount() != null ? 1 : 0, 0, 1), 
				checkCardinality("additionalPaymentDate", (AdjustableOrRelativeDate) o.getAdditionalPaymentDate() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnSwapAdditionalPayment", ValidationResult.ValidationType.CARDINALITY, "ReturnSwapAdditionalPayment", path, "", res.getError());
				}
				return success("ReturnSwapAdditionalPayment", ValidationResult.ValidationType.CARDINALITY, "ReturnSwapAdditionalPayment", path, "");
			})
			.collect(toList());
	}
