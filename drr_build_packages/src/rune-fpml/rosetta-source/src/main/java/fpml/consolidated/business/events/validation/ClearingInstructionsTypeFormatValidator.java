		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingInstructions", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingInstructions", path, "", res.getError());
				}
				return success("ClearingInstructions", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingInstructions", path, "");
			})
			.collect(toList());
	}
