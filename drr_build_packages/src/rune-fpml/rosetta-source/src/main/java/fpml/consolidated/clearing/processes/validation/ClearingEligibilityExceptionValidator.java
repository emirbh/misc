		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingEligibilityException", ValidationResult.ValidationType.CARDINALITY, "ClearingEligibilityException", path, "", res.getError());
				}
				return success("ClearingEligibilityException", ValidationResult.ValidationType.CARDINALITY, "ClearingEligibilityException", path, "");
			})
			.collect(toList());
	}
