		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PeriodRate", ValidationResult.ValidationType.CARDINALITY, "PeriodRate", path, "", res.getError());
				}
				return success("PeriodRate", ValidationResult.ValidationType.CARDINALITY, "PeriodRate", path, "");
			})
			.collect(toList());
	}
