		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionFeature", ValidationResult.ValidationType.CARDINALITY, "OptionFeature", path, "", res.getError());
				}
				return success("OptionFeature", ValidationResult.ValidationType.CARDINALITY, "OptionFeature", path, "");
			})
			.collect(toList());
	}
