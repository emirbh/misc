		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SecurityValuation", ValidationResult.ValidationType.CARDINALITY, "SecurityValuation", path, "", res.getError());
				}
				return success("SecurityValuation", ValidationResult.ValidationType.CARDINALITY, "SecurityValuation", path, "");
			})
			.collect(toList());
	}
