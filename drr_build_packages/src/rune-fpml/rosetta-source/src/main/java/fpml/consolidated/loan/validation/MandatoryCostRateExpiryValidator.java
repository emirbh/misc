...
public class MandatoryCostRateExpiryValidator implements Validator<MandatoryCostRateExpiry> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MandatoryCostRateExpiry", ValidationResult.ValidationType.CARDINALITY, "MandatoryCostRateExpiry", path, "", res.getError());
				}
				return success("MandatoryCostRateExpiry", ValidationResult.ValidationType.CARDINALITY, "MandatoryCostRateExpiry", path, "");
			})
			.collect(toList());
	}
