		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanAllocationSettlementTask", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocationSettlementTask", path, "", res.getError());
				}
				return success("LoanAllocationSettlementTask", ValidationResult.ValidationType.TYPE_FORMAT, "LoanAllocationSettlementTask", path, "");
			})
			.collect(toList());
	}
