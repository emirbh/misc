		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyRelationshipType", ValidationResult.ValidationType.CARDINALITY, "PartyRelationshipType", path, "", res.getError());
				}
				return success("PartyRelationshipType", ValidationResult.ValidationType.CARDINALITY, "PartyRelationshipType", path, "");
			})
			.collect(toList());
	}
