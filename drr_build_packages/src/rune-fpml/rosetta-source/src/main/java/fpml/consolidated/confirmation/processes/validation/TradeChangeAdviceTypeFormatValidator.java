...
public class TradeChangeAdviceTypeFormatValidator implements Validator<TradeChangeAdvice> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdvice", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdvice", path, "", res.getError());
				}
				return success("TradeChangeAdvice", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdvice", path, "");
			})
			.collect(toList());
	}
