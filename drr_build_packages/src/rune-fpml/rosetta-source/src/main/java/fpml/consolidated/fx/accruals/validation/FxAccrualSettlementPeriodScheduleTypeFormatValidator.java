		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualSettlementPeriodSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualSettlementPeriodSchedule", path, "", res.getError());
				}
				return success("FxAccrualSettlementPeriodSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "FxAccrualSettlementPeriodSchedule", path, "");
			})
			.collect(toList());
	}
