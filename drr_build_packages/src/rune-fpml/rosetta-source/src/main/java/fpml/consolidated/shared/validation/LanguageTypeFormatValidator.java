		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Language", ValidationResult.ValidationType.TYPE_FORMAT, "Language", path, "", res.getError());
				}
				return success("Language", ValidationResult.ValidationType.TYPE_FORMAT, "Language", path, "");
			})
			.collect(toList());
	}
