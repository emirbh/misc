		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AllocationApproved", ValidationResult.ValidationType.CARDINALITY, "AllocationApproved", path, "", res.getError());
				}
				return success("AllocationApproved", ValidationResult.ValidationType.CARDINALITY, "AllocationApproved", path, "");
			})
			.collect(toList());
	}
