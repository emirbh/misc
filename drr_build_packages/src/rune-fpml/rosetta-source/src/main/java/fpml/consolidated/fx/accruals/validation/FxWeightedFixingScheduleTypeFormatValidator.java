		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxWeightedFixingSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "FxWeightedFixingSchedule", path, "", res.getError());
				}
				return success("FxWeightedFixingSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "FxWeightedFixingSchedule", path, "");
			})
			.collect(toList());
	}
