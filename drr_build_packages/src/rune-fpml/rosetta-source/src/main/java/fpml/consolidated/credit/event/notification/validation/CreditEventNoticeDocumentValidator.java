...
public class CreditEventNoticeDocumentValidator implements Validator<CreditEventNoticeDocument> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNoticeDocument", ValidationResult.ValidationType.CARDINALITY, "CreditEventNoticeDocument", path, "", res.getError());
				}
				return success("CreditEventNoticeDocument", ValidationResult.ValidationType.CARDINALITY, "CreditEventNoticeDocument", path, "");
			})
			.collect(toList());
	}
