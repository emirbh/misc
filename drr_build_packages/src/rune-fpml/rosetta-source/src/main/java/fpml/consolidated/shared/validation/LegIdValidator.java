		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LegId", ValidationResult.ValidationType.CARDINALITY, "LegId", path, "", res.getError());
				}
				return success("LegId", ValidationResult.ValidationType.CARDINALITY, "LegId", path, "");
			})
			.collect(toList());
	}
