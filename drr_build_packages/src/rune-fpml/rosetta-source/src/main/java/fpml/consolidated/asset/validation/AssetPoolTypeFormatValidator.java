		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetPool", ValidationResult.ValidationType.TYPE_FORMAT, "AssetPool", path, "", res.getError());
				}
				return success("AssetPool", ValidationResult.ValidationType.TYPE_FORMAT, "AssetPool", path, "");
			})
			.collect(toList());
	}
