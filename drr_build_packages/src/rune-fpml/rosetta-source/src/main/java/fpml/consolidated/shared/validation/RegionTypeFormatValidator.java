		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Region", ValidationResult.ValidationType.TYPE_FORMAT, "Region", path, "", res.getError());
				}
				return success("Region", ValidationResult.ValidationType.TYPE_FORMAT, "Region", path, "");
			})
			.collect(toList());
	}
