		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("X509IssuerSerialType", ValidationResult.ValidationType.TYPE_FORMAT, "X509IssuerSerialType", path, "", res.getError());
				}
				return success("X509IssuerSerialType", ValidationResult.ValidationType.TYPE_FORMAT, "X509IssuerSerialType", path, "");
			})
			.collect(toList());
	}
