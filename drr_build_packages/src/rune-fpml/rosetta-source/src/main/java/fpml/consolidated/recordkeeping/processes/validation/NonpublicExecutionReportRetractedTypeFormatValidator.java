...
public class NonpublicExecutionReportRetractedTypeFormatValidator implements Validator<NonpublicExecutionReportRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonpublicExecutionReportRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "NonpublicExecutionReportRetracted", path, "", res.getError());
				}
				return success("NonpublicExecutionReportRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "NonpublicExecutionReportRetracted", path, "");
			})
			.collect(toList());
	}
