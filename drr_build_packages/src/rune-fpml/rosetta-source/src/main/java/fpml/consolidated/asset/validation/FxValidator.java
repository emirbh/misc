		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Fx", ValidationResult.ValidationType.CARDINALITY, "Fx", path, "", res.getError());
				}
				return success("Fx", ValidationResult.ValidationType.CARDINALITY, "Fx", path, "");
			})
			.collect(toList());
	}
