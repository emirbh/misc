		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationDatesReference", ValidationResult.ValidationType.CARDINALITY, "ValuationDatesReference", path, "", res.getError());
				}
				return success("ValuationDatesReference", ValidationResult.ValidationType.CARDINALITY, "ValuationDatesReference", path, "");
			})
			.collect(toList());
	}
