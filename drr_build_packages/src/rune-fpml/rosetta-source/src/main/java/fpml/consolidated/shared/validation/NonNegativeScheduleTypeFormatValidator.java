		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonNegativeSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "NonNegativeSchedule", path, "", res.getError());
				}
				return success("NonNegativeSchedule", ValidationResult.ValidationType.TYPE_FORMAT, "NonNegativeSchedule", path, "");
			})
			.collect(toList());
	}
