...
public class ClearingStatusTypeFormatValidator implements Validator<ClearingStatus> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingStatus", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingStatus", path, "", res.getError());
				}
				return success("ClearingStatus", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingStatus", path, "");
			})
			.collect(toList());
	}
