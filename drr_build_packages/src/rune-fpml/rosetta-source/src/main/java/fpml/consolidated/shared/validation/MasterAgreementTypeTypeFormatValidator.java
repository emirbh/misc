		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementType", ValidationResult.ValidationType.TYPE_FORMAT, "MasterAgreementType", path, "", res.getError());
				}
				return success("MasterAgreementType", ValidationResult.ValidationType.TYPE_FORMAT, "MasterAgreementType", path, "");
			})
			.collect(toList());
	}
