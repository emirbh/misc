...
public class LoanBulkServicingNotificationTypeFormatValidator implements Validator<LoanBulkServicingNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanBulkServicingNotification", ValidationResult.ValidationType.TYPE_FORMAT, "LoanBulkServicingNotification", path, "", res.getError());
				}
				return success("LoanBulkServicingNotification", ValidationResult.ValidationType.TYPE_FORMAT, "LoanBulkServicingNotification", path, "");
			})
			.collect(toList());
	}
