		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityNotification", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityNotification", path, "", res.getError());
				}
				return success("AbstractFacilityNotification", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityNotification", path, "");
			})
			.collect(toList());
	}
