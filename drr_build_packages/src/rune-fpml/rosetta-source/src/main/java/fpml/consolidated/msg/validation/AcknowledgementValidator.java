		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Acknowledgement", ValidationResult.ValidationType.CARDINALITY, "Acknowledgement", path, "", res.getError());
				}
				return success("Acknowledgement", ValidationResult.ValidationType.CARDINALITY, "Acknowledgement", path, "");
			})
			.collect(toList());
	}
