		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureProperty", ValidationResult.ValidationType.TYPE_FORMAT, "SignatureProperty", path, "", res.getError());
				}
				return success("SignatureProperty", ValidationResult.ValidationType.TYPE_FORMAT, "SignatureProperty", path, "");
			})
			.collect(toList());
	}
