		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClassifiablePayment", ValidationResult.ValidationType.TYPE_FORMAT, "ClassifiablePayment", path, "", res.getError());
				}
				return success("ClassifiablePayment", ValidationResult.ValidationType.TYPE_FORMAT, "ClassifiablePayment", path, "");
			})
			.collect(toList());
	}
