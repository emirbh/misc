...
public class QueryPortfolioValidator implements Validator<QueryPortfolio> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QueryPortfolio", ValidationResult.ValidationType.CARDINALITY, "QueryPortfolio", path, "", res.getError());
				}
				return success("QueryPortfolio", ValidationResult.ValidationType.CARDINALITY, "QueryPortfolio", path, "");
			})
			.collect(toList());
	}
