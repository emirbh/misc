		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Equity", ValidationResult.ValidationType.TYPE_FORMAT, "Equity", path, "", res.getError());
				}
				return success("Equity", ValidationResult.ValidationType.TYPE_FORMAT, "Equity", path, "");
			})
			.collect(toList());
	}
