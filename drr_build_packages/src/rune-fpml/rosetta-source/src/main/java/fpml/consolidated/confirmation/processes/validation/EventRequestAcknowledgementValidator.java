...
public class EventRequestAcknowledgementValidator implements Validator<EventRequestAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventRequestAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "EventRequestAcknowledgement", path, "", res.getError());
				}
				return success("EventRequestAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "EventRequestAcknowledgement", path, "");
			})
			.collect(toList());
	}
