		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasket", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasket", path, "", res.getError());
				}
				return success("CommodityBasket", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasket", path, "");
			})
			.collect(toList());
	}
