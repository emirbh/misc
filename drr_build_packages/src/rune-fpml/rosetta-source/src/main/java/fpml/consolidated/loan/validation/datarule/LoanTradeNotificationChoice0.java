 ...
 * @version 2.1.1
 */
@RosettaDataRule("LoanTradeNotificationChoice0")
@ImplementedBy(LoanTradeNotificationChoice0.Default.class)
public interface LoanTradeNotificationChoice0 extends Validator<LoanTradeNotification> {
	
	String NAME = "LoanTradeNotificationChoice0";
	String DEFINITION = "required choice loanTradeEventGroup, settlementTask";
	
	class Default implements LoanTradeNotificationChoice0 {
		...
		private ComparisonResult executeDataRule(LoanTradeNotification loanTradeNotification) {
			try {
				return choice(MapperS.of(loanTradeNotification), Arrays.asList("loanTradeEventGroup", "settlementTask"), ChoiceRuleValidationMethod.REQUIRED);
			}
			catch (Exception ex) {
	...
	class NoOp implements LoanTradeNotificationChoice0 {
