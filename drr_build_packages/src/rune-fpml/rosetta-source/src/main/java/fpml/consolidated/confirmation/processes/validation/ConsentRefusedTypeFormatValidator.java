...
public class ConsentRefusedTypeFormatValidator implements Validator<ConsentRefused> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentRefused", ValidationResult.ValidationType.TYPE_FORMAT, "ConsentRefused", path, "", res.getError());
				}
				return success("ConsentRefused", ValidationResult.ValidationType.TYPE_FORMAT, "ConsentRefused", path, "");
			})
			.collect(toList());
	}
