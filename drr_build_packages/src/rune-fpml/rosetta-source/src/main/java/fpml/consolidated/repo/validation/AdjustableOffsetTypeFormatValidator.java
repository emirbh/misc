		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableOffset", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableOffset", path, "", res.getError());
				}
				return success("AdjustableOffset", ValidationResult.ValidationType.TYPE_FORMAT, "AdjustableOffset", path, "");
			})
			.collect(toList());
	}
