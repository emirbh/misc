...
public class RequestRegulatoryReportingStatusTypeFormatValidator implements Validator<RequestRegulatoryReportingStatus> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestRegulatoryReportingStatus", ValidationResult.ValidationType.TYPE_FORMAT, "RequestRegulatoryReportingStatus", path, "", res.getError());
				}
				return success("RequestRegulatoryReportingStatus", ValidationResult.ValidationType.TYPE_FORMAT, "RequestRegulatoryReportingStatus", path, "");
			})
			.collect(toList());
	}
