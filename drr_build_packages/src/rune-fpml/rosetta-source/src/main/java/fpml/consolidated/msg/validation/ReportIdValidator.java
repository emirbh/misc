		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportId", ValidationResult.ValidationType.CARDINALITY, "ReportId", path, "", res.getError());
				}
				return success("ReportId", ValidationResult.ValidationType.CARDINALITY, "ReportId", path, "");
			})
			.collect(toList());
	}
