...
public class DigestMethodTypeTypeFormatValidator implements Validator<DigestMethodType> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DigestMethodType", ValidationResult.ValidationType.TYPE_FORMAT, "DigestMethodType", path, "", res.getError());
				}
				return success("DigestMethodType", ValidationResult.ValidationType.TYPE_FORMAT, "DigestMethodType", path, "");
			})
			.collect(toList());
	}
