...
public class AdditionalDisruptionEventsValidator implements Validator<AdditionalDisruptionEvents> {
	...
	private List<ComparisonResult> getComparisonResults(AdditionalDisruptionEvents o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("determiningPartyReference", (PartyReference) o.getDeterminingPartyReference() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdditionalDisruptionEvents", ValidationResult.ValidationType.CARDINALITY, "AdditionalDisruptionEvents", path, "", res.getError());
				}
				return success("AdditionalDisruptionEvents", ValidationResult.ValidationType.CARDINALITY, "AdditionalDisruptionEvents", path, "");
			})
			.collect(toList());
	}
