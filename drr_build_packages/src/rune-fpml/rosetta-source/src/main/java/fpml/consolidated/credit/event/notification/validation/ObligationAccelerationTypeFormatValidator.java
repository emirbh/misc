		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObligationAcceleration", ValidationResult.ValidationType.TYPE_FORMAT, "ObligationAcceleration", path, "", res.getError());
				}
				return success("ObligationAcceleration", ValidationResult.ValidationType.TYPE_FORMAT, "ObligationAcceleration", path, "");
			})
			.collect(toList());
	}
