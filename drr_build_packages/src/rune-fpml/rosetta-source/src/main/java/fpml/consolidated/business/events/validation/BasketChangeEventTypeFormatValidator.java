		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasketChangeEvent", ValidationResult.ValidationType.TYPE_FORMAT, "BasketChangeEvent", path, "", res.getError());
				}
				return success("BasketChangeEvent", ValidationResult.ValidationType.TYPE_FORMAT, "BasketChangeEvent", path, "");
			})
			.collect(toList());
	}
