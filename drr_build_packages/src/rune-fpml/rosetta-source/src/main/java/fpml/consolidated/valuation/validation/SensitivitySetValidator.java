		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SensitivitySet", ValidationResult.ValidationType.CARDINALITY, "SensitivitySet", path, "", res.getError());
				}
				return success("SensitivitySet", ValidationResult.ValidationType.CARDINALITY, "SensitivitySet", path, "");
			})
			.collect(toList());
	}
