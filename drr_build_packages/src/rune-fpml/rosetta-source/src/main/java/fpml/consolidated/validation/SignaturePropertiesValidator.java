...
public class SignaturePropertiesValidator implements Validator<SignatureProperties> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureProperties", ValidationResult.ValidationType.CARDINALITY, "SignatureProperties", path, "", res.getError());
				}
				return success("SignatureProperties", ValidationResult.ValidationType.CARDINALITY, "SignatureProperties", path, "");
			})
			.collect(toList());
	}
