		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OtherAgreementVersion", ValidationResult.ValidationType.CARDINALITY, "OtherAgreementVersion", path, "", res.getError());
				}
				return success("OtherAgreementVersion", ValidationResult.ValidationType.CARDINALITY, "OtherAgreementVersion", path, "");
			})
			.collect(toList());
	}
