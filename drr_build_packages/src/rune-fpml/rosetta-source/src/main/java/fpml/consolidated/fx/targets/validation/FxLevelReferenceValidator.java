		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxLevelReference", ValidationResult.ValidationType.CARDINALITY, "FxLevelReference", path, "", res.getError());
				}
				return success("FxLevelReference", ValidationResult.ValidationType.CARDINALITY, "FxLevelReference", path, "");
			})
			.collect(toList());
	}
