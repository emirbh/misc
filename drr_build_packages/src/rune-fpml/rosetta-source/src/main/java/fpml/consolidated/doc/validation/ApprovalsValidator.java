		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Approvals", ValidationResult.ValidationType.CARDINALITY, "Approvals", path, "", res.getError());
				}
				return success("Approvals", ValidationResult.ValidationType.CARDINALITY, "Approvals", path, "");
			})
			.collect(toList());
	}
