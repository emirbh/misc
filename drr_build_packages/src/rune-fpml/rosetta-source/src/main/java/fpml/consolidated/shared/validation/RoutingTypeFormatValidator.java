		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Routing", ValidationResult.ValidationType.TYPE_FORMAT, "Routing", path, "", res.getError());
				}
				return success("Routing", ValidationResult.ValidationType.TYPE_FORMAT, "Routing", path, "");
			})
			.collect(toList());
	}
