...
public class ClearingRefusedTypeFormatValidator implements Validator<ClearingRefused> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingRefused", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingRefused", path, "", res.getError());
				}
				return success("ClearingRefused", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingRefused", path, "");
			})
			.collect(toList());
	}
