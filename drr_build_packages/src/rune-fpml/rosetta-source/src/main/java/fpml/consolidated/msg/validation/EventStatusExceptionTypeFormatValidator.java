...
public class EventStatusExceptionTypeFormatValidator implements Validator<EventStatusException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventStatusException", ValidationResult.ValidationType.TYPE_FORMAT, "EventStatusException", path, "", res.getError());
				}
				return success("EventStatusException", ValidationResult.ValidationType.TYPE_FORMAT, "EventStatusException", path, "");
			})
			.collect(toList());
	}
