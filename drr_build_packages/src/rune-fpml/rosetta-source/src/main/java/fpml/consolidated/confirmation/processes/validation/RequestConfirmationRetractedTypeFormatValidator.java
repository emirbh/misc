		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConfirmationRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestConfirmationRetracted", path, "", res.getError());
				}
				return success("RequestConfirmationRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestConfirmationRetracted", path, "");
			})
			.collect(toList());
	}
