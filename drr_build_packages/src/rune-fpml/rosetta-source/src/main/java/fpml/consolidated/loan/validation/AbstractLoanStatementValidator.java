		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanStatement", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanStatement", path, "", res.getError());
				}
				return success("AbstractLoanStatement", ValidationResult.ValidationType.CARDINALITY, "AbstractLoanStatement", path, "");
			})
			.collect(toList());
	}
