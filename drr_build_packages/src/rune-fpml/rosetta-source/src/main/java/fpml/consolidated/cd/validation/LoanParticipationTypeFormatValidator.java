...
public class LoanParticipationTypeFormatValidator implements Validator<LoanParticipation> {
	...
	private List<ComparisonResult> getComparisonResults(LoanParticipation o) {
		return Lists.<ComparisonResult>newArrayList(
				checkString("qualifyingParticipationSeller", o.getQualifyingParticipationSeller(), 1, empty(), empty())
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanParticipation", ValidationResult.ValidationType.TYPE_FORMAT, "LoanParticipation", path, "", res.getError());
				}
				return success("LoanParticipation", ValidationResult.ValidationType.TYPE_FORMAT, "LoanParticipation", path, "");
			})
			.collect(toList());
	}
