...
public class AbstractFacilityPaymentEventValidator implements Validator<AbstractFacilityPaymentEvent> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractFacilityPaymentEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityPaymentEvent", path, "", res.getError());
				}
				return success("AbstractFacilityPaymentEvent", ValidationResult.ValidationType.CARDINALITY, "AbstractFacilityPaymentEvent", path, "");
			})
			.collect(toList());
	}
