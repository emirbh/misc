...
public class RequestExecutionRetractedValidator implements Validator<RequestExecutionRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestExecutionRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestExecutionRetracted", path, "", res.getError());
				}
				return success("RequestExecutionRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestExecutionRetracted", path, "");
			})
			.collect(toList());
	}
