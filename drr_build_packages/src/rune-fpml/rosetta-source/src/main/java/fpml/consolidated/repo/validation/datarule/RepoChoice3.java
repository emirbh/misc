 ...
 * @version 2.1.1
 */
@RosettaDataRule("RepoChoice3")
@ImplementedBy(RepoChoice3.Default.class)
public interface RepoChoice3 extends Validator<Repo> {
	
	String NAME = "RepoChoice3";
	String DEFINITION = "optional choice repoChoice, security, triParty";
	
	class Default implements RepoChoice3 {
		...
		private ComparisonResult executeDataRule(Repo repo) {
			try {
				return choice(MapperS.of(repo), Arrays.asList("repoChoice", "security", "triParty"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements RepoChoice3 {
