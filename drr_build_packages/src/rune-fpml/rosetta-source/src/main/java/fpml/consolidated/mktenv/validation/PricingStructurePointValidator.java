		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingStructurePoint", ValidationResult.ValidationType.CARDINALITY, "PricingStructurePoint", path, "", res.getError());
				}
				return success("PricingStructurePoint", ValidationResult.ValidationType.CARDINALITY, "PricingStructurePoint", path, "");
			})
			.collect(toList());
	}
