...
public class FxAveragingProcessValidator implements Validator<FxAveragingProcess> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAveragingProcess", ValidationResult.ValidationType.CARDINALITY, "FxAveragingProcess", path, "", res.getError());
				}
				return success("FxAveragingProcess", ValidationResult.ValidationType.CARDINALITY, "FxAveragingProcess", path, "");
			})
			.collect(toList());
	}
