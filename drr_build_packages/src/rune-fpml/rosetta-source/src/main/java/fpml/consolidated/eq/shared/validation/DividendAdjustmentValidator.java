		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DividendAdjustment", ValidationResult.ValidationType.CARDINALITY, "DividendAdjustment", path, "", res.getError());
				}
				return success("DividendAdjustment", ValidationResult.ValidationType.CARDINALITY, "DividendAdjustment", path, "");
			})
			.collect(toList());
	}
