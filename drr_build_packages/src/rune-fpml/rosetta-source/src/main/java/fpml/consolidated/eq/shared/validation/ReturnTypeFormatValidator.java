		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Return", ValidationResult.ValidationType.TYPE_FORMAT, "Return", path, "", res.getError());
				}
				return success("Return", ValidationResult.ValidationType.TYPE_FORMAT, "Return", path, "");
			})
			.collect(toList());
	}
