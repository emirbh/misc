		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OtcClassification", ValidationResult.ValidationType.CARDINALITY, "OtcClassification", path, "", res.getError());
				}
				return success("OtcClassification", ValidationResult.ValidationType.CARDINALITY, "OtcClassification", path, "");
			})
			.collect(toList());
	}
