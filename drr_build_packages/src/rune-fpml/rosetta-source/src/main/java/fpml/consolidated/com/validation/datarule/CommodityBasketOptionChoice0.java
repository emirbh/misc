 ...
 * @version 2.1.1
 */
@RosettaDataRule("CommodityBasketOptionChoice0")
@ImplementedBy(CommodityBasketOptionChoice0.Default.class)
public interface CommodityBasketOptionChoice0 extends Validator<CommodityBasketOption> {
	
	String NAME = "CommodityBasketOptionChoice0";
	String DEFINITION = "optional choice calculationPeriodsSchedule, calculationPeriods";
	
	class Default implements CommodityBasketOptionChoice0 {
		...
		private ComparisonResult executeDataRule(CommodityBasketOption commodityBasketOption) {
			try {
				return choice(MapperS.of(commodityBasketOption), Arrays.asList("calculationPeriodsSchedule", "calculationPeriods"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements CommodityBasketOptionChoice0 {
