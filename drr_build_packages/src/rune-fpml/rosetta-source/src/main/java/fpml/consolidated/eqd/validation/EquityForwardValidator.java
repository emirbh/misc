		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityForward", ValidationResult.ValidationType.CARDINALITY, "EquityForward", path, "", res.getError());
				}
				return success("EquityForward", ValidationResult.ValidationType.CARDINALITY, "EquityForward", path, "");
			})
			.collect(toList());
	}
