...
public class CommodityReturnLegValidator implements Validator<CommodityReturnLeg> {
	...
	private List<ComparisonResult> getComparisonResults(CommodityReturnLeg o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("commodityReturnCalculation", (CommodityReturnCalculation) o.getCommodityReturnCalculation() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityReturnLeg", ValidationResult.ValidationType.CARDINALITY, "CommodityReturnLeg", path, "", res.getError());
				}
				return success("CommodityReturnLeg", ValidationResult.ValidationType.CARDINALITY, "CommodityReturnLeg", path, "");
			})
			.collect(toList());
	}
