		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeSettlementTask", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeSettlementTask", path, "", res.getError());
				}
				return success("LoanTradeSettlementTask", ValidationResult.ValidationType.TYPE_FORMAT, "LoanTradeSettlementTask", path, "");
			})
			.collect(toList());
	}
