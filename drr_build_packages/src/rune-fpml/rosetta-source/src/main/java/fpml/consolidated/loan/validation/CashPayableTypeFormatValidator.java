		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CashPayable", ValidationResult.ValidationType.TYPE_FORMAT, "CashPayable", path, "", res.getError());
				}
				return success("CashPayable", ValidationResult.ValidationType.TYPE_FORMAT, "CashPayable", path, "");
			})
			.collect(toList());
	}
