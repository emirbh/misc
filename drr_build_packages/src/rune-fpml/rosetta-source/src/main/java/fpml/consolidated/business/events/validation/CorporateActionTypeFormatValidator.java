		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorporateAction", ValidationResult.ValidationType.TYPE_FORMAT, "CorporateAction", path, "", res.getError());
				}
				return success("CorporateAction", ValidationResult.ValidationType.TYPE_FORMAT, "CorporateAction", path, "");
			})
			.collect(toList());
	}
