		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationAccepted", ValidationResult.ValidationType.CARDINALITY, "CollateralAllocationAccepted", path, "", res.getError());
				}
				return success("CollateralAllocationAccepted", ValidationResult.ValidationType.CARDINALITY, "CollateralAllocationAccepted", path, "");
			})
			.collect(toList());
	}
