		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxOptionStrikePrice", ValidationResult.ValidationType.CARDINALITY, "FxOptionStrikePrice", path, "", res.getError());
				}
				return success("FxOptionStrikePrice", ValidationResult.ValidationType.CARDINALITY, "FxOptionStrikePrice", path, "");
			})
			.collect(toList());
	}
