		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxVolatilitySwap", ValidationResult.ValidationType.TYPE_FORMAT, "FxVolatilitySwap", path, "", res.getError());
				}
				return success("FxVolatilitySwap", ValidationResult.ValidationType.TYPE_FORMAT, "FxVolatilitySwap", path, "");
			})
			.collect(toList());
	}
