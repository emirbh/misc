...
public class ExceptionTypeFormatValidator implements Validator<fpml.consolidated.msg.Exception> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Exception", ValidationResult.ValidationType.TYPE_FORMAT, "Exception", path, "", res.getError());
				}
				return success("Exception", ValidationResult.ValidationType.TYPE_FORMAT, "Exception", path, "");
			})
			.collect(toList());
	}
