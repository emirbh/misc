		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestAccrualsCompoundingMethod", ValidationResult.ValidationType.TYPE_FORMAT, "InterestAccrualsCompoundingMethod", path, "", res.getError());
				}
				return success("InterestAccrualsCompoundingMethod", ValidationResult.ValidationType.TYPE_FORMAT, "InterestAccrualsCompoundingMethod", path, "");
			})
			.collect(toList());
	}
