...
public class ConfirmationDisputedTypeFormatValidator implements Validator<ConfirmationDisputed> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConfirmationDisputed", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationDisputed", path, "", res.getError());
				}
				return success("ConfirmationDisputed", ValidationResult.ValidationType.TYPE_FORMAT, "ConfirmationDisputed", path, "");
			})
			.collect(toList());
	}
