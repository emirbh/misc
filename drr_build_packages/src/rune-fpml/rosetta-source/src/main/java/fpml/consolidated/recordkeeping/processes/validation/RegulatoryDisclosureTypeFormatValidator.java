...
public class RegulatoryDisclosureTypeFormatValidator implements Validator<RegulatoryDisclosure> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RegulatoryDisclosure", ValidationResult.ValidationType.TYPE_FORMAT, "RegulatoryDisclosure", path, "", res.getError());
				}
				return success("RegulatoryDisclosure", ValidationResult.ValidationType.TYPE_FORMAT, "RegulatoryDisclosure", path, "");
			})
			.collect(toList());
	}
