		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxCurveValuation", ValidationResult.ValidationType.TYPE_FORMAT, "FxCurveValuation", path, "", res.getError());
				}
				return success("FxCurveValuation", ValidationResult.ValidationType.TYPE_FORMAT, "FxCurveValuation", path, "");
			})
			.collect(toList());
	}
