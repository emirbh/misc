		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotDomesticCurrency", ValidationResult.ValidationType.TYPE_FORMAT, "NotDomesticCurrency", path, "", res.getError());
				}
				return success("NotDomesticCurrency", ValidationResult.ValidationType.TYPE_FORMAT, "NotDomesticCurrency", path, "");
			})
			.collect(toList());
	}
