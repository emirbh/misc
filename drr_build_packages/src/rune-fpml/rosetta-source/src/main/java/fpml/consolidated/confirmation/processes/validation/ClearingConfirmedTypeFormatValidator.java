		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingConfirmed", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingConfirmed", path, "", res.getError());
				}
				return success("ClearingConfirmed", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingConfirmed", path, "");
			})
			.collect(toList());
	}
