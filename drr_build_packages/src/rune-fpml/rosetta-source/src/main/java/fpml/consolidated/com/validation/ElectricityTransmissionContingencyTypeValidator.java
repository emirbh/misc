		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ElectricityTransmissionContingencyType", ValidationResult.ValidationType.CARDINALITY, "ElectricityTransmissionContingencyType", path, "", res.getError());
				}
				return success("ElectricityTransmissionContingencyType", ValidationResult.ValidationType.CARDINALITY, "ElectricityTransmissionContingencyType", path, "");
			})
			.collect(toList());
	}
