		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdditionalFixedPayments", ValidationResult.ValidationType.CARDINALITY, "AdditionalFixedPayments", path, "", res.getError());
				}
				return success("AdditionalFixedPayments", ValidationResult.ValidationType.CARDINALITY, "AdditionalFixedPayments", path, "");
			})
			.collect(toList());
	}
