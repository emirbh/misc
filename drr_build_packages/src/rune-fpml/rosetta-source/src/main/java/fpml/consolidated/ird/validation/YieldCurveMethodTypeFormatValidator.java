		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("YieldCurveMethod", ValidationResult.ValidationType.TYPE_FORMAT, "YieldCurveMethod", path, "", res.getError());
				}
				return success("YieldCurveMethod", ValidationResult.ValidationType.TYPE_FORMAT, "YieldCurveMethod", path, "");
			})
			.collect(toList());
	}
