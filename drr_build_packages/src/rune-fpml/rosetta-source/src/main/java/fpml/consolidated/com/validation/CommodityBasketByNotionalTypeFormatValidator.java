		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasketByNotional", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasketByNotional", path, "", res.getError());
				}
				return success("CommodityBasketByNotional", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasketByNotional", path, "");
			})
			.collect(toList());
	}
