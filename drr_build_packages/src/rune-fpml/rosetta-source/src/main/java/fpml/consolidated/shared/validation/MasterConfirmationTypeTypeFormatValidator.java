		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterConfirmationType", ValidationResult.ValidationType.TYPE_FORMAT, "MasterConfirmationType", path, "", res.getError());
				}
				return success("MasterConfirmationType", ValidationResult.ValidationType.TYPE_FORMAT, "MasterConfirmationType", path, "");
			})
			.collect(toList());
	}
