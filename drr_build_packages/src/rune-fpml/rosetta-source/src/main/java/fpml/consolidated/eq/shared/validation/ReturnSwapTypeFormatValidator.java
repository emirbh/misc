		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReturnSwap", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnSwap", path, "", res.getError());
				}
				return success("ReturnSwap", ValidationResult.ValidationType.TYPE_FORMAT, "ReturnSwap", path, "");
			})
			.collect(toList());
	}
