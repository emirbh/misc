		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MessageRejected", ValidationResult.ValidationType.TYPE_FORMAT, "MessageRejected", path, "", res.getError());
				}
				return success("MessageRejected", ValidationResult.ValidationType.TYPE_FORMAT, "MessageRejected", path, "");
			})
			.collect(toList());
	}
