		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPhysicalExercise", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityPhysicalExercise", path, "", res.getError());
				}
				return success("CommodityPhysicalExercise", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityPhysicalExercise", path, "");
			})
			.collect(toList());
	}
