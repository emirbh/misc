		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxKnockoutCount", ValidationResult.ValidationType.CARDINALITY, "FxKnockoutCount", path, "", res.getError());
				}
				return success("FxKnockoutCount", ValidationResult.ValidationType.CARDINALITY, "FxKnockoutCount", path, "");
			})
			.collect(toList());
	}
