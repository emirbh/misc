		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyPortfolioName", ValidationResult.ValidationType.TYPE_FORMAT, "PartyPortfolioName", path, "", res.getError());
				}
				return success("PartyPortfolioName", ValidationResult.ValidationType.TYPE_FORMAT, "PartyPortfolioName", path, "");
			})
			.collect(toList());
	}
