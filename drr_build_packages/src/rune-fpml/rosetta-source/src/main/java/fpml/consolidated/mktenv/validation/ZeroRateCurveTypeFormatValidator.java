		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ZeroRateCurve", ValidationResult.ValidationType.TYPE_FORMAT, "ZeroRateCurve", path, "", res.getError());
				}
				return success("ZeroRateCurve", ValidationResult.ValidationType.TYPE_FORMAT, "ZeroRateCurve", path, "");
			})
			.collect(toList());
	}
