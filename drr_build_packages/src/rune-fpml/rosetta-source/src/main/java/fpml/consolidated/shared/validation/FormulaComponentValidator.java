...
public class FormulaComponentValidator implements Validator<FormulaComponent> {
	...
	private List<ComparisonResult> getComparisonResults(FormulaComponent o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("componentDescription", (String) o.getComponentDescription() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FormulaComponent", ValidationResult.ValidationType.CARDINALITY, "FormulaComponent", path, "", res.getError());
				}
				return success("FormulaComponent", ValidationResult.ValidationType.CARDINALITY, "FormulaComponent", path, "");
			})
			.collect(toList());
	}
