		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityLeg", ValidationResult.ValidationType.TYPE_FORMAT, "VolatilityLeg", path, "", res.getError());
				}
				return success("VolatilityLeg", ValidationResult.ValidationType.TYPE_FORMAT, "VolatilityLeg", path, "");
			})
			.collect(toList());
	}
