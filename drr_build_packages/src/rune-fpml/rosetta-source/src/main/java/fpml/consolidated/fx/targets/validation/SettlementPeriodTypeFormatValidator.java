		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SettlementPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPeriod", path, "", res.getError());
				}
				return success("SettlementPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "SettlementPeriod", path, "");
			})
			.collect(toList());
	}
