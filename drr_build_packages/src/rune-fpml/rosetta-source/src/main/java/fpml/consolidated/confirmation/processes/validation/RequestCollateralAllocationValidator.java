...
public class RequestCollateralAllocationValidator implements Validator<RequestCollateralAllocation> {
	...
	private List<ComparisonResult> getComparisonResults(RequestCollateralAllocation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("collateralAllocation", (List<? extends ProposedCollateralAllocation>) o.getCollateralAllocation() == null ? 0 : o.getCollateralAllocation().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestCollateralAllocation", ValidationResult.ValidationType.CARDINALITY, "RequestCollateralAllocation", path, "", res.getError());
				}
				return success("RequestCollateralAllocation", ValidationResult.ValidationType.CARDINALITY, "RequestCollateralAllocation", path, "");
			})
			.collect(toList());
	}
