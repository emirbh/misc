		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdditionalEvent", ValidationResult.ValidationType.TYPE_FORMAT, "AdditionalEvent", path, "", res.getError());
				}
				return success("AdditionalEvent", ValidationResult.ValidationType.TYPE_FORMAT, "AdditionalEvent", path, "");
			})
			.collect(toList());
	}
