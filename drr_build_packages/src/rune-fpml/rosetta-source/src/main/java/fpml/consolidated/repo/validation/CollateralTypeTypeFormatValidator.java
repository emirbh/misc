		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralType", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralType", path, "", res.getError());
				}
				return success("CollateralType", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralType", path, "");
			})
			.collect(toList());
	}
