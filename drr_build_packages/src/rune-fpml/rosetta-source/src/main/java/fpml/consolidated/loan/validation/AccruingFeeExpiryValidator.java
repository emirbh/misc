...
public class AccruingFeeExpiryValidator implements Validator<AccruingFeeExpiry> {
	...
	private List<ComparisonResult> getComparisonResults(AccruingFeeExpiry o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("type", (AccruingFeeType) o._getType() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccruingFeeExpiry", ValidationResult.ValidationType.CARDINALITY, "AccruingFeeExpiry", path, "", res.getError());
				}
				return success("AccruingFeeExpiry", ValidationResult.ValidationType.CARDINALITY, "AccruingFeeExpiry", path, "");
			})
			.collect(toList());
	}
