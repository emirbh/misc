...
public class NonCorrectableRequestMessageValidator implements Validator<NonCorrectableRequestMessage> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NonCorrectableRequestMessage", ValidationResult.ValidationType.CARDINALITY, "NonCorrectableRequestMessage", path, "", res.getError());
				}
				return success("NonCorrectableRequestMessage", ValidationResult.ValidationType.CARDINALITY, "NonCorrectableRequestMessage", path, "");
			})
			.collect(toList());
	}
