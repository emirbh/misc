		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MessageRejected", ValidationResult.ValidationType.CARDINALITY, "MessageRejected", path, "", res.getError());
				}
				return success("MessageRejected", ValidationResult.ValidationType.CARDINALITY, "MessageRejected", path, "");
			})
			.collect(toList());
	}
