		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "CoalDelivery", path, "", res.getError());
				}
				return success("CoalDelivery", ValidationResult.ValidationType.TYPE_FORMAT, "CoalDelivery", path, "");
			})
			.collect(toList());
	}
