		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAverageRateObservation", ValidationResult.ValidationType.TYPE_FORMAT, "FxAverageRateObservation", path, "", res.getError());
				}
				return success("FxAverageRateObservation", ValidationResult.ValidationType.TYPE_FORMAT, "FxAverageRateObservation", path, "");
			})
			.collect(toList());
	}
