		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Correlation", ValidationResult.ValidationType.CARDINALITY, "Correlation", path, "", res.getError());
				}
				return success("Correlation", ValidationResult.ValidationType.CARDINALITY, "Correlation", path, "");
			})
			.collect(toList());
	}
