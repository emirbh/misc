...
public class FacilityPositionStatementValidator implements Validator<FacilityPositionStatement> {
	...
	private List<ComparisonResult> getComparisonResults(FacilityPositionStatement o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("positionPartyReference", (PartyReference) o.getPositionPartyReference() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FacilityPositionStatement", ValidationResult.ValidationType.CARDINALITY, "FacilityPositionStatement", path, "", res.getError());
				}
				return success("FacilityPositionStatement", ValidationResult.ValidationType.CARDINALITY, "FacilityPositionStatement", path, "");
			})
			.collect(toList());
	}
