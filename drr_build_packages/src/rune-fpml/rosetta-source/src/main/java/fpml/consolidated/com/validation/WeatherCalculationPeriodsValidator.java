		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WeatherCalculationPeriods", ValidationResult.ValidationType.CARDINALITY, "WeatherCalculationPeriods", path, "", res.getError());
				}
				return success("WeatherCalculationPeriods", ValidationResult.ValidationType.CARDINALITY, "WeatherCalculationPeriods", path, "");
			})
			.collect(toList());
	}
