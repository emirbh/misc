...
public class LoanLegalStructureBeforeChoiceValidator implements Validator<LoanLegalStructureBeforeChoice> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanLegalStructureBeforeChoice", ValidationResult.ValidationType.CARDINALITY, "LoanLegalStructureBeforeChoice", path, "", res.getError());
				}
				return success("LoanLegalStructureBeforeChoice", ValidationResult.ValidationType.CARDINALITY, "LoanLegalStructureBeforeChoice", path, "");
			})
			.collect(toList());
	}
