		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InstrumentTradeDetails", ValidationResult.ValidationType.CARDINALITY, "InstrumentTradeDetails", path, "", res.getError());
				}
				return success("InstrumentTradeDetails", ValidationResult.ValidationType.CARDINALITY, "InstrumentTradeDetails", path, "");
			})
			.collect(toList());
	}
