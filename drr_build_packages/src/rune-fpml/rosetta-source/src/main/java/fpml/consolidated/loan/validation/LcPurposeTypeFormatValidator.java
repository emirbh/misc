		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcPurpose", ValidationResult.ValidationType.TYPE_FORMAT, "LcPurpose", path, "", res.getError());
				}
				return success("LcPurpose", ValidationResult.ValidationType.TYPE_FORMAT, "LcPurpose", path, "");
			})
			.collect(toList());
	}
