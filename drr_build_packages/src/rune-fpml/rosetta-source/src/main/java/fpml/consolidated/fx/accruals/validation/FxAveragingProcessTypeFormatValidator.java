		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAveragingProcess", ValidationResult.ValidationType.TYPE_FORMAT, "FxAveragingProcess", path, "", res.getError());
				}
				return success("FxAveragingProcess", ValidationResult.ValidationType.TYPE_FORMAT, "FxAveragingProcess", path, "");
			})
			.collect(toList());
	}
