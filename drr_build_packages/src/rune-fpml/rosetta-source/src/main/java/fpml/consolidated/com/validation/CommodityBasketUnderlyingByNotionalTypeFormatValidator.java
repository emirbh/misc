		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityBasketUnderlyingByNotional", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasketUnderlyingByNotional", path, "", res.getError());
				}
				return success("CommodityBasketUnderlyingByNotional", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityBasketUnderlyingByNotional", path, "");
			})
			.collect(toList());
	}
