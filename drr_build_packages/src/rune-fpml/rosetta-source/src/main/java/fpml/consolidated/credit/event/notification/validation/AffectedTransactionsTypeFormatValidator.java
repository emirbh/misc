		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AffectedTransactions", ValidationResult.ValidationType.TYPE_FORMAT, "AffectedTransactions", path, "", res.getError());
				}
				return success("AffectedTransactions", ValidationResult.ValidationType.TYPE_FORMAT, "AffectedTransactions", path, "");
			})
			.collect(toList());
	}
