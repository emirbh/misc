		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTrade", ValidationResult.ValidationType.CARDINALITY, "LoanTrade", path, "", res.getError());
				}
				return success("LoanTrade", ValidationResult.ValidationType.CARDINALITY, "LoanTrade", path, "");
			})
			.collect(toList());
	}
