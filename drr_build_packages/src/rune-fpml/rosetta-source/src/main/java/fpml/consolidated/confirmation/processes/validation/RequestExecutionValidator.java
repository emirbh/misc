...
public class RequestExecutionValidator implements Validator<RequestExecution> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestExecution", ValidationResult.ValidationType.CARDINALITY, "RequestExecution", path, "", res.getError());
				}
				return success("RequestExecution", ValidationResult.ValidationType.CARDINALITY, "RequestExecution", path, "");
			})
			.collect(toList());
	}
