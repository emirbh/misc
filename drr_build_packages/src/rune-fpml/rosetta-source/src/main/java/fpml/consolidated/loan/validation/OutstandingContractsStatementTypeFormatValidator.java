...
public class OutstandingContractsStatementTypeFormatValidator implements Validator<OutstandingContractsStatement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OutstandingContractsStatement", ValidationResult.ValidationType.TYPE_FORMAT, "OutstandingContractsStatement", path, "", res.getError());
				}
				return success("OutstandingContractsStatement", ValidationResult.ValidationType.TYPE_FORMAT, "OutstandingContractsStatement", path, "");
			})
			.collect(toList());
	}
