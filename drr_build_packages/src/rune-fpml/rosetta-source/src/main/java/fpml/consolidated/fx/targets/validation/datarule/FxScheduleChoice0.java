 ...
 * @version 2.1.1
 */
@RosettaDataRule("FxScheduleChoice0")
@ImplementedBy(FxScheduleChoice0.Default.class)
public interface FxScheduleChoice0 extends Validator<FxSchedule> {
	
	String NAME = "FxScheduleChoice0";
	String DEFINITION = "optional choice dateAdjustments, businessCentersReference, businessCenters, dateOffset";
	
	class Default implements FxScheduleChoice0 {
		...
		private ComparisonResult executeDataRule(FxSchedule fxSchedule) {
			try {
				return choice(MapperS.of(fxSchedule), Arrays.asList("dateAdjustments", "businessCentersReference", "businessCenters", "dateOffset"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements FxScheduleChoice0 {
