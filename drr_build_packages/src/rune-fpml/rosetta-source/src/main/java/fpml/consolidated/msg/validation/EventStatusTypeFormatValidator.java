		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventStatus", ValidationResult.ValidationType.TYPE_FORMAT, "EventStatus", path, "", res.getError());
				}
				return success("EventStatus", ValidationResult.ValidationType.TYPE_FORMAT, "EventStatus", path, "");
			})
			.collect(toList());
	}
