...
public class RolloverValidator implements Validator<Rollover> {
	...
	private List<ComparisonResult> getComparisonResults(Rollover o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("currentContracts", (LoanContractsReference) o.getCurrentContracts() != null ? 1 : 0, 1, 1), 
				checkCardinality("newContracts", (LoanContractList) o.getNewContracts() != null ? 1 : 0, 1, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Rollover", ValidationResult.ValidationType.CARDINALITY, "Rollover", path, "", res.getError());
				}
				return success("Rollover", ValidationResult.ValidationType.CARDINALITY, "Rollover", path, "");
			})
			.collect(toList());
	}
