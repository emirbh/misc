...
public class FxTargetSettlementPeriodScheduleValidator implements Validator<FxTargetSettlementPeriodSchedule> {
	...
	private List<ComparisonResult> getComparisonResults(FxTargetSettlementPeriodSchedule o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("settlementPeriod", (List<? extends FxTargetSettlementPeriod>) o.getSettlementPeriod() == null ? 0 : o.getSettlementPeriod().size(), 1, 0)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxTargetSettlementPeriodSchedule", ValidationResult.ValidationType.CARDINALITY, "FxTargetSettlementPeriodSchedule", path, "", res.getError());
				}
				return success("FxTargetSettlementPeriodSchedule", ValidationResult.ValidationType.CARDINALITY, "FxTargetSettlementPeriodSchedule", path, "");
			})
			.collect(toList());
	}
