		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EntityName", ValidationResult.ValidationType.TYPE_FORMAT, "EntityName", path, "", res.getError());
				}
				return success("EntityName", ValidationResult.ValidationType.TYPE_FORMAT, "EntityName", path, "");
			})
			.collect(toList());
	}
