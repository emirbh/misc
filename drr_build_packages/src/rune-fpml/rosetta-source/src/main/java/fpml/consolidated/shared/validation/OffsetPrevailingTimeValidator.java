...
public class OffsetPrevailingTimeValidator implements Validator<OffsetPrevailingTime> {
	...
	private List<ComparisonResult> getComparisonResults(OffsetPrevailingTime o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("time", (PrevailingTime) o.getTime() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OffsetPrevailingTime", ValidationResult.ValidationType.CARDINALITY, "OffsetPrevailingTime", path, "", res.getError());
				}
				return success("OffsetPrevailingTime", ValidationResult.ValidationType.CARDINALITY, "OffsetPrevailingTime", path, "");
			})
			.collect(toList());
	}
