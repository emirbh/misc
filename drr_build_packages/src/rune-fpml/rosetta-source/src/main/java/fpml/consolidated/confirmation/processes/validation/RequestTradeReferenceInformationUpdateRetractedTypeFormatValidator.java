...
public class RequestTradeReferenceInformationUpdateRetractedTypeFormatValidator implements Validator<RequestTradeReferenceInformationUpdateRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestTradeReferenceInformationUpdateRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdateRetracted", path, "", res.getError());
				}
				return success("RequestTradeReferenceInformationUpdateRetracted", ValidationResult.ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdateRetracted", path, "");
			})
			.collect(toList());
	}
