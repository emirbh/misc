		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeChangeAdviceException", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdviceException", path, "", res.getError());
				}
				return success("TradeChangeAdviceException", ValidationResult.ValidationType.TYPE_FORMAT, "TradeChangeAdviceException", path, "");
			})
			.collect(toList());
	}
