...
public class CreditEventNotificationTypeFormatValidator implements Validator<CreditEventNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventNotification", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventNotification", path, "", res.getError());
				}
				return success("CreditEventNotification", ValidationResult.ValidationType.TYPE_FORMAT, "CreditEventNotification", path, "");
			})
			.collect(toList());
	}
