		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResourceType", ValidationResult.ValidationType.TYPE_FORMAT, "ResourceType", path, "", res.getError());
				}
				return success("ResourceType", ValidationResult.ValidationType.TYPE_FORMAT, "ResourceType", path, "");
			})
			.collect(toList());
	}
