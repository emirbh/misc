...
public class OptionExpirationNotificationTypeFormatValidator implements Validator<OptionExpirationNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionExpirationNotification", ValidationResult.ValidationType.TYPE_FORMAT, "OptionExpirationNotification", path, "", res.getError());
				}
				return success("OptionExpirationNotification", ValidationResult.ValidationType.TYPE_FORMAT, "OptionExpirationNotification", path, "");
			})
			.collect(toList());
	}
