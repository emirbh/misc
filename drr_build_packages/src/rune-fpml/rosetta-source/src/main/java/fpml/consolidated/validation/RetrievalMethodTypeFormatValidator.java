...
public class RetrievalMethodTypeFormatValidator implements Validator<RetrievalMethod> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RetrievalMethod", ValidationResult.ValidationType.TYPE_FORMAT, "RetrievalMethod", path, "", res.getError());
				}
				return success("RetrievalMethod", ValidationResult.ValidationType.TYPE_FORMAT, "RetrievalMethod", path, "");
			})
			.collect(toList());
	}
