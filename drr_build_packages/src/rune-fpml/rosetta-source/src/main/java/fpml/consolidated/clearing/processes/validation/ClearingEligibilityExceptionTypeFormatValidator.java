		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingEligibilityException", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingEligibilityException", path, "", res.getError());
				}
				return success("ClearingEligibilityException", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingEligibilityException", path, "");
			})
			.collect(toList());
	}
