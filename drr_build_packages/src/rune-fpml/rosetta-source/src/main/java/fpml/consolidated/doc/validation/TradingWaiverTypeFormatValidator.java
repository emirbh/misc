		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradingWaiver", ValidationResult.ValidationType.TYPE_FORMAT, "TradingWaiver", path, "", res.getError());
				}
				return success("TradingWaiver", ValidationResult.ValidationType.TYPE_FORMAT, "TradingWaiver", path, "");
			})
			.collect(toList());
	}
