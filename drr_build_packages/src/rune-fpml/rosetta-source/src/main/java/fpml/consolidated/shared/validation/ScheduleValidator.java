		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Schedule", ValidationResult.ValidationType.CARDINALITY, "Schedule", path, "", res.getError());
				}
				return success("Schedule", ValidationResult.ValidationType.CARDINALITY, "Schedule", path, "");
			})
			.collect(toList());
	}
