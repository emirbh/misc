		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MoneyWithParticipantShare", ValidationResult.ValidationType.CARDINALITY, "MoneyWithParticipantShare", path, "", res.getError());
				}
				return success("MoneyWithParticipantShare", ValidationResult.ValidationType.CARDINALITY, "MoneyWithParticipantShare", path, "");
			})
			.collect(toList());
	}
