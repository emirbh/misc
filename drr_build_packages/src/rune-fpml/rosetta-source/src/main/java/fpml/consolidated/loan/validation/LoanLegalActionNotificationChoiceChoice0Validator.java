...
public class LoanLegalActionNotificationChoiceChoice0Validator implements Validator<LoanLegalActionNotificationChoiceChoice0> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanLegalActionNotificationChoiceChoice0", ValidationResult.ValidationType.CARDINALITY, "LoanLegalActionNotificationChoiceChoice0", path, "", res.getError());
				}
				return success("LoanLegalActionNotificationChoiceChoice0", ValidationResult.ValidationType.CARDINALITY, "LoanLegalActionNotificationChoiceChoice0", path, "");
			})
			.collect(toList());
	}
