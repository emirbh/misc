		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityDerivativeBase", ValidationResult.ValidationType.CARDINALITY, "EquityDerivativeBase", path, "", res.getError());
				}
				return success("EquityDerivativeBase", ValidationResult.ValidationType.CARDINALITY, "EquityDerivativeBase", path, "");
			})
			.collect(toList());
	}
