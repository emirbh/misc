...
public class BankruptcyValidator implements Validator<Bankruptcy> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Bankruptcy", ValidationResult.ValidationType.CARDINALITY, "Bankruptcy", path, "", res.getError());
				}
				return success("Bankruptcy", ValidationResult.ValidationType.CARDINALITY, "Bankruptcy", path, "");
			})
			.collect(toList());
	}
