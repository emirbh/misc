		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexReferenceInformation", ValidationResult.ValidationType.TYPE_FORMAT, "IndexReferenceInformation", path, "", res.getError());
				}
				return success("IndexReferenceInformation", ValidationResult.ValidationType.TYPE_FORMAT, "IndexReferenceInformation", path, "");
			})
			.collect(toList());
	}
