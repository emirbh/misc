		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccrualPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "AccrualPeriod", path, "", res.getError());
				}
				return success("AccrualPeriod", ValidationResult.ValidationType.TYPE_FORMAT, "AccrualPeriod", path, "");
			})
			.collect(toList());
	}
