		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PackageHeader", ValidationResult.ValidationType.TYPE_FORMAT, "PackageHeader", path, "", res.getError());
				}
				return success("PackageHeader", ValidationResult.ValidationType.TYPE_FORMAT, "PackageHeader", path, "");
			})
			.collect(toList());
	}
