...
public class LcIssuanceFeePaymentValidator implements Validator<LcIssuanceFeePayment> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LcIssuanceFeePayment", ValidationResult.ValidationType.CARDINALITY, "LcIssuanceFeePayment", path, "", res.getError());
				}
				return success("LcIssuanceFeePayment", ValidationResult.ValidationType.CARDINALITY, "LcIssuanceFeePayment", path, "");
			})
			.collect(toList());
	}
