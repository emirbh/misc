...
public class LoanPartyProfileStatementTypeFormatValidator implements Validator<LoanPartyProfileStatement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanPartyProfileStatement", ValidationResult.ValidationType.TYPE_FORMAT, "LoanPartyProfileStatement", path, "", res.getError());
				}
				return success("LoanPartyProfileStatement", ValidationResult.ValidationType.TYPE_FORMAT, "LoanPartyProfileStatement", path, "");
			})
			.collect(toList());
	}
