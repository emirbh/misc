		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriodDatesReference", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodDatesReference", path, "", res.getError());
				}
				return success("CalculationPeriodDatesReference", ValidationResult.ValidationType.TYPE_FORMAT, "CalculationPeriodDatesReference", path, "");
			})
			.collect(toList());
	}
