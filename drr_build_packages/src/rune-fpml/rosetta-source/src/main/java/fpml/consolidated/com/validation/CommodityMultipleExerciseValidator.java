...
public class CommodityMultipleExerciseValidator implements Validator<CommodityMultipleExercise> {
	...
	private List<ComparisonResult> getComparisonResults(CommodityMultipleExercise o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("minimumNotionalQuantity", (CommodityNotionalQuantity) o.getMinimumNotionalQuantity() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityMultipleExercise", ValidationResult.ValidationType.CARDINALITY, "CommodityMultipleExercise", path, "", res.getError());
				}
				return success("CommodityMultipleExercise", ValidationResult.ValidationType.CARDINALITY, "CommodityMultipleExercise", path, "");
			})
			.collect(toList());
	}
