...
public class CommodityNotionalAmountValidator implements Validator<CommodityNotionalAmount> {
	...
	private List<ComparisonResult> getComparisonResults(CommodityNotionalAmount o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("reinvestmentFeature", (Boolean) o.getReinvestmentFeature() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityNotionalAmount", ValidationResult.ValidationType.CARDINALITY, "CommodityNotionalAmount", path, "", res.getError());
				}
				return success("CommodityNotionalAmount", ValidationResult.ValidationType.CARDINALITY, "CommodityNotionalAmount", path, "");
			})
			.collect(toList());
	}
