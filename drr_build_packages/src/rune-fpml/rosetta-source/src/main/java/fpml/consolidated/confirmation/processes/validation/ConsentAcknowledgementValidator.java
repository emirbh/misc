...
public class ConsentAcknowledgementValidator implements Validator<ConsentAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "ConsentAcknowledgement", path, "", res.getError());
				}
				return success("ConsentAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "ConsentAcknowledgement", path, "");
			})
			.collect(toList());
	}
