		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestTradeReferenceInformationUpdate", ValidationResult.ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdate", path, "", res.getError());
				}
				return success("RequestTradeReferenceInformationUpdate", ValidationResult.ValidationType.TYPE_FORMAT, "RequestTradeReferenceInformationUpdate", path, "");
			})
			.collect(toList());
	}
