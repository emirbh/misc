		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementId", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementId", path, "", res.getError());
				}
				return success("MasterAgreementId", ValidationResult.ValidationType.CARDINALITY, "MasterAgreementId", path, "");
			})
			.collect(toList());
	}
