		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxFallbackReferencePrice", ValidationResult.ValidationType.CARDINALITY, "FxFallbackReferencePrice", path, "", res.getError());
				}
				return success("FxFallbackReferencePrice", ValidationResult.ValidationType.CARDINALITY, "FxFallbackReferencePrice", path, "");
			})
			.collect(toList());
	}
