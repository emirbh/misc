		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TimestampTypeScheme", ValidationResult.ValidationType.TYPE_FORMAT, "TimestampTypeScheme", path, "", res.getError());
				}
				return success("TimestampTypeScheme", ValidationResult.ValidationType.TYPE_FORMAT, "TimestampTypeScheme", path, "");
			})
			.collect(toList());
	}
