		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AccountName", ValidationResult.ValidationType.TYPE_FORMAT, "AccountName", path, "", res.getError());
				}
				return success("AccountName", ValidationResult.ValidationType.TYPE_FORMAT, "AccountName", path, "");
			})
			.collect(toList());
	}
