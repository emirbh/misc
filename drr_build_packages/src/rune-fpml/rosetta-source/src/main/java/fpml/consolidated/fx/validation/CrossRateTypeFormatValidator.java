		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CrossRate", ValidationResult.ValidationType.TYPE_FORMAT, "CrossRate", path, "", res.getError());
				}
				return success("CrossRate", ValidationResult.ValidationType.TYPE_FORMAT, "CrossRate", path, "");
			})
			.collect(toList());
	}
