		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DividendPeriodDividend", ValidationResult.ValidationType.TYPE_FORMAT, "DividendPeriodDividend", path, "", res.getError());
				}
				return success("DividendPeriodDividend", ValidationResult.ValidationType.TYPE_FORMAT, "DividendPeriodDividend", path, "");
			})
			.collect(toList());
	}
