		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeCategory", ValidationResult.ValidationType.CARDINALITY, "TradeCategory", path, "", res.getError());
				}
				return success("TradeCategory", ValidationResult.ValidationType.CARDINALITY, "TradeCategory", path, "");
			})
			.collect(toList());
	}
