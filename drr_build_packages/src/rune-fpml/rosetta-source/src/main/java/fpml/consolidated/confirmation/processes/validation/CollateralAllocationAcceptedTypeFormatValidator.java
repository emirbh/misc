...
public class CollateralAllocationAcceptedTypeFormatValidator implements Validator<CollateralAllocationAccepted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralAllocationAccepted", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralAllocationAccepted", path, "", res.getError());
				}
				return success("CollateralAllocationAccepted", ValidationResult.ValidationType.TYPE_FORMAT, "CollateralAllocationAccepted", path, "");
			})
			.collect(toList());
	}
