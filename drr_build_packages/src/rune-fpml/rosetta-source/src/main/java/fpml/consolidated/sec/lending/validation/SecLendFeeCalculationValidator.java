...
public class SecLendFeeCalculationValidator implements Validator<SecLendFeeCalculation> {
	...
	private List<ComparisonResult> getComparisonResults(SecLendFeeCalculation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("minimumFee", (PositiveMoney) o.getMinimumFee() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SecLendFeeCalculation", ValidationResult.ValidationType.CARDINALITY, "SecLendFeeCalculation", path, "", res.getError());
				}
				return success("SecLendFeeCalculation", ValidationResult.ValidationType.CARDINALITY, "SecLendFeeCalculation", path, "");
			})
			.collect(toList());
	}
