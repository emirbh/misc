 ...
 * @version 2.1.1
 */
@RosettaDataRule("TradeNovationContentChoice0")
@ImplementedBy(TradeNovationContentChoice0.Default.class)
public interface TradeNovationContentChoice0 extends Validator<TradeNovationContent> {
	
	String NAME = "TradeNovationContentChoice0";
	String DEFINITION = "optional choice newTradeIdentifier, newTrade";
	
	class Default implements TradeNovationContentChoice0 {
		...
		private ComparisonResult executeDataRule(TradeNovationContent tradeNovationContent) {
			try {
				return choice(MapperS.of(tradeNovationContent), Arrays.asList("newTradeIdentifier", "newTrade"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements TradeNovationContentChoice0 {
