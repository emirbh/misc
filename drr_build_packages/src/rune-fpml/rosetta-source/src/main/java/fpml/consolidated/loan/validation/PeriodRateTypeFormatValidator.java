		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PeriodRate", ValidationResult.ValidationType.TYPE_FORMAT, "PeriodRate", path, "", res.getError());
				}
				return success("PeriodRate", ValidationResult.ValidationType.TYPE_FORMAT, "PeriodRate", path, "");
			})
			.collect(toList());
	}
