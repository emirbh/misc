		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MultiDimensionalPricingData", ValidationResult.ValidationType.CARDINALITY, "MultiDimensionalPricingData", path, "", res.getError());
				}
				return success("MultiDimensionalPricingData", ValidationResult.ValidationType.CARDINALITY, "MultiDimensionalPricingData", path, "");
			})
			.collect(toList());
	}
