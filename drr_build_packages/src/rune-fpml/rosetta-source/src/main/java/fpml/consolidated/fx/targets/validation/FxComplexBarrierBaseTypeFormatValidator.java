		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxComplexBarrierBase", ValidationResult.ValidationType.TYPE_FORMAT, "FxComplexBarrierBase", path, "", res.getError());
				}
				return success("FxComplexBarrierBase", ValidationResult.ValidationType.TYPE_FORMAT, "FxComplexBarrierBase", path, "");
			})
			.collect(toList());
	}
