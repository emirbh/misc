...
public class PartyMessageInformationValidator implements Validator<PartyMessageInformation> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyMessageInformation", ValidationResult.ValidationType.CARDINALITY, "PartyMessageInformation", path, "", res.getError());
				}
				return success("PartyMessageInformation", ValidationResult.ValidationType.CARDINALITY, "PartyMessageInformation", path, "");
			})
			.collect(toList());
	}
