		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ElectricityPhysicalLeg", ValidationResult.ValidationType.TYPE_FORMAT, "ElectricityPhysicalLeg", path, "", res.getError());
				}
				return success("ElectricityPhysicalLeg", ValidationResult.ValidationType.TYPE_FORMAT, "ElectricityPhysicalLeg", path, "");
			})
			.collect(toList());
	}
