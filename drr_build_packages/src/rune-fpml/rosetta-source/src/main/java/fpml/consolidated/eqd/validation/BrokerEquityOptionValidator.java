...
public class BrokerEquityOptionValidator implements Validator<BrokerEquityOption> {
	...
	private List<ComparisonResult> getComparisonResults(BrokerEquityOption o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("deltaCrossed", (Boolean) o.getDeltaCrossed() != null ? 1 : 0, 0, 1), 
				checkCardinality("brokerageFee", (Money) o.getBrokerageFee() != null ? 1 : 0, 0, 1), 
				checkCardinality("brokerNotes", (String) o.getBrokerNotes() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BrokerEquityOption", ValidationResult.ValidationType.CARDINALITY, "BrokerEquityOption", path, "", res.getError());
				}
				return success("BrokerEquityOption", ValidationResult.ValidationType.CARDINALITY, "BrokerEquityOption", path, "");
			})
			.collect(toList());
	}
