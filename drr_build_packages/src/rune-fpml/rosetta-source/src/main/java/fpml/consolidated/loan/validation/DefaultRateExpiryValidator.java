		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DefaultRateExpiry", ValidationResult.ValidationType.CARDINALITY, "DefaultRateExpiry", path, "", res.getError());
				}
				return success("DefaultRateExpiry", ValidationResult.ValidationType.CARDINALITY, "DefaultRateExpiry", path, "");
			})
			.collect(toList());
	}
