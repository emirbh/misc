		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalAttributeDecimal", ValidationResult.ValidationType.CARDINALITY, "CoalAttributeDecimal", path, "", res.getError());
				}
				return success("CoalAttributeDecimal", ValidationResult.ValidationType.CARDINALITY, "CoalAttributeDecimal", path, "");
			})
			.collect(toList());
	}
