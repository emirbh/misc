...
public class LoanTradeFeeOwedValidator implements Validator<LoanTradeFeeOwed> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanTradeFeeOwed", ValidationResult.ValidationType.CARDINALITY, "LoanTradeFeeOwed", path, "", res.getError());
				}
				return success("LoanTradeFeeOwed", ValidationResult.ValidationType.CARDINALITY, "LoanTradeFeeOwed", path, "");
			})
			.collect(toList());
	}
