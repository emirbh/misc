 ...
 * @version 2.1.1
 */
@RosettaDataRule("TradeNovationContentChoice2")
@ImplementedBy(TradeNovationContentChoice2.Default.class)
public interface TradeNovationContentChoice2 extends Validator<TradeNovationContent> {
	
	String NAME = "TradeNovationContentChoice2";
	String DEFINITION = "if novatedAmount exists then remainingNumberOfOptions is absent and remainingNumberOfUnits is absent and optional choice oldTradeIdentifier, oldTrade and if oldTradeIdentifier exists then optional choice feeTradeIdentifier, feeTrade and if oldTrade exists then optional choice feeTradeIdentifier, feeTrade and if oldTradeIdentifier is absent and oldTrade is absent then optional choice feeTradeIdentifier, feeTrade";
	
	class Default implements TradeNovationContentChoice2 {
		...
		private ComparisonResult executeDataRule(TradeNovationContent tradeNovationContent) {
			try {
				if (exists(MapperS.of(tradeNovationContent).<Money>mapC("getNovatedAmount", _tradeNovationContent -> _tradeNovationContent.getNovatedAmount())).getOrDefault(false)) {
	...
	class NoOp implements TradeNovationContentChoice2 {
