		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CoalProductSource", ValidationResult.ValidationType.TYPE_FORMAT, "CoalProductSource", path, "", res.getError());
				}
				return success("CoalProductSource", ValidationResult.ValidationType.TYPE_FORMAT, "CoalProductSource", path, "");
			})
			.collect(toList());
	}
