...
public class LoanContractNotificationValidator implements Validator<LoanContractNotification> {
	...
	private List<ComparisonResult> getComparisonResults(LoanContractNotification o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("loanContractEventGroup", (AbstractLoanServicingEvent) o.getLoanContractEventGroup() != null ? 1 : 0, 1, 1), 
				...
				checkCardinality("loanContractNotificationChoice", (List<? extends LoanContractNotificationChoice>) o.getLoanContractNotificationChoice() == null ? 0 : o.getLoanContractNotificationChoice().size(), 1, 0), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractNotification", ValidationResult.ValidationType.CARDINALITY, "LoanContractNotification", path, "", res.getError());
				}
				return success("LoanContractNotification", ValidationResult.ValidationType.CARDINALITY, "LoanContractNotification", path, "");
			})
			.collect(toList());
	}
