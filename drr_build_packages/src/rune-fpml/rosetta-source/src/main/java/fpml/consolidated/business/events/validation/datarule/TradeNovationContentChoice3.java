 ...
 * @version 2.1.1
 */
@RosettaDataRule("TradeNovationContentChoice3")
@ImplementedBy(TradeNovationContentChoice3.Default.class)
public interface TradeNovationContentChoice3 extends Validator<TradeNovationContent> {
	
	String NAME = "TradeNovationContentChoice3";
	String DEFINITION = "if novatedNumberOfOptions exists then remainingAmount is absent and remainingNumberOfUnits is absent and optional choice oldTradeIdentifier, oldTrade and if oldTradeIdentifier exists then optional choice feeTradeIdentifier, feeTrade and if oldTrade exists then optional choice feeTradeIdentifier, feeTrade and if oldTradeIdentifier is absent and oldTrade is absent then optional choice feeTradeIdentifier, feeTrade";
	
	class Default implements TradeNovationContentChoice3 {
		...
		private ComparisonResult executeDataRule(TradeNovationContent tradeNovationContent) {
			try {
				if (exists(MapperS.of(tradeNovationContent).<BigDecimal>map("getNovatedNumberOfOptions", _tradeNovationContent -> _tradeNovationContent.getNovatedNumberOfOptions())).getOrDefault(false)) {
	...
	class NoOp implements TradeNovationContentChoice3 {
