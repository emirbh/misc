...
public interface LoanTradingCostOfCarryAccrualChoice extends Validator<LoanTradingCostOfCarryAccrual> {
	...
	class Default implements LoanTradingCostOfCarryAccrualChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, LoanTradingCostOfCarryAccrual loanTradingCostOfCarryAccrual) {
			ComparisonResult result = executeDataRule(loanTradingCostOfCarryAccrual);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "LoanTradingCostOfCarryAccrual", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "LoanTradingCostOfCarryAccrual", path, DEFINITION, failureMessage));
		}
