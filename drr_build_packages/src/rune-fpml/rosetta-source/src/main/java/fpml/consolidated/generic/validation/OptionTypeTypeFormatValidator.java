		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OptionType", ValidationResult.ValidationType.TYPE_FORMAT, "OptionType", path, "", res.getError());
				}
				return success("OptionType", ValidationResult.ValidationType.TYPE_FORMAT, "OptionType", path, "");
			})
			.collect(toList());
	}
