		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClearingEligibilityAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingEligibilityAcknowledgement", path, "", res.getError());
				}
				return success("ClearingEligibilityAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "ClearingEligibilityAcknowledgement", path, "");
			})
			.collect(toList());
	}
