		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SimpleFra", ValidationResult.ValidationType.TYPE_FORMAT, "SimpleFra", path, "", res.getError());
				}
				return success("SimpleFra", ValidationResult.ValidationType.TYPE_FORMAT, "SimpleFra", path, "");
			})
			.collect(toList());
	}
