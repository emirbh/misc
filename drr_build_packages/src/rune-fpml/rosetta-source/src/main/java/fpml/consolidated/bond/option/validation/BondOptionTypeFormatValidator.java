		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BondOption", ValidationResult.ValidationType.TYPE_FORMAT, "BondOption", path, "", res.getError());
				}
				return success("BondOption", ValidationResult.ValidationType.TYPE_FORMAT, "BondOption", path, "");
			})
			.collect(toList());
	}
