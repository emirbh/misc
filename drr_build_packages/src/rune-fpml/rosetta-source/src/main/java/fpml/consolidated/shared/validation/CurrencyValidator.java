		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Currency", ValidationResult.ValidationType.CARDINALITY, "Currency", path, "", res.getError());
				}
				return success("Currency", ValidationResult.ValidationType.CARDINALITY, "Currency", path, "");
			})
			.collect(toList());
	}
