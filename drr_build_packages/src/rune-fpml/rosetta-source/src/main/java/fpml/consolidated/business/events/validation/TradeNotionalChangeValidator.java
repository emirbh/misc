		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeNotionalChange", ValidationResult.ValidationType.CARDINALITY, "TradeNotionalChange", path, "", res.getError());
				}
				return success("TradeNotionalChange", ValidationResult.ValidationType.CARDINALITY, "TradeNotionalChange", path, "");
			})
			.collect(toList());
	}
