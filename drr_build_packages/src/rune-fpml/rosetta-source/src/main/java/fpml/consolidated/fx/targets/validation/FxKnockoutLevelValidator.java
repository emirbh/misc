		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxKnockoutLevel", ValidationResult.ValidationType.CARDINALITY, "FxKnockoutLevel", path, "", res.getError());
				}
				return success("FxKnockoutLevel", ValidationResult.ValidationType.CARDINALITY, "FxKnockoutLevel", path, "");
			})
			.collect(toList());
	}
