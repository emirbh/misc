		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PenaltyRateChange", ValidationResult.ValidationType.CARDINALITY, "PenaltyRateChange", path, "", res.getError());
				}
				return success("PenaltyRateChange", ValidationResult.ValidationType.CARDINALITY, "PenaltyRateChange", path, "");
			})
			.collect(toList());
	}
