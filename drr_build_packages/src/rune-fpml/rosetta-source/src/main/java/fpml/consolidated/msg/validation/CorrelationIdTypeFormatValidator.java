		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CorrelationId", ValidationResult.ValidationType.TYPE_FORMAT, "CorrelationId", path, "", res.getError());
				}
				return success("CorrelationId", ValidationResult.ValidationType.TYPE_FORMAT, "CorrelationId", path, "");
			})
			.collect(toList());
	}
