...
public class ConsentExceptionValidator implements Validator<ConsentException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConsentException", ValidationResult.ValidationType.CARDINALITY, "ConsentException", path, "", res.getError());
				}
				return success("ConsentException", ValidationResult.ValidationType.CARDINALITY, "ConsentException", path, "");
			})
			.collect(toList());
	}
