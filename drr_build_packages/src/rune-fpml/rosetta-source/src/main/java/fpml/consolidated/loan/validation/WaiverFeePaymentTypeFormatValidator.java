		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WaiverFeePayment", ValidationResult.ValidationType.TYPE_FORMAT, "WaiverFeePayment", path, "", res.getError());
				}
				return success("WaiverFeePayment", ValidationResult.ValidationType.TYPE_FORMAT, "WaiverFeePayment", path, "");
			})
			.collect(toList());
	}
