		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Security", ValidationResult.ValidationType.TYPE_FORMAT, "Security", path, "", res.getError());
				}
				return success("Security", ValidationResult.ValidationType.TYPE_FORMAT, "Security", path, "");
			})
			.collect(toList());
	}
