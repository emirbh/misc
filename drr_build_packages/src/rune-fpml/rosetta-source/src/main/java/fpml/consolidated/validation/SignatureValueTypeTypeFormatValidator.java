		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SignatureValueType", ValidationResult.ValidationType.TYPE_FORMAT, "SignatureValueType", path, "", res.getError());
				}
				return success("SignatureValueType", ValidationResult.ValidationType.TYPE_FORMAT, "SignatureValueType", path, "");
			})
			.collect(toList());
	}
