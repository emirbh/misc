 ...
 * @version 2.1.1
 */
@RosettaDataRule("LoanTradeNotificationChoice2")
@ImplementedBy(LoanTradeNotificationChoice2.Default.class)
public interface LoanTradeNotificationChoice2 extends Validator<LoanTradeNotification> {
	
	String NAME = "LoanTradeNotificationChoice2";
	String DEFINITION = "optional choice dealIdentifier, dealSummary";
	
	class Default implements LoanTradeNotificationChoice2 {
		...
		private ComparisonResult executeDataRule(LoanTradeNotification loanTradeNotification) {
			try {
				return choice(MapperS.of(loanTradeNotification), Arrays.asList("dealIdentifier", "dealSummary"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements LoanTradeNotificationChoice2 {
