		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityPremium", ValidationResult.ValidationType.CARDINALITY, "CommodityPremium", path, "", res.getError());
				}
				return success("CommodityPremium", ValidationResult.ValidationType.CARDINALITY, "CommodityPremium", path, "");
			})
			.collect(toList());
	}
