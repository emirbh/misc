		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxRateObservable", ValidationResult.ValidationType.TYPE_FORMAT, "FxRateObservable", path, "", res.getError());
				}
				return success("FxRateObservable", ValidationResult.ValidationType.TYPE_FORMAT, "FxRateObservable", path, "");
			})
			.collect(toList());
	}
