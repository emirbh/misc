		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyProfileId", ValidationResult.ValidationType.TYPE_FORMAT, "PartyProfileId", path, "", res.getError());
				}
				return success("PartyProfileId", ValidationResult.ValidationType.TYPE_FORMAT, "PartyProfileId", path, "");
			})
			.collect(toList());
	}
