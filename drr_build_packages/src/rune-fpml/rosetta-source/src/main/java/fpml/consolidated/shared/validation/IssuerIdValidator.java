		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IssuerId", ValidationResult.ValidationType.CARDINALITY, "IssuerId", path, "", res.getError());
				}
				return success("IssuerId", ValidationResult.ValidationType.CARDINALITY, "IssuerId", path, "");
			})
			.collect(toList());
	}
