...
public class MaturityAcknowledgementValidator implements Validator<MaturityAcknowledgement> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MaturityAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "MaturityAcknowledgement", path, "", res.getError());
				}
				return success("MaturityAcknowledgement", ValidationResult.ValidationType.CARDINALITY, "MaturityAcknowledgement", path, "");
			})
			.collect(toList());
	}
