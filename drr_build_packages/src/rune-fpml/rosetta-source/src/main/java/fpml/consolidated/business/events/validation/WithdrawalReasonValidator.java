		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("WithdrawalReason", ValidationResult.ValidationType.CARDINALITY, "WithdrawalReason", path, "", res.getError());
				}
				return success("WithdrawalReason", ValidationResult.ValidationType.CARDINALITY, "WithdrawalReason", path, "");
			})
			.collect(toList());
	}
