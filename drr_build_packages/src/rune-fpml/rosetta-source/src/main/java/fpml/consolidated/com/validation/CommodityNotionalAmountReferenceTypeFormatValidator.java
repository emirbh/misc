		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommodityNotionalAmountReference", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityNotionalAmountReference", path, "", res.getError());
				}
				return success("CommodityNotionalAmountReference", ValidationResult.ValidationType.TYPE_FORMAT, "CommodityNotionalAmountReference", path, "");
			})
			.collect(toList());
	}
