...
public class ExecutionAdviceExceptionValidator implements Validator<ExecutionAdviceException> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdviceException", ValidationResult.ValidationType.CARDINALITY, "ExecutionAdviceException", path, "", res.getError());
				}
				return success("ExecutionAdviceException", ValidationResult.ValidationType.CARDINALITY, "ExecutionAdviceException", path, "");
			})
			.collect(toList());
	}
