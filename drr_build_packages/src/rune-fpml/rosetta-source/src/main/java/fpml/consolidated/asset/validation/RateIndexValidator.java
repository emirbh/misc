...
public class RateIndexValidator implements Validator<RateIndex> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RateIndex", ValidationResult.ValidationType.CARDINALITY, "RateIndex", path, "", res.getError());
				}
				return success("RateIndex", ValidationResult.ValidationType.CARDINALITY, "RateIndex", path, "");
			})
			.collect(toList());
	}
