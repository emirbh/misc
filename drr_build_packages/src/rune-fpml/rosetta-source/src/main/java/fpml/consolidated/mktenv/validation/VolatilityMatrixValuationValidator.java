...
public class VolatilityMatrixValuationValidator implements Validator<VolatilityMatrixValuation> {
	...
	private List<ComparisonResult> getComparisonResults(VolatilityMatrixValuation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("dataPoints", (MultiDimensionalPricingData) o.getDataPoints() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VolatilityMatrixValuation", ValidationResult.ValidationType.CARDINALITY, "VolatilityMatrixValuation", path, "", res.getError());
				}
				return success("VolatilityMatrixValuation", ValidationResult.ValidationType.CARDINALITY, "VolatilityMatrixValuation", path, "");
			})
			.collect(toList());
	}
