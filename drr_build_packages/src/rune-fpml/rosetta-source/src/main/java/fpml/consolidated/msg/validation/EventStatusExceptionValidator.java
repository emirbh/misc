		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventStatusException", ValidationResult.ValidationType.CARDINALITY, "EventStatusException", path, "", res.getError());
				}
				return success("EventStatusException", ValidationResult.ValidationType.CARDINALITY, "EventStatusException", path, "");
			})
			.collect(toList());
	}
