		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventRequestAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "EventRequestAcknowledgement", path, "", res.getError());
				}
				return success("EventRequestAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "EventRequestAcknowledgement", path, "");
			})
			.collect(toList());
	}
