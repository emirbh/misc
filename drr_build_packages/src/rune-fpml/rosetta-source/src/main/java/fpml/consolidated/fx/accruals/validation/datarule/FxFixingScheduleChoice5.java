 ...
 * @version 2.1.1
 */
@RosettaDataRule("FxFixingScheduleChoice5")
@ImplementedBy(FxFixingScheduleChoice5.Default.class)
public interface FxFixingScheduleChoice5 extends Validator<FxFixingSchedule> {
	
	String NAME = "FxFixingScheduleChoice5";
	String DEFINITION = "optional choice dateAdjustments, dayType";
	
	class Default implements FxFixingScheduleChoice5 {
		...
		private ComparisonResult executeDataRule(FxFixingSchedule fxFixingSchedule) {
			try {
				return choice(MapperS.of(fxFixingSchedule), Arrays.asList("dateAdjustments", "dayType"), ChoiceRuleValidationMethod.OPTIONAL);
			}
			catch (Exception ex) {
	...
	class NoOp implements FxFixingScheduleChoice5 {
