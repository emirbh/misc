...
public class MutualFundValidator implements Validator<MutualFund> {
	...
	private List<ComparisonResult> getComparisonResults(MutualFund o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("openEndedFund", (Boolean) o.getOpenEndedFund() != null ? 1 : 0, 0, 1), 
				checkCardinality("fundManager", (String) o.getFundManager() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MutualFund", ValidationResult.ValidationType.CARDINALITY, "MutualFund", path, "", res.getError());
				}
				return success("MutualFund", ValidationResult.ValidationType.CARDINALITY, "MutualFund", path, "");
			})
			.collect(toList());
	}
