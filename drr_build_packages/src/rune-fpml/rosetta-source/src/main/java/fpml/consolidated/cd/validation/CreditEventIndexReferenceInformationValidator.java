...
public class CreditEventIndexReferenceInformationValidator implements Validator<CreditEventIndexReferenceInformation> {
	...
	private List<ComparisonResult> getComparisonResults(CreditEventIndexReferenceInformation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("indexReversionDate", (ZonedDateTime) o.getIndexReversionDate() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditEventIndexReferenceInformation", ValidationResult.ValidationType.CARDINALITY, "CreditEventIndexReferenceInformation", path, "", res.getError());
				}
				return success("CreditEventIndexReferenceInformation", ValidationResult.ValidationType.CARDINALITY, "CreditEventIndexReferenceInformation", path, "");
			})
			.collect(toList());
	}
