		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IssuerTradeId", ValidationResult.ValidationType.TYPE_FORMAT, "IssuerTradeId", path, "", res.getError());
				}
				return success("IssuerTradeId", ValidationResult.ValidationType.TYPE_FORMAT, "IssuerTradeId", path, "");
			})
			.collect(toList());
	}
