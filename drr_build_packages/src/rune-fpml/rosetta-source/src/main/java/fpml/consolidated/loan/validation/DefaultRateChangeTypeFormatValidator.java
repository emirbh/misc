		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DefaultRateChange", ValidationResult.ValidationType.TYPE_FORMAT, "DefaultRateChange", path, "", res.getError());
				}
				return success("DefaultRateChange", ValidationResult.ValidationType.TYPE_FORMAT, "DefaultRateChange", path, "");
			})
			.collect(toList());
	}
