		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("VerificationStatusException", ValidationResult.ValidationType.TYPE_FORMAT, "VerificationStatusException", path, "", res.getError());
				}
				return success("VerificationStatusException", ValidationResult.ValidationType.TYPE_FORMAT, "VerificationStatusException", path, "");
			})
			.collect(toList());
	}
