		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxSingleLeg", ValidationResult.ValidationType.TYPE_FORMAT, "FxSingleLeg", path, "", res.getError());
				}
				return success("FxSingleLeg", ValidationResult.ValidationType.TYPE_FORMAT, "FxSingleLeg", path, "");
			})
			.collect(toList());
	}
