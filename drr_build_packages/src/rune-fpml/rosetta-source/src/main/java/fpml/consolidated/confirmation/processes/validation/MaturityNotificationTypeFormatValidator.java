...
public class MaturityNotificationTypeFormatValidator implements Validator<MaturityNotification> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MaturityNotification", ValidationResult.ValidationType.TYPE_FORMAT, "MaturityNotification", path, "", res.getError());
				}
				return success("MaturityNotification", ValidationResult.ValidationType.TYPE_FORMAT, "MaturityNotification", path, "");
			})
			.collect(toList());
	}
