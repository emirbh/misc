		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AbstractLoanStatement", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractLoanStatement", path, "", res.getError());
				}
				return success("AbstractLoanStatement", ValidationResult.ValidationType.TYPE_FORMAT, "AbstractLoanStatement", path, "");
			})
			.collect(toList());
	}
