		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ConstituentWeight", ValidationResult.ValidationType.TYPE_FORMAT, "ConstituentWeight", path, "", res.getError());
				}
				return success("ConstituentWeight", ValidationResult.ValidationType.TYPE_FORMAT, "ConstituentWeight", path, "");
			})
			.collect(toList());
	}
