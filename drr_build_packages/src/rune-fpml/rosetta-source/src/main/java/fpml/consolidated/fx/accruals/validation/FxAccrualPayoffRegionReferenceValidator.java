		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAccrualPayoffRegionReference", ValidationResult.ValidationType.CARDINALITY, "FxAccrualPayoffRegionReference", path, "", res.getError());
				}
				return success("FxAccrualPayoffRegionReference", ValidationResult.ValidationType.CARDINALITY, "FxAccrualPayoffRegionReference", path, "");
			})
			.collect(toList());
	}
