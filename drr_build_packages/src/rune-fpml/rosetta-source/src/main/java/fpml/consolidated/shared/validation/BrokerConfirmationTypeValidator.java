		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BrokerConfirmationType", ValidationResult.ValidationType.CARDINALITY, "BrokerConfirmationType", path, "", res.getError());
				}
				return success("BrokerConfirmationType", ValidationResult.ValidationType.CARDINALITY, "BrokerConfirmationType", path, "");
			})
			.collect(toList());
	}
