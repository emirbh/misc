		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExecutionAdviceAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionAdviceAcknowledgement", path, "", res.getError());
				}
				return success("ExecutionAdviceAcknowledgement", ValidationResult.ValidationType.TYPE_FORMAT, "ExecutionAdviceAcknowledgement", path, "");
			})
			.collect(toList());
	}
