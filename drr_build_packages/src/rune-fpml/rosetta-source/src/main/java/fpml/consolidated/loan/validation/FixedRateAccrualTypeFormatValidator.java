		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FixedRateAccrual", ValidationResult.ValidationType.TYPE_FORMAT, "FixedRateAccrual", path, "", res.getError());
				}
				return success("FixedRateAccrual", ValidationResult.ValidationType.TYPE_FORMAT, "FixedRateAccrual", path, "");
			})
			.collect(toList());
	}
