		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestConsentRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestConsentRetracted", path, "", res.getError());
				}
				return success("RequestConsentRetracted", ValidationResult.ValidationType.CARDINALITY, "RequestConsentRetracted", path, "");
			})
			.collect(toList());
	}
