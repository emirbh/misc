		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxAverageRate", ValidationResult.ValidationType.TYPE_FORMAT, "FxAverageRate", path, "", res.getError());
				}
				return success("FxAverageRate", ValidationResult.ValidationType.TYPE_FORMAT, "FxAverageRate", path, "");
			})
			.collect(toList());
	}
