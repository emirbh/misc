		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractReference", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractReference", path, "", res.getError());
				}
				return success("LoanContractReference", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractReference", path, "");
			})
			.collect(toList());
	}
