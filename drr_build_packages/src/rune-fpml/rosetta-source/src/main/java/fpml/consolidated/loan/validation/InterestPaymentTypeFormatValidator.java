		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestPayment", ValidationResult.ValidationType.TYPE_FORMAT, "InterestPayment", path, "", res.getError());
				}
				return success("InterestPayment", ValidationResult.ValidationType.TYPE_FORMAT, "InterestPayment", path, "");
			})
			.collect(toList());
	}
