		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FxDisruptionEvent", ValidationResult.ValidationType.TYPE_FORMAT, "FxDisruptionEvent", path, "", res.getError());
				}
				return success("FxDisruptionEvent", ValidationResult.ValidationType.TYPE_FORMAT, "FxDisruptionEvent", path, "");
			})
			.collect(toList());
	}
