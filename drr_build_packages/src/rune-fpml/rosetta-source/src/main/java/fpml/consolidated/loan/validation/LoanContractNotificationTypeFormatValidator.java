		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanContractNotification", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractNotification", path, "", res.getError());
				}
				return success("LoanContractNotification", ValidationResult.ValidationType.TYPE_FORMAT, "LoanContractNotification", path, "");
			})
			.collect(toList());
	}
