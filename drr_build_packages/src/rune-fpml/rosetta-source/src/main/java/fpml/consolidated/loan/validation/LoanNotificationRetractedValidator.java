...
public class LoanNotificationRetractedValidator implements Validator<LoanNotificationRetracted> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("LoanNotificationRetracted", ValidationResult.ValidationType.CARDINALITY, "LoanNotificationRetracted", path, "", res.getError());
				}
				return success("LoanNotificationRetracted", ValidationResult.ValidationType.CARDINALITY, "LoanNotificationRetracted", path, "");
			})
			.collect(toList());
	}
