		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ObligationAccelerationEvent", ValidationResult.ValidationType.TYPE_FORMAT, "ObligationAccelerationEvent", path, "", res.getError());
				}
				return success("ObligationAccelerationEvent", ValidationResult.ValidationType.TYPE_FORMAT, "ObligationAccelerationEvent", path, "");
			})
			.collect(toList());
	}
