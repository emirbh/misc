		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CreditSupportAgreementIdentifier", ValidationResult.ValidationType.CARDINALITY, "CreditSupportAgreementIdentifier", path, "", res.getError());
				}
				return success("CreditSupportAgreementIdentifier", ValidationResult.ValidationType.CARDINALITY, "CreditSupportAgreementIdentifier", path, "");
			})
			.collect(toList());
	}
