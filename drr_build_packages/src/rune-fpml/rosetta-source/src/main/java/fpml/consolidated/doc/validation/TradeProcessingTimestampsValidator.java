		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeProcessingTimestamps", ValidationResult.ValidationType.CARDINALITY, "TradeProcessingTimestamps", path, "", res.getError());
				}
				return success("TradeProcessingTimestamps", ValidationResult.ValidationType.CARDINALITY, "TradeProcessingTimestamps", path, "");
			})
			.collect(toList());
	}
