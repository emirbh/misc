		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ForecastRateIndex", ValidationResult.ValidationType.TYPE_FORMAT, "ForecastRateIndex", path, "", res.getError());
				}
				return success("ForecastRateIndex", ValidationResult.ValidationType.TYPE_FORMAT, "ForecastRateIndex", path, "");
			})
			.collect(toList());
	}
