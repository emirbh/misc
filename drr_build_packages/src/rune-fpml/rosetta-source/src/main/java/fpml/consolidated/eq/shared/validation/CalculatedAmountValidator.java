		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculatedAmount", ValidationResult.ValidationType.CARDINALITY, "CalculatedAmount", path, "", res.getError());
				}
				return success("CalculatedAmount", ValidationResult.ValidationType.CARDINALITY, "CalculatedAmount", path, "");
			})
			.collect(toList());
	}
