...
public class RequestCollateralAllocationTypeFormatValidator implements Validator<RequestCollateralAllocation> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestCollateralAllocation", ValidationResult.ValidationType.TYPE_FORMAT, "RequestCollateralAllocation", path, "", res.getError());
				}
				return success("RequestCollateralAllocation", ValidationResult.ValidationType.TYPE_FORMAT, "RequestCollateralAllocation", path, "");
			})
			.collect(toList());
	}
