...
public class RequestClearingEligibilityValidator implements Validator<RequestClearingEligibility> {

	private List<ComparisonResult> getComparisonResults(RequestClearingEligibility o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("requestingPartyReference", (PartyReference) o.getRequestingPartyReference() != null ? 1 : 0, 0, 1), 
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, RequestClearingEligibility o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("RequestClearingEligibility", ValidationResult.ValidationType.CARDINALITY, "RequestClearingEligibility", path, "", res.getError());
				}
				return success("RequestClearingEligibility", ValidationResult.ValidationType.CARDINALITY, "RequestClearingEligibility", path, "");
			})
			.collect(toList());
	}
