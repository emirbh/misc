...
public class CalculationPeriodValidator implements Validator<CalculationPeriod> {
	...
	private List<ComparisonResult> getComparisonResults(CalculationPeriod o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("unadjustedStartDate", (ZonedDateTime) o.getUnadjustedStartDate() != null ? 1 : 0, 0, 1), 
				checkCardinality("unadjustedEndDate", (ZonedDateTime) o.getUnadjustedEndDate() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculationPeriod", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriod", path, "", res.getError());
				}
				return success("CalculationPeriod", ValidationResult.ValidationType.CARDINALITY, "CalculationPeriod", path, "");
			})
			.collect(toList());
	}
