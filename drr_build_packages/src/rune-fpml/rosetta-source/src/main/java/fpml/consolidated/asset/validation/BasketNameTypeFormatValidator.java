		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BasketName", ValidationResult.ValidationType.TYPE_FORMAT, "BasketName", path, "", res.getError());
				}
				return success("BasketName", ValidationResult.ValidationType.TYPE_FORMAT, "BasketName", path, "");
			})
			.collect(toList());
	}
