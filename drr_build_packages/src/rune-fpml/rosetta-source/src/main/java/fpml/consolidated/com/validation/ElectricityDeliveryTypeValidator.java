		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ElectricityDeliveryType", ValidationResult.ValidationType.CARDINALITY, "ElectricityDeliveryType", path, "", res.getError());
				}
				return success("ElectricityDeliveryType", ValidationResult.ValidationType.CARDINALITY, "ElectricityDeliveryType", path, "");
			})
			.collect(toList());
	}
