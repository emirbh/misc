		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ServiceStatus", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceStatus", path, "", res.getError());
				}
				return success("ServiceStatus", ValidationResult.ValidationType.TYPE_FORMAT, "ServiceStatus", path, "");
			})
			.collect(toList());
	}
