		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdditionalDisruptionEvents", ValidationResult.ValidationType.TYPE_FORMAT, "AdditionalDisruptionEvents", path, "", res.getError());
				}
				return success("AdditionalDisruptionEvents", ValidationResult.ValidationType.TYPE_FORMAT, "AdditionalDisruptionEvents", path, "");
			})
			.collect(toList());
	}
