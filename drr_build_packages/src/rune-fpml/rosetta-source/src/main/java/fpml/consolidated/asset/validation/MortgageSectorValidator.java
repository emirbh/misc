		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MortgageSector", ValidationResult.ValidationType.CARDINALITY, "MortgageSector", path, "", res.getError());
				}
				return success("MortgageSector", ValidationResult.ValidationType.CARDINALITY, "MortgageSector", path, "");
			})
			.collect(toList());
	}
