		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Step", ValidationResult.ValidationType.CARDINALITY, "Step", path, "", res.getError());
				}
				return success("Step", ValidationResult.ValidationType.CARDINALITY, "Step", path, "");
			})
			.collect(toList());
	}
