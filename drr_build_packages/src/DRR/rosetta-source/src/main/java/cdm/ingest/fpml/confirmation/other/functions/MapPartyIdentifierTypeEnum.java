	...
	*/
	...
	public static class MapPartyIdentifierTypeEnumDefault extends MapPartyIdentifierTypeEnum {
		...
		protected PartyIdentifierTypeEnum assignOutput(PartyIdentifierTypeEnum result, String value) {
			...
			} else if (areEqual(switchArgument, MapperS.of("http://www.fpml.org/coding-scheme/external/exchange-id-MIC"), CardinalityOperator.All).get()) {
				result = PartyIdentifierTypeEnum.MIC;
			} else if (areEqual(switchArgument, MapperS.of("http://www.fpml.org/coding-scheme/external/mifir/extension-iso10383"), CardinalityOperator.All).get()) {
				result = PartyIdentifierTypeEnum.MIC;
			} else {
				result = null;
			}
