	...
	public static class MapPeriodExtendedEnumDefault extends MapPeriodExtendedEnum {
		...
		protected PeriodExtendedEnum assignOutput(PeriodExtendedEnum result, String value) {
			...
			if (switchArgument.get() == null) {
				...
				result = PeriodExtendedEnum.D;
			} else if (areEqual(switchArgument, MapperS.of("PerCalculationDay"), CardinalityOperator.All).get()) {
				result = PeriodExtendedEnum.D;
			...
			} else if (areEqual(switchArgument, MapperS.of("PerSettlementPeriod"), CardinalityOperator.All).get()) {
				result = PeriodExtendedEnum.C;
			} else if (areEqual(switchArgument, MapperS.of("PerHour"), CardinalityOperator.All).get()) {
				result = PeriodExtendedEnum.H;
			} else {
				result = null;
			}
