 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3002_03")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3002_03.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3002_03 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3002_03";
	String DEFINITION = "API_GetLeiData(reportSubmittingEntityID) -> generalCategory <> LeiGeneralCategoryEnum -> BRANCH";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3002_03 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				return notEqual(MapperS.of(aPI_GetLeiData.evaluate(MapperS.of(eSMAEMIRMarginReport).<String>map("getReportSubmittingEntityID", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getReportSubmittingEntityID()).get())).<LeiGeneralCategoryEnum>map("getGeneralCategory", leiData -> leiData.getGeneralCategory()), MapperS.of(LeiGeneralCategoryEnum.BRANCH), CardinalityOperator.Any);
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3002_03 {
