 ...
 * @version 7.7.0
 */
@RosettaDataRule("HKMATransactionReportDOC0020")
@ImplementedBy(HKMATransactionReportDOC0020.Default.class)
public interface HKMATransactionReportDOC0020 extends Validator<HKMATransactionReport> {
	
	String NAME = "HKMATransactionReportDOC0020";
	...
	class Default implements HKMATransactionReportDOC0020 {
		...
		private ComparisonResult executeDataRule(HKMATransactionReport hKMATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(hKMATransactionReport).<ActionTypeEnum>map("getActionType", _hKMATransactionReport -> _hKMATransactionReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					if (areEqual(MapperS.of(hKMATransactionReport).<ClearedEnum>map("getCleared", _hKMATransactionReport -> _hKMATransactionReport.getCleared()), MapperS.of(ClearedEnum.Y), CardinalityOperator.All).getOrDefault(false)) {
						return exists(MapperS.of(hKMATransactionReport).<String>map("getClearingMember", _hKMATransactionReport -> _hKMATransactionReport.getClearingMember()));
					}
	...
	class NoOp implements HKMATransactionReportDOC0020 {
