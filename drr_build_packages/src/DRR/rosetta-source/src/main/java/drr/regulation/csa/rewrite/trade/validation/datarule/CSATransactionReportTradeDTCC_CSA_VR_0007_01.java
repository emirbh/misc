 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportTradeDTCC_CSA_VR_0007_01")
@ImplementedBy(CSATransactionReportTradeDTCC_CSA_VR_0007_01.Default.class)
public interface CSATransactionReportTradeDTCC_CSA_VR_0007_01 extends Validator<CSATransactionReportTrade> {
	
	String NAME = "CSATransactionReportTradeDTCC_CSA_VR_0007_01";
	String DEFINITION = "if leg1 -> payerIdentifier exists then leg1 -> receiverIdentifier exists";
	
	class Default implements CSATransactionReportTradeDTCC_CSA_VR_0007_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReportTrade cSATransactionReportTrade) {
			try {
				if (exists(MapperS.of(cSATransactionReportTrade).<CSALeg>map("getLeg1", _cSATransactionReportTrade -> _cSATransactionReportTrade.getLeg1()).<String>map("getPayerIdentifier", cSALeg -> cSALeg.getPayerIdentifier())).getOrDefault(false)) {
					return exists(MapperS.of(cSATransactionReportTrade).<CSALeg>map("getLeg1", _cSATransactionReportTrade -> _cSATransactionReportTrade.getLeg1()).<String>map("getReceiverIdentifier", cSALeg -> cSALeg.getReceiverIdentifier()));
				}
	...
	class NoOp implements CSATransactionReportTradeDTCC_CSA_VR_0007_01 {
