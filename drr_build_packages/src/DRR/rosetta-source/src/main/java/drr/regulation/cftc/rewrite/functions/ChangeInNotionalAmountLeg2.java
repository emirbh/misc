...
@ImplementedBy(ChangeInNotionalAmountLeg2.ChangeInNotionalAmountLeg2Default.class)
public abstract class ChangeInNotionalAmountLeg2 implements RosettaFunction {
	...
	*/
	...
	public static class ChangeInNotionalAmountLeg2Default extends ChangeInNotionalAmountLeg2 {
		@Override
		...
		protected MapperS<? extends PayoutLegWithAuxiliary> payoutBefore(TransactionReportInstruction transactionReportInstruction, BigDecimal defaultValue) {
			...
			return thenArg
				.mapSingleToItem(item -> MapperS.of(payoutFromProductLeg2.evaluate(item.<NonTransferableProduct>map("getProduct", trade -> trade.getProduct()).get(), item.<TradeLot>mapC("getTradeLot", trade -> trade.getTradeLot()).get(), MapperS.of(transactionReportInstruction).<ReportableInformationBase>map("getReportableInformation", _transactionReportInstruction -> _transactionReportInstruction.getReportableInformation()).get())));
		}
		...
		protected MapperS<BigDecimal> NotionalAmountAfter(TransactionReportInstruction transactionReportInstruction, BigDecimal defaultValue) {
			return MapperS.of(notionalAmountFormat.evaluate(payoutForQuantityLeg2Rule.evaluate(transactionReportInstruction), defaultValue));
		}
	}
}
