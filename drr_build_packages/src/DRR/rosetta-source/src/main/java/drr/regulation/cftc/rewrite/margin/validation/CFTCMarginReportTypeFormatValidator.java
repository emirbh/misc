...
public class CFTCMarginReportTypeFormatValidator implements Validator<CFTCMarginReport> {
	...
	private List<ComparisonResult> getComparisonResults(CFTCMarginReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkString("uti", o.getUti(), 0, empty(), of(Pattern.compile("[A-Z0-9]{18}[0-9]{2}(?:[A-Z0-9\\:.\\-_]{0,31}[A-Z0-9])?"))), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "CFTCMarginReport", path, "", res.getError());
				}
				return success("CFTCMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "CFTCMarginReport", path, "");
			})
			.collect(toList());
	}
