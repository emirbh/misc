...
@ImplementedBy(InitialMarginCollectedByReportingCounterpartyPreHaircut.InitialMarginCollectedByReportingCounterpartyPreHaircutDefault.class)
public abstract class InitialMarginCollectedByReportingCounterpartyPreHaircut implements RosettaFunction {
	...
	*/
	...
	public static class InitialMarginCollectedByReportingCounterpartyPreHaircutDefault extends InitialMarginCollectedByReportingCounterpartyPreHaircut {
		@Override
		...
		protected BigDecimal assignOutput(BigDecimal marginValue, ReportableCollateralBase reportableCollateral, Party reportingCounterparty) {
			final MapperS<BigDecimal> thenArg = MapperS.of(getMarginValue.evaluate(MapperC.<CollateralBalance>of(getCollateralBalancesForMarginType.evaluate(reportableCollateral, CollateralMarginTypeEnum.INITIAL_MARGIN))
				.filterItemNullSafe(item -> areEqual(item.<PartyReferencePayerReceiver>map("getPayerReceiver", collateralBalance -> collateralBalance.getPayerReceiver()).<ReferenceWithMetaParty>map("getReceiverPartyReference", partyReferencePayerReceiver -> partyReferencePayerReceiver.getReceiverPartyReference()).<Party>map("Type coercion", referenceWithMetaParty -> referenceWithMetaParty == null ? null : referenceWithMetaParty.getValue()), MapperS.of(reportingCounterparty), CardinalityOperator.All).get()).getMulti(), HaircutIndicatorEnum.PRE_HAIRCUT)).<BigDecimal>map("getValue", money -> money.getValue());
