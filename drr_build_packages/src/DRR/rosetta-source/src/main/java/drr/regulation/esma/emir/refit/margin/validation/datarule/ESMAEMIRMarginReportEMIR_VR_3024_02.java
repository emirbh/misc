 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3024_02")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3024_02.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3024_02 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3024_02";
	String DEFINITION = "if [CollateralisationType3Code -> UNCL, CollateralisationType3Code -> PRC1, CollateralisationType3Code -> OWC1] any = collateralisationCategory then variationMarginCollectedByTheCounterparty1PostHaircut is absent";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3024_02 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				if (areEqual(MapperC.<CollateralisationType3Code>of(MapperS.of(CollateralisationType3Code.UNCL), MapperS.of(CollateralisationType3Code.PRC1), MapperS.of(CollateralisationType3Code.OWC1)), MapperS.of(eSMAEMIRMarginReport).<CollateralisationType3Code>map("getCollateralisationCategory", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCollateralisationCategory()), CardinalityOperator.Any).getOrDefault(false)) {
					return notExists(MapperS.of(eSMAEMIRMarginReport).<BigDecimal>map("getVariationMarginCollectedByTheCounterparty1PostHaircut", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getVariationMarginCollectedByTheCounterparty1PostHaircut()));
				}
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3024_02 {
