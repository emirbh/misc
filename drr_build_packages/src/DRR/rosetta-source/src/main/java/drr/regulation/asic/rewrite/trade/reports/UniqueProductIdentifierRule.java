...
public abstract class UniqueProductIdentifierRule implements ReportFunction<TransactionReportInstruction, String> {
	...
	@Inject protected drr.standards.iosco.upi.reports.UniqueProductIdentifierRule uniqueProductIdentifierRule;
	...
	public static class UniqueProductIdentifierRuleDefault extends UniqueProductIdentifierRule {
		...
		protected String assignOutput(String output, TransactionReportInstruction input) {
			final MapperS<TransactionReportInstruction> thenArg = MapperS.of(input)
				.filterSingleNullSafe(item -> ComparisonResult.ofNullSafe(MapperS.of(isAllowableActionForASIC.evaluate(item.get()))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isActionTypeTERM.evaluate(item.get())))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isActionTypePRTO.evaluate(item.get())))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isActionTypeEROR.evaluate(item.get())))).get());
