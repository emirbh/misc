...
public interface MASTransactionReportDTCC_MAS_BR_0076a_01 extends Validator<MASTransactionReport> {
	...
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType then (if leg1 -> notionalAmountSchedule -> value count <> leg1 -> notionalAmountSchedule -> effectiveDate count then False else True) and if [CommonAssetClass -> CRDT, CommonAssetClass -> EQUI, CommonAssetClass -> CURR, CommonAssetClass -> INTR] any = assetClass and leg1 -> notionalAmountSchedule exists then leg1 -> notionalAmountSchedule -> value all >= 0 else True";
