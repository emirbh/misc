 ...
 * @version 7.7.0
 */
@RosettaDataRule("ASICMarginReportDTCC_ASIC_BR_3011_01")
@ImplementedBy(ASICMarginReportDTCC_ASIC_BR_3011_01.Default.class)
public interface ASICMarginReportDTCC_ASIC_BR_3011_01 extends Validator<ASICMarginReport> {
	
	String NAME = "ASICMarginReportDTCC_ASIC_BR_3011_01";
	String DEFINITION = "if actionType <> MarginActionEnum -> MARU then if initialMarginCollateralPortfolioCode = \"NOAP\" then variationMarginCollateralPortfolioCode exists";
	
	class Default implements ASICMarginReportDTCC_ASIC_BR_3011_01 {
			...
			try {
				if (notEqual(MapperS.of(aSICMarginReport).<MarginActionEnum>map("getActionType", _aSICMarginReport -> _aSICMarginReport.getActionType()), MapperS.of(MarginActionEnum.MARU), CardinalityOperator.Any).getOrDefault(false)) {
					if (areEqual(MapperS.of(aSICMarginReport).<String>map("getInitialMarginCollateralPortfolioCode", _aSICMarginReport -> _aSICMarginReport.getInitialMarginCollateralPortfolioCode()), MapperS.of("NOAP"), CardinalityOperator.All).getOrDefault(false)) {
						return exists(MapperS.of(aSICMarginReport).<String>map("getVariationMarginCollateralPortfolioCode", _aSICMarginReport -> _aSICMarginReport.getVariationMarginCollateralPortfolioCode()));
					}
	...
	class NoOp implements ASICMarginReportDTCC_ASIC_BR_3011_01 {
