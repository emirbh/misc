 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportTradeDTCC_CSA_VR_0037_01")
@ImplementedBy(CSATransactionReportTradeDTCC_CSA_VR_0037_01.Default.class)
public interface CSATransactionReportTradeDTCC_CSA_VR_0037_01 extends Validator<CSATransactionReportTrade> {
	
	String NAME = "CSATransactionReportTradeDTCC_CSA_VR_0037_01";
	String DEFINITION = "if nonReportable -> postUpiData -> Header -> AssetClass = Commodities or dtccAdditionalFields -> primaryAssetClass = Commodity then common.quantity.NotionalScheduleEffectiveDateDTCC_Validation( leg1 -> notionalQuantitySchedule -> value, leg1 -> notionalQuantitySchedule -> effectiveDate ) else leg1 -> notionalQuantitySchedule -> effectiveDate is absent";
	
	class Default implements CSATransactionReportTradeDTCC_CSA_VR_0037_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReportTrade cSATransactionReportTrade) {
			try {
				...
				return notExists(MapperS.of(cSATransactionReportTrade).<CSALeg>map("getLeg1", _cSATransactionReportTrade -> _cSATransactionReportTrade.getLeg1()).<NotionalPeriod>mapC("getNotionalQuantitySchedule", cSALeg -> cSALeg.getNotionalQuantitySchedule()).<Date>map("getEffectiveDate", notionalPeriod -> notionalPeriod.getEffectiveDate()));
			}
			catch (Exception ex) {
	...
	class NoOp implements CSATransactionReportTradeDTCC_CSA_VR_0037_01 {
