 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAValuationReportUKEMIR_VR_1003_02")
@ImplementedBy(FCAValuationReportUKEMIR_VR_1003_02.Default.class)
public interface FCAValuationReportUKEMIR_VR_1003_02 extends Validator<FCAValuationReport> {
	
	String NAME = "FCAValuationReportUKEMIR_VR_1003_02";
	String DEFINITION = "EntityResponsibleForReporting_02_Validation( nonReportable -> enrichment, entityResponsibleForReporting )";
	
	class Default implements FCAValuationReportUKEMIR_VR_1003_02 {
		...
		private ComparisonResult executeDataRule(FCAValuationReport fCAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(entityResponsibleForReporting_02_Validation.evaluate(MapperS.of(fCAValuationReport).<NonReportable>map("getNonReportable", _fCAValuationReport -> _fCAValuationReport.getNonReportable()).<EnrichmentData>map("getEnrichment", nonReportable -> nonReportable.getEnrichment()).get(), MapperS.of(fCAValuationReport).<String>map("getEntityResponsibleForReporting", _fCAValuationReport -> _fCAValuationReport.getEntityResponsibleForReporting()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements FCAValuationReportUKEMIR_VR_1003_02 {
