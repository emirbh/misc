 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3012_01")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3012_01.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3012_01 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3012_01";
	String DEFINITION = "if [CollateralisationType3Code -> OWC1, CollateralisationType3Code -> OWP1, CollateralisationType3Code -> FLCL] any = collateralisationCategory then initialMarginPostedByTheCounterparty1PreHaircut exists else initialMarginPostedByTheCounterparty1PreHaircut is absent";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3012_01 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				if (areEqual(MapperC.<CollateralisationType3Code>of(MapperS.of(CollateralisationType3Code.OWC1), MapperS.of(CollateralisationType3Code.OWP1), MapperS.of(CollateralisationType3Code.FLCL)), MapperS.of(eSMAEMIRMarginReport).<CollateralisationType3Code>map("getCollateralisationCategory", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCollateralisationCategory()), CardinalityOperator.Any).getOrDefault(false)) {
					return exists(MapperS.of(eSMAEMIRMarginReport).<BigDecimal>map("getInitialMarginPostedByTheCounterparty1PreHaircut", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getInitialMarginPostedByTheCounterparty1PreHaircut()));
				}
				return notExists(MapperS.of(eSMAEMIRMarginReport).<BigDecimal>map("getInitialMarginPostedByTheCounterparty1PreHaircut", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getInitialMarginPostedByTheCounterparty1PreHaircut()));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3012_01 {
