...
public abstract class GetNrgySpcfcAttrbts implements RosettaFunction {
	...
	*/
	...
	protected abstract EnergySpecificAttribute9__1.EnergySpecificAttribute9__1Builder doEvaluate(FCAUKEMIRTransactionReport drrReport);
	...
	public static class GetNrgySpcfcAttrbtsDefault extends GetNrgySpcfcAttrbts {
		...
		protected EnergySpecificAttribute9__1.EnergySpecificAttribute9__1Builder assignOutput(EnergySpecificAttribute9__1.EnergySpecificAttribute9__1Builder nrgySpcfcAttrbts, FCAUKEMIRTransactionReport drrReport) {
			...
			if (areEqual(MapperS.of(drrReport).<CommonAssetClass>map("getAssetClass", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getAssetClass()), MapperS.of(CommonAssetClass.COMM), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(drrReport).<String>map("getBaseProduct", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getBaseProduct()), MapperS.of("NRGY"), CardinalityOperator.All)).andNullSafe(areEqual(MapperS.of(drrReport).<String>map("getSubProduct", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getSubProduct()), MapperS.of("ELEC"), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(drrReport).<String>map("getSubProduct", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getSubProduct()), MapperS.of("NGAS"), CardinalityOperator.All))).getOrDefault(false)) {
				ifThenElseResult0 = MapperS.of(drrReport).<String>map("getInterconnectionPoint", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getInterconnectionPoint()).map("to-string", Object::toString).get();
			}
			EnergyLoadType1Code ifThenElseResult1 = null;
			if (exists(MapperS.of(drrReport).<String>mapC("getDeliveryPoint", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getDeliveryPoint())).getOrDefault(false)) {
				ifThenElseResult1 = MapperS.of(drrReport).<drr.standards.iso.EnergyLoadType1Code>map("getLoadType", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getLoadType()).checkedMap("to-enum", e -> EnergyLoadType1Code.valueOf(e.name()), IllegalArgumentException.class).get();
			}
			nrgySpcfcAttrbts = toBuilder(EnergySpecificAttribute9__1.builder()
				.setDlvryPtOrZone(MapperS.of(MapperS.of(drrReport).<String>mapC("getDeliveryPoint", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getDeliveryPoint()).get())
					.mapSingleToItem(item -> MapperS.of(DeliveryInterconnectionPoint1Choice__1.builder()
						.setCd(item.get())
						.build())).get())
				...
				.setDlvryAttr(MapperS.of(drrReport).<ReportableDelivery>mapC("getDeliveryReport", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getDeliveryReport())
					.mapItem(item -> {
						...
						return MapperS.of(EnergyDeliveryAttribute10__1.builder()
							...
							.setDlvryDt(DatePeriod1.builder()
								...
								.build())
							.setDrtn(item.<DurationType1Code>map("getDuration", reportableDelivery -> reportableDelivery.getDuration()).checkedMap("to-enum", e -> iso20022.auth030.fca.DurationType1Code.valueOf(e.name()), IllegalArgumentException.class).get())
							...
							.setQtyUnit(EnergyQuantityUnit2Choice__1.builder()
								.setCd(item.<EnergyQuantityUnit2Code>map("getQuantityUnit", reportableDelivery -> reportableDelivery.getQuantityUnit()).checkedMap("to-enum", e -> iso20022.auth030.fca.EnergyQuantityUnit2Code.valueOf(e.name()), IllegalArgumentException.class).get())
								.build())
