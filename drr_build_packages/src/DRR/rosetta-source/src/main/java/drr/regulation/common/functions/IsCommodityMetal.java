...
@ImplementedBy(IsCommodityMetal.IsCommodityMetalDefault.class)
public abstract class IsCommodityMetal implements RosettaFunction {
	
	// RosettaFunction dependencies
	//
	@Inject protected EconomicTermsForProduct economicTermsForProduct;
	@Inject protected IsCRPMetal isCRPMetal;
	...
	*/
	...
	public static class IsCommodityMetalDefault extends IsCommodityMetal {
		@Override
		...
		protected Boolean assignOutput(Boolean result, NonTransferableProduct product) {
			if (exists(productClass(product)).andNullSafe(exists(commodityReferencePrice(product))).getOrDefault(false)) {
				...
				result = ComparisonResult.ofNullSafe(MapperS.of(stringContains.evaluate((fieldWithMetaString0 == null ? null : fieldWithMetaString0.getValue()), "Metals:NonPrecious"))).andNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isCRPMetal.evaluate((fieldWithMetaString1 == null ? null : fieldWithMetaString1.getValue()))))).get();
			} else {
				final FieldWithMetaString fieldWithMetaString2 = productClass(product).get();
				if (exists(productClass(product)).andNullSafe(ComparisonResult.ofNullSafe(MapperS.of(stringContains.evaluate((fieldWithMetaString2 == null ? null : fieldWithMetaString2.getValue()), "Metals:NonPrecious")))).getOrDefault(false)) {
					result = true;
				} else {
					final FieldWithMetaString fieldWithMetaString3 = commodityReferencePrice(product).get();
					if (exists(commodityReferencePrice(product)).andNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isCRPMetal.evaluate((fieldWithMetaString3 == null ? null : fieldWithMetaString3.getValue()))))).getOrDefault(false)) {
						result = true;
					} else {
						result = false;
					}
