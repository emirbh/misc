 ...
 * @version 7.7.0
 */
@RosettaDataRule("HKMATransactionReportDOC0009")
@ImplementedBy(HKMATransactionReportDOC0009.Default.class)
public interface HKMATransactionReportDOC0009 extends Validator<HKMATransactionReport> {
	
	String NAME = "HKMATransactionReportDOC0009";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType then reportingTimestamp >= executionTimestamp";
	
	class Default implements HKMATransactionReportDOC0009 {
		...
		private ComparisonResult executeDataRule(HKMATransactionReport hKMATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(hKMATransactionReport).<ActionTypeEnum>map("getActionType", _hKMATransactionReport -> _hKMATransactionReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					return greaterThanEquals(MapperS.of(hKMATransactionReport).<ZonedDateTime>map("getReportingTimestamp", _hKMATransactionReport -> _hKMATransactionReport.getReportingTimestamp()), MapperS.of(hKMATransactionReport).<ZonedDateTime>map("getExecutionTimestamp", _hKMATransactionReport -> _hKMATransactionReport.getExecutionTimestamp()), CardinalityOperator.All);
				}
	...
	class NoOp implements HKMATransactionReportDOC0009 {
