 ...
 * @version 7.7.0
 */
@RosettaDataRule("SECTransactionReportDTCC_SEC_VR_FixedRateLeg1")
@ImplementedBy(SECTransactionReportDTCC_SEC_VR_FixedRateLeg1.Default.class)
public interface SECTransactionReportDTCC_SEC_VR_FixedRateLeg1 extends Validator<SECTransactionReport> {
	
	String NAME = "SECTransactionReportDTCC_SEC_VR_FixedRateLeg1";
	String DEFINITION = "if dtccAdditionalFields -> primaryAssetClass = Credit or nonReportable -> postUpiData -> Header -> AssetClass = Credit then (if leg1 -> spread is absent and otherPayment -> paymentType any <> UFRO and postPricedSwapIndicator = False and nonReportable -> instrumentType <> Option then leg1 -> fixedRate exists else leg1 -> fixedRate is absent) else if dtccAdditionalFields -> primaryAssetClass = InterestRate or nonReportable -> postUpiData -> Header -> AssetClass = Rates then (if leg1 -> spread is absent and postPricedSwapIndicator = False and nonReportable -> instrumentType <> Option then leg1 -> fixedRate exists else leg1 -> fixedRate is absent)";
	
	class Default implements SECTransactionReportDTCC_SEC_VR_FixedRateLeg1 {
	...
	class NoOp implements SECTransactionReportDTCC_SEC_VR_FixedRateLeg1 {
