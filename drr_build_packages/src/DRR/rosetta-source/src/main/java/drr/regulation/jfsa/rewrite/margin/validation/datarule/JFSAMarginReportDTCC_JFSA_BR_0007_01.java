 ...
 * @version 7.7.0
 */
@RosettaDataRule("JFSAMarginReportDTCC_JFSA_BR_0007_01")
@ImplementedBy(JFSAMarginReportDTCC_JFSA_BR_0007_01.Default.class)
public interface JFSAMarginReportDTCC_JFSA_BR_0007_01 extends Validator<JFSAMarginReport> {
	
	String NAME = "JFSAMarginReportDTCC_JFSA_BR_0007_01";
	String DEFINITION = "if actionType = MarginActionEnum -> MARU then (if counterparty2IdentifierType = True then counterparty1 <> counterparty2)";
	
	class Default implements JFSAMarginReportDTCC_JFSA_BR_0007_01 {
			...
			try {
				if (areEqual(MapperS.of(jFSAMarginReport).<MarginActionEnum>map("getActionType", _jFSAMarginReport -> _jFSAMarginReport.getActionType()), MapperS.of(MarginActionEnum.MARU), CardinalityOperator.All).getOrDefault(false)) {
					if (areEqual(MapperS.of(jFSAMarginReport).<Boolean>map("getCounterparty2IdentifierType", _jFSAMarginReport -> _jFSAMarginReport.getCounterparty2IdentifierType()), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
						return notEqual(MapperS.of(jFSAMarginReport).<String>map("getCounterparty1", _jFSAMarginReport -> _jFSAMarginReport.getCounterparty1()), MapperS.of(jFSAMarginReport).<String>map("getCounterparty2", _jFSAMarginReport -> _jFSAMarginReport.getCounterparty2()), CardinalityOperator.Any);
					}
	...
	class NoOp implements JFSAMarginReportDTCC_JFSA_BR_0007_01 {
