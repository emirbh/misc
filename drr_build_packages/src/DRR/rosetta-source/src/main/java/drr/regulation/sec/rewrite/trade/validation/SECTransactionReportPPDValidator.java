...
public class SECTransactionReportPPDValidator implements Validator<SECTransactionReportPPD> {

	private List<ComparisonResult> getComparisonResults(SECTransactionReportPPD o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, SECTransactionReportPPD o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SECTransactionReportPPD", ValidationResult.ValidationType.CARDINALITY, "SECTransactionReportPPD", path, "", res.getError());
				}
				return success("SECTransactionReportPPD", ValidationResult.ValidationType.CARDINALITY, "SECTransactionReportPPD", path, "");
			})
			.collect(toList());
	}
