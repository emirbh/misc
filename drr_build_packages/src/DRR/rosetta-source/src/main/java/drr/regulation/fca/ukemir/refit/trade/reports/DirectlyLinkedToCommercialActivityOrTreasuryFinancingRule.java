	...
	public static class DirectlyLinkedToCommercialActivityOrTreasuryFinancingRuleDefault extends DirectlyLinkedToCommercialActivityOrTreasuryFinancingRule {
		...
		protected Boolean assignOutput(Boolean output, TransactionReportInstruction input) {
			...
			final MapperS<JurisdictionPartyInformation> thenArg2 = thenArg1
				.filterSingleNullSafe(item -> areEqual(item.<FCAPartyInformation>map("getFcaPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getFcaPartyInformation()).<NatureOfPartyEnum>map("getNatureOfParty", fCAPartyInformation -> fCAPartyInformation.getNatureOfParty()), MapperS.of(NatureOfPartyEnum.NON_FINANCIAL), CardinalityOperator.All).get());
			output = thenArg2
				.mapSingleToItem(item -> item.<FCAPartyInformation>map("getFcaPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getFcaPartyInformation()).<Boolean>map("getDirectlyLinkedActivity", fCAPartyInformation -> fCAPartyInformation.getDirectlyLinkedActivity())).get();
