 ...
 * @version 7.7.0
 */
@RosettaDataRule("MASMarginReportDTCC_MAS_BR_0134_03")
@ImplementedBy(MASMarginReportDTCC_MAS_BR_0134_03.Default.class)
public interface MASMarginReportDTCC_MAS_BR_0134_03 extends Validator<MASMarginReport> {
	
	String NAME = "MASMarginReportDTCC_MAS_BR_0134_03";
	String DEFINITION = "if [MarginActionEnum -> MARU, MarginActionEnum -> CORR] any = actionType then eventTimestamp <= reportingTimestamp";
	
	class Default implements MASMarginReportDTCC_MAS_BR_0134_03 {
		...
		private ComparisonResult executeDataRule(MASMarginReport mASMarginReport) {
			try {
				if (areEqual(MapperC.<MarginActionEnum>of(MapperS.of(MarginActionEnum.MARU), MapperS.of(MarginActionEnum.CORR)), MapperS.of(mASMarginReport).<MarginActionEnum>map("getActionType", _mASMarginReport -> _mASMarginReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					return lessThanEquals(MapperS.of(mASMarginReport).<ZonedDateTime>map("getEventTimestamp", _mASMarginReport -> _mASMarginReport.getEventTimestamp()), MapperS.of(mASMarginReport).<ZonedDateTime>map("getReportingTimestamp", _mASMarginReport -> _mASMarginReport.getReportingTimestamp()), CardinalityOperator.All);
				}
	...
	class NoOp implements MASMarginReportDTCC_MAS_BR_0134_03 {
