 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportTradeDTCC_CSA_VR_0126_01")
@ImplementedBy(CSATransactionReportTradeDTCC_CSA_VR_0126_01.Default.class)
public interface CSATransactionReportTradeDTCC_CSA_VR_0126_01 extends Validator<CSATransactionReportTrade> {
	
	String NAME = "CSATransactionReportTradeDTCC_CSA_VR_0126_01";
	String DEFINITION = "if basketConstituents -> numberOfUnits exists then basketConstituents -> numberOfUnits count = basketConstituents -> identifier count";
	
	class Default implements CSATransactionReportTradeDTCC_CSA_VR_0126_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReportTrade cSATransactionReportTrade) {
			try {
				if (exists(MapperS.of(cSATransactionReportTrade).<BasketConstituentsReport>mapC("getBasketConstituents", _cSATransactionReportTrade -> _cSATransactionReportTrade.getBasketConstituents()).<BigDecimal>map("getNumberOfUnits", basketConstituentsReport -> basketConstituentsReport.getNumberOfUnits())).getOrDefault(false)) {
	...
	class NoOp implements CSATransactionReportTradeDTCC_CSA_VR_0126_01 {
