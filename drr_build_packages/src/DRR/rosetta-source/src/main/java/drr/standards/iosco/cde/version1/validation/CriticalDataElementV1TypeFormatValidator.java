...
public class CriticalDataElementV1TypeFormatValidator implements Validator<CriticalDataElementV1> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CriticalDataElementV1", ValidationResult.ValidationType.TYPE_FORMAT, "CriticalDataElementV1", path, "", res.getError());
				}
				return success("CriticalDataElementV1", ValidationResult.ValidationType.TYPE_FORMAT, "CriticalDataElementV1", path, "");
			})
			.collect(toList());
	}
