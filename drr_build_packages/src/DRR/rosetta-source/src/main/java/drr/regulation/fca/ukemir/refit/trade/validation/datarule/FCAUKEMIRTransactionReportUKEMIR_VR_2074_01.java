 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRTransactionReportUKEMIR_VR_2074_01")
@ImplementedBy(FCAUKEMIRTransactionReportUKEMIR_VR_2074_01.Default.class)
public interface FCAUKEMIRTransactionReportUKEMIR_VR_2074_01 extends Validator<FCAUKEMIRTransactionReport> {
	
	String NAME = "FCAUKEMIRTransactionReportUKEMIR_VR_2074_01";
	String DEFINITION = "if (IsVRAllowedForActionType(actionType) or actionType = ActionTypeEnum -> TERM) and otherPayment exists then (otherPayment extract (if paymentType exists then amount >= 0 else amount is absent) then all = True)";
	
	class Default implements FCAUKEMIRTransactionReportUKEMIR_VR_2074_01 {
					...
					final MapperC<Boolean> thenArg = MapperS.of(fCAUKEMIRTransactionReport).<OtherPayment>mapC("getOtherPayment", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getOtherPayment())
						.mapItem(item -> {
							if (exists(item.<PaymentType4Code>map("getPaymentType", otherPayment -> otherPayment.getPaymentType())).getOrDefault(false)) {
								return greaterThanEquals(item.<BigDecimal>map("getAmount", otherPayment -> otherPayment.getAmount()), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All).asMapper();
							}
							return notExists(item.<BigDecimal>map("getAmount", otherPayment -> otherPayment.getAmount())).asMapper();
						});
	...
	class NoOp implements FCAUKEMIRTransactionReportUKEMIR_VR_2074_01 {
