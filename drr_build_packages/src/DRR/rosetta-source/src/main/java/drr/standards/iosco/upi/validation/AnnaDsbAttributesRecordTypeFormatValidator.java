...
public class AnnaDsbAttributesRecordTypeFormatValidator implements Validator<AnnaDsbAttributesRecord> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AnnaDsbAttributesRecord", ValidationResult.ValidationType.TYPE_FORMAT, "AnnaDsbAttributesRecord", path, "", res.getError());
				}
				return success("AnnaDsbAttributesRecord", ValidationResult.ValidationType.TYPE_FORMAT, "AnnaDsbAttributesRecord", path, "");
			})
			.collect(toList());
	}
