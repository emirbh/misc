...
public class CFTCPart45LegValidator implements Validator<CFTCPart45Leg> {
	...
	private List<ComparisonResult> getComparisonResults(CFTCPart45Leg o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("payerIdentifierFormat", (PartyIdentifierFormatEnum) o.getPayerIdentifierFormat() != null ? 1 : 0, 0, 1), 
				checkCardinality("receiverIdentifierFormat", (PartyIdentifierFormatEnum) o.getReceiverIdentifierFormat() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCPart45Leg", ValidationResult.ValidationType.CARDINALITY, "CFTCPart45Leg", path, "", res.getError());
				}
				return success("CFTCPart45Leg", ValidationResult.ValidationType.CARDINALITY, "CFTCPart45Leg", path, "");
			})
			.collect(toList());
	}
