 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3007_02")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3007_02.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3007_02 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3007_02";
	String DEFINITION = "CompareDateTo(collateralTimestamp -> date, 2014, 2, 12) >= 0";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3007_02 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				return greaterThanEquals(MapperS.of(compareDateTo.evaluate(MapperS.of(eSMAEMIRMarginReport).<ZonedDateTime>map("getCollateralTimestamp", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCollateralTimestamp()).<Date>map("Date", zdt -> Date.of(zdt.toLocalDate())).get(), 2014, 2, 12)), MapperS.of(0), CardinalityOperator.All);
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3007_02 {
