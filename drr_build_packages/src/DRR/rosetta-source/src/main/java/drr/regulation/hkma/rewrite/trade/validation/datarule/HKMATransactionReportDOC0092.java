 ...
 * @version 7.7.0
 */
@RosettaDataRule("HKMATransactionReportDOC0092")
@ImplementedBy(HKMATransactionReportDOC0092.Default.class)
public interface HKMATransactionReportDOC0092 extends Validator<HKMATransactionReport> {
	
	String NAME = "HKMATransactionReportDOC0092";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType then if priorUTI exists then priorUTI <> uniqueTransactionIdentifier";
	
	class Default implements HKMATransactionReportDOC0092 {
		...
		private ComparisonResult executeDataRule(HKMATransactionReport hKMATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(hKMATransactionReport).<ActionTypeEnum>map("getActionType", _hKMATransactionReport -> _hKMATransactionReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					if (exists(MapperS.of(hKMATransactionReport).<String>map("getPriorUTI", _hKMATransactionReport -> _hKMATransactionReport.getPriorUTI())).getOrDefault(false)) {
						return notEqual(MapperS.of(hKMATransactionReport).<String>map("getPriorUTI", _hKMATransactionReport -> _hKMATransactionReport.getPriorUTI()), MapperS.of(hKMATransactionReport).<String>map("getUniqueTransactionIdentifier", _hKMATransactionReport -> _hKMATransactionReport.getUniqueTransactionIdentifier()), CardinalityOperator.Any);
					}
	...
	class NoOp implements HKMATransactionReportDOC0092 {
