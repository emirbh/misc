 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportCSA_VR_0145_01")
@ImplementedBy(CSATransactionReportCSA_VR_0145_01.Default.class)
public interface CSATransactionReportCSA_VR_0145_01 extends Validator<CSATransactionReport> {
	
	String NAME = "CSATransactionReportCSA_VR_0145_01";
	String DEFINITION = "if optionPremiumAmount > 0 then optionPremiumCurrency exists else optionPremiumCurrency is absent";
	
	class Default implements CSATransactionReportCSA_VR_0145_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReport cSATransactionReport) {
			try {
				if (greaterThan(MapperS.of(cSATransactionReport).<BigDecimal>map("getOptionPremiumAmount", _cSATransactionReport -> _cSATransactionReport.getOptionPremiumAmount()), MapperS.of(BigDecimal.valueOf(0)), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(cSATransactionReport).<ISOCurrencyCodeEnum>map("getOptionPremiumCurrency", _cSATransactionReport -> _cSATransactionReport.getOptionPremiumCurrency()));
				}
				return notExists(MapperS.of(cSATransactionReport).<ISOCurrencyCodeEnum>map("getOptionPremiumCurrency", _cSATransactionReport -> _cSATransactionReport.getOptionPremiumCurrency()));
			}
			catch (Exception ex) {
	...
	class NoOp implements CSATransactionReportCSA_VR_0145_01 {
