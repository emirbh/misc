 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRMarginReportUKEMIR_VR_3006_05")
@ImplementedBy(FCAUKEMIRMarginReportUKEMIR_VR_3006_05.Default.class)
public interface FCAUKEMIRMarginReportUKEMIR_VR_3006_05 extends Validator<FCAUKEMIRMarginReport> {
	
	String NAME = "FCAUKEMIRMarginReportUKEMIR_VR_3006_05";
	String DEFINITION = "if counterparty2IdentifierType = False then counterparty2 exists";
	
	class Default implements FCAUKEMIRMarginReportUKEMIR_VR_3006_05 {
		...
		private ComparisonResult executeDataRule(FCAUKEMIRMarginReport fCAUKEMIRMarginReport) {
			try {
				if (areEqual(MapperS.of(fCAUKEMIRMarginReport).<Boolean>map("getCounterparty2IdentifierType", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getCounterparty2IdentifierType()), MapperS.of(false), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getCounterparty2", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getCounterparty2()));
				}
	...
	class NoOp implements FCAUKEMIRMarginReportUKEMIR_VR_3006_05 {
