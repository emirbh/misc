...
public class CommonValuationReportValidator implements Validator<CommonValuationReport> {

	private List<ComparisonResult> getComparisonResults(CommonValuationReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("reportSubmittingEntityID", (String) o.getReportSubmittingEntityID() != null ? 1 : 0, 0, 1), 
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, CommonValuationReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommonValuationReport", ValidationResult.ValidationType.CARDINALITY, "CommonValuationReport", path, "", res.getError());
				}
				return success("CommonValuationReport", ValidationResult.ValidationType.CARDINALITY, "CommonValuationReport", path, "");
			})
			.collect(toList());
	}
