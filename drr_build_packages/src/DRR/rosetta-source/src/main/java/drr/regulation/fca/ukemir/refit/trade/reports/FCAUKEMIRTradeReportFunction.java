...
import drr.regulation.fca.ukemir.refit.trade.labels.FCAUKEMIRTradeLabelProvider;
...
@RosettaReport(namespace="drr.regulation.fca.ukemir.refit.trade", body="FCA", corpusList={"UKEMIR", "Trade"})
@RuneLabelProvider(labelProvider=FCAUKEMIRTradeLabelProvider.class)
@ImplementedBy(FCAUKEMIRTradeReportFunction.FCAUKEMIRTradeReportFunctionDefault.class)
public abstract class FCAUKEMIRTradeReportFunction implements ReportFunction<TransactionReportInstruction, FCAUKEMIRTransactionReport> {
	...
	@Inject protected ExecutionAgentReportingCounterpartyRule executionAgentReportingCounterpartyRule;
	@Inject protected ExecutionAgentReportingPartyRule executionAgentReportingPartyRule;
	...
	*/
	@Override
	public FCAUKEMIRTransactionReport evaluate(TransactionReportInstruction input) {
		FCAUKEMIRTransactionReport.FCAUKEMIRTransactionReportBuilder outputBuilder = doEvaluate(input);
		
		final FCAUKEMIRTransactionReport output;
		if (outputBuilder == null) {
			output = null;
		} else {
			output = outputBuilder.build();
			objectValidator.validate(FCAUKEMIRTransactionReport.class, output);
		}
		
		return output;
	}

	protected abstract FCAUKEMIRTransactionReport.FCAUKEMIRTransactionReportBuilder doEvaluate(TransactionReportInstruction input);

	public static class FCAUKEMIRTradeReportFunctionDefault extends FCAUKEMIRTradeReportFunction {
		@Override
		protected FCAUKEMIRTransactionReport.FCAUKEMIRTransactionReportBuilder doEvaluate(TransactionReportInstruction input) {
			FCAUKEMIRTransactionReport.FCAUKEMIRTransactionReportBuilder output = FCAUKEMIRTransactionReport.builder();
			return assignOutput(output, input);
		}
		
		protected FCAUKEMIRTransactionReport.FCAUKEMIRTransactionReportBuilder assignOutput(FCAUKEMIRTransactionReport.FCAUKEMIRTransactionReportBuilder output, TransactionReportInstruction input) {
			output
			...
			final BigDecimal bigDecimal2 = floatingRateReferencePeriodOfLeg1MultiplierRule.evaluate(input);
			output
				.getOrCreateLeg1()
				.getOrCreatePeriodicPayment()
			...
			final BigDecimal bigDecimal5 = floatingRateReferencePeriodOfLeg2MultiplierRule.evaluate(input);
			output
				.getOrCreateLeg2()
				.getOrCreatePeriodicPayment()
			...
			output
				.setExecutionAgentCounterparty1(executionAgentReportingPartyRule.evaluate(input));
			
			output
				.setExecutionAgentCounterparty2(executionAgentReportingCounterpartyRule.evaluate(input));
