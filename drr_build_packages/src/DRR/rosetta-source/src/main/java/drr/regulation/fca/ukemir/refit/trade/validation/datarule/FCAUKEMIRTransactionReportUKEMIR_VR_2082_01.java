 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRTransactionReportUKEMIR_VR_2082_01")
@ImplementedBy(FCAUKEMIRTransactionReportUKEMIR_VR_2082_01.Default.class)
public interface FCAUKEMIRTransactionReportUKEMIR_VR_2082_01 extends Validator<FCAUKEMIRTransactionReport> {
	
	String NAME = "FCAUKEMIRTransactionReportUKEMIR_VR_2082_01";
	String DEFINITION = "if IsVRAllowedForActionType(actionType) then if leg1 -> fixedRate exists then if [FrequencyPeriodEnum -> EXPI, FrequencyPeriodEnum -> YEAR, FrequencyPeriodEnum -> MNTH, FrequencyPeriodEnum -> WEEK, FrequencyPeriodEnum -> DAIL] any = leg1 -> periodicPayment -> fixedRatePaymentFrequencyPeriod and leg1 -> periodicPayment -> fixedRatePaymentFrequencyPeriodMultiplier >= 0 then True else False else leg1 -> periodicPayment -> fixedRatePaymentFrequencyPeriodMultiplier is absent";
	
	class Default implements FCAUKEMIRTransactionReportUKEMIR_VR_2082_01 {
		...
		private ComparisonResult executeDataRule(FCAUKEMIRTransactionReport fCAUKEMIRTransactionReport) {
			try {
				...
				if ((_boolean == null ? false : _boolean)) {
					if (exists(MapperS.of(fCAUKEMIRTransactionReport).<CommonLeg>map("getLeg1", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getLeg1()).<BigDecimal>map("getFixedRate", commonLeg -> commonLeg.getFixedRate())).getOrDefault(false)) {
	...
	class NoOp implements FCAUKEMIRTransactionReportUKEMIR_VR_2082_01 {
