...
@ImplementedBy(FormatToBaseOne18Rate.FormatToBaseOne18RateDefault.class)
public abstract class FormatToBaseOne18Rate implements RosettaFunction {
	...
	*/
	...
	public static class FormatToBaseOne18RateDefault extends FormatToBaseOne18Rate {
		@Override
		...
		protected BigDecimal assignOutput(BigDecimal formattedNumber, BigDecimal inputNumber) {
			if (lessThanEquals(MapperS.of(abs.evaluate(inputNumber)), MapperS.of(BigDecimal.valueOf(999999999999999999l)), CardinalityOperator.All).getOrDefault(false)) {
				final MapperS<BigDecimal> thenArg = MapperS.of(roundToPrecision.evaluate(inputNumber, 13, RoundingDirectionEnum.NEAREST, true));
				formattedNumber = MapperS.of(roundToSignificantFigures.evaluate(thenArg.get(), 18, RoundingDirectionEnum.NEAREST)).get();
			} else {
