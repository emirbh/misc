 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3022_01")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3022_01.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3022_01 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3022_01";
	String DEFINITION = "if [CollateralisationType3Code -> OWC2, CollateralisationType3Code -> OWP2, CollateralisationType3Code -> FLCL] any = collateralisationCategory then initialMarginCollectedByCounterparty1Currency exists else initialMarginCollectedByCounterparty1Currency is absent";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3022_01 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				if (areEqual(MapperC.<CollateralisationType3Code>of(MapperS.of(CollateralisationType3Code.OWC2), MapperS.of(CollateralisationType3Code.OWP2), MapperS.of(CollateralisationType3Code.FLCL)), MapperS.of(eSMAEMIRMarginReport).<CollateralisationType3Code>map("getCollateralisationCategory", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCollateralisationCategory()), CardinalityOperator.Any).getOrDefault(false)) {
					return exists(MapperS.of(eSMAEMIRMarginReport).<ISOCurrencyCodeEnum>map("getInitialMarginCollectedByCounterparty1Currency", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getInitialMarginCollectedByCounterparty1Currency()));
				}
				return notExists(MapperS.of(eSMAEMIRMarginReport).<ISOCurrencyCodeEnum>map("getInitialMarginCollectedByCounterparty1Currency", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getInitialMarginCollectedByCounterparty1Currency()));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3022_01 {
