 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAValuationReportEMIR_VR_2153_04")
@ImplementedBy(ESMAValuationReportEMIR_VR_2153_04.Default.class)
public interface ESMAValuationReportEMIR_VR_2153_04 extends Validator<ESMAValuationReport> {
	
	String NAME = "ESMAValuationReportEMIR_VR_2153_04";
	String DEFINITION = "EventDate_04_Validation(eventDate, reportingTimestamp)";
	
	class Default implements ESMAValuationReportEMIR_VR_2153_04 {
		...
		private ComparisonResult executeDataRule(ESMAValuationReport eSMAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(eventDate_04_Validation.evaluate(MapperS.of(eSMAValuationReport).<Date>map("getEventDate", _eSMAValuationReport -> _eSMAValuationReport.getEventDate()).get(), MapperS.of(eSMAValuationReport).<ZonedDateTime>map("getReportingTimestamp", _eSMAValuationReport -> _eSMAValuationReport.getReportingTimestamp()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAValuationReportEMIR_VR_2153_04 {
