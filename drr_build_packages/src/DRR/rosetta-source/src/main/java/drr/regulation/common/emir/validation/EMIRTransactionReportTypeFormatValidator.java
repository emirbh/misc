...
public class EMIRTransactionReportTypeFormatValidator implements Validator<EMIRTransactionReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EMIRTransactionReport", ValidationResult.ValidationType.TYPE_FORMAT, "EMIRTransactionReport", path, "", res.getError());
				}
				return success("EMIRTransactionReport", ValidationResult.ValidationType.TYPE_FORMAT, "EMIRTransactionReport", path, "");
			})
			.collect(toList());
	}
