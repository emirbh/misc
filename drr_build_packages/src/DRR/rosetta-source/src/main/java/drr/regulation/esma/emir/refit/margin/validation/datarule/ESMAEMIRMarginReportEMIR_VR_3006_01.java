 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3006_01")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3006_01.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3006_01 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3006_01";
	String DEFINITION = "if counterparty2IdentifierType = True then API_GetLeiData(counterparty2) -> lei exists";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3006_01 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				if (areEqual(MapperS.of(eSMAEMIRMarginReport).<Boolean>map("getCounterparty2IdentifierType", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCounterparty2IdentifierType()), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(aPI_GetLeiData.evaluate(MapperS.of(eSMAEMIRMarginReport).<String>map("getCounterparty2", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCounterparty2()).get())).<String>map("getLei", leiData -> leiData.getLei()));
				}
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3006_01 {
