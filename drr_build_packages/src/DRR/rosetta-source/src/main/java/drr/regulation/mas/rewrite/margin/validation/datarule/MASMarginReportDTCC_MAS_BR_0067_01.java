 ...
 * @version 7.7.0
 */
@RosettaDataRule("MASMarginReportDTCC_MAS_BR_0067_01")
@ImplementedBy(MASMarginReportDTCC_MAS_BR_0067_01.Default.class)
public interface MASMarginReportDTCC_MAS_BR_0067_01 extends Validator<MASMarginReport> {
	
	String NAME = "MASMarginReportDTCC_MAS_BR_0067_01";
	String DEFINITION = "if [MarginActionEnum -> CORR, MarginActionEnum -> MARU] any = actionType and variationMarginPostedByTheCounterparty1PreHaircut exists then variationMarginPostedByCounterparty1Currency exists and [ISOCurrencyCodeEnum -> XAG, ISOCurrencyCodeEnum -> XAU, ISOCurrencyCodeEnum -> XBA, ISOCurrencyCodeEnum -> XBB, ISOCurrencyCodeEnum -> XBC, ISOCurrencyCodeEnum -> XBD, ISOCurrencyCodeEnum -> XDR, ISOCurrencyCodeEnum -> XPD, ISOCurrencyCodeEnum -> XPT, ISOCurrencyCodeEnum -> XTS, ISOCurrencyCodeEnum -> XXX] all <> variationMarginPostedByCounterparty1Currency";
	
	class Default implements MASMarginReportDTCC_MAS_BR_0067_01 {
		...
		private ComparisonResult executeDataRule(MASMarginReport mASMarginReport) {
			try {
				if (areEqual(MapperC.<MarginActionEnum>of(MapperS.of(MarginActionEnum.CORR), MapperS.of(MarginActionEnum.MARU)), MapperS.of(mASMarginReport).<MarginActionEnum>map("getActionType", _mASMarginReport -> _mASMarginReport.getActionType()), CardinalityOperator.Any).andNullSafe(exists(MapperS.of(mASMarginReport).<BigDecimal>map("getVariationMarginPostedByTheCounterparty1PreHaircut", _mASMarginReport -> _mASMarginReport.getVariationMarginPostedByTheCounterparty1PreHaircut()))).getOrDefault(false)) {
	...
	class NoOp implements MASMarginReportDTCC_MAS_BR_0067_01 {
