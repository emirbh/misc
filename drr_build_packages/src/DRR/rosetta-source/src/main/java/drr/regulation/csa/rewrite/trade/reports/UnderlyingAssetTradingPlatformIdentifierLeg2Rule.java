...
@ImplementedBy(UnderlyingAssetTradingPlatformIdentifierLeg2Rule.UnderlyingAssetTradingPlatformIdentifierLeg2RuleDefault.class)
public abstract class UnderlyingAssetTradingPlatformIdentifierLeg2Rule implements ReportFunction<TransactionReportInstruction, String> {
	...
	@Inject protected drr.standards.iosco.cde.version3.underlier.reports.UnderlyingAssetTradingPlatformIdentifierLeg2Rule underlyingAssetTradingPlatformIdentifierLeg2Rule;
	...
	*/
	...
	public static class UnderlyingAssetTradingPlatformIdentifierLeg2RuleDefault extends UnderlyingAssetTradingPlatformIdentifierLeg2Rule {
		@Override
			...
			final FieldWithMetaString fieldWithMetaString = thenArg
				.mapSingleToItem(item -> {
					if (ComparisonResult.ofNullSafe(MapperS.of(isEquity.evaluate(productForEvent.evaluate(item.get())))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isEquityForward.evaluate(productForEvent.evaluate(item.get()))))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(qualify_AssetClass_Credit.evaluate(economicTermsForProduct.evaluate(productForEvent.evaluate(item.get())))))).getOrDefault(false)) {
						if (areEqual(MapperS.of(isCSALeg2Aligned.evaluate(item.get())), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
							final String string0 = underlyingAssetTradingPlatformIdentifierLeg2Rule.evaluate(item.get());
