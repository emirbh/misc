...
@ImplementedBy(ExecutionAgentReportingCounterpartyRule.ExecutionAgentReportingCounterpartyRuleDefault.class)
public abstract class ExecutionAgentReportingCounterpartyRule implements ReportFunction<TransactionReportInstruction, String> {
	...
	*/
	...
	public static class ExecutionAgentReportingCounterpartyRuleDefault extends ExecutionAgentReportingCounterpartyRule {
		@Override
		...
		protected String assignOutput(String output, TransactionReportInstruction input) {
			...
			output = thenArg
				.mapSingleToItem(item -> {
					if (areEqual(MapperS.of(executionAgentCounterparty2Rule.evaluate(item.get())), MapperS.of("NOAP"), CardinalityOperator.All).getOrDefault(false)) {
						return MapperS.<String>ofNull();
					}
					return MapperS.of(executionAgentCounterparty2Rule.evaluate(item.get()));
				}).get();
