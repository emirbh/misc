...
public class FCAValuationReportTypeFormatValidator implements Validator<FCAValuationReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FCAValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "FCAValuationReport", path, "", res.getError());
				}
				return success("FCAValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "FCAValuationReport", path, "");
			})
			.collect(toList());
	}
