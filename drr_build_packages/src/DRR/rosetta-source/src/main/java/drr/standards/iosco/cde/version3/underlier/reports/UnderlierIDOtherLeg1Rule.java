		...
		protected String assignOutput(String output, TransactionReportInstructionBase input) {
			...
			output = thenArg
				.mapSingleToItem(item -> {
					if (exists(MapperC.<AssetIdentifier>of(getUnderlierProductIdentifierLeg1.evaluate(item.get()))).andNullSafe(notExists(MapperS.of(filterAssetIdentifier.evaluate(getUnderlierProductIdentifierLeg1.evaluate(item.get()), AssetIdTypeEnum.ISIN)))).getOrDefault(false)) {
