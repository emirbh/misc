 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAValuationReportEMIR_VR_1003_02")
@ImplementedBy(ESMAValuationReportEMIR_VR_1003_02.Default.class)
public interface ESMAValuationReportEMIR_VR_1003_02 extends Validator<ESMAValuationReport> {
	
	String NAME = "ESMAValuationReportEMIR_VR_1003_02";
	String DEFINITION = "EntityResponsibleForReporting_02_Validation( nonReportable -> enrichment, entityResponsibleForReporting )";
	
	class Default implements ESMAValuationReportEMIR_VR_1003_02 {
		...
		private ComparisonResult executeDataRule(ESMAValuationReport eSMAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(entityResponsibleForReporting_02_Validation.evaluate(MapperS.of(eSMAValuationReport).<NonReportable>map("getNonReportable", _eSMAValuationReport -> _eSMAValuationReport.getNonReportable()).<EnrichmentData>map("getEnrichment", nonReportable -> nonReportable.getEnrichment()).get(), MapperS.of(eSMAValuationReport).<String>map("getEntityResponsibleForReporting", _eSMAValuationReport -> _eSMAValuationReport.getEntityResponsibleForReporting()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAValuationReportEMIR_VR_1003_02 {
