		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Leg", ValidationResult.ValidationType.CARDINALITY, "Leg", path, "", res.getError());
				}
				return success("Leg", ValidationResult.ValidationType.CARDINALITY, "Leg", path, "");
			})
			.collect(toList());
	}
