 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRTransactionReportUKEMIR_VR_2030_01")
@ImplementedBy(FCAUKEMIRTransactionReportUKEMIR_VR_2030_01.Default.class)
public interface FCAUKEMIRTransactionReportUKEMIR_VR_2030_01 extends Validator<FCAUKEMIRTransactionReport> {
	
	String NAME = "FCAUKEMIRTransactionReportUKEMIR_VR_2030_01";
	String DEFINITION = "if IsVRAllowedForActionType(actionType) then if (venueOfExecution exists and GetOrFetchMicData( nonReportable -> enrichment -> micData, venueOfExecution ) extract (marketCategory = MicMarketCategoryEnum -> RMKT or IsFCAThirdCountryEquivalentMarket(mic) = True)) = False then clearingObligation exists and [ClearingObligationEnum -> TRUE, ClearingObligationEnum -> FLSE, ClearingObligationEnum -> UKWN] any = clearingObligation";
	
	class Default implements FCAUKEMIRTransactionReportUKEMIR_VR_2030_01 {
			...
			try {
				...
				if ((_boolean == null ? false : _boolean)) {
					if (areEqual(exists(MapperS.of(fCAUKEMIRTransactionReport).<String>map("getVenueOfExecution", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getVenueOfExecution())).andNullSafe(ComparisonResult.ofNullSafe(MapperS.of(getOrFetchMicData.evaluate(MapperS.of(fCAUKEMIRTransactionReport).<FCAUKEMIRNonReportable>map("getNonReportable", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getNonReportable()).<EnrichmentData>map("getEnrichment", fCAUKEMIRNonReportable -> fCAUKEMIRNonReportable.getEnrichment()).<MicData>mapC("getMicData", enrichmentData -> enrichmentData.getMicData()).getMulti(), MapperS.of(fCAUKEMIRTransactionReport).<String>map("getVenueOfExecution", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getVenueOfExecution()).get()))
						...
						return exists(MapperS.of(fCAUKEMIRTransactionReport).<ClearingObligationEnum>map("getClearingObligation", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getClearingObligation())).andNullSafe(areEqual(MapperC.<ClearingObligationEnum>of(MapperS.of(ClearingObligationEnum.TRUE), MapperS.of(ClearingObligationEnum.FLSE), MapperS.of(ClearingObligationEnum.UKWN)), MapperS.of(fCAUKEMIRTransactionReport).<ClearingObligationEnum>map("getClearingObligation", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getClearingObligation()), CardinalityOperator.Any));
					}
	...
	class NoOp implements FCAUKEMIRTransactionReportUKEMIR_VR_2030_01 {
