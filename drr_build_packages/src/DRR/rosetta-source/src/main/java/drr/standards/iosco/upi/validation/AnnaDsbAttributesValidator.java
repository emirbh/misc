...
public class AnnaDsbAttributesValidator implements Validator<AnnaDsbAttributes> {
	...
	private List<ComparisonResult> getComparisonResults(AnnaDsbAttributes o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("Category", (AnnaDsbCategory) o.getCategory() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AnnaDsbAttributes", ValidationResult.ValidationType.CARDINALITY, "AnnaDsbAttributes", path, "", res.getError());
				}
				return success("AnnaDsbAttributes", ValidationResult.ValidationType.CARDINALITY, "AnnaDsbAttributes", path, "");
			})
			.collect(toList());
	}
