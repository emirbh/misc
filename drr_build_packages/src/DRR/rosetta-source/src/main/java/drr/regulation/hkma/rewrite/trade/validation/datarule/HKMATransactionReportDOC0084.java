 ...
 * @version 7.7.0
 */
@RosettaDataRule("HKMATransactionReportDOC0084")
@ImplementedBy(HKMATransactionReportDOC0084.Default.class)
public interface HKMATransactionReportDOC0084 extends Validator<HKMATransactionReport> {
	
	String NAME = "HKMATransactionReportDOC0084";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI, ActionTypeEnum -> TERM] any = actionType then if otherPayment -> paymentType exists then otherPayment -> date exists";
	
	class Default implements HKMATransactionReportDOC0084 {
			...
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI), MapperS.of(ActionTypeEnum.TERM)), MapperS.of(hKMATransactionReport).<ActionTypeEnum>map("getActionType", _hKMATransactionReport -> _hKMATransactionReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					if (exists(MapperS.of(hKMATransactionReport).<OtherPayment>mapC("getOtherPayment", _hKMATransactionReport -> _hKMATransactionReport.getOtherPayment()).<PaymentType4Code>map("getPaymentType", otherPayment -> otherPayment.getPaymentType())).getOrDefault(false)) {
						return exists(MapperS.of(hKMATransactionReport).<OtherPayment>mapC("getOtherPayment", _hKMATransactionReport -> _hKMATransactionReport.getOtherPayment()).<Date>map("getDate", otherPayment -> otherPayment.getDate()));
					}
	...
	class NoOp implements HKMATransactionReportDOC0084 {
