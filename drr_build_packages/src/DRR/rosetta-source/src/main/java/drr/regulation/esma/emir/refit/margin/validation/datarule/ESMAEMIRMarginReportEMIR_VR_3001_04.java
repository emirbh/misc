...
public interface ESMAEMIRMarginReportEMIR_VR_3001_04 extends Validator<ESMAEMIRMarginReport> {
	...
	String DEFINITION = "CompareDateTo(reportingTimestamp -> date, 2024, 4, 29) >= 0";
	...
	class Default implements ESMAEMIRMarginReportEMIR_VR_3001_04 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				return greaterThanEquals(MapperS.of(compareDateTo.evaluate(MapperS.of(eSMAEMIRMarginReport).<ZonedDateTime>map("getReportingTimestamp", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getReportingTimestamp()).<Date>map("Date", zdt -> Date.of(zdt.toLocalDate())).get(), 2024, 4, 29)), MapperS.of(0), CardinalityOperator.All);
			}
			catch (Exception ex) {
