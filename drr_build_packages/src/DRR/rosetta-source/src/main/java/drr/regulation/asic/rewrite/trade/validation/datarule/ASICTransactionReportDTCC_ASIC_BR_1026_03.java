 ...
 * @version 7.7.0
 */
@RosettaDataRule("ASICTransactionReportDTCC_ASIC_BR_1026_03")
@ImplementedBy(ASICTransactionReportDTCC_ASIC_BR_1026_03.Default.class)
public interface ASICTransactionReportDTCC_ASIC_BR_1026_03 extends Validator<ASICTransactionReport> {
	
	String NAME = "ASICTransactionReportDTCC_ASIC_BR_1026_03";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType and assetClass = CommonAssetClass -> CURR and contractType = CommonContractType -> OPTN then if nonReportable -> postUpiData -> Header -> InstrumentType = Option and nonReportable -> postUpiData -> Header -> AssetClass = Foreign_Exchange and nonReportable -> postUpiData -> Header -> UseCase <> Digital_Option then leg1 -> notionalAmount = callAmount or leg1 -> notionalAmount = putAmount or leg1 -> notionalAmount = 9999999999999999999999999 and (if [CommonAssetClass -> CRDT, CommonAssetClass -> EQUI, CommonAssetClass -> CURR, CommonAssetClass -> INTR] any = assetClass and leg1 -> notionalAmount < 0 then False else True)";
	
	class Default implements ASICTransactionReportDTCC_ASIC_BR_1026_03 {
	...
	class NoOp implements ASICTransactionReportDTCC_ASIC_BR_1026_03 {
