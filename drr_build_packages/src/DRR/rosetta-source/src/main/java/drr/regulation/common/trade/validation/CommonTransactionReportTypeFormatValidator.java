...
public class CommonTransactionReportTypeFormatValidator implements Validator<CommonTransactionReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommonTransactionReport", ValidationResult.ValidationType.TYPE_FORMAT, "CommonTransactionReport", path, "", res.getError());
				}
				return success("CommonTransactionReport", ValidationResult.ValidationType.TYPE_FORMAT, "CommonTransactionReport", path, "");
			})
			.collect(toList());
	}
