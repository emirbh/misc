...
public class CSATransactionReportPPDTypeFormatValidator implements Validator<CSATransactionReportPPD> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CSATransactionReportPPD", ValidationResult.ValidationType.TYPE_FORMAT, "CSATransactionReportPPD", path, "", res.getError());
				}
				return success("CSATransactionReportPPD", ValidationResult.ValidationType.TYPE_FORMAT, "CSATransactionReportPPD", path, "");
			})
			.collect(toList());
	}
