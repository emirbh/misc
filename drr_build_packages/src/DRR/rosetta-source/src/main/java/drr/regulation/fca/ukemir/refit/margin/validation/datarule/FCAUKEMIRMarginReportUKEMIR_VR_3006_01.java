 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRMarginReportUKEMIR_VR_3006_01")
@ImplementedBy(FCAUKEMIRMarginReportUKEMIR_VR_3006_01.Default.class)
public interface FCAUKEMIRMarginReportUKEMIR_VR_3006_01 extends Validator<FCAUKEMIRMarginReport> {
	
	String NAME = "FCAUKEMIRMarginReportUKEMIR_VR_3006_01";
	String DEFINITION = "if counterparty2IdentifierType = True then API_GetLeiData(counterparty2) -> lei exists";
	
	class Default implements FCAUKEMIRMarginReportUKEMIR_VR_3006_01 {
		...
		private ComparisonResult executeDataRule(FCAUKEMIRMarginReport fCAUKEMIRMarginReport) {
			try {
				if (areEqual(MapperS.of(fCAUKEMIRMarginReport).<Boolean>map("getCounterparty2IdentifierType", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getCounterparty2IdentifierType()), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(aPI_GetLeiData.evaluate(MapperS.of(fCAUKEMIRMarginReport).<String>map("getCounterparty2", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getCounterparty2()).get())).<String>map("getLei", leiData -> leiData.getLei()));
				}
	...
	class NoOp implements FCAUKEMIRMarginReportUKEMIR_VR_3006_01 {
