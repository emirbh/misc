...
@ImplementedBy(BrokerIdentifierFormatRule.BrokerIdentifierFormatRuleDefault.class)
public abstract class BrokerIdentifierFormatRule implements ReportFunction<TransactionReportInstruction, PartyIdentifierFormat2Enum> {
	...
	*/
	...
	public static class BrokerIdentifierFormatRuleDefault extends BrokerIdentifierFormatRule {
		@Override
			...
			output = MapperS.of(input)
				.mapSingleToItem(reportInstruction -> {
					final MapperS<Party> thenArg = MapperS.of(input)
						.mapSingleToItem(item -> MapperS.of(extract_BrokerId.evaluate(item.get())));
					return thenArg
