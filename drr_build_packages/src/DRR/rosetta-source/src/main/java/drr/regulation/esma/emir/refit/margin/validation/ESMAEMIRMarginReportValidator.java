...
public class ESMAEMIRMarginReportValidator implements Validator<ESMAEMIRMarginReport> {
	...
	private List<ComparisonResult> getComparisonResults(ESMAEMIRMarginReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("nonReportable", (ESMAEMIRNonReportableCollateralData) o.getNonReportable() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ESMAEMIRMarginReport", ValidationResult.ValidationType.CARDINALITY, "ESMAEMIRMarginReport", path, "", res.getError());
				}
				return success("ESMAEMIRMarginReport", ValidationResult.ValidationType.CARDINALITY, "ESMAEMIRMarginReport", path, "");
			})
			.collect(toList());
	}
