 ...
 * @version 7.7.0
 */
@RosettaDataRule("ReportableJurisdictionInformationFCAReportingRegime")
@ImplementedBy(ReportableJurisdictionInformationFCAReportingRegime.Default.class)
public interface ReportableJurisdictionInformationFCAReportingRegime extends Validator<ReportableJurisdictionInformation> {
	
	String NAME = "ReportableJurisdictionInformationFCAReportingRegime";
	String DEFINITION = "if supervisoryBody = FCA then partyInformation -> fcaPartyInformation exists else partyInformation -> fcaPartyInformation is absent";
	
	class Default implements ReportableJurisdictionInformationFCAReportingRegime {
		...
		private ComparisonResult executeDataRule(ReportableJurisdictionInformation reportableJurisdictionInformation) {
			try {
				if (areEqual(MapperS.of(reportableJurisdictionInformation).<FieldWithMetaSupervisoryBodyEnum>map("getSupervisoryBody", _reportableJurisdictionInformation -> _reportableJurisdictionInformation.getSupervisoryBody()).<SupervisoryBodyEnum>map("Type coercion", fieldWithMetaSupervisoryBodyEnum -> fieldWithMetaSupervisoryBodyEnum == null ? null : fieldWithMetaSupervisoryBodyEnum.getValue()), MapperS.of(SupervisoryBodyEnum.FCA), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(reportableJurisdictionInformation).<JurisdictionPartyInformation>mapC("getPartyInformation", _reportableJurisdictionInformation -> _reportableJurisdictionInformation.getPartyInformation()).<FCAPartyInformation>map("getFcaPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getFcaPartyInformation()));
				}
				return notExists(MapperS.of(reportableJurisdictionInformation).<JurisdictionPartyInformation>mapC("getPartyInformation", _reportableJurisdictionInformation -> _reportableJurisdictionInformation.getPartyInformation()).<FCAPartyInformation>map("getFcaPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getFcaPartyInformation()));
			}
			catch (Exception ex) {
	...
	class NoOp implements ReportableJurisdictionInformationFCAReportingRegime {
