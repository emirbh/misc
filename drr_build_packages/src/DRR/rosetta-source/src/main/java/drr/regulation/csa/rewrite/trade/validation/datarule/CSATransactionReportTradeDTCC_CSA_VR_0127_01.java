 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportTradeDTCC_CSA_VR_0127_01")
@ImplementedBy(CSATransactionReportTradeDTCC_CSA_VR_0127_01.Default.class)
public interface CSATransactionReportTradeDTCC_CSA_VR_0127_01 extends Validator<CSATransactionReportTrade> {
	
	String NAME = "CSATransactionReportTradeDTCC_CSA_VR_0127_01";
	String DEFINITION = "if basketConstituents -> unitOfMeasure exists then basketConstituents -> unitOfMeasure count = basketConstituents -> identifier count";
	
	class Default implements CSATransactionReportTradeDTCC_CSA_VR_0127_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReportTrade cSATransactionReportTrade) {
			try {
				if (exists(MapperS.of(cSATransactionReportTrade).<BasketConstituentsReport>mapC("getBasketConstituents", _cSATransactionReportTrade -> _cSATransactionReportTrade.getBasketConstituents()).<String>map("getUnitOfMeasure", basketConstituentsReport -> basketConstituentsReport.getUnitOfMeasure())).getOrDefault(false)) {
	...
	class NoOp implements CSATransactionReportTradeDTCC_CSA_VR_0127_01 {
