...
public class FCAUKEMIRMarginReportTypeFormatValidator implements Validator<FCAUKEMIRMarginReport> {
	...
	private List<ComparisonResult> getComparisonResults(FCAUKEMIRMarginReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkString("executionAgentOfCounterparty2", o.getExecutionAgentOfCounterparty2(), 0, empty(), of(Pattern.compile("[A-Z0-9]{18,18}[0-9]{2,2}")))
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FCAUKEMIRMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "FCAUKEMIRMarginReport", path, "", res.getError());
				}
				return success("FCAUKEMIRMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "FCAUKEMIRMarginReport", path, "");
			})
			.collect(toList());
	}
