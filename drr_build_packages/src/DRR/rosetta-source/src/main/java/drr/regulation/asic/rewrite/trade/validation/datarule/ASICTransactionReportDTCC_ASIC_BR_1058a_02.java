 ...
 * @version 7.7.0
 */
@RosettaDataRule("ASICTransactionReportDTCC_ASIC_BR_1058a_02")
@ImplementedBy(ASICTransactionReportDTCC_ASIC_BR_1058a_02.Default.class)
public interface ASICTransactionReportDTCC_ASIC_BR_1058a_02 extends Validator<ASICTransactionReport> {
	
	String NAME = "ASICTransactionReportDTCC_ASIC_BR_1058a_02";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType and [CommonContractType -> OPTN, CommonContractType -> SWPT, CommonContractType -> FORW] any = contractType then if barrier -> lower -> price -> monetary exists or barrier -> lower -> price -> decimal exists then barrier -> lower -> notation exists";
	
	class Default implements ASICTransactionReportDTCC_ASIC_BR_1058a_02 {
		...
		private ComparisonResult executeDataRule(ASICTransactionReport aSICTransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(aSICTransactionReport).<ActionTypeEnum>map("getActionType", _aSICTransactionReport -> _aSICTransactionReport.getActionType()), CardinalityOperator.Any).andNullSafe(areEqual(MapperC.<CommonContractType>of(MapperS.of(CommonContractType.OPTN), MapperS.of(CommonContractType.SWPT), MapperS.of(CommonContractType.FORW)), MapperS.of(aSICTransactionReport).<CommonContractType>map("getContractType", _aSICTransactionReport -> _aSICTransactionReport.getContractType()), CardinalityOperator.Any)).getOrDefault(false)) {
					if (exists(MapperS.of(aSICTransactionReport).<SingleOrUpperAndLowerBarrier>map("getBarrier", _aSICTransactionReport -> _aSICTransactionReport.getBarrier()).<BarrierPrice>map("getLower", singleOrUpperAndLowerBarrier -> singleOrUpperAndLowerBarrier.getLower()).<PriceFormat>map("getPrice", barrierPrice -> barrierPrice.getPrice()).<BigDecimal>map("getMonetary", priceFormat -> priceFormat.getMonetary())).orNullSafe(exists(MapperS.of(aSICTransactionReport).<SingleOrUpperAndLowerBarrier>map("getBarrier", _aSICTransactionReport -> _aSICTransactionReport.getBarrier()).<BarrierPrice>map("getLower", singleOrUpperAndLowerBarrier -> singleOrUpperAndLowerBarrier.getLower()).<PriceFormat>map("getPrice", barrierPrice -> barrierPrice.getPrice()).<BigDecimal>map("getDecimal", priceFormat -> priceFormat.getDecimal()))).getOrDefault(false)) {
						return exists(MapperS.of(aSICTransactionReport).<SingleOrUpperAndLowerBarrier>map("getBarrier", _aSICTransactionReport -> _aSICTransactionReport.getBarrier()).<BarrierPrice>map("getLower", singleOrUpperAndLowerBarrier -> singleOrUpperAndLowerBarrier.getLower()).<PriceNotationEnum>map("getNotation", barrierPrice -> barrierPrice.getNotation()));
					}
	...
	class NoOp implements ASICTransactionReportDTCC_ASIC_BR_1058a_02 {
