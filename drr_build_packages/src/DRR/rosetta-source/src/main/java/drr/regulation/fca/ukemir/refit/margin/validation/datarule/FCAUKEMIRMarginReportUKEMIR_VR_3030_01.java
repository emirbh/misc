 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRMarginReportUKEMIR_VR_3030_01")
@ImplementedBy(FCAUKEMIRMarginReportUKEMIR_VR_3030_01.Default.class)
public interface FCAUKEMIRMarginReportUKEMIR_VR_3030_01 extends Validator<FCAUKEMIRMarginReport> {
	
	String NAME = "FCAUKEMIRMarginReportUKEMIR_VR_3030_01";
	String DEFINITION = "if (executionAgentOfCounterparty1 exists and executionAgentOfCounterparty2 exists) or (executionAgentOfCounterparty1 is absent and executionAgentOfCounterparty2 is absent) then nonReportable -> relationshipRecord is absent";
	
	class Default implements FCAUKEMIRMarginReportUKEMIR_VR_3030_01 {
		...
		private ComparisonResult executeDataRule(FCAUKEMIRMarginReport fCAUKEMIRMarginReport) {
			try {
				if (exists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty1", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty1())).andNullSafe(exists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty2", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty2()))).orNullSafe(notExists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty1", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty1())).andNullSafe(notExists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty2", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty2())))).getOrDefault(false)) {
					return notExists(MapperS.of(fCAUKEMIRMarginReport).<FCAUKEMIRNonReportableCollateralData>map("getNonReportable", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getNonReportable()).<String>map("getRelationshipRecord", fCAUKEMIRNonReportableCollateralData -> fCAUKEMIRNonReportableCollateralData.getRelationshipRecord()));
				}
	...
	class NoOp implements FCAUKEMIRMarginReportUKEMIR_VR_3030_01 {
