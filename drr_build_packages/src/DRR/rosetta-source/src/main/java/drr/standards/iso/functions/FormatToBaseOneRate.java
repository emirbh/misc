...
@ImplementedBy(FormatToBaseOneRate.FormatToBaseOneRateDefault.class)
public abstract class FormatToBaseOneRate implements RosettaFunction {
	...
	*/
	...
	public static class FormatToBaseOneRateDefault extends FormatToBaseOneRate {
		@Override
		...
		protected BigDecimal assignOutput(BigDecimal formattedNumber, BigDecimal inputNumber) {
			if (lessThanEquals(MapperS.of(abs.evaluate(inputNumber)), MapperS.of(BigDecimal.valueOf(99999999999l)), CardinalityOperator.All).getOrDefault(false)) {
				final MapperS<BigDecimal> thenArg = MapperS.of(roundToPrecision.evaluate(inputNumber, 10, RoundingDirectionEnum.NEAREST, true));
				formattedNumber = MapperS.of(roundToSignificantFigures.evaluate(thenArg.get(), 11, RoundingDirectionEnum.NEAREST)).get();
			} else {
