	...
	*/
	...
	public static class Counterparty2FinancialEntityIndicatorRuleDefault extends Counterparty2FinancialEntityIndicatorRule {
		...
		protected Boolean assignOutput(Boolean output, TransactionReportInstruction input) {
			...
			if (exists(thenArg1.<CFTCPartyInformation>map("getCftcPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getCftcPartyInformation()).<CFTCFinancialEntityClassificationEnum>map("getFinancialEntity", cFTCPartyInformation -> cFTCPartyInformation.getFinancialEntity())).getOrDefault(false)) {
