...
import drr.standards.iosco.cde.version3.underlier.reports.UnderlierIDOtherLeg2Rule;
import javax.inject.Inject;

import static com.rosetta.model.lib.expression.ExpressionOperatorsNullSafe.*;

@ImplementedBy(UnderlierIDOtherLeg1Rule.UnderlierIDOtherLeg1RuleDefault.class)
public abstract class UnderlierIDOtherLeg1Rule implements ReportFunction<TransactionReportInstructionBase, String> {
	
	// RosettaFunction dependencies
	//
	@Inject protected IsAllowableActionForCSA isAllowableActionForCSA;
	@Inject protected IsCSALeg1Aligned isCSALeg1Aligned;
	@Inject protected drr.standards.iosco.cde.version3.underlier.reports.UnderlierIDOtherLeg1Rule underlierIDOtherLeg1Rule;
	...
	public static class UnderlierIDOtherLeg1RuleDefault extends UnderlierIDOtherLeg1Rule {
		...
		protected String assignOutput(String output, TransactionReportInstructionBase input) {
			...
			if (areEqual(MapperS.of(isCSALeg1Aligned.evaluate(thenArg.get())), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
				ifThenElseResult = MapperS.of(underlierIDOtherLeg1Rule.evaluate(thenArg.get()));
			} else {
				ifThenElseResult = MapperS.of(underlierIDOtherLeg2Rule.evaluate(thenArg.get()));
			}
