...
public class CommonValuationReportTypeFormatValidator implements Validator<CommonValuationReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommonValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "CommonValuationReport", path, "", res.getError());
				}
				return success("CommonValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "CommonValuationReport", path, "");
			})
			.collect(toList());
	}
