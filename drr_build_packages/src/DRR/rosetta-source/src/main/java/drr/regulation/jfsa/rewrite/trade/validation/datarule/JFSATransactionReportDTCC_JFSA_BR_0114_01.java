 ...
 * @version 7.7.0
 */
@RosettaDataRule("JFSATransactionReportDTCC_JFSA_BR_0114_01")
@ImplementedBy(JFSATransactionReportDTCC_JFSA_BR_0114_01.Default.class)
public interface JFSATransactionReportDTCC_JFSA_BR_0114_01 extends Validator<JFSATransactionReport> {
	
	String NAME = "JFSATransactionReportDTCC_JFSA_BR_0114_01";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType then (if underlyingIdentificationType is absent then referenceEntity exists)";
	
	class Default implements JFSATransactionReportDTCC_JFSA_BR_0114_01 {
		...
		private ComparisonResult executeDataRule(JFSATransactionReport jFSATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(jFSATransactionReport).<ActionTypeEnum>map("getActionType", _jFSATransactionReport -> _jFSATransactionReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					if (notExists(MapperS.of(jFSATransactionReport).<UnderlyingIdentificationTypeEnum>map("getUnderlyingIdentificationType", _jFSATransactionReport -> _jFSATransactionReport.getUnderlyingIdentificationType())).getOrDefault(false)) {
						return exists(MapperS.of(jFSATransactionReport).<String>map("getReferenceEntity", _jFSATransactionReport -> _jFSATransactionReport.getReferenceEntity()));
					}
	...
	class NoOp implements JFSATransactionReportDTCC_JFSA_BR_0114_01 {
