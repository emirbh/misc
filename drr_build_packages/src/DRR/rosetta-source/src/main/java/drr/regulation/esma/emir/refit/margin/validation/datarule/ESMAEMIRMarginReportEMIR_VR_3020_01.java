 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3020_01")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3020_01.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3020_01 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3020_01";
	String DEFINITION = "if [CollateralisationType3Code -> OWC2, CollateralisationType3Code -> OWP2, CollateralisationType3Code -> FLCL] any = collateralisationCategory then initialMarginCollectedByCounterparty1PreHaircut exists else initialMarginCollectedByCounterparty1PreHaircut is absent";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3020_01 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				if (areEqual(MapperC.<CollateralisationType3Code>of(MapperS.of(CollateralisationType3Code.OWC2), MapperS.of(CollateralisationType3Code.OWP2), MapperS.of(CollateralisationType3Code.FLCL)), MapperS.of(eSMAEMIRMarginReport).<CollateralisationType3Code>map("getCollateralisationCategory", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCollateralisationCategory()), CardinalityOperator.Any).getOrDefault(false)) {
					return exists(MapperS.of(eSMAEMIRMarginReport).<BigDecimal>map("getInitialMarginCollectedByCounterparty1PreHaircut", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getInitialMarginCollectedByCounterparty1PreHaircut()));
				}
				return notExists(MapperS.of(eSMAEMIRMarginReport).<BigDecimal>map("getInitialMarginCollectedByCounterparty1PreHaircut", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getInitialMarginCollectedByCounterparty1PreHaircut()));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3020_01 {
