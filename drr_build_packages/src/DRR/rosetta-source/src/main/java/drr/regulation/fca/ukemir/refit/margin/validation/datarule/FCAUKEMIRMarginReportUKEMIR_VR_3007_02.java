 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRMarginReportUKEMIR_VR_3007_02")
@ImplementedBy(FCAUKEMIRMarginReportUKEMIR_VR_3007_02.Default.class)
public interface FCAUKEMIRMarginReportUKEMIR_VR_3007_02 extends Validator<FCAUKEMIRMarginReport> {
	
	String NAME = "FCAUKEMIRMarginReportUKEMIR_VR_3007_02";
	String DEFINITION = "CompareDateTo(collateralTimestamp -> date, 2014, 2, 12) >= 0";
	
	class Default implements FCAUKEMIRMarginReportUKEMIR_VR_3007_02 {
		...
		private ComparisonResult executeDataRule(FCAUKEMIRMarginReport fCAUKEMIRMarginReport) {
			try {
				return greaterThanEquals(MapperS.of(compareDateTo.evaluate(MapperS.of(fCAUKEMIRMarginReport).<ZonedDateTime>map("getCollateralTimestamp", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getCollateralTimestamp()).<Date>map("Date", zdt -> Date.of(zdt.toLocalDate())).get(), 2014, 2, 12)), MapperS.of(0), CardinalityOperator.All);
			}
			catch (Exception ex) {
	...
	class NoOp implements FCAUKEMIRMarginReportUKEMIR_VR_3007_02 {
