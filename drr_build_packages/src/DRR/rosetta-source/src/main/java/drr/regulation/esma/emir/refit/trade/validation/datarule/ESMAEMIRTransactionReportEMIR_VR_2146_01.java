 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRTransactionReportEMIR_VR_2146_01")
@ImplementedBy(ESMAEMIRTransactionReportEMIR_VR_2146_01.Default.class)
public interface ESMAEMIRTransactionReportEMIR_VR_2146_01 extends Validator<ESMAEMIRTransactionReport> {
	
	String NAME = "ESMAEMIRTransactionReportEMIR_VR_2146_01";
	...
	class Default implements ESMAEMIRTransactionReportEMIR_VR_2146_01 {
			...
			try {
				...
				if ((_boolean == null ? false : _boolean)) {
					if (areEqual(MapperS.of(eSMAEMIRTransactionReport).<CommonAssetClass>map("getAssetClass", _eSMAEMIRTransactionReport -> _eSMAEMIRTransactionReport.getAssetClass()), MapperS.of(CommonAssetClass.CRDT), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(eSMAEMIRTransactionReport).<UnderlyingIdentificationTypeEnum>map("getUnderlyingIdentificationType", _eSMAEMIRTransactionReport -> _eSMAEMIRTransactionReport.getUnderlyingIdentificationType()), MapperS.of(UnderlyingIdentificationTypeEnum.X), CardinalityOperator.All)).getOrDefault(false)) {
						return exists(MapperS.of(eSMAEMIRTransactionReport).<Integer>map("getSeriesVersion", _eSMAEMIRTransactionReport -> _eSMAEMIRTransactionReport.getSeriesVersion())).andNullSafe(greaterThanEquals(MapperS.of(eSMAEMIRTransactionReport).<Integer>map("getSeriesVersion", _eSMAEMIRTransactionReport -> _eSMAEMIRTransactionReport.getSeriesVersion()), MapperS.of(0), CardinalityOperator.All));
					}
					return notExists(MapperS.of(eSMAEMIRTransactionReport).<Integer>map("getSeriesVersion", _eSMAEMIRTransactionReport -> _eSMAEMIRTransactionReport.getSeriesVersion()));
				}
	...
	class NoOp implements ESMAEMIRTransactionReportEMIR_VR_2146_01 {
