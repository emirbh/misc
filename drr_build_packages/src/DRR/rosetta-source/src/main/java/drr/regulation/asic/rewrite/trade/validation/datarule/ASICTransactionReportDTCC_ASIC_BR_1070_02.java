 ...
 * @version 7.7.0
 */
@RosettaDataRule("ASICTransactionReportDTCC_ASIC_BR_1070_02")
@ImplementedBy(ASICTransactionReportDTCC_ASIC_BR_1070_02.Default.class)
public interface ASICTransactionReportDTCC_ASIC_BR_1070_02 extends Validator<ASICTransactionReport> {
	
	String NAME = "ASICTransactionReportDTCC_ASIC_BR_1070_02";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType and [CommonAssetClass -> CRDT, CommonAssetClass -> EQUI, CommonAssetClass -> COMM, CommonAssetClass -> INTR] any = assetClass and [CommonContractType -> FRAS, CommonContractType -> OPTN, CommonContractType -> SWAP, CommonContractType -> SWPT, CommonContractType -> OTHR] any = contractType then if [FrequencyPeriodEnum -> YEAR, FrequencyPeriodEnum -> MNTH, FrequencyPeriodEnum -> WEEK, FrequencyPeriodEnum -> DAIL, FrequencyPeriodEnum -> EXPI] any = leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriod then leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriodMultiplier exists and (leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriodMultiplier < 1000 and leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriodMultiplier > 0) else if leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriod = FrequencyPeriodEnum -> EXPI then leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriodMultiplier = 1 else if leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriod is absent or leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriod = FrequencyPeriodEnum -> ADHO then leg1 -> periodicPayment -> floatingRatePaymentFrequencyPeriodMultiplier is absent";
	
	class Default implements ASICTransactionReportDTCC_ASIC_BR_1070_02 {
	...
	class NoOp implements ASICTransactionReportDTCC_ASIC_BR_1070_02 {
