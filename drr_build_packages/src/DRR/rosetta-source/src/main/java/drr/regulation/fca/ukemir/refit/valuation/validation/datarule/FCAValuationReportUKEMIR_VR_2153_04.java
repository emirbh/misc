 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAValuationReportUKEMIR_VR_2153_04")
@ImplementedBy(FCAValuationReportUKEMIR_VR_2153_04.Default.class)
public interface FCAValuationReportUKEMIR_VR_2153_04 extends Validator<FCAValuationReport> {
	
	String NAME = "FCAValuationReportUKEMIR_VR_2153_04";
	String DEFINITION = "EventDate_04_Validation(eventDate, reportingTimestamp)";
	
	class Default implements FCAValuationReportUKEMIR_VR_2153_04 {
		...
		private ComparisonResult executeDataRule(FCAValuationReport fCAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(eventDate_04_Validation.evaluate(MapperS.of(fCAValuationReport).<Date>map("getEventDate", _fCAValuationReport -> _fCAValuationReport.getEventDate()).get(), MapperS.of(fCAValuationReport).<ZonedDateTime>map("getReportingTimestamp", _fCAValuationReport -> _fCAValuationReport.getReportingTimestamp()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements FCAValuationReportUKEMIR_VR_2153_04 {
