...
public class CFTCPart43TransactionReportTypeFormatValidator implements Validator<CFTCPart43TransactionReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCPart43TransactionReport", ValidationResult.ValidationType.TYPE_FORMAT, "CFTCPart43TransactionReport", path, "", res.getError());
				}
				return success("CFTCPart43TransactionReport", ValidationResult.ValidationType.TYPE_FORMAT, "CFTCPart43TransactionReport", path, "");
			})
			.collect(toList());
	}
