...
public class SECTransactionReportValidator implements Validator<SECTransactionReport> {

	private List<ComparisonResult> getComparisonResults(SECTransactionReport o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, SECTransactionReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SECTransactionReport", ValidationResult.ValidationType.CARDINALITY, "SECTransactionReport", path, "", res.getError());
				}
				return success("SECTransactionReport", ValidationResult.ValidationType.CARDINALITY, "SECTransactionReport", path, "");
			})
			.collect(toList());
	}
