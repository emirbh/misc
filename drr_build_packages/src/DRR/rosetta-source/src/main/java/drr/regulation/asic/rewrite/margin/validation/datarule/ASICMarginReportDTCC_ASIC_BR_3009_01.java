 ...
 * @version 7.7.0
 */
@RosettaDataRule("ASICMarginReportDTCC_ASIC_BR_3009_01")
@ImplementedBy(ASICMarginReportDTCC_ASIC_BR_3009_01.Default.class)
public interface ASICMarginReportDTCC_ASIC_BR_3009_01 extends Validator<ASICMarginReport> {
	
	String NAME = "ASICMarginReportDTCC_ASIC_BR_3009_01";
	String DEFINITION = "if actionType = MarginActionEnum -> MARU or actionType = MarginActionEnum -> CORR then if collateralPortfolioIndicator = True then portfolioContainingNonReportedComponentIndicator exists";
	
	class Default implements ASICMarginReportDTCC_ASIC_BR_3009_01 {
			...
			try {
				if (areEqual(MapperS.of(aSICMarginReport).<MarginActionEnum>map("getActionType", _aSICMarginReport -> _aSICMarginReport.getActionType()), MapperS.of(MarginActionEnum.MARU), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(aSICMarginReport).<MarginActionEnum>map("getActionType", _aSICMarginReport -> _aSICMarginReport.getActionType()), MapperS.of(MarginActionEnum.CORR), CardinalityOperator.All)).getOrDefault(false)) {
					if (areEqual(MapperS.of(aSICMarginReport).<Boolean>map("getCollateralPortfolioIndicator", _aSICMarginReport -> _aSICMarginReport.getCollateralPortfolioIndicator()), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
						return exists(MapperS.of(aSICMarginReport).<Boolean>map("getPortfolioContainingNonReportedComponentIndicator", _aSICMarginReport -> _aSICMarginReport.getPortfolioContainingNonReportedComponentIndicator()));
					}
	...
	class NoOp implements ASICMarginReportDTCC_ASIC_BR_3009_01 {
