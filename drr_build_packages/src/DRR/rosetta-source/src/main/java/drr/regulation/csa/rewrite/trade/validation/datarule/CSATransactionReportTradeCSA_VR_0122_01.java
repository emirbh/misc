 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportTradeCSA_VR_0122_01")
@ImplementedBy(CSATransactionReportTradeCSA_VR_0122_01.Default.class)
public interface CSATransactionReportTradeCSA_VR_0122_01 extends Validator<CSATransactionReportTrade> {
	
	String NAME = "CSATransactionReportTradeCSA_VR_0122_01";
	String DEFINITION = "if customBasketIndicator = True then customBasketCode exists else customBasketCode is absent";
	
	class Default implements CSATransactionReportTradeCSA_VR_0122_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReportTrade cSATransactionReportTrade) {
			try {
				if (areEqual(MapperS.of(cSATransactionReportTrade).<Boolean>map("getCustomBasketIndicator", _cSATransactionReportTrade -> _cSATransactionReportTrade.getCustomBasketIndicator()), MapperS.of(true), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(cSATransactionReportTrade).<String>map("getCustomBasketCode", _cSATransactionReportTrade -> _cSATransactionReportTrade.getCustomBasketCode()));
				}
				return notExists(MapperS.of(cSATransactionReportTrade).<String>map("getCustomBasketCode", _cSATransactionReportTrade -> _cSATransactionReportTrade.getCustomBasketCode()));
			}
			catch (Exception ex) {
	...
	class NoOp implements CSATransactionReportTradeCSA_VR_0122_01 {
