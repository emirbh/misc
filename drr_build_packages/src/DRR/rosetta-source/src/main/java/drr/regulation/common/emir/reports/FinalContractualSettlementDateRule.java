		...
		protected Date assignOutput(Date output, TransactionReportInstruction input) {
			...
			output = thenArg
				.mapSingleToItem(item -> {
					final Boolean _boolean = isFRA.evaluate(productForEvent.evaluate(item.get()));
					if ((_boolean == null ? false : _boolean)) {
						return MapperS.of(interestRateLeg2.evaluate(productForEvent.evaluate(item.get()))).<AdjustableDate>map("getPaymentDate", interestRatePayout -> interestRatePayout.getPaymentDate()).<Date>map("getUnadjustedDate", adjustableDate -> adjustableDate.getUnadjustedDate());
					}
