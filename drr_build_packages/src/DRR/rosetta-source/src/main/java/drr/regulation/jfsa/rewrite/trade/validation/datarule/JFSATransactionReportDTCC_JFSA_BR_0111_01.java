...
public interface JFSATransactionReportDTCC_JFSA_BR_0111_01 extends Validator<JFSATransactionReport> {
	...
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType and assetClass = CommonAssetClass -> INTR then (if underlyingIdentificationType = I then underlyingIdentification exists else if underlyingIdentificationType = X and underlyingIndexIndicator is absent and nameOfTheUnderlyingIndex is absent then underlyingIdentification exists)";
		...
		private ComparisonResult executeDataRule(JFSATransactionReport jFSATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(jFSATransactionReport).<ActionTypeEnum>map("getActionType", _jFSATransactionReport -> _jFSATransactionReport.getActionType()), CardinalityOperator.Any).andNullSafe(areEqual(MapperS.of(jFSATransactionReport).<CommonAssetClass>map("getAssetClass", _jFSATransactionReport -> _jFSATransactionReport.getAssetClass()), MapperS.of(CommonAssetClass.INTR), CardinalityOperator.All)).getOrDefault(false)) {
					...
					if (areEqual(MapperS.of(jFSATransactionReport).<UnderlyingIdentificationTypeEnum>map("getUnderlyingIdentificationType", _jFSATransactionReport -> _jFSATransactionReport.getUnderlyingIdentificationType()), MapperS.of(UnderlyingIdentificationTypeEnum.X), CardinalityOperator.All).andNullSafe(notExists(MapperS.of(jFSATransactionReport).<IndexEnum>map("getUnderlyingIndexIndicator", _jFSATransactionReport -> _jFSATransactionReport.getUnderlyingIndexIndicator()))).andNullSafe(notExists(MapperS.of(jFSATransactionReport).<String>map("getNameOfTheUnderlyingIndex", _jFSATransactionReport -> _jFSATransactionReport.getNameOfTheUnderlyingIndex()))).getOrDefault(false)) {
						return exists(MapperS.of(jFSATransactionReport).<String>map("getUnderlyingIdentification", _jFSATransactionReport -> _jFSATransactionReport.getUnderlyingIdentification()));
					}
