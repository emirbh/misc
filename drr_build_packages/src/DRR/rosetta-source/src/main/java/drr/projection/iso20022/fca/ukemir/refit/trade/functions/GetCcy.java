...
public abstract class GetCcy implements RosettaFunction {
	...
	*/
	...
	protected abstract CurrencyExchange22__1.CurrencyExchange22__1Builder doEvaluate(FCAUKEMIRTransactionReport drrReport);
	...
	public static class GetCcyDefault extends GetCcy {
		...
		protected CurrencyExchange22__1.CurrencyExchange22__1Builder assignOutput(CurrencyExchange22__1.CurrencyExchange22__1Builder ccy, FCAUKEMIRTransactionReport drrReport) {
			...
			if (areEqual(MapperS.of(drrReport).<CommonAssetClass>map("getAssetClass", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getAssetClass()), MapperS.of(CommonAssetClass.CURR), CardinalityOperator.All).getOrDefault(false)) {
				ifThenElseResult0 = subString.evaluate(MapperS.of(drrReport).<String>map("getExchangeRateBasis", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getExchangeRateBasis()).get(), 1, 3);
			}
			String ifThenElseResult1 = null;
			if (areEqual(MapperS.of(drrReport).<CommonAssetClass>map("getAssetClass", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getAssetClass()), MapperS.of(CommonAssetClass.CURR), CardinalityOperator.All).getOrDefault(false)) {
				ifThenElseResult1 = subString.evaluate(MapperS.of(drrReport).<String>map("getExchangeRateBasis", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getExchangeRateBasis()).get(), 5, 3);
			}
			ccy = toBuilder(CurrencyExchange22__1.builder()
				.setXchgRate(MapperS.of(drrReport).<BigDecimal>map("getExchangeRate", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getExchangeRate()).get())
				.setFwdXchgRate(MapperS.of(drrReport).<BigDecimal>map("getForwardExchangeRate", fCAUKEMIRTransactionReport -> fCAUKEMIRTransactionReport.getForwardExchangeRate()).get())
