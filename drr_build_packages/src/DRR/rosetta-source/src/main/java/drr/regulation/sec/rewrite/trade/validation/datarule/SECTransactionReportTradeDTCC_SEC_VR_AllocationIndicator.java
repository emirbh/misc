 ...
 * @version 7.7.0
 */
@RosettaDataRule("SECTransactionReportTradeDTCC_SEC_VR_AllocationIndicator")
@ImplementedBy(SECTransactionReportTradeDTCC_SEC_VR_AllocationIndicator.Default.class)
public interface SECTransactionReportTradeDTCC_SEC_VR_AllocationIndicator extends Validator<SECTransactionReportTrade> {
	
	String NAME = "SECTransactionReportTradeDTCC_SEC_VR_AllocationIndicator";
	String DEFINITION = "if actionType = NEWT and eventType = ALOC then allocationIndicator <> UNAL";
	
	class Default implements SECTransactionReportTradeDTCC_SEC_VR_AllocationIndicator {
		...
		private ComparisonResult executeDataRule(SECTransactionReportTrade sECTransactionReportTrade) {
			try {
				if (areEqual(MapperS.of(sECTransactionReportTrade).<ActionTypeEnum>map("getActionType", _sECTransactionReportTrade -> _sECTransactionReportTrade.getActionType()), MapperS.of(ActionTypeEnum.NEWT), CardinalityOperator.All).andNullSafe(areEqual(MapperS.of(sECTransactionReportTrade).<EventTypeEnum>map("getEventType", _sECTransactionReportTrade -> _sECTransactionReportTrade.getEventType()), MapperS.of(EventTypeEnum.ALOC), CardinalityOperator.All)).getOrDefault(false)) {
					return notEqual(MapperS.of(sECTransactionReportTrade).<AllocationIndicatorEnum>map("getAllocationIndicator", _sECTransactionReportTrade -> _sECTransactionReportTrade.getAllocationIndicator()), MapperS.of(AllocationIndicatorEnum.UNAL), CardinalityOperator.Any);
				}
	...
	class NoOp implements SECTransactionReportTradeDTCC_SEC_VR_AllocationIndicator {
