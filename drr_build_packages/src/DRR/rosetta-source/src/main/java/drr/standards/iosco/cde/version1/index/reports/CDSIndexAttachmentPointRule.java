	...
	public static class CDSIndexAttachmentPointRuleDefault extends CDSIndexAttachmentPointRule {
		...
		protected BigDecimal assignOutput(BigDecimal output, TransactionReportInstructionBase input) {
			...
			final MapperS<BigDecimal> thenArg1 = thenArg0
				.mapSingleToItem(item -> MapperS.of(item.<CreditIndex>map("getIndexReferenceInformation", generalTerms -> generalTerms.getIndexReferenceInformation()).<Tranche>map("getTranche", creditIndex -> creditIndex.getTranche()).<BigDecimal>map("getAttachmentPoint", tranche -> tranche.getAttachmentPoint()).getOrDefault(item.<BasketReferenceInformation>map("getBasketReferenceInformation", generalTerms -> generalTerms.getBasketReferenceInformation()).<Tranche>map("getTranche", basketReferenceInformation -> basketReferenceInformation.getTranche()).<BigDecimal>map("getAttachmentPoint", tranche -> tranche.getAttachmentPoint()).get())));
