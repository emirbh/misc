		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CSAMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "CSAMarginReport", path, "", res.getError());
				}
				return success("CSAMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "CSAMarginReport", path, "");
			})
			.collect(toList());
	}
