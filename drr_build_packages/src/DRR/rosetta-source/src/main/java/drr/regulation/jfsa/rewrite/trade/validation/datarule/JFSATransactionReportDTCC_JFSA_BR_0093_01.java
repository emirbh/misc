 ...
 * @version 7.7.0
 */
@RosettaDataRule("JFSATransactionReportDTCC_JFSA_BR_0093_01")
@ImplementedBy(JFSATransactionReportDTCC_JFSA_BR_0093_01.Default.class)
public interface JFSATransactionReportDTCC_JFSA_BR_0093_01 extends Validator<JFSATransactionReport> {
	
	String NAME = "JFSATransactionReportDTCC_JFSA_BR_0093_01";
	String DEFINITION = "if CommonAssetClass -> EQUI = assetClass then common.quantity.QuantityUnitOfMeasureLeg_Validation( actionType, leg1 -> totalNotionalQuantity, leg1 -> quantityUnitOfMeasure )";
	
	class Default implements JFSATransactionReportDTCC_JFSA_BR_0093_01 {
		...
		private ComparisonResult executeDataRule(JFSATransactionReport jFSATransactionReport) {
			try {
				if (areEqual(MapperS.of(CommonAssetClass.EQUI), MapperS.of(jFSATransactionReport).<CommonAssetClass>map("getAssetClass", _jFSATransactionReport -> _jFSATransactionReport.getAssetClass()), CardinalityOperator.All).getOrDefault(false)) {
					return ComparisonResult.ofNullSafe(MapperS.of(quantityUnitOfMeasureLeg_Validation.evaluate(MapperS.of(jFSATransactionReport).<ActionTypeEnum>map("getActionType", _jFSATransactionReport -> _jFSATransactionReport.getActionType()).get(), MapperS.of(jFSATransactionReport).<CommonLeg>map("getLeg1", _jFSATransactionReport -> _jFSATransactionReport.getLeg1()).<BigDecimal>map("getTotalNotionalQuantity", commonLeg -> commonLeg.getTotalNotionalQuantity()).get(), MapperS.of(jFSATransactionReport).<CommonLeg>map("getLeg1", _jFSATransactionReport -> _jFSATransactionReport.getLeg1()).<String>map("getQuantityUnitOfMeasure", commonLeg -> commonLeg.getQuantityUnitOfMeasure()).get())));
				}
	...
	class NoOp implements JFSATransactionReportDTCC_JFSA_BR_0093_01 {
