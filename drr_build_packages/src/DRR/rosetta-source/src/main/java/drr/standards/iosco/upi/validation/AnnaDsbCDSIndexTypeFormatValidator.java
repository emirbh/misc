...
public class AnnaDsbCDSIndexTypeFormatValidator implements Validator<AnnaDsbCDSIndex> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AnnaDsbCDSIndex", ValidationResult.ValidationType.TYPE_FORMAT, "AnnaDsbCDSIndex", path, "", res.getError());
				}
				return success("AnnaDsbCDSIndex", ValidationResult.ValidationType.TYPE_FORMAT, "AnnaDsbCDSIndex", path, "");
			})
			.collect(toList());
	}
