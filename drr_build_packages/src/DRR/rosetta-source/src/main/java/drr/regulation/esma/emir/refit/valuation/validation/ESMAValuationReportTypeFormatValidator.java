...
public class ESMAValuationReportTypeFormatValidator implements Validator<ESMAValuationReport> {

	private List<ComparisonResult> getComparisonResults(ESMAValuationReport o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, ESMAValuationReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ESMAValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "ESMAValuationReport", path, "", res.getError());
				}
				return success("ESMAValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "ESMAValuationReport", path, "");
			})
			.collect(toList());
	}
