 ...
 * @version 7.7.0
 */
@RosettaDataRule("SECTransactionReportTradeDTCC_SEC_VR_PriorUSI_02")
@ImplementedBy(SECTransactionReportTradeDTCC_SEC_VR_PriorUSI_02.Default.class)
public interface SECTransactionReportTradeDTCC_SEC_VR_PriorUSI_02 extends Validator<SECTransactionReportTrade> {
	
	String NAME = "SECTransactionReportTradeDTCC_SEC_VR_PriorUSI_02";
	String DEFINITION = "if actionType = NEWT then priorUSI <> uniqueSwapIdentifier";
	
	class Default implements SECTransactionReportTradeDTCC_SEC_VR_PriorUSI_02 {
		...
		private ComparisonResult executeDataRule(SECTransactionReportTrade sECTransactionReportTrade) {
			try {
				if (areEqual(MapperS.of(sECTransactionReportTrade).<ActionTypeEnum>map("getActionType", _sECTransactionReportTrade -> _sECTransactionReportTrade.getActionType()), MapperS.of(ActionTypeEnum.NEWT), CardinalityOperator.All).getOrDefault(false)) {
					return notEqual(MapperS.of(sECTransactionReportTrade).<String>map("getPriorUSI", _sECTransactionReportTrade -> _sECTransactionReportTrade.getPriorUSI()), MapperS.of(sECTransactionReportTrade).<String>map("getUniqueSwapIdentifier", _sECTransactionReportTrade -> _sECTransactionReportTrade.getUniqueSwapIdentifier()), CardinalityOperator.Any);
				}
	...
	class NoOp implements SECTransactionReportTradeDTCC_SEC_VR_PriorUSI_02 {
