 ...
 * @version 7.7.0
 */
@RosettaDataRule("CFTCMarginReportDTCC_CFTC_VR_0115_01")
@ImplementedBy(CFTCMarginReportDTCC_CFTC_VR_0115_01.Default.class)
public interface CFTCMarginReportDTCC_CFTC_VR_0115_01 extends Validator<CFTCMarginReport> {
	
	String NAME = "CFTCMarginReportDTCC_CFTC_VR_0115_01";
	String DEFINITION = "if actionType = MARU then collateralisationCategory exists";
	
	class Default implements CFTCMarginReportDTCC_CFTC_VR_0115_01 {
		...
		private ComparisonResult executeDataRule(CFTCMarginReport cFTCMarginReport) {
			try {
				if (areEqual(MapperS.of(cFTCMarginReport).<MarginActionEnum>map("getActionType", _cFTCMarginReport -> _cFTCMarginReport.getActionType()), MapperS.of(MarginActionEnum.MARU), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(cFTCMarginReport).<CollateralisationType3Code>map("getCollateralisationCategory", _cFTCMarginReport -> _cFTCMarginReport.getCollateralisationCategory()));
				}
	...
	class NoOp implements CFTCMarginReportDTCC_CFTC_VR_0115_01 {
