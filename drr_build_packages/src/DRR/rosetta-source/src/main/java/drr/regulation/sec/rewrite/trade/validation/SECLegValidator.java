...
public class SECLegValidator implements Validator<SECLeg> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SECLeg", ValidationResult.ValidationType.CARDINALITY, "SECLeg", path, "", res.getError());
				}
				return success("SECLeg", ValidationResult.ValidationType.CARDINALITY, "SECLeg", path, "");
			})
			.collect(toList());
	}
