package drr.regulation.common.margin.validation;
...
public class CommonMarginReportTypeFormatValidator implements Validator<CommonMarginReport> {

	private List<ComparisonResult> getComparisonResults(CommonMarginReport o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, CommonMarginReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CommonMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "CommonMarginReport", path, "", res.getError());
				}
				return success("CommonMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "CommonMarginReport", path, "");
			})
			.collect(toList());
	}
