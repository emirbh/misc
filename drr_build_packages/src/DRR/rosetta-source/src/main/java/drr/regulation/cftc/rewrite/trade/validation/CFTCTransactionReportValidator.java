...
public class CFTCTransactionReportValidator implements Validator<CFTCTransactionReport> {

	private List<ComparisonResult> getComparisonResults(CFTCTransactionReport o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, CFTCTransactionReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCTransactionReport", ValidationResult.ValidationType.CARDINALITY, "CFTCTransactionReport", path, "", res.getError());
				}
				return success("CFTCTransactionReport", ValidationResult.ValidationType.CARDINALITY, "CFTCTransactionReport", path, "");
			})
			.collect(toList());
	}
