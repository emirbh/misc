...
@ImplementedBy(IsCommoditySwap.IsCommoditySwapDefault.class)
public abstract class IsCommoditySwap implements RosettaFunction {
	...
	*/
	...
	public static class IsCommoditySwapDefault extends IsCommoditySwap {
		@Override
		...
		protected Boolean assignOutput(Boolean result, NonTransferableProduct product) {
			result = areEqual(MapperS.of(qualify_Commodity_Swap_FixedFloat.evaluate(economicTermsForProduct.evaluate(product))), MapperS.of(true), CardinalityOperator.All).orNullSafe(areEqual(MapperS.of(qualify_Commodity_Swap_Basis.evaluate(economicTermsForProduct.evaluate(product))), MapperS.of(true), CardinalityOperator.All)).get();
