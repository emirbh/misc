 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportTradeCSA_VR_0050_01")
@ImplementedBy(CSATransactionReportTradeCSA_VR_0050_01.Default.class)
public interface CSATransactionReportTradeCSA_VR_0050_01 extends Validator<CSATransactionReportTrade> {
	
	String NAME = "CSATransactionReportTradeCSA_VR_0050_01";
	String DEFINITION = "if ([AnnaDsbAssetClassEnum -> Equity, AnnaDsbAssetClassEnum -> Commodities] any = nonReportable -> postUpiData -> Header -> AssetClass or [AssetClassEnum -> Equity, AssetClassEnum -> Commodity] any = dtccAdditionalFields -> primaryAssetClass) and (priceSchedule -> price exists) then priceSchedule -> effectiveDate exists else priceSchedule -> effectiveDate is absent";
	
	class Default implements CSATransactionReportTradeCSA_VR_0050_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReportTrade cSATransactionReportTrade) {
			try {
				if (areEqual(MapperC.<AnnaDsbAssetClassEnum>of(MapperS.of(AnnaDsbAssetClassEnum.EQUITY), MapperS.of(AnnaDsbAssetClassEnum.COMMODITIES)), MapperS.of(cSATransactionReportTrade).<CSANonReportable>map("getNonReportable", _cSATransactionReportTrade -> _cSATransactionReportTrade.getNonReportable()).<AnnaDsbUpiRecord>map("getPostUpiData", cSANonReportable -> cSANonReportable.getPostUpiData()).<AnnaDsbHeader>map("getHeader", annaDsbUpiRecord -> annaDsbUpiRecord.getHeader()).<AnnaDsbAssetClassEnum>map("getAssetClass", annaDsbHeader -> annaDsbHeader.getAssetClass()), CardinalityOperator.Any).orNullSafe(areEqual(MapperC.<AssetClassEnum>of(MapperS.of(AssetClassEnum.EQUITY), MapperS.of(AssetClassEnum.COMMODITY)), MapperS.of(cSATransactionReportTrade).<DTCCAdditionalFields>map("getDtccAdditionalFields", _cSATransactionReportTrade -> _cSATransactionReportTrade.getDtccAdditionalFields()).<AssetClassEnum>map("getPrimaryAssetClass", dTCCAdditionalFields -> dTCCAdditionalFields.getPrimaryAssetClass()), CardinalityOperator.Any)).andNullSafe(exists(MapperS.of(cSATransactionReportTrade).<PricePeriod>mapC("getPriceSchedule", _cSATransactionReportTrade -> _cSATransactionReportTrade.getPriceSchedule()).<PriceFormat>map("getPrice", pricePeriod -> pricePeriod.getPrice()))).getOrDefault(false)) {
					return exists(MapperS.of(cSATransactionReportTrade).<PricePeriod>mapC("getPriceSchedule", _cSATransactionReportTrade -> _cSATransactionReportTrade.getPriceSchedule()).<Date>map("getEffectiveDate", pricePeriod -> pricePeriod.getEffectiveDate()));
				}
				return notExists(MapperS.of(cSATransactionReportTrade).<PricePeriod>mapC("getPriceSchedule", _cSATransactionReportTrade -> _cSATransactionReportTrade.getPriceSchedule()).<Date>map("getEffectiveDate", pricePeriod -> pricePeriod.getEffectiveDate()));
			}
			catch (Exception ex) {
	...
	class NoOp implements CSATransactionReportTradeCSA_VR_0050_01 {
