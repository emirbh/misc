 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAEMIRMarginReportEMIR_VR_3007_04")
@ImplementedBy(ESMAEMIRMarginReportEMIR_VR_3007_04.Default.class)
public interface ESMAEMIRMarginReportEMIR_VR_3007_04 extends Validator<ESMAEMIRMarginReport> {
	
	String NAME = "ESMAEMIRMarginReportEMIR_VR_3007_04";
	String DEFINITION = "collateralTimestamp -> date = eventDate";
	
	class Default implements ESMAEMIRMarginReportEMIR_VR_3007_04 {
		...
		private ComparisonResult executeDataRule(ESMAEMIRMarginReport eSMAEMIRMarginReport) {
			try {
				return areEqual(MapperS.of(eSMAEMIRMarginReport).<ZonedDateTime>map("getCollateralTimestamp", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getCollateralTimestamp()).<Date>map("Date", zdt -> Date.of(zdt.toLocalDate())), MapperS.of(eSMAEMIRMarginReport).<Date>map("getEventDate", _eSMAEMIRMarginReport -> _eSMAEMIRMarginReport.getEventDate()), CardinalityOperator.All);
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAEMIRMarginReportEMIR_VR_3007_04 {
