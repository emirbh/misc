...
public class CSATransactionReportPPDValidator implements Validator<CSATransactionReportPPD> {

	private List<ComparisonResult> getComparisonResults(CSATransactionReportPPD o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, CSATransactionReportPPD o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CSATransactionReportPPD", ValidationResult.ValidationType.CARDINALITY, "CSATransactionReportPPD", path, "", res.getError());
				}
				return success("CSATransactionReportPPD", ValidationResult.ValidationType.CARDINALITY, "CSATransactionReportPPD", path, "");
			})
			.collect(toList());
	}
