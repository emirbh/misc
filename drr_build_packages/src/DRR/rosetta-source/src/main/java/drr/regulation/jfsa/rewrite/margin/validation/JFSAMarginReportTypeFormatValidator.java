...
public class JFSAMarginReportTypeFormatValidator implements Validator<JFSAMarginReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("JFSAMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "JFSAMarginReport", path, "", res.getError());
				}
				return success("JFSAMarginReport", ValidationResult.ValidationType.TYPE_FORMAT, "JFSAMarginReport", path, "");
			})
			.collect(toList());
	}
