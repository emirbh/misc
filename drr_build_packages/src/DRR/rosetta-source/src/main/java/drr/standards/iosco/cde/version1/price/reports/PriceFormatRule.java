...
@ImplementedBy(PriceFormatRule.PriceFormatRuleDefault.class)
public abstract class PriceFormatRule implements ReportFunction<TransactionReportInstructionBase, PriceFormat> {
	...
	*/
	...
	public static class PriceFormatRuleDefault extends PriceFormatRule {
		@Override
		...
		protected PriceFormat.PriceFormatBuilder assignOutput(PriceFormat.PriceFormatBuilder output, TransactionReportInstructionBase input) {
			output = toBuilder(priceFormatFromNotation.evaluate(priceValueRule.evaluate(input), priceNotationRule.evaluate(input)));
