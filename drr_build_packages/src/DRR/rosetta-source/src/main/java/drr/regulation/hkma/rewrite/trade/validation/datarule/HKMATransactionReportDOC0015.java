 ...
 * @version 7.7.0
 */
@RosettaDataRule("HKMATransactionReportDOC0015")
@ImplementedBy(HKMATransactionReportDOC0015.Default.class)
public interface HKMATransactionReportDOC0015 extends Validator<HKMATransactionReport> {
	
	String NAME = "HKMATransactionReportDOC0015";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType then counterparty1 <> counterparty2";
	
	class Default implements HKMATransactionReportDOC0015 {
		...
		private ComparisonResult executeDataRule(HKMATransactionReport hKMATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(hKMATransactionReport).<ActionTypeEnum>map("getActionType", _hKMATransactionReport -> _hKMATransactionReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					return notEqual(MapperS.of(hKMATransactionReport).<String>map("getCounterparty1", _hKMATransactionReport -> _hKMATransactionReport.getCounterparty1()), MapperS.of(hKMATransactionReport).<String>map("getCounterparty2", _hKMATransactionReport -> _hKMATransactionReport.getCounterparty2()), CardinalityOperator.Any);
				}
	...
	class NoOp implements HKMATransactionReportDOC0015 {
