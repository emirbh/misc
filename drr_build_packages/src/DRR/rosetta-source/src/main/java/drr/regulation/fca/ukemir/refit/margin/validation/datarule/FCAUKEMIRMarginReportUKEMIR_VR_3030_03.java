 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRMarginReportUKEMIR_VR_3030_03")
@ImplementedBy(FCAUKEMIRMarginReportUKEMIR_VR_3030_03.Default.class)
public interface FCAUKEMIRMarginReportUKEMIR_VR_3030_03 extends Validator<FCAUKEMIRMarginReport> {
	
	String NAME = "FCAUKEMIRMarginReportUKEMIR_VR_3030_03";
	String DEFINITION = "if [MarginActionEnum -> MARU, MarginActionEnum -> CORR] any = actionType then ( if executionAgentOfCounterparty1 exists and executionAgentOfCounterparty2 is absent then GetOrFetchLeiData( nonReportable -> enrichment -> leiData, executionAgentOfCounterparty1 ) exists else if executionAgentOfCounterparty2 exists and executionAgentOfCounterparty1 is absent then GetOrFetchLeiData( nonReportable -> enrichment -> leiData, executionAgentOfCounterparty2 ) exists else if executionAgentOfCounterparty1 exists and executionAgentOfCounterparty2 exists then GetOrFetchLeiData( nonReportable -> enrichment -> leiData, executionAgentOfCounterparty1 ) exists and GetOrFetchLeiData( nonReportable -> enrichment -> leiData, executionAgentOfCounterparty2 ) exists)";
	
	class Default implements FCAUKEMIRMarginReportUKEMIR_VR_3030_03 {
			...
			try {
				if (areEqual(MapperC.<MarginActionEnum>of(MapperS.of(MarginActionEnum.MARU), MapperS.of(MarginActionEnum.CORR)), MapperS.of(fCAUKEMIRMarginReport).<MarginActionEnum>map("getActionType", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getActionType()), CardinalityOperator.Any).getOrDefault(false)) {
					...
					if (exists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty1", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty1())).andNullSafe(exists(MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty2", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty2()))).getOrDefault(false)) {
						return exists(MapperS.of(getOrFetchLeiData.evaluate(MapperS.of(fCAUKEMIRMarginReport).<FCAUKEMIRNonReportableCollateralData>map("getNonReportable", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getNonReportable()).<EnrichmentData>map("getEnrichment", fCAUKEMIRNonReportableCollateralData -> fCAUKEMIRNonReportableCollateralData.getEnrichment()).<LeiData>mapC("getLeiData", enrichmentData -> enrichmentData.getLeiData()).getMulti(), MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty1", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty1()).get()))).andNullSafe(exists(MapperS.of(getOrFetchLeiData.evaluate(MapperS.of(fCAUKEMIRMarginReport).<FCAUKEMIRNonReportableCollateralData>map("getNonReportable", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getNonReportable()).<EnrichmentData>map("getEnrichment", fCAUKEMIRNonReportableCollateralData -> fCAUKEMIRNonReportableCollateralData.getEnrichment()).<LeiData>mapC("getLeiData", enrichmentData -> enrichmentData.getLeiData()).getMulti(), MapperS.of(fCAUKEMIRMarginReport).<String>map("getExecutionAgentOfCounterparty2", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getExecutionAgentOfCounterparty2()).get()))));
					}
	...
	class NoOp implements FCAUKEMIRMarginReportUKEMIR_VR_3030_03 {
