 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRMarginReportUKEMIR_VR_3001_04")
@ImplementedBy(FCAUKEMIRMarginReportUKEMIR_VR_3001_04.Default.class)
public interface FCAUKEMIRMarginReportUKEMIR_VR_3001_04 extends Validator<FCAUKEMIRMarginReport> {
	
	String NAME = "FCAUKEMIRMarginReportUKEMIR_VR_3001_04";
	String DEFINITION = "CompareDateTo(reportingTimestamp -> date, 2024, 9, 30) >= 0";
	
	class Default implements FCAUKEMIRMarginReportUKEMIR_VR_3001_04 {
		...
		private ComparisonResult executeDataRule(FCAUKEMIRMarginReport fCAUKEMIRMarginReport) {
			try {
				return greaterThanEquals(MapperS.of(compareDateTo.evaluate(MapperS.of(fCAUKEMIRMarginReport).<ZonedDateTime>map("getReportingTimestamp", _fCAUKEMIRMarginReport -> _fCAUKEMIRMarginReport.getReportingTimestamp()).<Date>map("Date", zdt -> Date.of(zdt.toLocalDate())).get(), 2024, 9, 30)), MapperS.of(0), CardinalityOperator.All);
			}
			catch (Exception ex) {
	...
	class NoOp implements FCAUKEMIRMarginReportUKEMIR_VR_3001_04 {
