 ...
 * @version 7.7.0
 */
@RosettaDataRule("CSATransactionReportCSA_VR_0049_01")
@ImplementedBy(CSATransactionReportCSA_VR_0049_01.Default.class)
public interface CSATransactionReportCSA_VR_0049_01 extends Validator<CSATransactionReport> {
	
	String NAME = "CSATransactionReportCSA_VR_0049_01";
	String DEFINITION = "if ([AnnaDsbAssetClassEnum -> Equity, AnnaDsbAssetClassEnum -> Commodities] any = nonReportable -> postUpiData -> Header -> AssetClass or [AssetClassEnum -> Equity, AssetClassEnum -> Commodity] any = dtccAdditionalFields -> primaryAssetClass) and price exists then priceUnitOfMeasure exists else priceUnitOfMeasure is absent";
	
	class Default implements CSATransactionReportCSA_VR_0049_01 {
		...
		private ComparisonResult executeDataRule(CSATransactionReport cSATransactionReport) {
			try {
				if (areEqual(MapperC.<AnnaDsbAssetClassEnum>of(MapperS.of(AnnaDsbAssetClassEnum.EQUITY), MapperS.of(AnnaDsbAssetClassEnum.COMMODITIES)), MapperS.of(cSATransactionReport).<CSANonReportable>map("getNonReportable", _cSATransactionReport -> _cSATransactionReport.getNonReportable()).<AnnaDsbUpiRecord>map("getPostUpiData", cSANonReportable -> cSANonReportable.getPostUpiData()).<AnnaDsbHeader>map("getHeader", annaDsbUpiRecord -> annaDsbUpiRecord.getHeader()).<AnnaDsbAssetClassEnum>map("getAssetClass", annaDsbHeader -> annaDsbHeader.getAssetClass()), CardinalityOperator.Any).orNullSafe(areEqual(MapperC.<AssetClassEnum>of(MapperS.of(AssetClassEnum.EQUITY), MapperS.of(AssetClassEnum.COMMODITY)), MapperS.of(cSATransactionReport).<DTCCAdditionalFields>map("getDtccAdditionalFields", _cSATransactionReport -> _cSATransactionReport.getDtccAdditionalFields()).<AssetClassEnum>map("getPrimaryAssetClass", dTCCAdditionalFields -> dTCCAdditionalFields.getPrimaryAssetClass()), CardinalityOperator.Any)).andNullSafe(exists(MapperS.of(cSATransactionReport).<PriceFormat>map("getPrice", _cSATransactionReport -> _cSATransactionReport.getPrice()))).getOrDefault(false)) {
					return exists(MapperS.of(cSATransactionReport).<String>map("getPriceUnitOfMeasure", _cSATransactionReport -> _cSATransactionReport.getPriceUnitOfMeasure()));
				}
				return notExists(MapperS.of(cSATransactionReport).<String>map("getPriceUnitOfMeasure", _cSATransactionReport -> _cSATransactionReport.getPriceUnitOfMeasure()));
			}
			catch (Exception ex) {
	...
	class NoOp implements CSATransactionReportCSA_VR_0049_01 {
