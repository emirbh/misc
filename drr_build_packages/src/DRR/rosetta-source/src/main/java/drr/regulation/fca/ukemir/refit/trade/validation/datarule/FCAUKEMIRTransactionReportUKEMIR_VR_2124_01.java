 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRTransactionReportUKEMIR_VR_2124_01")
@ImplementedBy(FCAUKEMIRTransactionReportUKEMIR_VR_2124_01.Default.class)
public interface FCAUKEMIRTransactionReportUKEMIR_VR_2124_01 extends Validator<FCAUKEMIRTransactionReport> {
	
	String NAME = "FCAUKEMIRTransactionReportUKEMIR_VR_2124_01";
	String DEFINITION = "if IsVRAllowedForActionType(actionType) and deliveryReport exists then (deliveryReport extract dr [ if IsAcceptedEicCode(deliveryPoint) or IsAcceptedEicCode(interconnectionPoint) then dr -> deliveryStartDate exists else dr -> deliveryStartDate is absent ] then all = True)";
	
	class Default implements FCAUKEMIRTransactionReportUKEMIR_VR_2124_01 {
					...
					final MapperC<Boolean> thenArg = MapperS.of(fCAUKEMIRTransactionReport).<ReportableDelivery>mapC("getDeliveryReport", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getDeliveryReport())
						.mapItem(dr -> {
							if (ComparisonResult.ofNullSafe(MapperS.of(isAcceptedEicCode.evaluate(MapperS.of(fCAUKEMIRTransactionReport).<String>mapC("getDeliveryPoint", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getDeliveryPoint()).getMulti()))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isAcceptedEicCode.evaluate(MapperS.of(fCAUKEMIRTransactionReport).<String>map("getInterconnectionPoint", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getInterconnectionPoint()).getMulti())))).getOrDefault(false)) {
								return exists(dr.<Date>map("getDeliveryStartDate", reportableDelivery -> reportableDelivery.getDeliveryStartDate())).asMapper();
							}
							return notExists(dr.<Date>map("getDeliveryStartDate", reportableDelivery -> reportableDelivery.getDeliveryStartDate())).asMapper();
						});
	...
	class NoOp implements FCAUKEMIRTransactionReportUKEMIR_VR_2124_01 {
