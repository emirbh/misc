 ...
 * @version 7.7.0
 */
@RosettaDataRule("JFSATransactionReportDTCC_JFSA_BR_0110_01")
@ImplementedBy(JFSATransactionReportDTCC_JFSA_BR_0110_01.Default.class)
public interface JFSATransactionReportDTCC_JFSA_BR_0110_01 extends Validator<JFSATransactionReport> {
	
	String NAME = "JFSATransactionReportDTCC_JFSA_BR_0110_01";
	String DEFINITION = "if [ActionTypeEnum -> NEWT, ActionTypeEnum -> MODI, ActionTypeEnum -> CORR, ActionTypeEnum -> REVI] any = actionType and assetClass = CommonAssetClass -> CRDT then (if referenceEntity is absent then underlyingIdentificationType exists)";
	
	class Default implements JFSATransactionReportDTCC_JFSA_BR_0110_01 {
		...
		private ComparisonResult executeDataRule(JFSATransactionReport jFSATransactionReport) {
			try {
				if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(jFSATransactionReport).<ActionTypeEnum>map("getActionType", _jFSATransactionReport -> _jFSATransactionReport.getActionType()), CardinalityOperator.Any).andNullSafe(areEqual(MapperS.of(jFSATransactionReport).<CommonAssetClass>map("getAssetClass", _jFSATransactionReport -> _jFSATransactionReport.getAssetClass()), MapperS.of(CommonAssetClass.CRDT), CardinalityOperator.All)).getOrDefault(false)) {
					if (notExists(MapperS.of(jFSATransactionReport).<String>map("getReferenceEntity", _jFSATransactionReport -> _jFSATransactionReport.getReferenceEntity())).getOrDefault(false)) {
	...
	class NoOp implements JFSATransactionReportDTCC_JFSA_BR_0110_01 {
