...
public class SECTransactionReportTradeValidator implements Validator<SECTransactionReportTrade> {

	private List<ComparisonResult> getComparisonResults(SECTransactionReportTrade o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("leg1", (SECLeg) o.getLeg1() != null ? 1 : 0, 0, 1), 
				checkCardinality("leg2", (SECLeg) o.getLeg2() != null ? 1 : 0, 0, 1), 
				...
				checkCardinality("allocationIndicator", (AllocationIndicatorEnum) o.getAllocationIndicator() != null ? 1 : 0, 1, 1), 
				checkCardinality("jurisdiction", (JurisdictionEnum) o.getJurisdiction() != null ? 1 : 0, 1, 1)
			);
	}

	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, SECTransactionReportTrade o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SECTransactionReportTrade", ValidationResult.ValidationType.CARDINALITY, "SECTransactionReportTrade", path, "", res.getError());
				}
				return success("SECTransactionReportTrade", ValidationResult.ValidationType.CARDINALITY, "SECTransactionReportTrade", path, "");
			})
			.collect(toList());
	}
