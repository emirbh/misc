	...
	*/
	...
	public static class ReportingObligationOfTheCounterparty2RuleDefault extends ReportingObligationOfTheCounterparty2Rule {
		...
		protected Boolean assignOutput(Boolean output, TransactionReportInstruction input) {
			...
			output = thenArg2
				.mapSingleToItem(item -> areEqual(item.<NatureOfPartyEnum>map("getNatureOfParty", fCAPartyInformation -> fCAPartyInformation.getNatureOfParty()), MapperS.of(NatureOfPartyEnum.FINANCIAL), CardinalityOperator.All).orNullSafe(areEqual(item.<NatureOfPartyEnum>map("getNatureOfParty", fCAPartyInformation -> fCAPartyInformation.getNatureOfParty()), MapperS.of(NatureOfPartyEnum.NON_FINANCIAL), CardinalityOperator.All).andNullSafe(areEqual(item.<Boolean>map("getExceedsClearingThreshold", fCAPartyInformation -> fCAPartyInformation.getExceedsClearingThreshold()), MapperS.of(true), CardinalityOperator.All))).asMapper()).get();
