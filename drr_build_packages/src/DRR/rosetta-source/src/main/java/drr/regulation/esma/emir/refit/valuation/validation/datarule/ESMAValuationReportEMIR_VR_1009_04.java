 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAValuationReportEMIR_VR_1009_04")
@ImplementedBy(ESMAValuationReportEMIR_VR_1009_04.Default.class)
public interface ESMAValuationReportEMIR_VR_1009_04 extends Validator<ESMAValuationReport> {
	
	String NAME = "ESMAValuationReportEMIR_VR_1009_04";
	String DEFINITION = "Counterparty2_04_Validation( nonReportable -> enrichment, counterparty2IdentifierType, counterparty2 )";
	
	class Default implements ESMAValuationReportEMIR_VR_1009_04 {
		...
		private ComparisonResult executeDataRule(ESMAValuationReport eSMAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(counterparty2_04_Validation.evaluate(MapperS.of(eSMAValuationReport).<NonReportable>map("getNonReportable", _eSMAValuationReport -> _eSMAValuationReport.getNonReportable()).<EnrichmentData>map("getEnrichment", nonReportable -> nonReportable.getEnrichment()).get(), MapperS.of(eSMAValuationReport).<Boolean>map("getCounterparty2IdentifierType", _eSMAValuationReport -> _eSMAValuationReport.getCounterparty2IdentifierType()).get(), MapperS.of(eSMAValuationReport).<String>map("getCounterparty2", _eSMAValuationReport -> _eSMAValuationReport.getCounterparty2()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAValuationReportEMIR_VR_1009_04 {
