...
public class CFTCPart45LegTypeFormatValidator implements Validator<CFTCPart45Leg> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCPart45Leg", ValidationResult.ValidationType.TYPE_FORMAT, "CFTCPart45Leg", path, "", res.getError());
				}
				return success("CFTCPart45Leg", ValidationResult.ValidationType.TYPE_FORMAT, "CFTCPart45Leg", path, "");
			})
			.collect(toList());
	}
