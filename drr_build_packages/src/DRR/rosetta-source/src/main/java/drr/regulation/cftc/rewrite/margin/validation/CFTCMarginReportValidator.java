...
public class CFTCMarginReportValidator implements Validator<CFTCMarginReport> {
	...
	private List<ComparisonResult> getComparisonResults(CFTCMarginReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("uniqueSwapIdentifier", (String) o.getUniqueSwapIdentifier() != null ? 1 : 0, 1, 1), 
				checkCardinality("affiliatedCounterpartyForMarginAndCapitalIndicator", (Boolean) o.getAffiliatedCounterpartyForMarginAndCapitalIndicator() != null ? 1 : 0, 1, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCMarginReport", ValidationResult.ValidationType.CARDINALITY, "CFTCMarginReport", path, "", res.getError());
				}
				return success("CFTCMarginReport", ValidationResult.ValidationType.CARDINALITY, "CFTCMarginReport", path, "");
			})
			.collect(toList());
	}
