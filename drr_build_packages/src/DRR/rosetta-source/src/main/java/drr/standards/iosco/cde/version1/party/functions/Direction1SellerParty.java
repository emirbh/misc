...
@ImplementedBy(Direction1SellerParty.Direction1SellerPartyDefault.class)
public abstract class Direction1SellerParty implements RosettaFunction {
	...
	*/
	...
	public static class Direction1SellerPartyDefault extends Direction1SellerParty {
		@Override
		...
		protected MapperC<? extends Payout> productPayout(Trade trade) {
			return MapperS.of(economicTermsForProduct.evaluate(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get())).<Payout>mapC("getPayout", _economicTerms -> _economicTerms.getPayout());
		}
		...
		protected MapperS<? extends Counterparty> counterparty(Trade trade) {
			...
			if (ComparisonResult.ofNullSafe(MapperS.of(isVarianceSwap.evaluate(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get()))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isVolatilitySwap.evaluate(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get())))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isTotalReturnSwapDebtUnderlier.evaluate(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get())))).getOrDefault(false)) {
				return MapperS.of(extractCounterpartyByRole.evaluate(MapperS.of(trade).<Counterparty>mapC("getCounterparty", _trade -> _trade.getCounterparty()).getMulti(), productPayout(trade).<PerformancePayout>map("getPerformancePayout", payout -> payout.getPerformancePayout()).<PayerReceiver>map("getPayerReceiver", performancePayout -> performancePayout.getPayerReceiver()).<CounterpartyRoleEnum>map("getPayer", payerReceiver -> payerReceiver.getPayer()).get()));
			}
			...
			if ((boolean1 == null ? false : boolean1)) {
				return MapperS.of(extractCounterpartyByRole.evaluate(MapperS.of(trade).<Counterparty>mapC("getCounterparty", _trade -> _trade.getCounterparty()).getMulti(), MapperS.of(capFloorPayerReceiver.evaluate(productPayout(trade).<InterestRatePayout>map("getInterestRatePayout", payout -> payout.getInterestRatePayout()).get(), Direction1Enum.SLLR)).getOrDefault(productPayout(trade).<InterestRatePayout>map("getInterestRatePayout", payout -> payout.getInterestRatePayout()).<PayerReceiver>map("getPayerReceiver", interestRatePayout -> interestRatePayout.getPayerReceiver()).<CounterpartyRoleEnum>map("getPayer", payerReceiver -> payerReceiver.getPayer()).get())));
			}
			...
			if ((boolean2 == null ? false : boolean2)) {
				return MapperS.of(extractCounterpartyByRole.evaluate(MapperS.of(trade).<Counterparty>mapC("getCounterparty", _trade -> _trade.getCounterparty()).getMulti(), productPayout(trade).<CommodityPayout>map("getCommodityPayout", payout -> payout.getCommodityPayout()).<PayerReceiver>map("getPayerReceiver", commodityPayout -> commodityPayout.getPayerReceiver()).<CounterpartyRoleEnum>map("getReceiver", payerReceiver -> payerReceiver.getReceiver()).get()));
			}
			if (ComparisonResult.ofNullSafe(MapperS.of(isProductETD.evaluate(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get()))).andNullSafe(exists(productPayout(trade).<SettlementPayout>map("getSettlementPayout", payout -> payout.getSettlementPayout()))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(isEquityForward.evaluate(MapperS.of(trade).<NonTransferableProduct>map("getProduct", _trade -> _trade.getProduct()).get())))).orNullSafe(ComparisonResult.ofNullSafe(MapperS.of(qualify_InterestRate_Forward_Debt.evaluate(economicTerms(trade).get())))).getOrDefault(false)) {
