 ...
 * @version 7.7.0
 */
@RosettaDataRule("ESMAValuationReportEMIR_VR_2023_01")
@ImplementedBy(ESMAValuationReportEMIR_VR_2023_01.Default.class)
public interface ESMAValuationReportEMIR_VR_2023_01 extends Validator<ESMAValuationReport> {
	
	String NAME = "ESMAValuationReportEMIR_VR_2023_01";
	String DEFINITION = "ValuationTimestamp_01_Validation(valuationAmount, valuationTimestamp)";
	
	class Default implements ESMAValuationReportEMIR_VR_2023_01 {
		...
		private ComparisonResult executeDataRule(ESMAValuationReport eSMAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(valuationTimestamp_01_Validation.evaluate(MapperS.of(eSMAValuationReport).<BigDecimal>map("getValuationAmount", _eSMAValuationReport -> _eSMAValuationReport.getValuationAmount()).get(), MapperS.of(eSMAValuationReport).<ZonedDateTime>map("getValuationTimestamp", _eSMAValuationReport -> _eSMAValuationReport.getValuationTimestamp()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements ESMAValuationReportEMIR_VR_2023_01 {
