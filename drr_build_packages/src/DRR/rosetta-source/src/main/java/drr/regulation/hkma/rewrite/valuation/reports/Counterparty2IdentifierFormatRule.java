...
public abstract class Counterparty2IdentifierFormatRule implements ReportFunction<ValuationReportInstruction, PartyIdentifierFormat2Enum> {
	...
	*/
	@Override
	public PartyIdentifierFormat2Enum evaluate(ValuationReportInstruction input) {
		PartyIdentifierFormat2Enum output = doEvaluate(input);
		
		return output;
	}

	protected abstract PartyIdentifierFormat2Enum doEvaluate(ValuationReportInstruction input);

	public static class Counterparty2IdentifierFormatRuleDefault extends Counterparty2IdentifierFormatRule {
		@Override
		protected PartyIdentifierFormat2Enum doEvaluate(ValuationReportInstruction input) {
			PartyIdentifierFormat2Enum output = null;
			return assignOutput(output, input);
		}
		
		protected PartyIdentifierFormat2Enum assignOutput(PartyIdentifierFormat2Enum output, ValuationReportInstruction input) {
