...
@ImplementedBy(NotionalQuantityScheduleValue_Validation.NotionalQuantityScheduleValue_ValidationDefault.class)
public abstract class NotionalQuantityScheduleValue_Validation implements RosettaFunction {
	...
	*/
	...
	public static class NotionalQuantityScheduleValue_ValidationDefault extends NotionalQuantityScheduleValue_Validation {
		@Override
		...
		protected Boolean assignOutput(Boolean result, ActionTypeEnum actionType, CommonAssetClass assetClass, List<? extends NotionalPeriod> notionalQuantitySchedule) {
			if (areEqual(MapperC.<ActionTypeEnum>of(MapperS.of(ActionTypeEnum.NEWT), MapperS.of(ActionTypeEnum.MODI), MapperS.of(ActionTypeEnum.CORR), MapperS.of(ActionTypeEnum.REVI)), MapperS.of(actionType), CardinalityOperator.Any).andNullSafe(areEqual(MapperC.<CommonAssetClass>of(MapperS.of(CommonAssetClass.EQUI), MapperS.of(CommonAssetClass.COMM)), MapperS.of(assetClass), CardinalityOperator.Any)).getOrDefault(false)) {
				...
				if (exists(MapperC.<NotionalPeriod>of(notionalQuantitySchedule).<Date>map("getEffectiveDate", notionalPeriod -> notionalPeriod.getEffectiveDate())).getOrDefault(false)) {
					ifThenElseResult = exists(MapperC.<NotionalPeriod>of(notionalQuantitySchedule).<BigDecimal>map("getValue", notionalPeriod -> notionalPeriod.getValue()));
				} else {
