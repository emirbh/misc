...
@ImplementedBy(FormatToMax3Number.FormatToMax3NumberDefault.class)
public abstract class FormatToMax3Number implements RosettaFunction {
	...
	*/
	...
	public static class FormatToMax3NumberDefault extends FormatToMax3Number {
		@Override
		...
		protected BigDecimal assignOutput(BigDecimal formattedNumber, BigDecimal inputNumber) {
			if (lessThanEquals(MapperS.of(abs.evaluate(inputNumber)), MapperS.of(BigDecimal.valueOf(99999)), CardinalityOperator.All).getOrDefault(false)) {
				...
				formattedNumber = MapperS.of(roundToSignificantFigures.evaluate(thenArg.get(), 3, RoundingDirectionEnum.NEAREST)).get();
			} else {
