...
import drr.regulation.sec.rewrite.trade.labels.SECPPDLabelProvider;
...
@RosettaReport(namespace="drr.regulation.sec.rewrite.trade", body="SEC", corpusList={"PPD"})
@RuneLabelProvider(labelProvider=SECPPDLabelProvider.class)
@ImplementedBy(SECPPDReportFunction.SECPPDReportFunctionDefault.class)
public abstract class SECPPDReportFunction implements ReportFunction<TransactionReportInstruction, SECTransactionReportPPD> {
	...
	@Inject protected OtherPaymentPPDRule otherPaymentPPDRule;
	...
	*/
	@Override
	public SECTransactionReportPPD evaluate(TransactionReportInstruction input) {
		SECTransactionReportPPD.SECTransactionReportPPDBuilder outputBuilder = doEvaluate(input);
		
		final SECTransactionReportPPD output;
		if (outputBuilder == null) {
			output = null;
		} else {
			output = outputBuilder.build();
			objectValidator.validate(SECTransactionReportPPD.class, output);
		}
		
		return output;
	}

	protected abstract SECTransactionReportPPD.SECTransactionReportPPDBuilder doEvaluate(TransactionReportInstruction input);

	public static class SECPPDReportFunctionDefault extends SECPPDReportFunction {
		@Override
		protected SECTransactionReportPPD.SECTransactionReportPPDBuilder doEvaluate(TransactionReportInstruction input) {
			SECTransactionReportPPD.SECTransactionReportPPDBuilder output = SECTransactionReportPPD.builder();
			return assignOutput(output, input);
		}
		
		protected SECTransactionReportPPD.SECTransactionReportPPDBuilder assignOutput(SECTransactionReportPPD.SECTransactionReportPPDBuilder output, TransactionReportInstruction input) {
			output
			...
			output
				.setOtherPayment(otherPaymentPPDRule.evaluate(input));
