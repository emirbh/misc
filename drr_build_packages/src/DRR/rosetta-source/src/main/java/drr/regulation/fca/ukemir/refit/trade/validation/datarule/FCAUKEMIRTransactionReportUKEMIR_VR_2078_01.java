 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRTransactionReportUKEMIR_VR_2078_01")
@ImplementedBy(FCAUKEMIRTransactionReportUKEMIR_VR_2078_01.Default.class)
public interface FCAUKEMIRTransactionReportUKEMIR_VR_2078_01 extends Validator<FCAUKEMIRTransactionReport> {
	
	String NAME = "FCAUKEMIRTransactionReportUKEMIR_VR_2078_01";
	String DEFINITION = "if (IsVRAllowedForActionType(actionType) or actionType = ActionTypeEnum -> TERM) and otherPayment exists then (otherPayment extract (if paymentType exists then if receiverFormat = Lei then GetOrFetchLeiData(empty, receiver) -> lei exists else receiver exists else receiver is absent) then all = True)";
	
	class Default implements FCAUKEMIRTransactionReportUKEMIR_VR_2078_01 {
						...
						.mapItem(item -> {
							if (exists(item.<PaymentType4Code>map("getPaymentType", otherPayment -> otherPayment.getPaymentType())).getOrDefault(false)) {
								if (areEqual(item.<PartyIdentifierFormatEnum>map("getReceiverFormat", otherPayment -> otherPayment.getReceiverFormat()), MapperS.of(PartyIdentifierFormatEnum.LEI), CardinalityOperator.All).getOrDefault(false)) {
									return exists(MapperS.of(getOrFetchLeiData.evaluate(Collections.<LeiData>emptyList(), item.<String>map("getReceiver", otherPayment -> otherPayment.getReceiver()).get())).<String>map("getLei", leiData -> leiData.getLei())).asMapper();
								}
								return exists(item.<String>map("getReceiver", otherPayment -> otherPayment.getReceiver())).asMapper();
							}
							return notExists(item.<String>map("getReceiver", otherPayment -> otherPayment.getReceiver())).asMapper();
						});
	...
	class NoOp implements FCAUKEMIRTransactionReportUKEMIR_VR_2078_01 {
