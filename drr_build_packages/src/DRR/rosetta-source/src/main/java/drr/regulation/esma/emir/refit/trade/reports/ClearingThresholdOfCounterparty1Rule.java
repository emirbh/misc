	...
	public static class ClearingThresholdOfCounterparty1RuleDefault extends ClearingThresholdOfCounterparty1Rule {
		...
		protected Boolean assignOutput(Boolean output, TransactionReportInstruction input) {
			...
			final MapperS<JurisdictionPartyInformation> thenArg2 = thenArg1
				.filterSingleNullSafe(item -> areEqual(MapperC.<NatureOfPartyEnum>of(MapperS.of(NatureOfPartyEnum.FINANCIAL), MapperS.of(NatureOfPartyEnum.NON_FINANCIAL)), item.<ESMAPartyInformation>map("getEsmaPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getEsmaPartyInformation()).<NatureOfPartyEnum>map("getNatureOfParty", eSMAPartyInformation -> eSMAPartyInformation.getNatureOfParty()), CardinalityOperator.Any).get());
			output = thenArg2
				.mapSingleToItem(item -> item.<ESMAPartyInformation>map("getEsmaPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getEsmaPartyInformation()).<Boolean>map("getExceedsClearingThreshold", eSMAPartyInformation -> eSMAPartyInformation.getExceedsClearingThreshold())).get();
