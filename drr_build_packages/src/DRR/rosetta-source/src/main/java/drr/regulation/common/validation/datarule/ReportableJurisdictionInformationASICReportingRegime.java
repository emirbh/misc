 ...
 * @version 7.7.0
 */
@RosettaDataRule("ReportableJurisdictionInformationASICReportingRegime")
@ImplementedBy(ReportableJurisdictionInformationASICReportingRegime.Default.class)
public interface ReportableJurisdictionInformationASICReportingRegime extends Validator<ReportableJurisdictionInformation> {
	
	String NAME = "ReportableJurisdictionInformationASICReportingRegime";
	String DEFINITION = "if supervisoryBody = ASIC then partyInformation -> asicPartyInformation exists else partyInformation -> asicPartyInformation is absent";
	
	class Default implements ReportableJurisdictionInformationASICReportingRegime {
		...
		private ComparisonResult executeDataRule(ReportableJurisdictionInformation reportableJurisdictionInformation) {
			try {
				if (areEqual(MapperS.of(reportableJurisdictionInformation).<FieldWithMetaSupervisoryBodyEnum>map("getSupervisoryBody", _reportableJurisdictionInformation -> _reportableJurisdictionInformation.getSupervisoryBody()).<SupervisoryBodyEnum>map("Type coercion", fieldWithMetaSupervisoryBodyEnum -> fieldWithMetaSupervisoryBodyEnum == null ? null : fieldWithMetaSupervisoryBodyEnum.getValue()), MapperS.of(SupervisoryBodyEnum.ASIC), CardinalityOperator.All).getOrDefault(false)) {
					return exists(MapperS.of(reportableJurisdictionInformation).<JurisdictionPartyInformation>mapC("getPartyInformation", _reportableJurisdictionInformation -> _reportableJurisdictionInformation.getPartyInformation()).<ASICPartyInformation>map("getAsicPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getAsicPartyInformation()));
				}
				return notExists(MapperS.of(reportableJurisdictionInformation).<JurisdictionPartyInformation>mapC("getPartyInformation", _reportableJurisdictionInformation -> _reportableJurisdictionInformation.getPartyInformation()).<ASICPartyInformation>map("getAsicPartyInformation", jurisdictionPartyInformation -> jurisdictionPartyInformation.getAsicPartyInformation()));
			}
			catch (Exception ex) {
	...
	class NoOp implements ReportableJurisdictionInformationASICReportingRegime {
