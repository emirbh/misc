 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAValuationReportUKEMIR_VR_1009_04")
@ImplementedBy(FCAValuationReportUKEMIR_VR_1009_04.Default.class)
public interface FCAValuationReportUKEMIR_VR_1009_04 extends Validator<FCAValuationReport> {
	
	String NAME = "FCAValuationReportUKEMIR_VR_1009_04";
	String DEFINITION = "Counterparty2_04_Validation( nonReportable -> enrichment, counterparty2IdentifierType, counterparty2 )";
	
	class Default implements FCAValuationReportUKEMIR_VR_1009_04 {
		...
		private ComparisonResult executeDataRule(FCAValuationReport fCAValuationReport) {
			try {
				return ComparisonResult.ofNullSafe(MapperS.of(counterparty2_04_Validation.evaluate(MapperS.of(fCAValuationReport).<NonReportable>map("getNonReportable", _fCAValuationReport -> _fCAValuationReport.getNonReportable()).<EnrichmentData>map("getEnrichment", nonReportable -> nonReportable.getEnrichment()).get(), MapperS.of(fCAValuationReport).<Boolean>map("getCounterparty2IdentifierType", _fCAValuationReport -> _fCAValuationReport.getCounterparty2IdentifierType()).get(), MapperS.of(fCAValuationReport).<String>map("getCounterparty2", _fCAValuationReport -> _fCAValuationReport.getCounterparty2()).get())));
			}
			catch (Exception ex) {
	...
	class NoOp implements FCAValuationReportUKEMIR_VR_1009_04 {
