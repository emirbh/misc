 ...
 * @version 7.7.0
 */
@RosettaDataRule("FCAUKEMIRTransactionReportUKEMIR_VR_2073_01")
@ImplementedBy(FCAUKEMIRTransactionReportUKEMIR_VR_2073_01.Default.class)
public interface FCAUKEMIRTransactionReportUKEMIR_VR_2073_01 extends Validator<FCAUKEMIRTransactionReport> {
	
	String NAME = "FCAUKEMIRTransactionReportUKEMIR_VR_2073_01";
	String DEFINITION = "if (IsVRAllowedForActionType(actionType) or actionType = ActionTypeEnum -> TERM) and otherPayment exists then (otherPayment extract (if paymentType exists then [PaymentType4Code -> UFRO, PaymentType4Code -> UWIN, PaymentType4Code -> PEXH] contains paymentType else True) then all = True)";
	
	class Default implements FCAUKEMIRTransactionReportUKEMIR_VR_2073_01 {
					...
					final MapperC<Boolean> thenArg = MapperS.of(fCAUKEMIRTransactionReport).<OtherPayment>mapC("getOtherPayment", _fCAUKEMIRTransactionReport -> _fCAUKEMIRTransactionReport.getOtherPayment())
						.mapItem(item -> {
							if (exists(item.<PaymentType4Code>map("getPaymentType", otherPayment -> otherPayment.getPaymentType())).getOrDefault(false)) {
								return contains(MapperC.<PaymentType4Code>of(MapperS.of(PaymentType4Code.UFRO), MapperS.of(PaymentType4Code.UWIN), MapperS.of(PaymentType4Code.PEXH)), item.<PaymentType4Code>map("getPaymentType", otherPayment -> otherPayment.getPaymentType())).asMapper();
							}
	...
	class NoOp implements FCAUKEMIRTransactionReportUKEMIR_VR_2073_01 {
