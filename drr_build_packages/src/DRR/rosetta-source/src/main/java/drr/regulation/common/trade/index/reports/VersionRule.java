	...
	public static class VersionRuleDefault extends VersionRule {
		...
		protected BigDecimal assignOutput(BigDecimal output, TransactionReportInstruction input) {
			...
			final MapperC<Integer> thenArg2 = thenArg1
				.mapItem(item -> item.<GeneralTerms>map("getGeneralTerms", creditDefaultPayout -> creditDefaultPayout.getGeneralTerms()).<CreditIndex>map("getIndexReferenceInformation", generalTerms -> generalTerms.getIndexReferenceInformation()).<Integer>map("getIndexAnnexVersion", creditIndex -> creditIndex.getIndexAnnexVersion()));
