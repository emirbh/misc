package drr.regulation.common.emir.validation;
...
public class EMIRTransactionReportValidator implements Validator<EMIRTransactionReport> {

	private List<ComparisonResult> getComparisonResults(EMIRTransactionReport o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, EMIRTransactionReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EMIRTransactionReport", ValidationResult.ValidationType.CARDINALITY, "EMIRTransactionReport", path, "", res.getError());
				}
				return success("EMIRTransactionReport", ValidationResult.ValidationType.CARDINALITY, "EMIRTransactionReport", path, "");
			})
			.collect(toList());
	}
