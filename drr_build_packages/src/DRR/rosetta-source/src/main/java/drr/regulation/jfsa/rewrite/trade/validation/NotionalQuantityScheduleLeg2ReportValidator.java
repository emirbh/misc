...
public class NotionalQuantityScheduleLeg2ReportValidator implements Validator<NotionalQuantityScheduleLeg2Report> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NotionalQuantityScheduleLeg2Report", ValidationResult.ValidationType.CARDINALITY, "NotionalQuantityScheduleLeg2Report", path, "", res.getError());
				}
				return success("NotionalQuantityScheduleLeg2Report", ValidationResult.ValidationType.CARDINALITY, "NotionalQuantityScheduleLeg2Report", path, "");
			})
			.collect(toList());
	}
