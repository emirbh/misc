...
public abstract class Counterparty2SchemeNameRule implements ReportFunction<TransactionReportInstruction, HKTRPartyScheme> {
	...
	public static class Counterparty2SchemeNameRuleDefault extends Counterparty2SchemeNameRule {
		...
		protected HKTRPartyScheme assignOutput(HKTRPartyScheme output, TransactionReportInstruction input) {
			...
			if (areEqual(MapperS.of(counterparty2IdentifierFormatRule.evaluate(input)), MapperS.of(PartyIdentifierFormat2Enum.OTHER), CardinalityOperator.All).getOrDefault(false)) {
				thenArg0 = MapperS.of(input).<ReportingSide>map("getReportingSide", transactionReportInstruction -> transactionReportInstruction.getReportingSide()).<ReferenceWithMetaParty>map("getReportingCounterparty", reportingSide -> reportingSide.getReportingCounterparty());
			} else {
