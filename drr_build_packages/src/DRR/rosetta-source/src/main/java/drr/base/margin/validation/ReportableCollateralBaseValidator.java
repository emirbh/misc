...
public class ReportableCollateralBaseValidator implements Validator<ReportableCollateralBase> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ReportableCollateralBase", ValidationResult.ValidationType.CARDINALITY, "ReportableCollateralBase", path, "", res.getError());
				}
				return success("ReportableCollateralBase", ValidationResult.ValidationType.CARDINALITY, "ReportableCollateralBase", path, "");
			})
			.collect(toList());
	}
