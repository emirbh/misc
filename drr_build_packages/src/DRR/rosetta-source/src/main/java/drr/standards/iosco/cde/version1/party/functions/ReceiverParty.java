	...
	*/
	...
	public static class ReceiverPartyDefault extends ReceiverParty {
		...
		protected Party.PartyBuilder assignOutput(Party.PartyBuilder party, PayoutLegWithAuxiliary payout, List<? extends Counterparty> counterparties) {
			final ReferenceWithMetaParty referenceWithMetaParty = MapperS.of(extractCounterpartyByRole.evaluate(counterparties, MapperS.of(payerReceiverFromPayout.evaluate(payout)).<CounterpartyRoleEnum>map("getReceiver", payerReceiver -> payerReceiver.getReceiver()).get())).<ReferenceWithMetaParty>map("getPartyReference", counterparty -> counterparty.getPartyReference()).get();
