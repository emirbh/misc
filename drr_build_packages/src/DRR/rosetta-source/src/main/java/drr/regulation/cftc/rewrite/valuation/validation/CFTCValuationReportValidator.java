package drr.regulation.cftc.rewrite.valuation.validation;
...
public class CFTCValuationReportValidator implements Validator<CFTCValuationReport> {

	private List<ComparisonResult> getComparisonResults(CFTCValuationReport o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("initialMarginCollateralPortfolioCode", (String) o.getInitialMarginCollateralPortfolioCode() != null ? 1 : 0, 1, 1), 
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, CFTCValuationReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CFTCValuationReport", ValidationResult.ValidationType.CARDINALITY, "CFTCValuationReport", path, "", res.getError());
				}
				return success("CFTCValuationReport", ValidationResult.ValidationType.CARDINALITY, "CFTCValuationReport", path, "");
			})
			.collect(toList());
	}
