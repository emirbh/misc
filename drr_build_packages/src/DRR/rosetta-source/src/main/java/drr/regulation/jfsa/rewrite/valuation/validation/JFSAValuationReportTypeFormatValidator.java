package drr.regulation.jfsa.rewrite.valuation.validation;
...
public class JFSAValuationReportTypeFormatValidator implements Validator<JFSAValuationReport> {

	private List<ComparisonResult> getComparisonResults(JFSAValuationReport o) {
	...
	@Override
	public List<ValidationResult<?>> getValidationResults(RosettaPath path, JFSAValuationReport o) {
		return getComparisonResults(o)
			.stream()
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("JFSAValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "JFSAValuationReport", path, "", res.getError());
				}
				return success("JFSAValuationReport", ValidationResult.ValidationType.TYPE_FORMAT, "JFSAValuationReport", path, "");
			})
			.collect(toList());
	}
