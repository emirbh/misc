...
public class ESMAValuationReportValidator implements Validator<ESMAValuationReport> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ESMAValuationReport", ValidationResult.ValidationType.CARDINALITY, "ESMAValuationReport", path, "", res.getError());
				}
				return success("ESMAValuationReport", ValidationResult.ValidationType.CARDINALITY, "ESMAValuationReport", path, "");
			})
			.collect(toList());
	}
