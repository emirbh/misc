		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PortfolioIdentification3", ValidationResult.ValidationType.TYPE_FORMAT, "PortfolioIdentification3", path, "", res.getError());
				}
				return success("PortfolioIdentification3", ValidationResult.ValidationType.TYPE_FORMAT, "PortfolioIdentification3", path, "");
			})
			.collect(toList());
	}
