...
public class PaperCommodityOther1Validator implements Validator<PaperCommodityOther1> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PaperCommodityOther1", ValidationResult.ValidationType.CARDINALITY, "PaperCommodityOther1", path, "", res.getError());
				}
				return success("PaperCommodityOther1", ValidationResult.ValidationType.CARDINALITY, "PaperCommodityOther1", path, "");
			})
			.collect(toList());
	}
