		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DataSubmission", ValidationResult.ValidationType.TYPE_FORMAT, "DataSubmission", path, "", res.getError());
				}
				return success("DataSubmission", ValidationResult.ValidationType.TYPE_FORMAT, "DataSubmission", path, "");
			})
			.collect(toList());
	}
