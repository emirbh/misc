...
public class PartyIdentification248Choice__3Validator implements Validator<PartyIdentification248Choice__3> {
	...
	private List<ComparisonResult> getComparisonResults(PartyIdentification248Choice__3 o) {
		return Lists.<ComparisonResult>newArrayList(
				checkCardinality("lgl", (LegalPersonIdentification1__3) o.getLgl() != null ? 1 : 0, 0, 1), 
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PartyIdentification248Choice__3", ValidationResult.ValidationType.CARDINALITY, "PartyIdentification248Choice__3", path, "", res.getError());
				}
				return success("PartyIdentification248Choice__3", ValidationResult.ValidationType.CARDINALITY, "PartyIdentification248Choice__3", path, "");
			})
			.collect(toList());
	}
