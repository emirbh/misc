...
public class TradeData55ChoiceValidator implements Validator<TradeData55Choice> {
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeData55Choice", ValidationResult.ValidationType.CARDINALITY, "TradeData55Choice", path, "", res.getError());
				}
				return success("TradeData55Choice", ValidationResult.ValidationType.CARDINALITY, "TradeData55Choice", path, "");
			})
			.collect(toList());
	}
