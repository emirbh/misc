		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MetalCommodityPrecious2", ValidationResult.ValidationType.TYPE_FORMAT, "MetalCommodityPrecious2", path, "", res.getError());
				}
				return success("MetalCommodityPrecious2", ValidationResult.ValidationType.TYPE_FORMAT, "MetalCommodityPrecious2", path, "");
			})
			.collect(toList());
	}
