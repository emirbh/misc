		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OrganisationIdentification15Choice__3", ValidationResult.ValidationType.TYPE_FORMAT, "OrganisationIdentification15Choice__3", path, "", res.getError());
				}
				return success("OrganisationIdentification15Choice__3", ValidationResult.ValidationType.TYPE_FORMAT, "OrganisationIdentification15Choice__3", path, "");
			})
			.collect(toList());
	}
