		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DateAndDateTime2Choice__1", ValidationResult.ValidationType.CARDINALITY, "DateAndDateTime2Choice__1", path, "", res.getError());
				}
				return success("DateAndDateTime2Choice__1", ValidationResult.ValidationType.CARDINALITY, "DateAndDateTime2Choice__1", path, "");
			})
			.collect(toList());
	}
