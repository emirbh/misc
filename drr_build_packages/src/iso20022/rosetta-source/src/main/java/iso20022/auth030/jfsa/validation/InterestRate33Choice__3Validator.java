...
public class InterestRate33Choice__3Validator implements Validator<InterestRate33Choice__3> {
	...
	private List<ComparisonResult> getComparisonResults(InterestRate33Choice__3 o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("fltg", (FloatingRate13__3) o.getFltg() != null ? 1 : 0, 0, 1)
			);
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestRate33Choice__3", ValidationResult.ValidationType.CARDINALITY, "InterestRate33Choice__3", path, "", res.getError());
				}
				return success("InterestRate33Choice__3", ValidationResult.ValidationType.CARDINALITY, "InterestRate33Choice__3", path, "");
			})
			.collect(toList());
	}
