		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PolypropyleneCommodityPlastic2", ValidationResult.ValidationType.CARDINALITY, "PolypropyleneCommodityPlastic2", path, "", res.getError());
				}
				return success("PolypropyleneCommodityPlastic2", ValidationResult.ValidationType.CARDINALITY, "PolypropyleneCommodityPlastic2", path, "");
			})
			.collect(toList());
	}
