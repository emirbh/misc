		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AssetClassCommodityIndex1", ValidationResult.ValidationType.TYPE_FORMAT, "AssetClassCommodityIndex1", path, "", res.getError());
				}
				return success("AssetClassCommodityIndex1", ValidationResult.ValidationType.TYPE_FORMAT, "AssetClassCommodityIndex1", path, "");
			})
			.collect(toList());
	}
