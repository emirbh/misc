		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EnergyCommodityElectricity2", ValidationResult.ValidationType.CARDINALITY, "EnergyCommodityElectricity2", path, "", res.getError());
				}
				return success("EnergyCommodityElectricity2", ValidationResult.ValidationType.CARDINALITY, "EnergyCommodityElectricity2", path, "");
			})
			.collect(toList());
	}
