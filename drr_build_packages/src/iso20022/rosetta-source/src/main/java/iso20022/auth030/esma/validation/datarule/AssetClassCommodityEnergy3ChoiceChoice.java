...
public interface AssetClassCommodityEnergy3ChoiceChoice extends Validator<AssetClassCommodityEnergy3Choice> {
	...
	class Default implements AssetClassCommodityEnergy3ChoiceChoice {
		...
		@Override
		public List<ValidationResult<?>> getValidationResults(RosettaPath path, AssetClassCommodityEnergy3Choice assetClassCommodityEnergy3Choice) {
			ComparisonResult result = executeDataRule(assetClassCommodityEnergy3Choice);
			if (result.getOrDefault(true)) {
				return Arrays.asList(ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "AssetClassCommodityEnergy3Choice", path, DEFINITION));
			}
			...
			return Arrays.asList(ValidationResult.failure(NAME, ValidationResult.ValidationType.DATA_RULE, "AssetClassCommodityEnergy3Choice", path, DEFINITION, failureMessage));
		}
