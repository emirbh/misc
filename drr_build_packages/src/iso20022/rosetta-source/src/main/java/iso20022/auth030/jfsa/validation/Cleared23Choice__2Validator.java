		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Cleared23Choice__2", ValidationResult.ValidationType.CARDINALITY, "Cleared23Choice__2", path, "", res.getError());
				}
				return success("Cleared23Choice__2", ValidationResult.ValidationType.CARDINALITY, "Cleared23Choice__2", path, "");
			})
			.collect(toList());
	}
