		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Tranche3", ValidationResult.ValidationType.CARDINALITY, "Tranche3", path, "", res.getError());
				}
				return success("Tranche3", ValidationResult.ValidationType.CARDINALITY, "Tranche3", path, "");
			})
			.collect(toList());
	}
